...
const GENERATIONS_PATH = path.resolve(__dirname, '../GENERATIONS/images');

// Cache for storage and faster reads
 ...
 * @param {Object} fileMetadata - File metadata from cache
...
async function getGeneration(fileMetadata, includeImageData = false) {
    try {
        let result = {
            src: `/${fileMetadata.url.replace(/^\/+/, '')}`,  // Ensure clean URL without double slashes
...
function createFileMetadata(fileName, basePath, stats, resolvedPath) {
    ...
    // Create URL relative to the GENERATIONS/images directory
...
function initCache(imageDir = GENERATIONS_PATH) {
    ...
    if (!fs.existsSync(imageDir)) {
        console.warn(`Creating directory: ${imageDir}`);
    ...
    try {
        ...
        function scanDir(dir) {
            try {
                // Normalize the path to remove any duplicate 'images' segments
                ...
                // Check if we're not going deeper than GENERATIONS_PATH/images
                ...
                for (const item of items) {
                    ...
                    try {
                        ...
                        if (stats.isDirectory()) {
                            ...
                            processFile(item, normalizedDir);
                        }
                    } catch (itemError) {
...
async function getNewestGeneration() {
    ...
    Object.values(_cache).forEach(folderFiles => {
        folderFiles.forEach(file => {
            const fileName = path.basename(file.fullPath);
            if (!fileName.startsWith('info') && file.ext.toLowerCase() === '.png') {
...
async function addFile(folderName, fileName) {
    ...
    // Build path using the folder name for proper directory structure
    const fileAbsolutePath = path.join(_imgPath, folderName, path.basename(fileName));
