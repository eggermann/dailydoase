...
//https://blog.hubspot.com/website/how-to-get-youtube-api-key
                    ...
                    console.log('hallelulioss-----1', response.data)




                    response.data.items.map(async (item, index) => {
                        console.log('hallelulioss-----2',item)


                        const id = item.id.videoId;
                        item.link = "https://www.youtube.com/watch?v=" + id;
                        const url2 = `https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id=${id}&key=${_.youtubeApiKey}`;

                        try {
                            const responseDetails = await axios.get(url2);
