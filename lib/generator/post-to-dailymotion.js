...
import store from '../store.cjs'; // Assuming store is a shared module for file tracking

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const saveItemPath = this.imageDir = path.resolve(__dirname, '../../GENERATIONS/dailymotion');
...
const _ = {
    ...
    DailymotionAPI: class extends PostTo {
        constructor(config) {
            ...
            config.saveItemPath = saveItemPath;
            this.config = config;
        ...
        getQuery(q) {
            ...
            return `https://api.dailymotion.com/videos?fields=id,title,url,owner.screenname,created_time,thumbnail_url,duration,allow_embed&search=${q}&limit=${limit}`;
            //&api_key=${_DM_API_KEY}
        }
...
const i = new _.DailymotionAPI(
    {words: [['Robotics', 'en']]//only describing purpose
