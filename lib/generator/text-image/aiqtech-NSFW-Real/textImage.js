...
import { joinOutPath, withTimeout } from './../../utils.js';
...
export class PostToAIQTech_NSFWReal_TextImage extends PostTo {
  ...
  async prompt(promptText, options = {}) {
    ...
    console.log('[AIQTech NSFW-Real] Requesting /infer with payload:', JSON.stringify(payload, null, 2));
    const result = await withTimeout(
      this._cli.predict('/infer', payload),
      15 * 60 * 1000,
      `${SPACE} /infer`
    );
    ...
    if (url) {
      ...
      console.log(`[AIQTech NSFW-Real] Downloading result from: ${url}`);
      await downloadToFile(url, savePath, { timeoutMs: 15 * 60 * 1000 });
      console.log(`[AIQTech NSFW-Real] Saved image to: ${savePath}`);
    } else if (buffer) {
      await fs.writeFile(savePath, buffer);
      console.log(`[AIQTech NSFW-Real] Saved image buffer to: ${savePath}`);
    } else {
