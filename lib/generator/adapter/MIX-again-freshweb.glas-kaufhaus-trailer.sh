...
export FRESHWEB_WORDS=${FRESHWEB_WORDS:-shopping,en | medicine,en | terror,en | love,en}
...
# the initial background, two confirmed changes, or a two-minute heartbeat.
...
if [[ -n "${EXHIBITION_STARTER_ROOT:-}" ]]; then
  export FRESHWEB_CAMERA_PERSON_QUEUE_PATH=${FRESHWEB_CAMERA_PERSON_QUEUE_PATH:-"$EXHIBITION_STARTER_ROOT/runtime/camera-shot-guard/accepted-person-queue.json"}
  export FRESHWEB_CAMERA_PERSON_QUEUE_CONSUMED_PATH=${FRESHWEB_CAMERA_PERSON_QUEUE_CONSUMED_PATH:-"$EXHIBITION_STARTER_ROOT/runtime/camera-shot-guard/accepted-person-queue-consumed.json"}
fi
# Keep the branch self-contained: OpenAI reads the live person and room first;
# the Mini's local vision service remains available as its offline fallback.
export OPENAI_VISION_MODEL=${OPENAI_VISION_MODEL:-gpt-4o-mini}
export FRESHWEB_VISION_PROVIDERS=${FRESHWEB_VISION_PROVIDERS:-openai,lmstudio}
export FRESHWEB_CAMERA_PRESENCE_VISION_PROVIDERS=${FRESHWEB_CAMERA_PRESENCE_VISION_PROVIDERS:-openai,lmstudio}
export FRESHWEB_PERSONA_DESCRIPTION_VISION_PROVIDERS=${FRESHWEB_PERSONA_DESCRIPTION_VISION_PROVIDERS:-openai,lmstudio}
...
# GPT owns rhythm. Runtime requests enough semantic cues for seven scenes, then
# accepts only a 2-7 scene plan with every clip between 2 and 6 seconds.
...
export FRESHWEB_GPT_OWNS_RHYTHM=${FRESHWEB_GPT_OWNS_RHYTHM:-1}
export FRESHWEB_GPT_RHYTHM_MIN_SCENE_COUNT=${FRESHWEB_GPT_RHYTHM_MIN_SCENE_COUNT:-2}
export FRESHWEB_GPT_RHYTHM_MAX_SCENE_COUNT=${FRESHWEB_GPT_RHYTHM_MAX_SCENE_COUNT:-7}
export FRESHWEB_GPT_RHYTHM_MIN_DURATION_SECONDS=${FRESHWEB_GPT_RHYTHM_MIN_DURATION_SECONDS:-2}
export FRESHWEB_GPT_RHYTHM_MAX_DURATION_SECONDS=${FRESHWEB_GPT_RHYTHM_MAX_DURATION_SECONDS:-6}
...
export FRESHWEB_CAMERA_FIRST_LAST_MAX_DURATION=${FRESHWEB_CAMERA_FIRST_LAST_MAX_DURATION:-6}
...
# captured synchronously after the previous shot. FLUX.2 flex accepts all ten
# real Runware reference images, so room, person, and context buffer arrive
# together instead of reducing drift correction to one last-frame image.
