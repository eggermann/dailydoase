...
if [ -z "${FRESHWEB_SCENE_VISUAL_DIRECTION:-}" ]; then
  FRESHWEB_SCENE_VISUAL_DIRECTION="Build a compact BRD television trailer from 1989 inside the supplied photographed Kaufhaus in Germany. The photograph is the immutable location plate: preserve its exact viewpoint, crop, floor plan, ceiling, ducts, windows, columns, elevators, walls, exposure, color balance, and at least eighty percent of the visible room in every generated still. The result must remain immediately recognizable as this Kaufhaus, never a generic warehouse, fantasy interior, or globally restyled set. Keep the isolated green monster as primary protagonist through its choices, preserving its face, glowing eyes, plant-like anatomy, hanging lamps, and dark green industrial identity whenever it is visible. It must not become a permanent large foreground mascot: vary its presence between distant figure, partial body, reflection, occluded trace, and deliberate off-screen consequence. Compose every scene from its semantic baton collision: carry the previous fresh term forward, collide it with the current stream's fresh getNext term, and derive the monster's intention, physical tactic, local surreal mutation, viewpoint, subject motion, room response, and camera movement from that exact friction. Confine impossible transformations to the protagonist, people when called for, movable objects, light, reflections, shadows, and a local architectural detail; never dissolve or replace the whole location. $FRESHWEB_PEOPLE_DIRECTION Scenes must be causally linked, visually concrete, strange but readable. Choose one camera behavior per scene only after deciding what the collision makes the protagonist do. The camera may hold, pan, tilt, track sideways, arc, push, pull back, rise, descend, or change focus when that action best reveals the semantic event. Avoid repeating the same camera behavior in adjacent scenes and do not impose a global travel direction. Every image and motion prompt must specify subject, surreal event, mood, lighting, color, texture, composition, lens or framing, physical motion, and the motivated camera behavior. No invented modern objects, subtitles, readable lettering, labels, callout lines, or typography."
fi
export FRESHWEB_SCENE_VISUAL_DIRECTION

if [ -z "${FRESHWEB_CAMERA_SCENE_PLAN_SYSTEM_PROMPT:-}" ]; then
  FRESHWEB_CAMERA_SCENE_PLAN_SYSTEM_PROMPT="Create the requested number of short scene plans for a green-monster trailer grounded in supplied photographed Kaufhaus interiors. The protagonist reference contains only one isolated green plant-like monster; use it only for creature identity, never as composition or background. Treat each Kaufhaus photograph as an immutable location plate. In every stillPrompt preserve its exact viewpoint, crop, floor plan, ceiling, ducts, windows, columns, elevators, walls, exposure, color balance, and at least eighty percent of the room; the location must remain immediately recognizable, not become a generic warehouse or fantasy set. $FRESHWEB_PEOPLE_DIRECTION Each source cue labels a carried Anchor and fresh getNext Collision. Let that collision compose every variable part of the scene: storyCause, monsterIntent, semanticAction, local roomConsequence, monsterPresence, viewpoint, motionCue, cameraCue, stillPrompt, and singleImagePrompt. Do not add a generic trailer action, generic warehouse atmosphere, or default camera move independent of the words. Infer one specific surprising causal event: the monster discovers, misunderstands, uses, protects, rejects, imitates, or transforms something because of the collision. Keep surreal mutation local to protagonist, people when semantically justified, movable objects, light, reflections, shadows, or one nearby architectural detail; never replace or dissolve the whole Kaufhaus. The next scene inherits the visible consequence, not merely the terms. The monster is protagonist by agency, never a permanent large advertising mascot; vary scale and presence. Select viewpoint and cameraCue only after semanticAction: use hold, pan, tilt, sideways track, arc, push, pullback, rise, descent, or focus shift according to what best reveals that action. Do not default to forward movement and do not repeat the same dominant camera behavior in adjacent scenes. Every stillPrompt captures the decisive action inside the recognizable location. Every singleImagePrompt shows tactic beginning, changing, and leaving its local consequence, including the chosen cameraCue. Preserve monster identity and photographed Kaufhaus continuity; no live camera, poster layout, panels, portraits, callout lines, readable text, or modern logos. Return required JSON scene plan only."
fi
