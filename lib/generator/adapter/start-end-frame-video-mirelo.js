...
import AiqtechNSFW from '../aiqtech-NSFW-Real/textImage.js';
...
import { checkAndLogDuration, concatVideos, uploadToYouTube } from '../../helper/yt-upload/concat-and-upload.js';
...
class Generator extends PostTo {
    ...
    async init() {
        ...
        const imageModel = (this.config.image.type === 'aiqtech-NSFW-Real') ? AiqtechNSFW : FLUX;
        this.flux = await imageModel.init(this.config.image);
    ...
    async generateImage(streams, options) {
        ...
        if (options?.prompts?.create) {
            prompt = options.prompts.create(prompt);
        }
    ...
    async prompt(streams, options = {}) {
        ...
        try {
         ...
         console.log('Generator startFrame image path:', startFrame);
            ...
            */
            ...
            if (videoPrompt.create) {
                ...
                prompt = await videoPrompt.create(startFrame.json.metadata.prompt, this.lastEndFRame.json.metadata.prompt);
            ...
            mergedConfig.prompt = prompt ?? (options.useImagePrompt ? startFrame.json.metadata.prompt : '');


            console.log('GenImgVideo final video prompt:', totalPrompt, 'mergedConfig:', mergedConfig.prompt);
