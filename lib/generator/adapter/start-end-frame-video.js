...
import videoModel from '../wan22/firstLastFrame.js';
...
process.exit
    ...
    async prompt(streams, options = {}) {
        ...
        try {
            ...
            const startFrame = await generateImage(streams, options);
            options.endImageStream = await generateImage(streams, options);

            const videoPrompt = options.video.prompts || {};
            console.log('GenImgVideo videoOptions:', videoPrompt,'options.video',options.video);
         

            if (videoPrompt.create) {
                prompt = videoPrompt.create(startFrame, options.video);
