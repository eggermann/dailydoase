...
class Generator extends PostTo {
    ...
    async prompt(streams, options = {}) {
        ...
        try {
            ...
            // If requested, reuse the image prompt from the image stage
            ...
            // Fallback to the image prompt if video prompt is empty
            mergedConfig.prompt = (totalPrompt && String(totalPrompt).trim().length > 0)
                ? totalPrompt
                : (startFrame?.json?.metadata?.prompt || '');
