...
const normalizeProviderName = (value) => {
  ...
  if (
    normalized === 'localmistral'
    || normalized === 'local-mistral'
    || normalized === 'mac-mini-vision'
  ) {
    return 'lmstudio';
  }
  return normalized;
...
async function createOpenAiClient(apiKey) {
  ...
  const configuredTimeout = Number(process.env.FRESHWEB_OPENAI_VISION_TIMEOUT_MS);
...
export async function visionModel(input = {}, runtime = {}) {
  ...
  const providers = (
    Array.isArray(options.providers) && options.providers.length > 0
      ? options.providers
      : FALLBACK_PROVIDERS
  ).map((provider) => normalizeProviderName(provider)).filter(Boolean);
