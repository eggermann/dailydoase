...
test('repairSemanticScenePlanEntry preserves immutable cue identity without a stream', async () => {
  ...
  const repaired = await repairSemanticScenePlanEntry({
    ...
    invalidScene: {
      index: 3,
      durationSeconds: 2,
      event: 'Die Ablehnung: Rauchkanister und Kinderschuh',
    },
    validationErrors: ['Terrorism policy violation'],
  });
  ...
  expect(client.chat.completions.create.mock.calls[0][0].messages[1].content)
    .not.toMatch(/Rauchkanister|Kinderschuh|Terrorism policy violation/);
});
...
test('createSceneGenerator tells the model to use source cues as the story spine', async () => {
  ...
  expect(request.messages[0].content).toContain('short English-language monster-trailer scenes');
  ...
  expect(requestOptions).toEqual({ timeout: 45000 });
});
...
test('createSceneGenerator prepends semantic priority to custom system prompts', async () => {
  ...
  expect(systemMessage).toContain('short English-language monster-trailer scenes');
  expect(systemMessage).toContain('Custom camera planner instructions.');
});

test('createSceneGenerator isolates raw rejection text from the clean repair prompt', async () => {
  const create = jest.fn(async () => ({
    choices: [{
      message: {
        content: JSON.stringify({
          scenes: [{
            ...
            sceneFocus: 'location',
            event: 'Temporary partitions change the visible route.',
            monsterPresence: 'absent',
            consequence: 'The passage now ends at a mirrored column.',
            nextHook: 'A reflected line remains.',
            stillPrompt: 'The real hall with temporary partitions forming a changed route.',
            videoPrompt: 'The nearest partition shifts once and redirects the route.',
            cameraCue: 'Mostly held handheld phone shot.',
            startFrameStrategy: 'locationReanchor',
            startFrameReason: 'Establish the real location.',
          }],
        }),
      },
    }],
  ...
  const rawRefusal = 'Die Ablehnung: Terrorism, Rauchkanister, Kinderschuh';

  await generateScenes({
    sceneCount: 1,
    sceneLengths: [2],
    repairContext: {
      retryReason: 'invalid-scene-plan',
      invalidScenePlan: rawRefusal,
      validationErrors: [rawRefusal],
    },
  });

  const repairPrompt = create.mock.calls[0][0].messages[1].content;
  expect(repairPrompt).toMatch(/previous response was not a valid scene plan/i);
  expect(repairPrompt).toMatch(/new complete JSON scene plan in concise English/i);
  expect(repairPrompt).toMatch(/Do not mention the rejected response/i);
  expect(repairPrompt).not.toContain(rawRefusal);
  expect(repairPrompt).not.toMatch(/Terrorism|Rauchkanister|Kinderschuh/i);
});
...
test('generateScenePlanWithFallback keeps requested count in one complete plan', async () => {
  const generateScenes = jest.fn(async ({ sceneCount }) => Array.from({ length: sceneCount }, (_, index) => ({
    ...
    semanticAnchorEnglish: `anchor ${index + 1}`,
    semanticCollision: `collision-${index + 1}`,
    semanticCollisionEnglish: `collision ${index + 1}`,
    sceneFocus: 'location',
    event: 'A physical event.',
    monsterPresence: 'absent',
    consequence: 'A visible result.',
    nextHook: 'The visible result remains.',
    stillPrompt: 'A decisive still.',
    videoPrompt: 'The event moves.',
    cameraCue: 'A small handheld pan.',
    startFrameStrategy: index === 0 ? 'locationReanchor' : 'rawLastFrame',
    startFrameReason: 'Keep location continuity.',
  })));
...
test('generateScenePlanWithFallback uses a neutral fallback after one clean repair', async () => {
  ...
  expect(generateScenes.mock.calls[1][0].repairContext).toEqual({
    retryReason: 'invalid-scene-plan',
  });
  expect(result.semanticValidation.valid).toBe(true);
  expect(result.planningAttempts.at(-1).fallbackUsed).toBe(true);
  expect(result.scenePlan[0].event).toBe('Temporary partitions slowly change the visible route through the hall.');
});

test('generateScenePlanWithFallback stops before rendering when neutral fallback is disabled', async () => {
  const generateScenes = jest.fn(async () => {
    throw new Error('Scene plan request timed out after 45000ms');
  });
  ...
  await expect(generateScenePlanWithFallback({
    ...
    sourceCueRecords: [{
      anchor: { term: 'horror' },
      collision: { term: 'fries' },
    }],
    allowNeutralFallback: false,
  })).rejects.toThrow('neutral fallback disabled');

  expect(generateScenes).toHaveBeenCalledTimes(2);
});

test('neutral fallback never schedules a monster appearance', async () => {
  const result = await generateScenePlanWithFallback({
    generateScenes: async () => [{ semanticAnchor: 'Kaufhaus', semanticCollision: 'Betriebsform' }],
    sceneCount: 6,
    sceneLengths: [2, 2, 2, 2, 2, 2],
    sourceCueRecords: Array.from({ length: 6 }, (_, index) => ({
      anchor: { term: `anchor-${index}` },
      collision: { term: `collision-${index}` },
    })),
  });

  expect(result.scenePlan.every((scene) => scene.monsterPresence === 'absent')).toBe(true);
  expect(result.scenePlan.every((scene) => !['monster', 'mixed'].includes(scene.sceneFocus))).toBe(true);
});
...
test('generateScenePlanWithFallback repairs malformed JSON once with saved cue records', async () => {
  ...
  expect(JSON.stringify(generateScenes.mock.calls[1][0])).not.toContain('{not json');
});

test('rejected objects and policy wording never enter the retry or fallback plan', async () => {
  const rawRefusal = 'Die Ablehnung: Terrorism, Rauchkanister, Kinderschuh';
  const generateScenes = jest.fn(async () => {
    const error = new Error('planner rejected');
    error.name = 'InvalidScenePlanContentError';
    error.rawScenePlan = rawRefusal;
    throw error;
  });
  const result = await generateScenePlanWithFallback({
    generateScenes,
    ...
    sourceCueRecords: [{
      anchor: { term: 'exhibition' },
      collision: { term: 'light' },
    }],
  });

  expect(generateScenes).toHaveBeenCalledTimes(2);
  expect(JSON.stringify(generateScenes.mock.calls[1][0])).not.toContain(rawRefusal);
  expect(JSON.stringify(result.scenePlan)).not.toMatch(/Terrorism|Rauchkanister|Kinderschuh|Die Ablehnung/i);
  expect(result.planningAttempts[0].rejectionType).toBe('planner-refusal');
});
