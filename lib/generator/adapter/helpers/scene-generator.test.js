...
test('reference-image-actor validation accepts GPT story-driven iteration starts', () => {
  const continuation = {
  ...
  expect(validateReferenceImageActorScenePlan([continuation], options)).toEqual([]);
  expect(validateReferenceImageActorScenePlan([transition], options)).toEqual([]);
  expect(validateReferenceImageActorScenePlan([continuation])).not.toEqual([]);
});
...
test('parseScenePlan parses JSON scene objects with scenes arrays', () => {
    ...
    scenes: [
      {
        ...
        actorTrajectory: 'The gardener steps from the potting bench toward the window while raising the seedling.',
        finalPose: 'The gardener stands beside the window with the seedling held at eye level.',
        actorsInteraction: '',
  ...
  expect(scenePlan[0].actorTrajectory).toBe('The gardener steps from the potting bench toward the window while raising the seedling.');
  expect(scenePlan[0].finalPose).toBe('The gardener stands beside the window with the seedling held at eye level.');
...
test('createSceneGenerator tells the model to use source cues as the story spine', async () => {
  ...
  expect(userMessage).toContain('primary stage and prop inventory');
  expect(userMessage).toContain('existing person, object, or room feature');
  expect(userMessage).toContain('real-camera origin remains readable');
