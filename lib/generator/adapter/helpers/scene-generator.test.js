...
test('createSceneGenerator tells the model to use source cues as the story spine', async () => {
  ...
  expect(userMessage).toContain('Camera-mode visible-anchor rule: only use people, props, furniture, appliances, and room elements that are explicitly visible in the vision story context.');
  expect(userMessage).toContain('If the context does not name a pan, bottle, stove, plate, or table, do not mention one');
