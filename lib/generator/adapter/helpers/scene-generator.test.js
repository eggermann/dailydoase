...
test('parseScenePlan parses JSON scene objects with scenes arrays', () => {
    ...
    scenes: [
      {
        ...
        exitMotion: 'The gardener keeps lifting the seedling upward, elbow still rising.',
  ...
  expect(scenePlan[0].exitMotion).toBe('The gardener keeps lifting the seedling upward, elbow still rising.');
  expect(scenePlan[0].requiresDocumentaryEndFrame).toBe(true);
  ...
  expect(scenePlan[1].requiresDocumentaryEndFrame).toBe(false);
});
...
test('generateScenePlanWithFallback keeps the live run moving after planning models fail', async () => {
  const generateScenes = jest.fn(async () => {
    throw new Error('both scene planners timed out');
  });
  ...
  const result = await generateScenePlanWithFallback({
    ...
    sceneCount: 2,
    sceneLengths: [3, 4],
    configMode: 'referenceImageActor',
    sourceCues: ['banana', 'bone'],
    visionStoryContext: 'a visitor in the exhibition room',
    emergencyScenePlan: createEmergencyScenePlan,
  });

  expect(result.effectiveSceneLengths).toEqual([3, 4]);
  expect(result.scenePlan).toHaveLength(2);
  expect(result.scenePlan[0]).toEqual(expect.objectContaining({
    frameSource: 'newImage',
    videoMode: 'singleImage',
    freshImage: true,
    useCameraShot: true,
  }));
  expect(result.scenePlan[1]).toEqual(expect.objectContaining({
    frameSource: 'lastFrame',
    videoMode: 'singleImage',
    freshImage: false,
    useCameraShot: false,
  ...
  expect(result.scenePlan[0]).toEqual(expect.objectContaining({
    title: 'Threshold response: banana',
    eventType: 'actorToLocation',
  }));
  expect(result.scenePlan[1]).toEqual(expect.objectContaining({
    title: 'Encounter at the display: bone',
    eventType: 'actorToActor',
  }));
  expect(result.scenePlan[0].videoPrompt).toContain('banana');
  expect(result.scenePlan[1].videoPrompt).toContain('bone');
  expect(result.scenePlan[0].motionCue).not.toEqual(result.scenePlan[1].motionCue);
});
