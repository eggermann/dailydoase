...
test('createSceneGenerator tells the model to use source cues as the story spine', async () => {
  ...
  expect(request.messages[0].content).toContain(
    'Write semanticAnchorEnglish and semanticCollisionEnglish as concise English translations'
  );
  expect(request.response_format.json_schema.schema.properties.scenes.items.required)
    .toEqual(expect.arrayContaining(['semanticAnchorEnglish', 'semanticCollisionEnglish']));
  ...
  expect(userMessage).toContain('Scene memory rule: after scene 1, let the previous scene leave a visible residue in the next one');
  ...
  expect(userMessage).toContain('Surreal description rule: name the exact impossible but visible event produced by the collision.');
  expect(userMessage).toContain('Semantic baton rule: Anchor is inherited from the previous scene, Collision is the fresh getNext term');
  expect(userMessage).toContain('FLUX stillPrompt contract: write a self-contained image prompt with subject, decisive frozen action, semantic collision');
  expect(userMessage).toContain('WAN singleImagePrompt contract: write a self-contained image-to-video prompt with starting state, temporal transformation');
  ...
  expect(userMessage).toContain('Reference-image-actor-mode reminder: keep the story inside the real visible shot');
  expect(userMessage).toContain('Reference-image-actor-mode visible-anchor rule: use the vision story context as the source of truth for location, actor identity, and visible scene setup.');
  ...
  expect(userMessage).toContain('Reference-image-actor-mode firstLast rule: use firstLast only for small believable changes between start and destination inside the same visible room');
});
