...
test('compactPlannerText keeps a bounded semantic planner request', () => {
  expect(compactPlannerText('  first\nsecond  ', 30)).toBe('first second');
  const compacted = compactPlannerText('First complete sentence. Second complete sentence. Third complete sentence.', 50);
  expect(compacted.length).toBeLessThanOrEqual(50);
  expect(compacted).toMatch(/…$/);
  expect(compacted).toBe('First complete sentence.…');
});

test('emergency plan keeps a rotated semantic topic as a concrete three-beat arc', () => {
  const plan = createEmergencyScenePlan({
    sceneCount: 3,
    sceneLengths: [3, 2, 2],
    sourceCues: ['unbounded raw stream text that must not dominate the image prompt'],
    visionStoryContext: 'one visitor in the exhibition room',
    storyTransport: 'Current topic word: Berlin Art Week. Current people: 1 visible person.',
    configMode: 'reference-image-actor',
  });

  expect(plan.map((scene) => scene.title)).toEqual([
    'Berlin Art Week: Semantic signal',
    'Berlin Art Week: Semantic encounter',
    'Berlin Art Week: Semantic consequence',
  ]);
  expect(plan.every((scene) => scene.storyBeat.includes('Berlin Art Week'))).toBe(true);
  expect(plan.every((scene) => !scene.videoPrompt.includes('unbounded raw stream text'))).toBe(true);
});
