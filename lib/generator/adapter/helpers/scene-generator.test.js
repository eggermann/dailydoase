...
test('GPT rhythm may choose 2-7 scenes and 2-6 seconds without runtime replacement', async () => {
  const scenes = [
    { title: 'Opening', durationSeconds: 6 },
    { title: 'Payoff', durationSeconds: 2 },
  ];
  ...
  expect(plan.map((scene) => scene.durationSeconds)).toEqual([6, 2]);
  expect(create.mock.calls[0][0].messages.at(-1).content).toContain('Scene count: choose 2-7.');
  expect(create.mock.calls[0][0].messages.at(-1).content).toContain('Scene durations: choose whole seconds 2-6 per scene.');
});

test('GPT rhythm rejects too many scenes and out-of-range clip lengths', () => {
  expect(validateScenePlanRhythm([
    { durationSeconds: 1 },
    { durationSeconds: 7 },
  ], {
    enabled: true,
    ...
    'scene 1: durationSeconds must be between 2 and 6; received 1',
    'scene 2: durationSeconds must be between 2 and 6; received 7',
  ]);
});
