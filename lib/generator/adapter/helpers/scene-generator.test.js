...
test('creative cuts stay in the source room while story action remains free', () => {
  expect(DEFAULT_SCENE_SYSTEM_PROMPT).toContain('same physical room and camera world as the source');
  expect(DEFAULT_SCENE_SYSTEM_PROMPT).toContain('never to an exterior, a different building, a street, or an animal absent from the source');
});
...
test('getVisiblePeopleCount reads story transport and vision summaries', () => {
  expect(getVisiblePeopleCount('Current people: 2 visible people: artist and visitor.')).toBe(2);
  expect(getVisiblePeopleCount('People: 3. Actors: three people around the table.')).toBe(3);
  expect(getVisiblePeopleCount('One artist visible.')).toBe(0);
});

test('reference-image-actor validation requires interaction for multiple visible people', () => {
  const scene = {
  ...
  expect(validateReferenceImageActorScenePlan([scene], {
    requireMultiPersonInteraction: true,
  })).toContain('scene 1: multiple visible people require a concrete actorsInteraction');

  expect(validateReferenceImageActorScenePlan([{
    ...scene,
    actorsInteraction: 'The artist hands the visitor a print; the visitor takes it and looks back.',
  }], {
    requireMultiPersonInteraction: true,
  })).toEqual([]);
});
...
test('reference-image-actor validation permits a Runware-only creative cut', () => {
  const plan = [
    {
      frameSource: 'newImage',
      videoMode: 'singleImage',
  ...
  expect(validateReferenceImageActorScenePlan(plan, {
    forceImageToVideoOnly: true,
    allowCreativeCuts: true,
  })).toEqual([]);
});
...
test('parseScenePlan parses JSON scene objects with scenes arrays', () => {
  ...
  expect(scenePlan[0].realityIntrusion).toBe(true);
...
test('createSceneGenerator tells the model to use source cues as the story spine', async () => {
            ...
            scenes: [
              {
                ...
                actorAction: 'The foreground actor freezes and grips the other actor’s sleeve.',
                actorsInteraction: 'The foreground actor grips the other actor’s sleeve; the second actor turns and pulls them toward the exit.',
                locationAction: '',
  ...
  expect(userMessage).toContain('Every scene must contain a concrete actorsInteraction between at least two of them.');
