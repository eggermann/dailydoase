...
import { normalizeSceneAppearance } from '../shorty-book/visual-prompt-compiler.js';
import { normalizeVisualPreparation } from '../shorty-book/visual-continuity-guard.js';
...
export const DEFAULT_SCENE_SYSTEM_PROMPT = [
  ...
  'appearance defines physical existence: subject, mode (inRoom, reflection, screen, artwork, transformation), surface, and integration. Use inRoom when no special surface applies.',
  'visualPreparation is continueFrame by default. Use fluxEdit only when the next scene requires a new visible still state before motion; never use it merely for style, lighting, atmosphere, or drift correction.',
        ...
        items: {
          ...
          properties: {
            ...
            appearance: {
              type: 'object',
              additionalProperties: false,
              required: ['subject', 'mode', 'surface', 'integration'],
              properties: {
                subject: { type: 'string' },
                mode: {
                  type: 'string',
                  enum: ['inRoom', 'reflection', 'screen', 'artwork', 'transformation'],
                },
                surface: { type: 'string' },
                integration: {
                  type: 'array',
  ...
  scenePlan.forEach((scene, index) => {
    ...
    if (index === 0) {
      ...
      const isCameraTransition = videoMode === 'firstLast'
        ...
        && !freshImage
        && useCameraShot;
      const validOpening = storyDrivenIterationStart
        ? isCameraReset || isContinuation || isCameraTransition
        : isCameraReset;
      if (!validOpening) {
        issues.push(
          storyDrivenIterationStart
            ? `${label}: story-driven iteration start must be camera reset, previous-frame continuation, or previous-frame firstLast camera transition`
            : `${label}: opening mode must be newImage/singleImage/freshImage=true/useCameraShot=true`
        );
      }
...
export const parseScenePlan = (value, sceneCount = 3, resolvedSceneLengths = []) => {
  ...
  return parsed.scenes.slice(0, count).map((item, index) => {
    ...
    return {
      ...
      locationAction,
      storyEvent,
      appearance: normalizeSceneAppearance(item?.appearance, item),
      visualPreparation: normalizeVisualPreparation(item?.visualPreparation),
      castSelection,
...
export const generateScenePlanWithFallback = async ({
  ...
  iterationStartMode = 'cameraReset',
...
export const createSceneGenerator = ({
  ...
  return async ({
    ...
    const storyDrivenIterationStart = referenceImageActorMode
      && normalizeString(iterationStartMode).toLowerCase() === 'storydriven'
    ...
    const requestRules = [
      ...
      'Treat detected people, visible objects, and room features as the primary stage and prop inventory. Before inventing anything, choose at least one of them as the action carrier for each scene.',
      'Let each semantic cue change how that existing person, object, or room feature is used, perceived, or related to the others.',
      'Across the sequence, vary whether the cue acts mainly on a person, on the room, or through an interaction between both.',
      'A new surreal element may appear when the story benefits, but it should visibly emerge from an existing form, object, light condition, or action so its real-camera origin remains readable.',
      'Mirrors and reflective transformations may distort or repeat the visible actor only as one continuous reflection; never turn that reflection into an independent duplicate person.',
      ...
      ...(referenceImageActorMode
        ? [
            forceImageToVideoOnly
              ...
              : storyDrivenIterationStart
                ? 'Story-driven iteration start: a previous film ending and the current camera/cast context are both available. For scene 1 choose exactly one story-motivated tuple: continue previous film = lastFrame/singleImage/false/false; transition toward current camera context = lastFrame/firstLast/false/true; reset to current camera = newImage/singleImage/true/true. A newly observed visitor is optional story material, never a mandatory reset or appearance. Later continuation = lastFrame/singleImage/false/false; later destination = lastFrame/firstLast/false/true.'
            ...
            'Keep the detected room and its existing features readable as the scene foundation. Surreal changes may grow from them instead of replacing them with an unrelated invented setup.',
            'For firstLast, motionCue contains one continuous visible movement or interaction. First and last images already carry people, room, CCTV style, angle, and destination state. Do not repeat those details. Never use static, hold steady, or camera movement for firstLast.',
          ]
        : []),
    ...
    const messages = [
      ...
      },
      {
        ...
        content: [
          ...
          `Iteration start mode: ${storyDrivenIterationStart ? 'storyDriven with previous film available' : 'cameraReset'}`,
          ...requestRules,
        ].join('\n'),
      },
    ];
    ...
    const validationOptions = { forceImageToVideoOnly, storyDrivenIterationStart };
