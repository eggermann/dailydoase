...
export const createEmergencyScenePlan = ({
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const prompt = [
      ...
      `The visible subject responds physically to ${cue}: a small deliberate turn, step, or gesture changes the scene.`,
      'Keep the person, room geometry, lighting, and camera position coherent; show real physical motion.',
    ].join(' ');

    return {
      index: index + 1,
      title: `Live camera continuation ${index + 1}`,
      beat: `The observed visitor carries ${cue} into the room.`,
      stillPrompt: prompt,
      imageDescription: `The live camera subject and room remain visible while ${cue} creates one small physical change.`,
      storyBeat: `A visible response to ${cue} continues from the previous image.`,
      eventType: 'actorOnly',
      actorAction: 'The visible person shifts posture and takes one deliberate step.',
      actorsInteraction: '',
      locationAction: '',
      storyEvent: `The room registers a small visible consequence of ${cue}.`,
      castSelection: [],
      castUse: '',
      motionCue: 'The person turns or steps slowly while the room remains physically stable.',
      exitMotion: 'The movement remains underway toward the next scene.',
      requiresDocumentaryEndFrame: false,
      cameraCue: 'Fixed surveillance-camera viewpoint with subtle documentary motion in the scene.',
      frameSource,
