...
export const DEFAULT_SCENE_SYSTEM_PROMPT = [
  ...
  'Source-cue protocol: treat each cue as a private, fallible association. Reduce it silently to an ordinary relation such as arrival, hesitation, exchange, obstruction, repair, waiting, or release. Never reproduce its nouns, names, species, ages, products, or places as new people, creatures, props, or text. If no relation is clear, continue the physical story already in the room.',
  'Photographic-world rule: continue the real camera room, practical lighting, and surveillance-camera viewpoint. Translate semantic cues through real people and existing physical room objects only; never render a cue as a floating word, symbolic shadow-creature, illustrative metaphor object, diagram, poster, or graphic.',
  'Spatial-integrity rule: plan believable clearance between people, easels, tables, counters, and props. A hand may touch only the explicitly named object; no limb, body, or object may pass through another solid object.',
  ...
  'Treat eventType as descriptive metadata, not a constraint on invention; keep every invention materially plausible in the photographed room.',
  ...
  'creativeCut means a deliberate hard cut to a newly generated but still realistic CCTV scene image in the same physical room and camera world as the source. It may change to another motivated fixed CCTV angle inside that room, but never to an exterior, a different building, a street, or an animal absent from the source. It is never a First/Last interpolation. realityIntrusion is false by default. Set it true only when a source cue deliberately breaks the fictional CCTV world into the live exhibition camera after this scene. It is a hard signal cut, never an interpolated camera move.',
...
export const getVisiblePeopleCount = (...contexts) => {
  const combinedContext = contexts
    .map((context) => normalizeString(context))
    .filter(Boolean)
    .join('\n');

  const countPatterns = [
    /\b(\d+)\s+visible\s+people\b/gi,
    /\bpeople\s*:\s*(\d+)\b/gi,
  ];

  return countPatterns.reduce((largestCount, pattern) => {
    for (const match of combinedContext.matchAll(pattern)) {
      largestCount = Math.max(largestCount, Number(match[1]) || 0);
    }
    return largestCount;
  }, 0);
};
...
export const validateReferenceImageActorScenePlan = (
  scenePlan = [],
  {
    ...
    allowCreativeCuts = false,
    requireMultiPersonInteraction = false,
  } = {}
) => {
  const issues = [];
  ...
  scenePlan.forEach((scene, index) => {
    ...
    if (requireMultiPersonInteraction && !normalizeSceneAction(scene?.actorsInteraction)) {
      issues.push(`${label}: multiple visible people require a concrete actorsInteraction`);
    }
    ...
    } else if (forceImageToVideoOnly && allowCreativeCuts && scene?.creativeCut === true) {
      if (videoMode !== 'singleImage' || frameSource !== 'newImage' || !freshImage || useCameraShot) {
        issues.push(`${label}: creativeCut must be newImage/singleImage/freshImage=true/useCameraShot=false`);
      }
...
export const parseScenePlan = (value, sceneCount = 3, resolvedSceneLengths = []) => {
  ...
  return parsed.scenes.slice(0, count).map((item, index) => {
    ...
    return {
      ...
      creativeCut: item?.creativeCut === true,
...
export const createSceneGenerator = ({
  ...
  return async ({
    ...
    const visiblePeopleCount = getVisiblePeopleCount(
      normalizedVisionStoryContext,
      storyTransport
    );
    const requireMultiPersonInteraction = visiblePeopleCount > 1;
    ...
    const requestRules = [
      'The current topic word is a private thematic undertone, never the stable subject, a character, a creature, a prop, visible text, or an instruction to add an exemplar of that word.',
      ...
      ...(requireMultiPersonInteraction
        ? [
            `Exactly ${visiblePeopleCount} people are visible. Every scene must contain a concrete actorsInteraction between at least two of them. Describe a reciprocal physical exchange, shared object, response, handoff, obstruction, touch, or gaze-and-reaction; parallel isolated actions do not count.`,
          ]
        : []),
      'locationAction and storyEvent describe a concrete cause and consequence in this room. They may be strange, but must remain an event between already visible people, architecture, or ordinary objects with a practical role.',
      ...
      ...(referenceImageActorMode
        ? [
            forceImageToVideoOnly
              ? (allowCreativeCuts
                ? 'Reference-image-actor state machine: scene 1 = newImage/singleImage/freshImage true/useCameraShot true. Choose exactly one later creativeCut=true scene: newImage/singleImage/freshImage true/useCameraShot false, a hard cut to a newly generated realistic CCTV angle inside the same physical source room. Keep its room geometry, architecture, lighting, and camera-world; never cut outdoors, to another building, street, or an animal absent from the source. Every other later scene = lastFrame/singleImage/false/false. firstLast is disabled for this run.'
            ...
            'Surreal changes are allowed inside the physical source room. Never replace its architecture or move the story to an exterior or unrelated location.',
          ]
        : []),
      ...(allowCreativeCuts
        ? []
        : ['Set creativeCut=false for every scene. Do not describe a hard cut or another CCTV angle.']),
