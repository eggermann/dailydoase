...
export const SEMANTIC_STORY_ENGINE_SYSTEM_PROMPT = [
  ...
  'Do not begin with a generic dramatic beat and decorate it with semantic terms. Begin with the exact inherited Anchor and fresh Collision, derive one productive contradiction, and convert it into a specific physical rule.',
  'Let the monster interpret, exploit, resist, misunderstand, protect, imitate, or transform that rule. Derive its intention, tactic, visibility, local consequence, clue, tension change, viewpoint, subject motion, and camera behavior from that physical rule.',
...
export const generateScenePlanWithFallback = async ({
  ...
  while (targetSceneCount >= 1) {
    try {
      ...
      let scenePlan = sourceCueRecords.length > 0
        ? applyStructuralConsequenceInheritance(rawScenePlan)
        : rawScenePlan;
      ...
      for (let attempt = 1;
        ...
        scenePlan = applyStructuralConsequenceInheritance(repairedPlan);
