...
export const DEFAULT_SCENE_SYSTEM_PROMPT = [
  ...
  'When configMode is "camera", treat the source screenshot as a stage for a curious spectacle shaped by the wordstream, not as a static frame to paraphrase.',
...
export const createSceneGenerator = ({
  ...
  return async ({
    ...
    const messages = [
      ...
      },
      {
        ...
        content: [
          ...
          `Trippy flavor rule: ${
            normalizeString(sceneFlavor, 'default').toLowerCase() === 'ltxtrippy'
    ...
    const requestScenePlan = async (client, activeModel) => client.chat.completions.create({
      model: activeModel,
      messages,
