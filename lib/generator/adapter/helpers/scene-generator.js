...
export const DEFAULT_SCENE_SYSTEM_PROMPT = [
  ...
  'For spatial story events, actorTrajectory = start position -> action -> end position; finalPose = visible end pose. Leave both empty for atmosphere.',
  ...
  'motionCue is subject movement. cameraCue is camera behavior; for CCTV use a fixed vantage or hard cut, never operator movement.',
  'videoPrompt and singleImagePrompt: 2-3 production sentences. Include trajectory/final pose for spatial events; never schema or meta instructions.',
      ...
      scenes: {
        ...
        items: {
          ...
          properties: {
            ...
            actorTrajectory: { type: 'string' },
            finalPose: { type: 'string' },
...
export const parseScenePlan = (value, sceneCount = 3, resolvedSceneLengths = []) => {
  ...
  return parsed.scenes.slice(0, count).map((item, index) => {
    ...
    const actorTrajectory = normalizeSceneAction(item?.actorTrajectory);
    const finalPose = normalizeSceneAction(item?.finalPose);
