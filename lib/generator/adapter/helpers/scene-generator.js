...
export const DEFAULT_SCENE_SYSTEM_PROMPT = [
  ...
  'Set requiresDocumentaryEndFrame=true for later visible object, material, liquid, projection, or room changes; describe physical scale, contact, occlusion, light, and texture, then use firstLast. Keep it false for motion of visible contents; singleImage must not invent major props or graphic effects.',
  'motionCue describes physical subject or room movement; cameraCue describes only the CCTV viewpoint. For CCTV cameraCue may say locked viewpoint, but never use quiet, still, static, steady, holding, or no movement to describe the person or room event.',
  'exitMotion describes the visible movement already underway at this scene\'s final frame. State subject or room, direction or destination, body state, and momentum in one short sentence. The next scene receives it as incoming motion. Leave it empty only when this scene deliberately settles.',
      ...
      scenes: {
        ...
        items: {
          ...
          properties: {
            ...
            requiresDocumentaryEndFrame: { type: 'boolean' },
...
export const parseScenePlan = (value, sceneCount = 3, resolvedSceneLengths = []) => {
  ...
  return parsed.scenes.slice(0, count).map((item, index) => {
    ...
    return {
      ...
      exitMotion: normalizeString(item?.exitMotion, ''),
      requiresDocumentaryEndFrame: item?.requiresDocumentaryEndFrame === true,
...
// This is deliberately plain. It is only used when both planning models are
// unavailable; a live exhibition should keep producing camera-grounded clips
// instead of waiting forever for a text-planning service.
export const createEmergencyScenePlan = ({
  sceneCount = 1,
  sceneLengths = [],
  sourceCues = [],
  visionStoryContext = '',
  ...
  const count = clampSceneCount(sceneCount);
  const lengths = Array.isArray(sceneLengths) ? sceneLengths : [];
  const cues = sourceCues.map((cue) => normalizeString(cue)).filter(Boolean);
  const visibleContext = normalizeString(visionStoryContext, 'the visible visitor and exhibition room');
  const referenceImageActorMode = isReferenceImageActorMode(configMode);
  const sceneForms = [
    {
      title: 'Threshold response',
      eventType: 'actorToLocation',
      actorAction: 'The visible person enters the nearest open passage and orients toward an existing display.',
      actorsInteraction: '',
      locationAction: 'The person crosses the existing doorway or aisle boundary toward the nearest visible room feature.',
      storyEvent: 'The semantic cue becomes a visible choice of direction through the actual room.',
      motionCue: 'The person moves one clear step from the threshold toward the display, momentum continuing.',
      exitMotion: 'The person remains in motion toward the existing room feature.',
      cameraCue: 'Continue from the current CCTV frame; preserve its real left-right room layout while the threshold movement becomes visible.',
    },
    {
      title: 'Encounter at the display',
      eventType: 'actorToActor',
      actorAction: 'The visible person pauses at the nearest display and turns their body toward another already visible person or reflection.',
      actorsInteraction: 'Two already visible people acknowledge the same existing room feature through gaze, distance, or a small hand gesture.',
      locationAction: 'The existing display, pedestal, or wall feature becomes the physical point between the people.',
      storyEvent: 'The semantic cue becomes an observable exchange around one real room feature.',
      motionCue: 'Bodies turn inward toward the display, with a small gesture carrying into the next shot.',
      exitMotion: 'The exchange remains unresolved as one person begins to move away from the display.',
      cameraCue: 'Continue the current CCTV coverage; let the changed body relation, not a fake camera move, make the new scene.',
    },
    {
      title: 'Room feature answers',
      eventType: 'locationToActor',
      actorAction: 'The visible person notices a small change in light, reflection, or material at an already visible room feature.',
      actorsInteraction: '',
      locationAction: 'An existing surface, display, window, or light source creates a physical cue that draws the person closer.',
      storyEvent: 'The semantic cue changes how the existing room feature catches light, contact, or attention.',
      motionCue: 'The person leans or steps toward the existing feature while its visible change continues.',
      exitMotion: 'Attention and body momentum remain directed toward the room feature.',
      cameraCue: 'Use the current CCTV composition as evidence; preserve room geometry and make the changed relation legible in deep focus.',
    },
    {
      title: 'Aisle departure',
      eventType: 'actorOnly',
      actorAction: 'The visible person leaves the immediate display area along an existing aisle, carrying the scene consequence in posture or hands.',
      actorsInteraction: '',
      locationAction: 'The existing aisle or doorway gives the departure a clear destination without inventing a new location.',
      storyEvent: 'The semantic cue becomes a visible departure that leaves one consequence behind in the room.',
      motionCue: 'The person walks along the existing aisle with a clear direction and continuing momentum.',
      exitMotion: 'The person continues out along the aisle, leaving the next scene a readable trace.',
      cameraCue: 'Keep the current CCTV framing and show the departure across its actual floor geometry.',
    },
  ];

  return Array.from({ length: count }, (_, index) => {
    const cue = cues[index % Math.max(1, cues.length)] || 'the exhibition room';
    const form = sceneForms[index % sceneForms.length];
    const openingScene = index === 0;
    const useCameraShot = referenceImageActorMode && openingScene;
    const frameSource = openingScene ? 'newImage' : 'lastFrame';
    const prompt = [
      `Documentary CCTV view of ${visibleContext}.`,
      `Semantic anchor: ${cue}. ${form.actorAction}`,
      `${form.locationAction} Keep person identity, room geometry, lighting, and the current CCTV layout coherent; show real physical motion.`,
    ].join(' ');

    return {
      index: index + 1,
      title: `${form.title}: ${cue}`,
      beat: `${form.storyEvent} Semantic anchor: ${cue}.`,
      stillPrompt: prompt,
      imageDescription: `${form.locationAction} Semantic anchor: ${cue}.`,
      storyBeat: `${form.storyEvent} The visible consequence of ${cue} carries into the next scene.`,
      eventType: form.eventType,
      actorAction: form.actorAction,
      actorsInteraction: form.actorsInteraction,
      locationAction: form.locationAction,
      storyEvent: `${form.storyEvent} Semantic anchor: ${cue}.`,
      castSelection: [],
      castUse: '',
      motionCue: form.motionCue,
      exitMotion: form.exitMotion,
      requiresDocumentaryEndFrame: false,
      cameraCue: form.cameraCue,
      frameSource,
      videoMode: 'singleImage',
      durationSeconds: normalizeSceneLengthValue(lengths[index], 3),
      videoPrompt: prompt,
      singleImagePrompt: prompt,
      freshImage: useCameraShot,
      useCameraShot,
      realityIntrusion: false,
    };
  });
};
...
export const generateScenePlanWithFallback = async ({
  ...
  while (targetSceneCount >= 1) {
    ...
    try {
      ...
      if (!mismatch) {
        if (typeof emergencyScenePlan === 'function') {
          const scenePlan = await emergencyScenePlan({
            error,
            sceneCount: targetSceneCount,
            sceneLengths: activeSceneLengths || await resolveSceneLengthsInput(sceneLengths, targetSceneCount, 3),
            configMode,
            sceneFlavor,
            visualDirection,
            visionStoryContext,
          ...
          if (Array.isArray(scenePlan) && scenePlan.length === targetSceneCount) {
            return {
              scenePlan,
              effectiveSceneCount: targetSceneCount,
              effectiveSceneLengths: activeSceneLengths || await resolveSceneLengthsInput(sceneLengths, targetSceneCount, 3),
            };
          }
        }
        throw error;
