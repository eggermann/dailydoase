...
export const DEFAULT_SCENE_SYSTEM_PROMPT = [
  ...
  'motionTransport states the screen/body direction that continues into the next scene, for example "left-to-right toward the table; still approaching". Keep it from the preceding scene. Reverse only when this scene visibly names a turn, stop, retreat, collision, or exit.',
...
export const parseScenePlan = (value, sceneCount = 3, resolvedSceneLengths = []) => {
  ...
  return parsed.scenes.slice(0, count).map((item, index) => {
    ...
    return {
      ...
      motionTransport: normalizeString(item?.motionTransport, ''),
...
export const createSceneGenerator = ({
  ...
  return async ({
    ...
    const requestRules = [
      ...
      'CAST MEMORY is optional. For each scene, choose at most three listed cast ids in castSelection when past people or frames help the story; otherwise return an empty array. castUse explains the selected use without imposing identity continuity.',
...
export const buildFallbackVideoPrompt = (scenePlanEntry, fallbackText) => compactScenePrompt(
  [
    ...
    normalizeString(scenePlanEntry?.motionTransport),
