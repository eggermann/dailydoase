...
// Semantic streams and cast memory can contain long scraped fragments. The
// planner needs their meaning, not several pages of source text: compact input
// keeps a live structured request below the latency cliff.
...
// A live exhibition must continue when both text planners are unavailable.
// This plain, camera-grounded plan keeps the visitor and room readable; it is
// deliberately used only after both normal planning attempts failed.
export const createEmergencyScenePlan = ({
  ...
  const useCameraShot = isReferenceImageActorMode(configMode);
  const topicMatch = normalizeString(storyTransport).match(/Current topic word:\s*([^\.]+)\./i);
  const semanticAnchor = normalizeString(topicMatch?.[1])
    || normalizeString(sourceCues[0], 'the exhibition room');
  const stages = [
    {
      title: 'Semantic signal',
      eventType: 'locationToActor',
      action: 'The visitor notices and leans toward the altered room feature.',
      locationAction: 'An existing display, light, or circulation path takes on a wordless material form of the semantic anchor.',
      consequence: 'The anchor becomes a concrete, visible pressure inside the room.',
      motion: 'The visitor pauses, turns toward the changed feature, and takes one cautious step.',
    },
    {
      title: 'Semantic encounter',
      eventType: 'actorToLocation',
      action: 'The visitor tests the changed feature with a deliberate gesture.',
      locationAction: 'The existing room feature visibly answers the gesture without changing the room geometry.',
      consequence: 'The semantic anchor shifts the relation between body and display.',
      motion: 'A hand, shoulder, or gaze reaches toward the feature and then recoils or follows it.',
    },
    {
      title: 'Semantic consequence',
      eventType: 'locationToActor',
      action: 'The visitor commits to a new position created by the encounter.',
      locationAction: 'The altered feature redirects attention toward a different existing part of the room.',
      consequence: 'The anchor leaves a visible residue that can begin the next iteration.',
      motion: 'The visitor crosses a short distance and settles into a changed posture facing the consequence.',
    },
  ];
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const prompt = [
      ...
      `Semantic anchor: ${semanticAnchor}.`,
      `${stage.locationAction} ${stage.action}`,
      'Make the anchor a wordless, physical event rather than text, a logo, or an explanation. Keep the person, room geometry, lighting, and camera position coherent; show real physical motion.',
    ].join(' ');

    return {
      index: index + 1,
      title: `${semanticAnchor}: ${stage.title}`,
      beat: `${stage.consequence} Semantic anchor: ${semanticAnchor}.`,
      stillPrompt: prompt,
      imageDescription: `${stage.consequence} The live camera subject and room remain visible; semantic anchor: ${semanticAnchor}.`,
      storyBeat: `${stage.consequence} The visitor's next action follows from ${semanticAnchor}.`,
      eventType: stage.eventType,
      actorAction: stage.action,
      actorsInteraction: '',
      locationAction: stage.locationAction,
      storyEvent: `${stage.consequence} Semantic anchor: ${semanticAnchor}.`,
      castSelection: [],
      castUse: '',
      motionCue: stage.motion,
      exitMotion: 'The changed relationship remains visible and leads directly into the next scene.',
      requiresDocumentaryEndFrame: false,
      cameraCue: 'Fixed surveillance-camera viewpoint; the event unfolds inside one readable wide shot.',
      frameSource: openingScene ? 'newImage' : 'lastFrame',
      videoMode: 'singleImage',
      durationSeconds: normalizeSceneLengthValue(lengths[index], 3),
      videoPrompt: prompt,
      singleImagePrompt: prompt,
      freshImage: useCameraShot && openingScene,
      useCameraShot: useCameraShot && openingScene,
