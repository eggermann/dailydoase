...
export const repairSemanticScenePlanEntry = async ({
  ...
  const repairPrompt = [
    'Repair only the supplied scene.',
    'Do not consume or invent another Semantic Stream term.',
    'The exact semanticAnchor and semanticCollision are immutable.',
    'Preserve valid fields unless they conflict with the reported errors.',
    'Return one corrected scene object only.',
    `Structured cue record: ${JSON.stringify(cueRecord)}`,
    `Invalid scene: ${JSON.stringify(invalidScene)}`,
    `Validation errors: ${JSON.stringify(validationErrors)}`,
    `Previous scene consequence: ${JSON.stringify(previousScene ? {
      consequenceId: previousScene.consequenceId,
      localConsequence: previousScene.localConsequence,
      endFrameContinuity: previousScene.endFrameContinuity,
    } : null)}`,
    `Next scene inheritance requirements: ${JSON.stringify(nextScene ? {
      inheritsConsequenceId: nextScene.inheritsConsequenceId,
      inheritedConsequence: nextScene.inheritedConsequence,
    } : null)}`,
...
export const generateScenePlanWithFallback = async ({
  ...
  while (targetSceneCount >= 1) {
    try {
      ...
      const rawScenePlan = await generateScenes({
        ...
        visionStoryContext,
        sourceCues,
        sourceCueRecords: sourceCueRecords.slice(0, targetSceneCount),
        startFrameStrategy,
      });

      let scenePlan = rawScenePlan;
      let semanticValidation = sourceCueRecords.length > 0
        ? validateSemanticDrivenScenePlan({
            scenePlan,
            sourceCueRecords: sourceCueRecords.slice(0, targetSceneCount),
          })
      ...
      for (let attempt = 1;
        semanticValidation && !semanticValidation.valid && attempt <= maxSemanticRepairAttempts;
        attempt += 1) {
        const repairedPlan = [...scenePlan];
        for (const report of semanticValidation.reports.filter((item) => item.sceneIndex >= 0 && !item.valid)) {
          const sceneIndex = report.sceneIndex;
          const [repairedScene] = await generateScenes({
            sceneCount: 1,
            sceneLengths: [activeSceneLengths[sceneIndex]],
            configMode,
            sceneFlavor,
            visualDirection,
            visionStoryContext,
            sourceCues: [sourceCues?.[sceneIndex] || ''],
            sourceCueRecords: [sourceCueRecords[sceneIndex]],
            startFrameStrategy,
            repairContext: {
              attempt,
              cueRecord: sourceCueRecords[sceneIndex],
              invalidScene: scenePlan[sceneIndex],
              validationErrors: report.errors,
              previousScene: scenePlan[sceneIndex - 1] || null,
              nextScene: scenePlan[sceneIndex + 1] || null,
            },
          });
          repairedPlan[sceneIndex] = { ...repairedScene, index: sceneIndex + 1 };
        }
        scenePlan = repairedPlan;
        semanticValidation = validateSemanticDrivenScenePlan({
          scenePlan,
          sourceCueRecords: sourceCueRecords.slice(0, targetSceneCount),
        });
      }

      if (semanticValidation && !semanticValidation.valid && strictSemanticValidation) {
        const error = new Error('Semantic scene validation failed after targeted repair');
        error.name = 'SemanticSceneValidationError';
        error.scenePlan = scenePlan;
        error.rawScenePlan = rawScenePlan;
...
export const createSceneGenerator = ({
  ...
  const configuredSystemPrompt = normalizeString(systemPrompt, DEFAULT_SCENE_SYSTEM_PROMPT);
  const resolvedSystemPrompt = configuredSystemPrompt.includes(
    'the live Semantic Stream collision is the sole source of new story content'
  )
    ? configuredSystemPrompt
    : `${SEMANTIC_STORY_ENGINE_SYSTEM_PROMPT} ${configuredSystemPrompt}`;
  ...
  return async ({
    ...
    const repairMessage = repairContext
      ? [
          'TARGETED SEMANTIC REPAIR: Return exactly one corrected scene.',
          `Original cue record: ${JSON.stringify(repairContext.cueRecord)}`,
          `Invalid scene: ${JSON.stringify(repairContext.invalidScene)}`,
          `Exact validation errors: ${JSON.stringify(repairContext.validationErrors)}`,
          `Previous scene: ${JSON.stringify(repairContext.previousScene)}`,
          `Next scene: ${JSON.stringify(repairContext.nextScene)}`,
          'Repair only the supplied scene. Do not consume or invent another Semantic Stream term.',
          'The exact semanticAnchor and semanticCollision are immutable.',
          'Preserve valid fields unless they conflict with the reported errors.',
          'Repair only listed causal and continuity failures. Preserve valid location, timing, and production constraints.',
        ].join('\n')
      : '';
