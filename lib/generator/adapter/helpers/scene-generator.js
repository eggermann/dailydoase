...
export const generateScenePlanWithFallback = async ({
  ...
  if (!semanticValidation.valid) {
    scenePlan = await generateScenes({
      ...plannerRequest,
      repairContext: {
        invalidScenePlan: rawScenePlan,
        validationErrors: semanticValidation.errors,
      },
    });
    semanticValidation = validateScenePlan({
      scenePlan,
  ...
  if (!semanticValidation.valid && strictSemanticValidation) {
    const error = new Error('Scene plan failed structural validation after one whole-plan repair');
...
export const createSceneGenerator = ({
  ...
  return async ({
    ...
    const repairMessage = repairContext
      ? [
          'Scene plan has structural JSON errors. Correct only listed problems.',
          'Preserve creative ideas. Do not change semanticAnchor or semanticCollision.',
          'Do not request or invent new Semantic Stream terms.',
          `Errors: ${JSON.stringify(repairContext.validationErrors)}`,
          `Original scene plan: ${JSON.stringify(repairContext.invalidScenePlan)}`,
          'Return complete corrected JSON scene array only.',
        ].join('\n')
      : '';
    ...
    const requestScenePlan = async (client, activeModel) => {
      ...
      const request = {
        model: activeModel,
        messages,
        response_format: { type: 'json_schema', json_schema: COMPACT_SCENE_PLAN_SCHEMA },
        ...samplingOptions,
      };
