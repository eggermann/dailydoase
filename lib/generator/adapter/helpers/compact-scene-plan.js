...
export const COMPACT_SCENE_PLAN_SYSTEM_PROMPT = [
  ...
  'Every scene uses a transformationMechanism materially different from the preceding scene. Each scene leaves one concrete consequence. The next scene must begin with that exact pose, object placement, residue, light direction and shadow state before its fresh collision redirects, contradicts, transfers, reveals or resolves it.',
  'Build one accumulating narrative action, not six adjacent examples. Make nextHook a physical handoff that is visibly present in the following stillPrompt and starts moving in the following videoPrompt.',
  'For cameraCue, describe only viewpoint movement: organic micro-sway, hesitant reframe, slight searching pan, cautious shoulder-height sidestep, short approach or short retreat. Give movement an imperfect settle and motivate it by the event. Do not describe a device, person recording, hands, phone, camera body, screen, viewfinder, filming or recording. Do not describe orbiting, flying, floating, crane, drone, gimbal or movement through solid objects.',
  'Preserve the original Kaufhaus light family while practical fluorescent light pulses subtly, reflections tremble and shadows move across real surfaces for a lightly uncanny live-image texture.',
