...
test('visionModel accepts localMistral as an alias for LM Studio', async () => {
  const fetchImpl = jest.fn().mockResolvedValue({
    ok: true,
    status: 200,
    statusText: 'OK',
...
test('visionModel uses local vision when primary OpenAI vision is unavailable', async () => {
  const fetchImpl = jest.fn().mockResolvedValue({
    ok: true,
    status: 200,
    statusText: 'OK',
    text: async () => JSON.stringify({
      choices: [{ message: { content: 'A real person stands in the exhibition room.' } }],
    }),
  });
  const openaiCreate = jest.fn().mockRejectedValue(new Error('OpenAI temporarily unavailable'));

  const result = await visionModel(
    {
      imageUrl: 'https://example.com/portrait.jpg',
      prompt: 'Describe only the visible person.',
  ...
  expect(result).toEqual(expect.objectContaining({
    provider: 'lmstudio',
    model: 'qwen3-vl',
    outputText: expect.stringContaining('real person'),
  }));
});
