...
test('buildCameraGroundedPrompt injects location and actor continuity into camera prompts', () => {
  ...
  expect(prompt).toContain(DOCUMENTARY_MATERIAL_REALISM_SENTENCE);
...
test('buildCameraGroundedPrompt binds action to observed actor and keeps CCTV lock camera-only', () => {
  const prompt = buildCameraGroundedPrompt({
    basePrompt: 'A man rises slowly while the static camera watches.',
    actorAction: 'Rises from the chair, carries the magazine to the display, and lifts the varnish bottle.',
    locationAction: 'His fingertips leave a glossy trace on the bottle.',
    cameraCue: 'Fixed high-corner CCTV; no tracking, static composition.',
    cameraStyle: 'Real CCTV footage with a fixed high security-camera angle and steady composition.',
    startVision: START_VISION,
    useSingleImage: true,
  });

  expect(prompt).toContain('Bald man with glasses and a dark sweater rises from the chair');
  expect(prompt).toContain('Inside this locked viewpoint, the planned subject or room event evolves continuously');
  expect(prompt).not.toMatch(/\b(?:static|steady|no tracking|fixed\s+(?:high|camera|CCTV))\b/i);
  expect(prompt).not.toContain('leaving a.');
});
