...
test('short single-image shots explicitly forbid frozen tableau motion', () => {
  const prompt = buildCameraGroundedPrompt({
    basePrompt: 'The visitor picks up the paper and walks toward the register.',
    storyBeat: 'The paper reroutes the visitor.',
    durationSeconds: 3,
    useSingleImage: true,
  });

  expect(prompt).toContain('Motion rule: action starts immediately');
  expect(prompt).toContain('no frozen tableau or still-image hold');
});
