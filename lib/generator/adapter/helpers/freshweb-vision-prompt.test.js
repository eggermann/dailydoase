...
import {
  AUTHENTIC_WEBCAM_REALITY_CONTRACT,
...
test('camera prompts keep invented people inside authentic webcam reality', () => {
  const prompt = buildCameraGroundedPrompt({
    basePrompt: 'A newly invented woman appears behind the film-strip structure.',
    storyBeat: 'The archival image becomes a visitor in the room.',
    startVision: START_VISION,
    useSingleImage: true,
    cameraStyle: 'Fixed exhibition webcam.',
  });

  expect(prompt).toContain(AUTHENTIC_WEBCAM_REALITY_CONTRACT);
  expect(prompt).toContain('A newly invented woman appears behind the film-strip structure');
  expect(prompt).toContain('Fixed exhibition webcam');
});
...
test('default freshweb vision prompt requests Location, Actors, Assets, and Description', () => {
  ...
  expect(DEFAULT_FRESHWEB_VISION_PROMPT).toContain('Assets must be a JSON array');
  expect(DEFAULT_FRESHWEB_VISION_PROMPT).toContain('position');
  expect(DEFAULT_FRESHWEB_VISION_PROMPT).toContain('orientation');
  expect(DEFAULT_FRESHWEB_VISION_PROMPT).toContain('two people');
});

test('custom vision prompts still require structured actor placement', () => {
  const prompt = resolveFreshwebVisionPrompt('Describe strict room continuity.');

  expect(prompt).toContain('Describe strict room continuity.');
  expect(prompt).toContain('Actors must be a JSON array');
  expect(prompt).toContain('position');
  expect(prompt).toContain('orientation');
  expect(prompt).toContain('Assets must be a JSON array');
});

test('extractVisionStoryContext keeps structured room assets', () => {
  const context = extractVisionStoryContext(JSON.stringify({
    Location: 'former department-store floor',
    Actors: [],
    Assets: [
      { reference: 'service counter', description: 'long pale counter', position: 'midground right' },
      { reference: 'painting', description: 'large abstract canvas', position: 'background left' },
    ],
  }));
  ...
  expect(summarizeVisionStoryContext(context)).toContain('Assets:');
});
...
test('extractVisionStoryContext accepts valid JSON followed by model commentary', () => {
  const context = extractVisionStoryContext(`{
    "Subject": "A bald man in a black tank top",
    "Setting": "A white exhibition room",
    "Actors": [{
      "reference": "man",
      "description": "A bald man in a black tank top",
      "position": "midground center",
      "orientation": "back-facing"
    }]
  }
  what should stay consistent for the next video shot`);

  expect(context.peopleCount).toBe(1);
  expect(context.actors).toEqual([{
    reference: 'man',
    description: 'A bald man in a black tank top',
    position: 'midground center',
  ...
  expect(context.location).toBe('A white exhibition room');
});
...
test('extractVisionStoryContext preserves two people with positions and orientations', () => {
  const context = extractVisionStoryContext([
    'Location: old department-store floor.',
    'Actors: [{"reference":"person 1","description":"visitor in a dark coat","position":"foreground left","orientation":"front-facing"},{"reference":"person 2","description":"visitor near the back wall","position":"background right","orientation":"side-facing"}].',
    'Description: two visitors stand at different depths in the old sales floor.',
  ].join(' '));

  expect(context.peopleCount).toBe(2);
  expect(context.actors).toEqual([
    {
      reference: 'person 1',
  ...
  const summary = summarizeVisionStoryContext([
    'Location: old department-store floor.',
    'Actors: [{"reference":"person 1","description":"visitor in a dark coat","position":"foreground left","orientation":"front-facing"},{"reference":"person 2","description":"visitor near the back wall","position":"background right","orientation":"side-facing"}].',
  ].join(' '));
  expect(summary).toContain('People: 2.');
  expect(summary).toContain('"position":"foreground left"');
  expect(summary).toContain('"orientation":"side-facing"');
});

test('extractVisionStoryContext parses a complete top-level JSON vision response', () => {
  const context = extractVisionStoryContext(JSON.stringify({
    Subject: { description: 'a bald visitor in a dark coat' },
    Setting: { description: 'an old white department-store floor' },
    Location: { description: 'the Glas Kaufhaus exhibition' },
    Actors: [{
      reference: 'visitor',
      description: 'a bald visitor in a dark coat',
      position: 'foreground right',
      orientation: 'front-facing',
    }],
    Description: 'The visitor looks into the camera.',
  }));
  ...
  expect(context.location).toBe('the Glas Kaufhaus exhibition');
});

test('extractVisionStoryContext recovers actors from malformed camera pseudo-JSON', () => {
  const context = extractVisionStoryContext('```json { "Subject": "Art studio workspace", "Setting": "Indoor art studio space with unfinished paintings on walls and a desk setup", "Framing": "Wide shot from a low angle", "Lighting": Natural light, Soft overhead lighting, "Actors": [ { "reference": "Artist", "description": "A person wearing a black sleeveless top", "position": "Foreground left", "orientation": "Side-facing" } ], "Description": A lone artist stands by an unfinished painting. } ```');

  expect(context.peopleCount).toBe(1);
  expect(context.location).toBe('Indoor art studio space with unfinished paintings on walls and a desk setup');
  expect(context.actors).toEqual([{
    reference: 'artist',
    description: 'A person wearing a black sleeveless top',
    position: 'foreground left',
    orientation: 'side-facing',
  }]);
});
...
test('buildCameraGroundedPrompt injects location and actor continuity into camera prompts', () => {
  ...
  expect(prompt).toContain('additional story-introduced people may enter as physically integrated webcam subjects');
...
test('buildCameraGroundedPrompt compiles LTX trippy prompts as prompt-dominant surreal scene play', () => {
  ...
  expect(prompt).toContain('Keep the source camera orientation unmirrored');
  expect(prompt).toContain('Keep the camera motion uneasy, unstable, and hallucinatory');
  expect(prompt).not.toContain('Close-up shot focusing on the subject’s face and upper chest');
  expectNaturalCameraPrompt(prompt);
});
...
test('buildCameraGroundedPrompt can describe remote reference images without webcam wording', () => {
  const prompt = buildCameraGroundedPrompt({
    basePrompt: 'The subject turns toward the doorway.',
    storyBeat: 'The subject turns toward the doorway.',
    imageDescription: 'A bald man with glasses in a calm indoor room.',
    startVision: [
      'Location: A calm indoor room.',
      'Actors: A bald man with glasses.',
      'Description: The subject turns toward the doorway.',
    ].join(' '),
    useSingleImage: true,
    promptFlavor: 'ltxTrippy',
  ...
  expect(prompt).toContain('source realism');
  expect(prompt).not.toContain('webcam shot');
  expectNaturalCameraPrompt(prompt);
});
