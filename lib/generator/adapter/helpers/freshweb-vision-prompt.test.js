...
test('buildCameraGroundedPrompt injects location and actor continuity into camera prompts', () => {
  ...
  expect(prompt).toContain('Keep continuity with the same actor, same setting, and same shot family');
  expect(prompt).toContain('Keep the location fixed in an industrial-style room');
  expect(prompt).toContain('If already visible, keep these anchors readable');
  expect(prompt).toContain('shadowy figure');
  expect(prompt).toContain('Drive the shot toward');
  expect(prompt).toContain('By the end, the same actor and same setting should still read clearly');
});
...
test('buildCameraGroundedPrompt keeps multiple actor identities available', () => {
  ...
  expect(prompt).toContain('Keep continuity with the same actors, same setting, and same shot family');
  expect(prompt).toContain('Keep the location fixed in the same office');
  ...
  expect(prompt).toContain('Let this held shot settle into');
  expect(prompt).toContain('Avoid a frozen hold');
});
...
test('buildCameraGroundedPrompt keeps dynamic transition energy when a single-image scene came from firstLast planning', () => {
  ...
  expect(prompt).toContain('Translate the scene change into one continuous shot');
  expect(prompt).toContain('camera pushes with him toward the window');
});
