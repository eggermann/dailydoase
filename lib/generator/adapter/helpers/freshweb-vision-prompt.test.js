...
test('buildCameraGroundedPrompt can leave people presence open for a non-human source', () => {
  const prompt = buildCameraGroundedPrompt({
    basePrompt: 'Fluorescent tubes bend around the green monster.',
    storyBeat: 'Fluorescent tubes bend around the green monster.',
    useSingleImage: true,
...
test('buildCameraGroundedPrompt preserves complete WAN cinematic direction when requested', () => {
  const prompt = buildCameraGroundedPrompt({
    basePrompt: 'The green organism begins motionless beneath jaundiced fluorescent light. Mirrored columns breathe outward while visitors drift backward, wet concrete reflections pulse amber, and a slow low-angle dolly arcs left without cuts.',
    storyBeat: 'The Kaufhaus becomes a breathing archive.',
    motionCue: 'columns expand and visitors drift backward',
    cameraCue: 'slow low-angle dolly arcs left',
    startVision: 'Location: old Kaufhaus hall. Actors: none visible.',
    useSingleImage: true,
    preserveBasePrompt: true,
    lockActorCount: false,
  });

  expect(prompt).toContain('jaundiced fluorescent light');
  expect(prompt).toContain('wet concrete reflections pulse amber');
  expect(prompt).toContain('slow low-angle dolly arcs left without cuts');
});
