...
const STOP_WORDS = new Set([
  'same', 'keep', 'maintain', 'consistency', 'consistent', 'with', 'the', 'and',
  'for', 'next', 'shot', 'angle', 'framing', 'subject', 'setting', 'actor',
  'actors', 'location', 'readable', 'visible', 'identity'
]);

const ACTION_GENERIC_WORDS = new Set([
  'action', 'hold', 'holds', 'holding', 'then', 'into', 'toward', 'towards',
  'look', 'looks', 'looking', 'glance', 'glances', 'glancing', 'notice',
  'notices', 'noticing', 'brace', 'braces', 'bracing', 'rise', 'rises',
  'rising', 'cross', 'crosses', 'crossing', 'startle', 'startles',
  'startling', 'leans', 'lean', 'leaning', 'stares', 'staring', 'faces',
  'facing', 'drives', 'driving', 'pushes', 'pushing', 'moves', 'moving',
  'turns', 'turning', 'back', 'over', 'after', 'before', 'through', 'from',
  'camera', 'scene', 'still', 'close', 'closeup', 'glimpse', 'moment',
  'himself', 'herself', 'myself', 'yourself', 'itself', 'themselves',
  'shoulder', 'shoulders', 'head', 'face', 'eyes', 'hands', 'hand',
  'arms', 'arm', 'body', 'torso'
]);
...
const extractMeaningfulKeywords = (value) => normalizeVisionText(value)
  .toLowerCase()
  .replace(/[^a-z0-9\s]/g, ' ')
  .split(/\s+/)
  .filter((word) => word.length >= 4 && !STOP_WORDS.has(word));

const extractVisibleActionKeywords = (value) => normalizeVisionText(value)
  .toLowerCase()
  .replace(/[^a-z0-9\s]/g, ' ')
  .split(/\s+/)
  .filter((word) => (
...
const shouldIncludeCameraAction = ({
  ...
  const actionKeywords = extractVisibleActionKeywords(basePrompt);
  const visibleKeywords = new Set([
    ...extractVisibleActionKeywords(actorsSummary),
    ...extractVisibleActionKeywords(locationSummary),
    ...extractVisibleActionKeywords(readableState),
    ...extractVisibleActionKeywords(endVisibleState),
    ...extractVisibleActionKeywords(motionCue),
    ...extractVisibleActionKeywords(cameraCue),
  ]);

  const introducesUnsupportedElements = actionKeywords.some((keyword) => !visibleKeywords.has(keyword));
...
export const buildCameraGroundedPrompt = ({
  ...
  if (shouldIncludeCameraAction({
    ...
    parts.push(buildLabeledPromptSentence('Action:', actionText, 2, 28));
  }
  if (timingSentence) {
  ...
  if (continuityConstraint && shouldIncludeContinuityConstraint({
    continuityText: startContinuity,
    actorsSummary: startActors,
    locationSummary: startLocation,
    readableState,
  ...
  return compactScenePrompt(parts.join(' '), 10, 140);
};
