...
export const AUTHENTIC_WEBCAM_REALITY_CONTRACT = [
  'Authentic unedited low-cost webcam footage',
  'all people, including invented or transformed people, look physically present through the same lens, perspective, exposure, focus, compression, room light, shadows, and occlusion',
  'surreal events stay photoreal, never illustration, CGI, collage, cutout, or separately lit portrait',
].join('; ');

const withAuthenticWebcamReality = (prompt = '') => {
  const combined = [
...
const buildActorContinuitySentence = ({
  actorCount = 0,
  ...
  if (actorCount === 1) {
    if (strengthenIdentity) {
      return 'Keep the same real person from the source frame recognizable with the same face and clothes; additional story-introduced people may enter as physically integrated webcam subjects.';
    }
    return '';
  }

  return 'Keep all source people recognizable while allowing story-introduced people to enter as physically integrated webcam subjects.';
};
...
export const FRESHWEB_ROOM_ASSET_PROMPT = [
  'Assets must be a JSON array of stable visible room features using keys reference, description, and position.',
  'Include useful story anchors such as existing furniture, artworks, windows, doors, counters, surfaces, fixtures, and light sources.',
  'Do not list a person or an object visible only inside artwork, posters, mirrors, or screens as a real room asset.',
].join(' ');

export const DEFAULT_FRESHWEB_VISION_PROMPT = [
  'Describe only the visible shot for continuity.',
  'Return concise labeled lines for Subject, Setting, Framing, Lighting, Location, Actors, Assets, Description, and what should stay consistent for the next video shot.',
  FRESHWEB_ACTOR_PLACEMENT_PROMPT,
  FRESHWEB_ROOM_ASSET_PROMPT,
].join(' ');
...
export const resolveFreshwebVisionPrompt = (...values) => {
  ...
  const withActors = basePrompt.includes('Actors must be a JSON array')
    ? basePrompt
    : `${basePrompt} ${FRESHWEB_ACTOR_PLACEMENT_PROMPT}`;
  return withActors.includes('Assets must be a JSON array')
    ? withActors
    : `${withActors} ${FRESHWEB_ROOM_ASSET_PROMPT}`;
};
...
export const buildCameraGroundedPrompt = ({
  ...
  const actionText = compactScenePrompt(normalizeVisionText(basePrompt), 2, 28);
  const motion = normalizeVisionText(motionCue);
  const camera = normalizeVisionText(cameraCue);
  const cameraStyleSentence = compactScenePrompt(normalizeVisionText(cameraStyle), 1, 36);
  ...
  const actorVisible = Boolean(startActors);
  const actorCountLockSentence = buildActorContinuitySentence({
  ...
  const includeAction = shouldIncludeCameraAction({
  ...
  if (useSingleImage && String(promptFlavor || '').toLowerCase() === 'ltx') {
    ...
    return withAuthenticWebcamReality(compactScenePrompt([
      setupSentence,
  ...
  ].filter(Boolean).join(' '), 5, useSingleImage ? 126 : 110);

  if (prompt) {
    return withAuthenticWebcamReality([
      prompt,
