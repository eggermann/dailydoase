...
const stripPromptLead = (value) => trimDanglingConnector(
  String(value || '')
    .replace(/^in\s+/i, '')
...
const buildActorIdentityAnchor = ({
  ...
  if (!actorText) {
    return webcam
      ? 'the same real person from the webcam shot'
      : 'the same real person from the source frame';
  }
  return webcam
    ? `${actorText}, the exact same real person from the webcam shot`
    : `${actorText}, the exact same real person from the source frame`;
};
...
const buildLtxTrippySetupSentence = ({
  ...
  const identityAnchor = actorText
    ? buildActorIdentityAnchor({ actorsSummary, maxWords: 10, webcam: true })
    : 'the same real person from the webcam shot';

  return compactScenePrompt(
    `Start from ${visibleLead} and let the image break away from plain webcam realism; keep ${identityAnchor} with the same face and clothes.`,
    1,
    24
  );
};
...
const buildLtxTrippyCameraSentence = ({
  ...
  const motionFragment = !looksLikeStaticCameraFragment(motion)
    ...
    : '';
  const resolvedCamera = cameraFragment || 'use a creeping push, unstable reframing, and restless drift';
...
export const buildCameraGroundedPrompt = ({
  ...
  ].filter(Boolean).join(' '), 4, useSingleImage ? 110 : 95);
