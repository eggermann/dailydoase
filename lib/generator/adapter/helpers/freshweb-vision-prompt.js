...
const buildShortSingleImageMotionRule = ({ durationSeconds, useSingleImage = false } = {}) => {
  const duration = Number(durationSeconds);
  if (!useSingleImage || !Number.isFinite(duration) || duration > 4) {
    return '';
  }
  return 'Motion rule: action starts immediately and remains visibly in motion until the final second; no frozen tableau or still-image hold.';
};
...
const buildFirstLastCctvMotionPrompt = ({
  actionText = '',
  beatText = '',
  motion = '',
} = {}) => {
  const action = buildNaturalPromptSentence([
    actionText,
    motion,
    beatText,
  ], 34, 18);
  return compactScenePrompt([
    'Fixed real 1989 Einkaufszentrum CCTV viewpoint with mild VHS interlace; camera stays locked while people and objects move continuously in frame.',
    action || 'One visible subject completes one continuous physical action from the start image to the end image.',
    'Show continuous motion throughout; do not freeze, hold a pose, or delay the change until the final moment.',
  ].join(' '), 3, 68);
};
...
export const buildCameraGroundedPrompt = ({
  ...
  // First/Last already receives both visual states as images. Repeating
  // identity, geometry, style, and endpoint prose makes Wan favour a stable
  // tableau and defer the change to its last frames. Keep CCTV in the video
  // prompt as a camera rule, then spend the remaining prompt on one action.
  if (!useSingleImage) {
    return buildFirstLastCctvMotionPrompt({
  ...
  const shortSingleImageMotionRule = buildShortSingleImageMotionRule({
    durationSeconds,
    useSingleImage,
  });
