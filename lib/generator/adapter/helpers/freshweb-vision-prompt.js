...
export const buildCameraGroundedPrompt = ({
  ...
  if (!useSingleImage) {
    return buildCameraFirstLastLeanPrompt({
      continuityParts,
      beatText,
  ...
  parts.push(buildCameraIdentityLock({
    actorsSummary: startActors,
  }));
  parts.push(buildCameraFramingLock({
    readableState,
    continuityText: startContinuity,
    locationSummary: startLocation,
    useSingleImage,
  ...
  if (readableState) {
    parts.push(buildLabeledSentence('Keep readable:', readableState, 20));
  }
  if (beatText) {
    parts.push(buildLabeledSentence('Beat:', beatText, 24));
  }
  if (includeAction) {
  ...
  return compactScenePrompt(parts.join(' '), 12, 170);
};
