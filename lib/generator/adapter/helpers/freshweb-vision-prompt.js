...
const buildDurationGuidance = ({
  ...
  if (useSingleImage) {
    if (duration <= 2) {
      return `Fit the action to a ${durationLabel} held shot: play one immediate readable beat, keep the motion minimal, and avoid multi-step action.`;
    }
    if (duration <= 4) {
      return `Fit the action to a ${durationLabel} held shot: keep one clear emotional progression, restrained motion, and a single simple camera emphasis.`;
    }
    return `Fit the action to a ${durationLabel} held shot: let the movement breathe, but keep it as one coherent on-screen progression.`;
  }
