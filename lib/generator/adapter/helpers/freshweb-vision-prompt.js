...
const buildLtxCueActionSentence = ({
  ...
  return compactScenePrompt(
    cueText ? `Let ${cueText} register through ${resolvedAction}.` : `Let the cue register through ${resolvedAction}.`,
    1,
    24
  );
};
...
const buildLtxTrippySetupSentence = ({
  ...
  return compactScenePrompt(
    `Start from ${visibleLead} and let the image break away from plain webcam realism.`,
    1,
    20
  );
};
...
const buildLtxTrippyTransformationSentence = ({
  ...
  return compactScenePrompt(
    `Let ${cueText} take over the scene; ${actionFragment || fallbackAction}.`,
    1,
    26
  );
};
