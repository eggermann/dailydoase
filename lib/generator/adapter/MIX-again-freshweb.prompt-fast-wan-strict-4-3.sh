...
# Private Qwen vision is normal path. This outer handoff starts a loopback-only
# SSH tunnel to dominikeggermann's Mac mini before the pipeline runs.
export FRESHWEB_VISION_CONFIG=${FRESHWEB_VISION_CONFIG:-mac-mini-vision}
if [ "${FRESHWEB_VISION_CONFIG}" = "mac-mini-vision" ] && [ "${MAC_MINI_VISION_TUNNEL_ACTIVE:-0}" != "1" ]; then
  exec sh "${SCRIPT_DIR}/../../../localMistral/run-freshweb-mac-mini-vision.sh" "$@"
fi
...
export FRESHWEB_IMAGE_NEGATIVE_PROMPT=${FRESHWEB_IMAGE_NEGATIVE_PROMPT:-text, letters, words, captions, subtitles, signage, readable writing, typography, logo, watermark, poster text, wall text, gallery label text, menu text, printed text, distorted text, blurry, low detail, warped anatomy, broken perspective}
export FRESHWEB_OPENING_START_NEGATIVE_PROMPT=${FRESHWEB_OPENING_START_NEGATIVE_PROMPT:-text, letters, words, captions, subtitles, signage, readable writing, typography, logo, watermark, poster text, wall text, gallery label text, menu text, printed text, distorted text, different person, different room, broken anatomy, blur, low detail, collage, split screen}
...
if [ "${FRESHWEB_VISION_CONFIG}" = "mac-mini-vision" ]; then
