...
import { getIamge, isCameraStartImageEnabled } from '../../helper/getIamge.js';
...
const defaultStartImagePath = path.resolve(__dirname, '..', 'test.datas', 'timba-lake.png');

const resolveStartImagePath = async () => {
    if (!isCameraStartImageEnabled()) {
        return defaultStartImagePath;
    }

    return getIamge({
        outputDir: path.resolve(__dirname, '..', 'test.datas', 'camera'),
        fallbackImagePath: defaultStartImagePath,
...
const startImagePath = await resolveStartImagePath();
