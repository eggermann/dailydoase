...
const video = {
  prompts: {
    create: async (startFramePrompt, endFramePrompt, sceneContext, frameContext = {}) => {
      ...
      const sceneLength = setActiveSceneDuration(scenePlanEntry?.durationSeconds ?? nextSceneDuration());
      const plannedPrompt = scenePlanEntry?.videoPrompt;
      if (plannedPrompt) {
        return compactScenePrompt(plannedPrompt);
      }
      return buildFallbackVideoPrompt(
        scenePlanEntry,
