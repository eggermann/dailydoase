...
import { createFrameVisionHelper, normalizeVisionText, summarizeVisionAnchor } from './helpers/frame-vision.js';
...
const DEFAULT_SCENE_COUNT = Number(process.env.FRESHWEB_MIDDLE_SCENE_COUNT)
...
const VISION_PROMPT = process.env.FRESHWEB_MIDDLE_VISION_PROMPT
...
const VISION_PROVIDERS = (process.env.FRESHWEB_MIDDLE_VISION_PROVIDERS || process.env.FRESHWEB_TEST_VISION_PROVIDERS || '')
  .split(',')
  .map((value) => value.trim())
  .filter(Boolean);
...
const imageModelData = {
  ...
  width: Number(process.env.FRESHWEB_MIDDLE_IMAGE_WIDTH) || 256,
  height: Number(process.env.FRESHWEB_MIDDLE_IMAGE_HEIGHT) || 256,
...
const buildVisionAwarePrompt = ({
  ...
  const startAnchor = summarizeVisionAnchor(startVision);
  const endAnchor = summarizeVisionAnchor(endVision);
...
const video2 = {
  ...
  model: {
    ...
    height: Number(process.env.FRESHWEB_MIDDLE_VIDEO_HEIGHT) || 512,
    width: Number(process.env.FRESHWEB_MIDDLE_VIDEO_WIDTH) || 512,
...
import('../../../semantic-stream.js')
  .then((module) =>
    module.default([
      {
        refresh: true,
        folderName: process.env.FRESHWEB_MIDDLE_FOLDER || 'freshweb-middle-cost-test',
        ...
        promptFunktion: async (streams, config) => {
          ...
          console.log('[freshweb-middle-cost] openingCameraShot:', openingCameraShot);
          if (STATIC_TEST_MODE) {
            console.log('[freshweb-middle-cost] staticTestMode: enabled');
            console.log('[freshweb-middle-cost] sourceCues:', sourceCues.join(' | '));
          }
          console.log('[freshweb-middle-cost] scenePlan:', normalizedScenePlan.map((scene) => scene.title).join(' | '));
          console.log(
            '[freshweb-middle-cost] sceneModes:',
            normalizedScenePlan.map((scene) => scene.videoMode).join(' | ')
          );
          console.log('[freshweb-middle-cost] IMG_SEED:', IMAGE_SEED, 'VID_SEED:', VIDEO_SEED);
          console.log('[freshweb-middle-cost] sceneLengths:', activeSceneLengths.join(','));
          return streams;
        },
  ...
  .catch((err) => {
    console.error('Error in MIX-again-freshweb.middle-cost.js:', err);
    process.exit(1);
  });
