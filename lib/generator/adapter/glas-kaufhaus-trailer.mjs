...
const words = configuredWords.length > 0 ? configuredWords : defaultWords;
...
const main = async () => {
  ...
  const visualDirection = [
    ...
    `Semantic input: ${words.map(([word]) => word).join(', ')}. Derive the scene arc from their live getNext sequence; do not assign fixed meanings or fixed scenes to the configured start words.`,
    `Artist-derived fictional motifs, never literal portraits: ${motifDossier}.`,
    creativeRule,
    'Compose every scene as a semantic baton collision: carry the previous fresh term forward as semantic inheritance, collide it with the current stream\'s fresh getNext term, and let the conflict visibly infect bodies, objects, light, behavior, or architecture. Never explain or harmonize the contradiction.',
  ...
  const scenePlanSystemPrompt = [
    ...
    'People may appear when the semantic streams naturally call for them; neither add nor exclude them by default.',
    `The configured semantic streams (${words.map(([word]) => word).join(', ')}) must repeatedly yield fresh terms that alter the protagonist, room, gesture, sound-implied rhythm, atmosphere, light, composition, or camera emphasis across the requested scenes.`,
