...
const locationImages = [
  ...
  'kaufhaus-location/location-wall-person-reference.jpeg',
];
...
const main = async () => {
  ...
  const openingPrompt = [
    ...
    'No readable lettering, no modern branding, no literal artist portraits, no live visitors.',
  ].join(' ');
  const visualDirection = [
    ...
    'Treat the supplied Kaufhaus photos as the fixed physical world for every scene. Preserve their architecture, mirrored columns, open technical ceiling, fluorescent fixtures, concrete floor, elevators, windows, and white partition walls. Ignore and never reproduce any person visible in a location reference.',
    ...
    creativeRule,
    'Keep the film causally linked, visually concrete, poetic, strange but readable. No live camera, crowd, modern logos, subtitles, or invented artist biographies.',
  ].join(' ');
  const scenePlanSystemPrompt = [
    ...
    'Location dossier: real empty Kaufhaus interior with raw concrete floor, exposed ducts and cables, suspended fluorescent fixtures, mirrored structural columns, elevators, windows, brick walls, and white exhibition partitions. Every scene happens inside this same photographed location. Do not reproduce people from reference photos.',
