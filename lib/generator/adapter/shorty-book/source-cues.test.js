...
test('structured semantic baton keeps anchor and collision lossless', async () => {
  const stream = createSemanticTestStream('exhibition', ['archive', 'hunger', 'mirror']);

  const records = await buildCollisionSourceCueRecords({
    streams: [stream],
    sceneCount: 3,
  });

  expect(records.map(({ anchor, collision }) => ({
    anchor: anchor.term,
    collision: collision.term,
  }))).toEqual([
    { anchor: 'exhibition', collision: 'archive' },
    { anchor: 'archive', collision: 'hunger' },
    { anchor: 'hunger', collision: 'mirror' },
  ]);
  expect(records[0].anchor.role).toBe('initialConfiguredTerm');
  expect(records[1].anchor.role).toBe('carriedSemanticInheritance');
  expect(stream.getNext).toHaveBeenCalledTimes(3);
});

test('source cue bundle consumes each stream step once and serializes its records', async () => {
  const stream = createSemanticTestStream('1983', ['archive', 'hunger']);
  ...
  expect(stream.getNext).toHaveBeenCalledTimes(2);
  expect(sourceCues).toEqual(sourceCueRecords.map(serializeCollisionSourceCue));
  expect(sourceCues[0]).toContain('Anchor (initial configured term): 1983.');
  expect(sourceCues[0]).toContain('Collision A (fresh getNext from 1983): archive.');
});
