...
describe('story transport', () => {
  ...
  test('adds a reality-intrusion visitor to FIFO cast memory without replacing the protagonist', () => {
    const controller = createStoryTransportController({ actorReferenceLimit: 2 });
    controller.beginIteration({
      iteration: 1,
      referenceImagePath: '/cast/original-protagonist.jpg',
      visionStoryContext: {
        actors: [{ reference: 'original protagonist', description: 'maroon shirt' }],
      },
    });

    const remembered = controller.rememberRealityIntrusion({
      iteration: 1,
      sceneIndex: 3,
      referenceImagePath: '/cast/live-intrusion.jpg',
      visionStoryContext: {
        actors: [{ reference: 'new visitor', description: 'foreground right' }],
      },
    });

    expect(remembered.actorReferences.map((actor) => actor.referenceImage)).toEqual([
      '/cast/original-protagonist.jpg',
      '/cast/live-intrusion.jpg',
    ]);
    expect(remembered.actorReferences.at(-1)).toMatchObject({
  ...
  test('makes every configured semantic word mandatory in the scene-plan prompt', () => {
    ...
    const draft = controller.beginIteration({
      ...
      words: [
        ['Kaufhaus', 'de'],
        ['animal', 'en'],
        ['isolation strategy', 'en'],
        ['Art_critic', 'en'],
      ],
    });

    const prompt = formatStoryTransportForPrompt(draft);
    expect(prompt).toContain('Semantic word stream (every term is mandatory for this iteration)');
    expect(prompt).toContain('Kaufhaus | animal | isolation strategy | Art_critic');
    expect(prompt).toContain('never silently drop a term');
  });
  ...
  test('transports story and detected people positions across two iterations', () => {
    ...
    const firstTransport = controller.completeIteration({
      ...
      scenePlan: [
        { storyBeat: 'The visitors enter the abandoned sales floor.' },
        {
          ...
          finalPose: 'Both visitors stand on opposite sides of the central display, facing the closing gate.',
        },
      ],
    });
    ...
    expect(secondDraft.previous).toEqual(expect.objectContaining({
      iteration: 1,
      topic: 'Kaufhaus',
      finalBeat: 'The closing signal traps them between old displays. Final visible pose: Both visitors stand on opposite sides of the central display, facing the closing gate.',
    }));
    ...
    expect(promptContext).toContain('Previous final beat: The closing signal traps them between old displays. Final visible pose: Both visitors stand on opposite sides of the central display, facing the closing gate.');
    expect(promptContext).toContain('Opening obligation: Continue from this consequence: The closing signal traps them between old displays. Final visible pose: Both visitors stand on opposite sides of the central display, facing the closing gate.');
  });
  ...
  test('feeds iteration one ending into the second scene-planner request', async () => {
    const create = jest.fn(async () => {
      ...
      const finalPose = iteration === 1
        ? 'The visitor kneels beside the dark display, facing its lit screen.'
        : 'The visitor stands at the aisle exit, turned toward the CCTV camera.';
          ...
          message: {
            content: JSON.stringify({
              scenes: [{
                ...
                actorTrajectory: 'The visitor crosses from the left aisle toward the dark display.',
                finalPose,
    ...
    expect(secondPlannerPrompt).toContain('Previous final beat: The visitor follows the Kaufhaus signal toward the dark display. Final visible pose: The visitor kneels beside the dark display, facing its lit screen.');
    expect(secondPlannerPrompt).toContain('Opening obligation: Continue from this consequence: The visitor follows the Kaufhaus signal toward the dark display. Final visible pose: The visitor kneels beside the dark display, facing its lit screen.');
  });
});
