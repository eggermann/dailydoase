...
describe('story transport', () => {
  ...
  test('rotates the lead topic while retaining all configured semantic topics', () => {
    const words = [
      ['Einkaufszentrum', 'de'],
      ['Berlin Art Week', 'en'],
      ['Rosa Winkel', 'de'],
    ...
    expect(createStoryTransportDraft({ iteration: 1, words }).topic).toBe('Einkaufszentrum');
    expect(createStoryTransportDraft({ iteration: 2, words }).topic).toBe('Berlin Art Week');
    expect(createStoryTransportDraft({ iteration: 3, words }).topic).toBe('Rosa Winkel');
    expect(createStoryTransportDraft({ iteration: 4, words }).topics).toEqual([
      'Einkaufszentrum',
      'Berlin Art Week',
      'Rosa Winkel',
    ]);
  ...
  test('transports story and detected people positions across two iterations', () => {
    ...
    expect(promptContext).toContain('Active semantic topics: 1989.');
