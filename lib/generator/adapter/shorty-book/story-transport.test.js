...
describe('story transport', () => {
  ...
  test('keeps a fixed world context separate from semantic stream words', () => {
    const draft = createStoryTransportController().beginIteration({
      iteration: 1,
      seriesContext: 'Kaufhaus 1983',
      words: [['animal', 'en']],
    });

    expect(draft.seriesContext).toBe('Kaufhaus 1983');
    expect(formatStoryTransportForPrompt(draft)).toContain('World and series context: Kaufhaus 1983.');
  });
