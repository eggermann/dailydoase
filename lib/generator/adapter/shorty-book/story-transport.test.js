...
describe('story transport', () => {
  test('keeps at most seven actor image references in FIFO order', () => {
    const cache = createActorReferenceFifo({ maxSize: 7 });
    cache.touchMany(Array.from({ length: 8 }, (_, index) => ({
      personaId: `persona-${index + 1}`,
      referenceImage: `/cast/persona-${index + 1}.jpg`,
    })));

    expect(cache.values()).toHaveLength(7);
    expect(cache.values()[0].personaId).toBe('persona-2');
    expect(cache.values().at(-1).personaId).toBe('persona-8');
  ...
  test('passes at most three selected people to one scene', () => {
    const cast = {
      actorReferences: Array.from({ length: 4 }, (_, index) => ({
        personaId: `cast-${index + 1}`,
        referenceImage: `/camera/${index + 1}.jpg`,
      })),
    };
    const [scene] = attachCastReferencesToScenePlan({
      scenePlan: [{ castSelection: ['cast-1', 'cast-2', 'cast-3', 'cast-4'] }],
      cast,
    });

    expect(scene.castReferences.map((actor) => actor.personaId)).toEqual([
      'cast-1',
      'cast-2',
      'cast-3',
    ]);
