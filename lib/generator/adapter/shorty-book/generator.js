...
class Generator extends PostTo {
  ...
  async finalizeGeneratedVideo({
    ...
    await normalizeVideoOutput(videoData.file, {
      ...
      targetWidth: Number(mergedConfig?.width) || null,
      targetHeight: Number(mergedConfig?.height) || null,
    });
  ...
  async prompt(streams, options = {}) {
    ...
    try {
      ...
      const requestedDurationRaw = mergedConfig?.duration_seconds;
      const requestedDuration = typeof requestedDurationRaw === 'function'
        ? Number(await requestedDurationRaw())
