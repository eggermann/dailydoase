...
const resolveCapturedImageOverride = async ({
  ...
  if (typeof captureConfig.captureFn === 'function') {
    const capturedPath = await captureConfig.captureFn();
    if (capturedPath) {
      return {
...
class Generator extends PostTo {
  ...
  async generateImage(streams, options, promptSource = null) {
    ...
    return imageModel.prompt(totalImagePrompt, mergedConfig);
  }
  ...
  updateOpeningPersonaReferencePath(nextPath = '') {
  ...
  async persistCapturedPersonaReferenceImage(capturedPath, sceneContext = null) {
  ...
  async refreshPersonaReferenceFromCameraShot(capturedPath, sceneContext = null) {
    const openingImageConfig = this.getOpeningImageConfig();
    if (openingImageConfig?.usePersonaReferenceForFreshImages !== true) {
      return '';
    }

    const savedReferencePath = await this.persistCapturedPersonaReferenceImage(
      capturedPath,
      sceneContext
    );
    const nextReferencePath = savedReferencePath || String(capturedPath || '').trim();
    if (!nextReferencePath) {
      return '';
    }

    this.updateOpeningPersonaReferencePath(nextReferencePath);
    logger.info(`Updated persona reference from camera shot: ${nextReferencePath}`);
    return nextReferencePath;
  }

  async scheduleAsyncPersonaReferenceCapture(sceneContext = null) {
    ...
    if (Number(sceneContext?.index) > 0 && intervalScenes > 1 && sceneContext.index % intervalScenes !== 0) {
      return null;
    }
    ...
    this.personaReferenceCapturePromise = (async () => {
      try {
        ...
        if (savedReferencePath) {
          this.updateOpeningPersonaReferencePath(savedReferencePath);
          logger.info(`Updated async persona reference: ${savedReferencePath}`);
        }
        return savedReferencePath;
      } catch (error) {
  ...
  async generateOpeningFluxContextFrame(
    ...
    const openingModelOptions = {
      ...
      imagePath: resolvedReferenceImagePath,
  ...
  async continueSceneLoop({
    ...
    for (let index = startIndex; index < clipCount; index += 1) {
      ...
      void this.scheduleAsyncPersonaReferenceCapture(sceneContext);
