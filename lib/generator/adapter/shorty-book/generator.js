...
import { isReferenceImageActorMode } from './LiveContextOrchestrator-config.js';
...
class Generator extends PostTo {
  ...
  async generateCastContextFrame({
    ...
    const model = castContextConfig?.model?.model || 'bfl:6@1';
    const compilation = compileFluxEditPrompt({
      scene: scenePlanEntry,
      currentState: 'The supplied current room frame is composition truth.',
      model,
      cameraStyle: this.config?.story?.cameraStyle,
      referenceRoles: [
        {
          role: 'current frame / composition and current room truth',
          path: startImagePath,
        },
        ...existingReferencePaths.map((referencePath, index) => ({
          role: index === 0 ? 'selected cast identity truth' : 'additional selected cast identity truth',
          path: referencePath,
        })),
      ],
    });
    const prompt = compilation.prompt;
    ...
    try {
      ...
      const contextFramePath = contextFrame?.image?.path || contextFrame?.imagePath || '';
      const continuityGuard = contextFramePath
        ? await compareVisualContinuity({
            sourcePath: startImagePath,
            generatedPath: contextFramePath,
          })
        : { comparable: false, rejected: true, reason: 'missing-generated-frame' };
      if (continuityGuard.rejected) {
        logger.warn(
          `cast FLUX frame rejected: ${continuityGuard.reason}`
          + ` mean=${continuityGuard.meanDifference ?? 'n/a'}`
          + ` changed=${continuityGuard.changedPixelRatio ?? 'n/a'}`
          + ` output=${contextFramePath || 'none'}`
        );
        return null;
      }
      ...
      return {
        ...
        json: {
          ...(contextFrame?.json || {}),
          metadata: {
            ...(contextFrame?.json?.metadata || {}),
            visualPromptCompiler: compilation,
            visualContinuityGuard: continuityGuard,
          },
        },
      };
    } catch (error) {
  ...
  async generateOpeningFluxContextFrame(
    ...
    try {
      ...
      const generatedFramePath = generatedFrame?.image?.path || generatedFrame?.imagePath || '';
      const continuityGuard = generatedFramePath
        ? await compareVisualContinuity({
            sourcePath: primaryImagePath,
            generatedPath: generatedFramePath,
          })
        : { comparable: false, rejected: true, reason: 'missing-generated-frame' };
      if (continuityGuard.rejected) {
        logger.warn(
          `opening FLUX frame rejected: ${continuityGuard.reason}`
          + ` mean=${continuityGuard.meanDifference ?? 'n/a'}`
          + ` changed=${continuityGuard.changedPixelRatio ?? 'n/a'}`
          + ` output=${generatedFramePath || 'none'}`
        );
        return null;
      }
      return {
  ...
  async generateSceneContextFluxFrame({ scenePlanEntry = {}, sceneContext = {}, imageOptions = {} } = {}) {
    ...
    try {
      ...
      const continuityGuard = generatedFramePath
        ? await compareVisualContinuity({
            sourcePath: contextImagePath,
            generatedPath: generatedFramePath,
          })
        : { comparable: false, rejected: true, reason: 'missing-generated-frame' };
      if (continuityGuard.rejected) {
        logger.warn(
          `scene context FLUX frame rejected: ${continuityGuard.reason}`
          + ` output=${generatedFramePath || 'none'}`
        );
        return null;
      }
  ...
  shouldApplyDriftCorrection({ nextScenePlanEntry } = {}) {
    ...
    if (!nextScenePlanEntry || typeof nextScenePlanEntry !== 'object') {
      return false;
    }
  ...
  buildDriftCorrectionPrompt({ nextScenePlanEntry, referenceRoles = [] }) {
    const driftCorrection = this.getDriftCorrectionConfig();
    return compileFluxEditPrompt({
      scene: nextScenePlanEntry || {},
      currentState: 'Reference 1 is the actual current WAN end frame and overrides every textual description of the previous scene.',
      model: driftCorrection?.model?.model || 'runware:106@1',
      cameraStyle: this.config?.story?.cameraStyle,
      referenceRoles,
    });
  }
  ...
  async correctLastFrameForNextScene({ lastFrame, nextScenePlanEntry, nextSceneContext }) {
    ...
    // Runware receives one primary input plus at most nine secondary images.
    // Keep the fresh person ahead of the opening room, then retain the newest
    // context frames only while the request remains a true ten-image edit.
    const secondaryReferencePaths = [...new Set([
      ...(referenceImagePath && referenceImagePath !== primaryInputPath ? [referenceImagePath] : []),
      ...(openingImagePath && openingImagePath !== primaryInputPath ? [openingImagePath] : []),
    ...
    const referenceRoles = [
      { role: 'current WAN end frame / composition truth', path: primaryInputPath },
      ...(referenceImagePath && referenceImagePath !== primaryInputPath
        ? [{ role: 'latest camera person / identity truth', path: referenceImagePath }]
        : []),
      ...(openingImagePath && openingImagePath !== primaryInputPath
        ? [{ role: 'opening webcam frame / original room truth', path: openingImagePath }]
        : []),
      ...secondaryReferencePaths
        .filter((entryPath) => ![referenceImagePath, openingImagePath].includes(entryPath))
        .map((entryPath, index) => ({ role: `older continuity frame ${index + 1}`, path: entryPath })),
    ];
    const correctionCompilation = this.buildDriftCorrectionPrompt({
      nextScenePlanEntry,
      referenceRoles,
    });
    const correctionPrompt = correctionCompilation.prompt;
    logger.payload('compiled FLUX drift prompt', correctionCompilation, { maxLength: 12000 });
    ...
    const correctedFramePath = correctedFrame?.image?.path || correctedFrame?.imagePath || '';
    const continuityGuard = correctedFramePath
      ? await compareVisualContinuity({
          sourcePath: lastFramePath,
          generatedPath: correctedFramePath,
        })
      : {
          comparable: false,
          rejected: true,
          reason: 'missing-generated-frame',
        };
    if (continuityGuard.rejected) {
      logger.warn(
        `FLUX visual preparation rejected: ${continuityGuard.reason}`
        + ` mean=${continuityGuard.meanDifference ?? 'n/a'}`
        + ` changed=${continuityGuard.changedPixelRatio ?? 'n/a'}`
      );
      return {
        ...
        json: {
          ...
          metadata: {
            ...
            visualContinuityGuard: continuityGuard,
            rejectedFluxVisualPreparationPath: correctedFramePath,
          },
        },
      };
    }
    ...
    return {
      image: { path: correctedFramePath || lastFramePath },
      json: {
        ...(correctedFrame?.json || {}),
        metadata: {
          ...(lastFrame?.json?.metadata || {}),
          ...
          visualPromptCompiler: correctionCompilation,
  ...
  async resolveChainedStartFrameFromLastEnd({ nextScenePlanEntry, nextSceneContext }) {
    ...
    if (!this.shouldApplyDriftCorrection({ nextScenePlanEntry, nextSceneContext })) {
      logger.info('visual preparation: continue raw WAN end frame (no FLUX edit)');
      return this.lastEndFRame;
    }
  ...
  buildVideoConfig(generatedPrompt, videoModel, startFrame, options) {
    const groundedPrompt = isReferenceImageActorMode(this.config?.story?.mode)
      ? generatedPrompt
      : [
  ...
  async continueSceneLoop({
    ...
    for (let index = startIndex; index < clipCount; index += 1) {
      ...
      // Cast compositing is an authored transition target, never a hidden
      // replacement for the real webcam start of a single-image shot.
      ...
      if (!plannedSingleImageContinuation && scenePlanEntry?.videoMode !== 'firstLast') {
        logger.info(
          `cast context skipped: scene ${sceneContext.index}, ${scenePlanEntry?.videoMode || 'default'} starts from its source frame`
        );
      }
      ...
      await saveJSON(decisionFilePath, {
        ...
        planned: {
          ...
          visualPreparation: scenePlanEntry?.visualPreparation || 'continueFrame',
        ...
        visualPromptCompiler: sceneContext.visualPromptCompiler || null,
        fluxPromptCompiler:
          currentStartFrame?.json?.metadata?.visualPromptCompiler
          || sceneContext.fluxPromptCompiler
          || null,
        visualContinuityGuard: currentStartFrame?.json?.metadata?.visualContinuityGuard || null,
      });
      ...
      logger.info(`  visual preparation: ${scenePlanEntry?.visualPreparation || 'continueFrame'}`);
      ...
      if (!isLastClip && typeof sceneLoop.onSceneBoundary === 'function') {
        ...
        try {
          ...
          if (decision?.command) {
            ...
            if (cameraTransportApplied && decision.cameraHasPeople && decision.cameraImagePath) {
