...
class Generator extends PostTo {
  ...
  buildSceneContextFluxPrompt({
    ...
    if (protagonistAlreadyComposited) {
      return [
        ...
        'Make the result feel like realistic handheld mobile-device footage: candid, slightly shaky, imperfectly exposed, with autofocus breathing and mild compression artifacts.',
        ...
        'Output one realistic handheld-mobile-device edited Kaufhaus photograph, not a poster or illustration.',
      ].filter(Boolean).join(' ');
    }
    if (sceneContextImageConfig?.allowPeople === false) {
      ...
      return [
        ...
        promptOnlyIdentity
          ? 'Build identity from the realistic Green Monster vocabulary: one solemn elongated green bark face, amber glowing eyes, two large leaf-shaped ears, one trunk-like mouth tendril, and a rooted botanical body. Preserve these identity traits without reproducing one fixed silhouette or any background.'
          : 'Use the separate creature image only as an identity and motion vocabulary: preserve the Green Monster face, glowing eyes, botanical fibers, leaf anatomy, roots, and limb structure. Ignore its flat magenta background completely.',
        'Construct exactly one fresh scene-specific incarnation of the Green Monster. Preserve identity, but let the scene-specific pose and silhouette be driven by the semantic stream and the location; do not paste or reproduce the reference pose.',
        semanticCollision.anchor ? `Semantic Anchor: ${semanticCollision.anchor}.` : '',
        semanticCollision.collision ? `Semantic Collision: ${semanticCollision.collision}.` : '',
        'Keep the two semantic terms in productive conflict; the reference image only anchors creature identity.',
        storyBeat ? `Surreal scene event: ${storyBeat}` : '',
        'The semantic conflict must become an unmistakable physical mutation of the monster, not merely a decorative glow or unchanged reference cutout. Make the planned surreal event clearly visible around and through its body, while the room and other scene content come from the semantic collision and location plate.',
        ...
        'Output one realistic handheld-mobile-device edited Kaufhaus photograph, not an illustration or concept-art environment.',
      ].filter(Boolean).join(' ');
    }
    const protagonistRule = sceneContextImageConfig?.allowPeople === false
      ...
      : 'Keep the protagonist identity anchored to the separate protagonist reference image, not to random people in the context image or to the reference background.';
    const referenceRule = 'Use the separate creature image only for Green Monster identity, face, glowing eyes, body silhouette, botanical material, and motion continuity. Ignore its flat magenta isolation background completely. Never copy a reference backdrop, former poster composition, or graphic page layout.';
    ...
    return translateSemanticTermsForProductionPrompt([
      ...
      'Derive all variable scene content from the semantic conflict and the protagonist action. Confine surreal mutation to the monster, semantically justified people, movable objects, light, reflections, shadows, and one nearby architectural detail; let the semantic stream and the location plate carry the rest of the scene.',
  ...
  buildSceneContextSemanticReconstructionPrompt({ scenePlanEntry = {}, sceneContext = {} } = {}) {
    ...
    return [
      ...
      'Output one realistic handheld-mobile-device Kaufhaus photograph.',
    ].filter(Boolean).join(' ');
  }
