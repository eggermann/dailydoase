...
const readSemanticCollision = (scenePlanEntry = {}) => {
  const structuredAnchor = String(scenePlanEntry?.semanticAnchor || '').trim();
  const structuredCollision = String(scenePlanEntry?.semanticCollision || '').trim();
...
class Generator extends PostTo {
  ...
  async generateSceneContextFluxFrame({
    ...
    try {
      ...
      if (sceneContextImageConfig?.semanticReconstructionPass === true && firstPassPath) {
        const reconstructionPrompt = this.buildSceneContextSemanticReconstructionPrompt({
          scenePlanEntry,
          sceneContext,
        });
        try {
  ...
  async promptSceneImagesOnly(options, fileName) {
    ...
    if (sceneNumberOffset > 0 && await fs.pathExists(summaryPath)) {
