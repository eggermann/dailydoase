...
class Generator extends PostTo {
  ...
  getContextBufferConfig() {
    ...
    return {
      ...
      maxReferenceImages: resolvePositiveInt(configured?.maxReferenceImages, 11),
  ...
  async correctLastFrameForNextScene({ lastFrame, nextScenePlanEntry, nextSceneContext }) {
    ...
    const maximumSecondaryReferences = Math.max(0, contextBufferConfig.maxReferenceImages - 1);
    const secondaryReferencePaths = [];
    const addSecondaryReference = (entryPath) => {
      const resolvedPath = String(entryPath || '').trim();
      if (!resolvedPath || resolvedPath === primaryInputPath || secondaryReferencePaths.includes(resolvedPath)) {
        return;
      }
      if (secondaryReferencePaths.length < maximumSecondaryReferences) {
        secondaryReferencePaths.push(resolvedPath);
      }
    };

    // Current person has precedence, followed by the opening room/person anchor
    // and only then the newest supporting camera history.
    addSecondaryReference(referenceImagePath);
    addSecondaryReference(openingImagePath);
    [...contextScreenshotPaths].reverse().forEach(addSecondaryReference);
      ...
      ...(secondaryReferencePaths.length > 0
        ? {
            ...
            contextReference: {
              ...
              layout: {
                maxImages: Math.min(contextBufferConfig.size, maximumSecondaryReferences),
