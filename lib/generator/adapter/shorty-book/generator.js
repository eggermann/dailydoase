...
class Generator extends PostTo {
  ...
  async resolveStartFrame(streams, imageOptions, openingImageOptions = null, sceneContext = null) {
    ...
    if (!startFrame) {
      ...
      const resolvedImageOptions = openingImageOptions
        ? { ...imageOptions, ...openingImageOptions, sceneContext }
        : { ...imageOptions, sceneContext };
  ...
  async promptSceneLoop(streams, options, fileName) {
    ...
    for (let index = 0; index < clipCount; index += 1) {
      ...
      const useFreshImage = !isFirstClip && (independentSceneStarts || scenePlanEntry?.freshImage === true);
      if (useFreshImage) {
      ...
      const useSingleImage = isFirstClip
        ? sceneLoop.firstClipUseSingleImage !== false
        : Boolean(sceneLoop.subsequentClipsUseSingleImage);
