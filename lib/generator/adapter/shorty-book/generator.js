...
const RUNWARE_FLUX_PROMPT_MAX_CHARACTERS = 1900;
...
class Generator extends PostTo {
  ...
  async generateOpeningFluxContextFrame(
    ...
    const rawGeneratedPrompt = String(
      openingImageOptions?.generatedPrompt
      || openingImageOptions?.promptSource
      || ''
    ).trim();

    if (!referenceImagePath || !rawGeneratedPrompt) {
      return null;
    }
    ...
    const openingScene = openingImageOptions?.scenePlanEntry
      ...
      || {};
    const generatedPrompt = compactFluxPromptForProvider({
      prompt: rawGeneratedPrompt,
      scenePlanEntry: openingScene,
  ...
  async correctLastFrameForNextScene({ lastFrame, nextScenePlanEntry, nextSceneContext }) {
    ...
    const cameraHoldNegativePrompt = driftCorrection?.cameraMode
      ...
      : '';
    const correctionPrompt = compactPromptFragment(this.buildDriftCorrectionPrompt({
      lastFrame,
      nextScenePlanEntry,
      nextSceneContext,
      localLocationReference,
    }), RUNWARE_FLUX_PROMPT_MAX_CHARACTERS);
