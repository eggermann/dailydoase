...
const PHOTOGRAPHIC_WORLD_RULE = [
  'Photographic-world rule:',
  'continue the exact real camera room, practical lighting, and surveillance-camera perspective from the source image.',
  'Stage the story through real people and existing physical room objects only.',
  'Never render a semantic cue as a floating word, symbolic shadow-creature, illustrative metaphor object, diagram, poster, or graphic.',
  'Preserve physical spatial integrity: people, hands, easels, tables, counters, and props have separate volumes; limbs never pass through furniture or objects, and contact happens only at a named touch point.',
].join(' ');

const PHOTOGRAPHIC_VIDEO_RULE = [
  'Photoreal continuous CCTV observation of the source frame, not an illustration or stylized reconstruction.',
  'Keep the room, people, objects, materials, and practical lighting photographic and physically present.',
  'If visible artwork, posters, labels, or screens appear, they remain flat objects in the room and never define the style of the footage.',
  'The semantic cue drives a concrete action or material consequence, not a visual stand-in for an idea.',
].join(' ');
...
class Generator extends PostTo {
  ...
  async generateImage(streams, options, promptSource = null) {
    ...
    const effectivePrompt = [
      ...
      PHOTOGRAPHIC_WORLD_RULE,
      prompt,
    ].filter(Boolean).join(' ');
    const mergedConfig = mergeImageConfig(this.config, resolvedOptions);
    // In camera-story mode this is the initialized Runware BFL image model.
    // Use it for every generated scene still; reference *images* remain an
    // independent choice in buildPersonaReferenceImageOptions.
    const imageModel = this.personaReferenceImageModel || this.flux;
  ...
  shouldUsePersonaReferenceImageModel(options = {}) {
    ...
    // Creative cuts still need the Runware image-to-image renderer. They use
    // a stable room frame as input, not the changing visitor/persona frame.
    if (options?.usePersonaReferenceImageModel === true) {
      return true;
    }
  ...
  buildPersonaReferenceImageOptions(options = {}) {
    if (options?.skipPersonaReferenceImages === true) {
      return options;
    }
  ...
  async correctLastFrameForNextScene({ lastFrame, nextScenePlanEntry, nextSceneContext }) {
    ...
    let correctedFrame;
    try {
      correctedFrame = await this.driftCorrectionModel.prompt(correctionPrompt, correctionOptions);
    } catch (error) {
      logger.warn(`drift correction skipped: ${error?.message || error}`);
      return lastFrame;
    }
  ...
  buildVideoConfig(generatedPrompt, videoModel, startFrame, options, { useSingleImage = true } = {}) {
    // A First/Last renderer already receives both visual identity anchors as images.
    // Repeating the large webcam identity description here dilutes its motion prompt
    // and made WAN hold the first image instead of performing the transition.
    const groundedPrompt = [
      useSingleImage ? this.buildPersonaReferenceIdentityClause() : '',
      PHOTOGRAPHIC_VIDEO_RULE,
      generatedPrompt,
    ].filter(Boolean).join(' ');
  ...
  async continueSceneLoop({
    ...
    for (let index = startIndex; index < clipCount; index += 1) {
      ...
      if (!sceneContextFrame && useFreshImage) {
        ...
        const isCreativeCut = scenePlanEntry?.creativeCut === true;
        const creativeCutRoomImagePath = isCreativeCut
          ? String(
            this.getOpeningImageConfig()?.imagePath
            || this.getOpeningImageConfig()?.referenceImagePath
            || this.getOpeningImageConfig()?.personaReferencePath
            || currentStartFrame?.image?.path
            || ''
          ).trim()
          : '';
          ...
          if (liveStartOverride?.path) {
            ...
            try {
              currentStartFrame = await this.generateImage(streams, {
                ...
                sceneContext,
                frameRole: 'start',
                // A creative cut must grow from the stable room reference, not
                // from an asynchronously refreshed visitor/persona image.
                disablePersonaReference: isCreativeCut,
                usePersonaReferenceImageModel: isCreativeCut,
                skipPersonaReferenceImages: isCreativeCut,
                imagePath: creativeCutRoomImagePath || imageOptions.imagePath,
              });
            } catch (error) {
              logger.warn(`fresh scene image generation failed; keeping previous frame: ${error?.message || error}`);
            }
          }
        } else {
          try {
      ...
      if (castContextFrame?.image?.path) {
        const preservesPreviousFrame = !useFreshImage && (!isFirstClip || loopStartsFromLastFrame);
        if (preservesPreviousFrame) {
          ...
          sceneContext.castContextCandidatePath = castContextEndFrame.path;
        } else {
      ...
      // The planner owns the transport mode. A cast-context image is an
      // optional destination only when GPT chose First/Last; it must never
      // silently convert a planned single-image scene into First/Last.
      ...
      let endFrameOverride = useSingleImage ? null : castContextEndFrame;
      if (endFrameOverride) {
        sceneContext.transitionTargetPath = endFrameOverride.path;
      }
