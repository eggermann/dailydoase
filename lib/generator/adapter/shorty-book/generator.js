...
import { createVisitorMemory, rememberVisitor } from './visitor-memory.js';
import {
  concatMp4Lossless,
  extractLastFrame,
  forceVideoEndImage,
...
class Generator extends PostTo {
  ...
  constructor(modelConfig) {
    ...
    this.pendingVisitorEvent = null;
    this.visitorMemory = createVisitorMemory();
  ...
  async generateNewVisitorTransitionTarget({
    ...
    if (
      !visitorEvent
      || (!forceVisitorTransition && String(scenePlanEntry?.videoMode || '').trim() !== 'firstLast')
      || !startImagePath
      || !cameraReferencePath
    ...
    const rememberedReferencePaths = visitorEvent.type === 'returning'
      ? (Array.isArray(visitorEvent.visitor.referencePaths) ? visitorEvent.visitor.referencePaths : [])
      : [];
    const referencePaths = [
      cameraReferencePath,
      ...rememberedReferencePaths,
      ...new Set(castReferencePaths),
    ].filter((referencePath, index, entries) => entries.indexOf(referencePath) === index).slice(0, 9);
    const virtualisedInteraction = [
      scenePlanEntry?.actorsInteraction,
    ...
    const prompt = [
      ...
      visitorEvent.type === 'returning'
        ? 'The fresh live camera reference shows a returning visitor from this run. Preserve current people and room. Let the current semantic-stream virtualisation determine their visible interaction; never invent dialogue, touch, or a separate action that the planned event does not support.'
      ...
      visitorEvent.type === 'returning' && virtualisedInteraction
        ? `Virtualise this returning-visitor interaction from the semantic stream: ${virtualisedInteraction}`
        : '',
      ...
      return {
        ...
        json: {
          ...
          metadata: {
            ...
            visitorMemoryEvent: visitorEvent.type,
            visitorMemoryId: visitorEvent.visitor.id,
            visitorInteraction: virtualisedInteraction || visitorEvent.interaction,
          },
        },
      };
    } catch (error) {
  ...
  async rememberVisitorReference({ referencePath, metadata, sceneContext } = {}) {
    const result = rememberVisitor(this.visitorMemory, {
      description: metadata?.personDescription,
      strength: metadata?.personStrength,
      referencePath,
      sceneIndex: Number(sceneContext?.index) || null,
    });
    this.visitorMemory = result.memory;
    this.pendingVisitorEvent = result.event;
    await saveJSON(path.join(this.imageDir, 'parts', 'visitor-memory.json'), {
      kind: 'lost-audience-visitor-memory',
      updatedAt: new Date().toISOString(),
      visitors: this.visitorMemory.visitors,
    ...
    this.personaReferenceCapturePromise = (async () => {
      try {
        ...
        if (savedReference?.path) {
          ...
          const visitorEvent = typeof this.rememberVisitorReference === 'function'
            ? await this.rememberVisitorReference({
              referencePath: savedReference.path,
              metadata: savedReference.metadata || capturedReference.metadata || null,
              sceneContext,
            })
            : null;
          this.pendingPersonaReferenceIsNew = visitorEvent
            ? visitorEvent.type === 'new'
            : isLikelyNewVisitorReference({
              previousDescription,
              nextDescription,
              nextStrength,
            });
          ...
          logger.info(
            ...
            + ` | visitorEvent=${visitorEvent?.type || 'none'}`
          );
        }
  ...
  async generateSceneClip({
    ...
    const maxRetriesOnFailure = resolvePositiveInt(
      process.env.FRESHWEB_SCENE_MAX_RETRIES_ON_FAILURE,
      resolvePositiveInt(videoModel?.model?.maxRetriesOnFailure, 1)
    );
  ...
  async continueSceneLoop({
    ...
    for (let index = startIndex; index < clipCount; index += 1) {
      ...
      const forceVisitorTransition = ['new', 'returning'].includes(this.pendingVisitorEvent?.type);
      ...
      const useSingleImage = castContextEndFrame || forceVisitorTransition
        ? false
      ...
      let clipResult;
      try {
        clipResult = await this.generateSceneClip({
          startFrame: currentStartFrame,
          streams,
          options: {
            ...options,
        ...
        logger.error(`scene ${sceneContext.index}/${sceneContext.total} skipped after retry: ${failure}`);
        await saveJSON(path.join(promptDir, `${sceneFilePrefix}-scene-failure.json`), {
