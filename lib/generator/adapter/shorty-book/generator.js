...
export const resolveRoomEvolutionPhase = ({ previousStep = 0, reanchorEvery = 4 } = {}) => {
  const every = resolvePositiveInt(reanchorEvery, 4);
  const step = (Math.max(0, Number(previousStep) || 0) % every) + 1;
  return {
    every,
    step,
    reanchor: step === every,
...
class Generator extends PostTo {
  ...
  constructor(modelConfig) {
    ...
    this.gelbnasenContinuationCount = 0;
    this.gelbnasenTraceStage = 0;
    this.roomEvolutionStep = 0;
    ...
    this.personaReferenceCapturePromise = (async () => {
      try {
        ...
        if (savedReference?.path) {
          const previousDescription = String(this.getOpeningImageConfig()?.personaReferenceDescription || '').trim();
          const nextDescription = String(savedReference.metadata?.personDescription || capturedReference.metadata?.personDescription || '').trim();
          const isNewVisitor = Boolean(nextDescription && nextDescription !== previousDescription);
          this.pendingPersonaReferenceIsNew = isNewVisitor;
          this.updateOpeningPersonaReferencePath(savedReference.path, savedReference.metadata || capturedReference.metadata || null);
          const falseVisitorReference = typeof this.getDriftCorrectionConfig === 'function'
            ? this.getDriftCorrectionConfig()?.falseVisitorReference || {}
            : {};
          const every = resolvePositiveInt(falseVisitorReference.every, 4);
          const falseReferencePath = String(falseVisitorReference.imagePath || '').trim();
          if (isNewVisitor) {
            this.visitorEntryCount = (Number(this.visitorEntryCount) || 0) + 1;
            this.gelbnasenContinuationCount = 0;
          }
          if (isNewVisitor && this.visitorEntryCount % every === 0 && await fs.pathExists(falseReferencePath)) {
            this.pendingPersonaReferencePath = falseReferencePath;
            this.pendingPersonaReferenceIsNew = true;
            this.pendingPersonaReferenceIsFalseVisitor = true;
            logger.warn(`false visitor reference injected: visitor ${this.visitorEntryCount}, ${falseReferencePath}`);
          }
          markPersonaReferenceCaptureTimestampOnContext(this);
  ...
  shouldApplyDriftCorrection({ nextScenePlanEntry } = {}) {
    ...
    const castContextConfig = typeof this.getCastContextConfig === 'function'
      ...
      : {};
    const hasCastTransitionTarget = castContextConfig?.enabled === true
    ...
    if (videoMode === 'firstLast' && driftCorrection.applyToFirstLast !== true) {
      return this.pendingPersonaReferenceIsNew === true;
    }
  ...
  buildDriftCorrectionPrompt({
    ...
    const hasFalseVisitorReference = referenceImage?.sourceType === 'falseVisitorReference';
    const roomEvolutionDirective = roomEvolution?.reanchor
      ? `Room re-anchor ${roomEvolution.step}/${roomEvolution.every}: restore the exact recognisable room geometry, camera angle, practical light, and existing objects. Keep exactly one small visible residue of the prior transformation; do not reset the story.`
      : `Room evolution ${roomEvolution?.step || 1}/${roomEvolution?.every || 4}: let one already visible room feature carry the semantic event further through light, reflection, shadow, material, or relation. Keep the overall room recognisable; do not invent a different location.`;
    ...
    return [
      ...
      roomEvolutionDirective,
      hasFreshCameraReference
        ? 'A fresh live camera reference may contain newly arrived visitors. Use that presence silently as optional reality evidence for room and cast correction; do not force a visitor into the story action.'
        : '',
      hasFalseVisitorReference
        ? 'A misplaced surveillance-memory reference has entered the buffer: preserve only a subtle yellow-hat, onion-nose visitor trace if it fits the frame; never announce or explain it.'
        : '',
  ...
  buildDriftReferenceRolesPrompt({
    ...
    const roles = [
      ...
      'Reference image 1 is the current generated last frame and controls composition, camera angle, and immediate visual continuity.',
    ];
    let nextIndex = 2;

    if (referenceImagePath && referenceImagePath !== primaryInputPath) {
      roles.push(`Reference image ${nextIndex} is the freshest real camera capture and controls current person, room state, and lighting.`);
      nextIndex += 1;
    }
    if (openingImagePath && openingImagePath !== primaryInputPath && openingImagePath !== referenceImagePath) {
      roles.push(roomEvolution?.reanchor
        ? `Reference image ${nextIndex} is the canonical opening room reference; restore its geometry and left-right layout now.`
        : `Reference image ${nextIndex} is the canonical opening room reference; keep its overall geometry and left-right layout recognisable while one visible feature develops.`);
      nextIndex += 1;
    }
    if (contextScreenshotPaths.length > 0) {
      roles.push(`Reference images ${nextIndex} onward are older verified continuity only; use them to resolve identity and room conflicts, never as a collage or a new scene.`);
    }
  ...
  async resolveDriftCorrectionReferenceImage({ lastFrame } = {}) {
    ...
    if (pendingPersonaReferencePath && await fs.pathExists(pendingPersonaReferencePath)) {
      ...
      const isFalseVisitor = this.pendingPersonaReferenceIsFalseVisitor === true;
      this.pendingPersonaReferenceIsFalseVisitor = false;
      return {
        path: pendingPersonaReferencePath,
        prompt: isFalseVisitor ? 'false visitor reference: yellow hat and onion nose' : this.buildPersonaReferenceIdentityClause(),
        sourceType: isFalseVisitor ? 'falseVisitorReference' : 'cameraShot',
      };
    }

    const falseVisitorReference = driftCorrection?.falseVisitorReference || {};
    const falseReferencePath = String(falseVisitorReference.imagePath || '').trim();
    const afterContinuations = resolvePositiveInt(
      falseVisitorReference.afterContinuations,
      3
    );
    const continuationInterval = resolvePositiveInt(
      falseVisitorReference.continuationInterval,
      afterContinuations
    );
    this.gelbnasenContinuationCount = (Number(this.gelbnasenContinuationCount) || 0) + 1;
    const continuationOffset = this.gelbnasenContinuationCount - afterContinuations;
    const shouldContinueGelbnasenTrace = continuationOffset >= 0
      && continuationOffset % continuationInterval === 0
      && await fs.pathExists(falseReferencePath);

    if (shouldContinueGelbnasenTrace) {
      this.gelbnasenTraceStage = (Number(this.gelbnasenTraceStage) || 0) + 1;
      const tracePrompt = this.gelbnasenTraceStage === 1
        ? 'a small yellow-hat, onion-nose visitor trace enters the surveillance memory'
        : 'the subtle yellow-hat, onion-nose trace remains as a quiet memory in the room';
      logger.warn(
        `false visitor reference continued after ${this.gelbnasenContinuationCount} story continuations: ${falseReferencePath}`
      );
      return {
        path: falseReferencePath,
        prompt: tracePrompt,
        sourceType: 'falseVisitorReference',
      };
    }
  ...
  async correctLastFrameForNextScene({ lastFrame, nextScenePlanEntry, nextSceneContext }) {
    ...
    const roomEvolution = resolveRoomEvolutionPhase({
      previousStep: this.roomEvolutionStep,
      reanchorEvery: driftCorrection?.roomEvolution?.reanchorEvery,
    });
    this.roomEvolutionStep = roomEvolution.step;
    ...
    // Runware FLUX.2 flex accepts ten images in total, including the primary
    // last frame. Keep the freshest camera and opening anchors, then spend the
    // remaining slots on the newest distinct continuity frames. The lower
    // image-edit layer also protects this limit, but selecting here keeps the
    // prompt, metadata, and actual request in agreement.
    ...
    const correctionPrompt = [
      ...
      this.buildDriftReferenceRolesPrompt({
        primaryInputPath,
        referenceImagePath,
        openingImagePath,
    ...
    return {
      ...
      json: {
        ...
        metadata: {
          ...
          driftCorrectionContextBufferPaths: selectedContextScreenshotPaths,
          driftCorrectionContextReferenceBoardPath:
            ...
            || '',
          driftCorrectionContextBufferSize: selectedContextScreenshotPaths.length,
          roomEvolutionStep: roomEvolution.step,
          roomReanchored: roomEvolution.reanchor,
        },
      },
    };
  }
  ...
  async continueSceneLoop({
    ...
    for (let index = startIndex; index < clipCount; index += 1) {
      ...
      const castContextFrame = typeof this.generateCastContextFrame === 'function'
      ...
      let useSingleImage = scenePlanEntry?.videoMode === 'firstLast'
        ? false
        : scenePlanEntry?.videoMode === 'singleImage'
          ? true
          : configuredSingleImage;
      if (castContextEndFrame && useSingleImage) {
        // A single-image scene has one paid WAN anchor.  Keep the cast
        // composition, but use it as that anchor instead of silently
        // upgrading the scene into a two-anchor First/Last render.
        currentStartFrame = castContextFrame;
        sceneContext.frameSource = 'castContextImage';
        sceneContext.transitionTargetPath = '';
        castContextEndFrame = null;
      } else if (castContextEndFrame) {
        useSingleImage = false;
      }
      ...
      try {
        ...
        // The clip file name is the stable per-scene identifier.  The old
        // variable name was never defined here, so a provider failure turned
        // the intended "skip and continue" path into a fatal generator error.
        await saveJSON(path.join(promptDir, `${clipFileName}-scene-failure.json`), {
