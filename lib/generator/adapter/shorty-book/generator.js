...
class Generator extends PostTo {
  ...
  async correctLastFrameForNextScene({ lastFrame, nextScenePlanEntry, nextSceneContext }) {
    ...
    const correctionOptions = {
      ...
      negative_prompt: driftCorrection?.model?.negative_prompt,
      seed: driftCorrection?.model?.seed,
      width: driftCorrection?.model?.width,
      height: driftCorrection?.model?.height,
      ...(contextBufferConfig.enabled
        ? {
            images: contextScreenshotPaths.map((entryPath) => ({ path: entryPath })),
            contextReference: {
              enabled: true,
  ...
  async generateSceneClip({
    ...
    const requestedDuration = await resolveRequestedDurationOption(
    ...
    for (let index = 0; index < clipCount; index += 1) {
      ...
      if (!useFreshImage && this.lastEndFRame) {
        ...
        const nextScenePlanEntry = nextSceneContext
          ...
          : null;
        startFrame = await this.resolveChainedStartFrameFromLastEnd({
          nextScenePlanEntry,
          nextSceneContext,
        });
      }
