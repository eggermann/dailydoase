...
export const buildEnvironmentSceneContextFluxPrompt = ({
  ...
  'Create one realistic cinematic Kaufhaus photograph. No readable text, labels, panels or logos.',
].filter(Boolean).join(' ');
...
class Generator extends PostTo {
  ...
  buildSceneContextFluxPrompt({
    ...
    // Legacy monster-first assembly remains below only for historical source
    // reference. The active routes return above.
    ...
    const personaReferenceIdentityClause = this.buildPersonaReferenceIdentityClause({ subjectType: 'creature' });
    ...
    const legacyProtagonistAlreadyComposited = typeof protagonistAlreadyCompositedOverride === 'boolean'
      ? protagonistAlreadyCompositedOverride
      : sceneContextImageConfig?.protagonistAlreadyComposited === true;
    if (legacyProtagonistAlreadyComposited) {
      return [
  ...
  buildSceneContextSemanticReconstructionPrompt({ scenePlanEntry = {}, sceneContext = {} } = {}) {
    ...
    // Legacy reconstruction wording remains unreachable for saved source
    // compatibility only; active reconstruction is focus-aware above.
    const semanticCollision = readSemanticCollision(scenePlanEntry);
  ...
  buildDriftCorrectionPrompt({ lastFrame, nextScenePlanEntry, nextSceneContext }) {
    ...
    const personaReferenceIdentityClause = buildPersonaReferenceIdentityClauseFromConfig(openingImageConfig, {
      subjectType: isMonsterPipeline ? 'creature' : 'person',
      enabled: isMonsterPipeline ? shouldIncludeMonsterReference(nextScenePlanEntry) : true,
    });
    const driftCorrection = this.getDriftCorrectionConfig();
    ...
    return [
      ...
      'Keep the same visible subject, location, framing, and lighting.',
