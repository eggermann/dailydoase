...
const MAX_DRIFT_CORRECTION_REFERENCE_IMAGES = 10;
const PUBLIC_WAN_FIRST_LAST_MAX_DURATION_SECONDS = 5.1;
const VISITOR_IDENTITY_NOISE_WORDS = new Set([
...
const normalizeVisitorDescription = (value) => String(value || '')
  .toLowerCase()
  .replace(/[^a-z0-9äöüß\s-]/g, ' ')
  .replace(/\s+/g, ' ')
  .trim();

const extractVisitorIdentityTokens = (value) => new Set(
  normalizeVisitorDescription(value)
    .split(/[\s-]+/)
    .filter((token) => token.length > 2 && !VISITOR_IDENTITY_NOISE_WORDS.has(token))
);

export const isLikelyNewVisitorReference = ({
  previousDescription = '',
  nextDescription = '',
  nextStrength = 0,
} = {}) => {
  const previous = normalizeVisitorDescription(previousDescription);
  const next = normalizeVisitorDescription(nextDescription);
  if (!next || Number(nextStrength) <= 0) {
    return false;
  }
  if (!previous) {
    return true;
  ...
  const previousTokens = extractVisitorIdentityTokens(previous);
  const nextTokens = extractVisitorIdentityTokens(next);
  if (previousTokens.size < 2 || nextTokens.size < 2) {
    return false;
  }
  const sharedTokenCount = [...previousTokens]
    .filter((token) => nextTokens.has(token))
    .length;
  const overlap = sharedTokenCount / Math.min(previousTokens.size, nextTokens.size);
  return overlap < 0.4;
};
...
const truncatePromptText = (value, maxCharacters = 600) => {
  ...
  return `${shortened.slice(0, lastBoundary > maxCharacters * 0.6 ? lastBoundary : maxCharacters).trim()}.`;
};
...
const buildPersonaReferenceIdentityClauseFromConfig = (openingImageConfig = {}) => {
  ...
  // A story may invent a visitor as a dramatic device. A real webcam person
  // stays the first identity reference, while the story can still add people
  // when that is part of the scene.
  const cameraPersonTakesPrecedence = personaReferencePath
    ? 'The real person in the saved webcam anchor has priority over story character details. When the story describes a visitor, guest, or character, adapt that character first to the real camera person instead of replacing their identity with invented hair, clothing, age, or gender.'
    : '';
  ...
  if (personaReferenceDescription) {
    ...
    return [normalizeIdentityClause(identityLock), cameraPersonTakesPrecedence]
      .filter(Boolean)
      .join(' ');
  }
  ...
  if (personaReferenceVisionText) {
    return [
      ...
      cameraPersonTakesPrecedence,
    ].filter(Boolean).join(' ');
  }

  return cameraPersonTakesPrecedence;
};
...
class Generator extends PostTo {
  ...
  constructor(modelConfig) {
    ...
    this.pendingPersonaReferenceIsNew = false;
  ...
  async generateCastContextFrame({
    ...
    const castReferences = Array.isArray(scenePlanEntry?.castReferences)
      ...
      : [];
    const maxReferences = resolvePositiveInt(castContextConfig?.maxReferences, 3);
    const selectedCastReferences = castReferences.slice(0, maxReferences);
    const referencePaths = [...new Set(selectedCastReferences
    ...
    const castLabels = selectedCastReferences
  ...
  async generateNewVisitorTransitionTarget({
    startFrame,
    scenePlanEntry = {},
    sceneContext = {},
    imageOptions = {},
    ...
    const startImagePath = String(startFrame?.image?.path || '').trim();
    const cameraReferencePath = String(this.pendingPersonaReferencePath || '').trim();
    if (
      this.pendingPersonaReferenceIsNew !== true
      || String(scenePlanEntry?.videoMode || '').trim() !== 'firstLast'
      || !startImagePath
      || !cameraReferencePath
      || !(await fs.pathExists(startImagePath))
      || !(await fs.pathExists(cameraReferencePath))
    ) {
      return null;
    }

    const imageModel = this.castContextImageModel || this.driftCorrectionModel || this.personaReferenceImageModel;
    if (!imageModel) {
      return null;
    }
    ...
    const castContextConfig = this.getCastContextConfig();
    const maxReferences = resolvePositiveInt(castContextConfig?.maxReferences, 3);
    const castReferencePaths = [];
    for (const reference of (Array.isArray(scenePlanEntry?.castReferences)
      ? scenePlanEntry.castReferences
      : []).slice(0, maxReferences)) {
      const referencePath = String(reference?.referenceImage || '').trim();
      if (
        referencePath
        && referencePath !== startImagePath
        && referencePath !== cameraReferencePath
        && await fs.pathExists(referencePath)
      ) {
        castReferencePaths.push(referencePath);
      }
    }
    const referencePaths = [cameraReferencePath, ...new Set(castReferencePaths)]
      .slice(0, maxReferences + 1);
    const prompt = [
      'Create only the physically realistic destination frame for a First/Last CCTV transition.',
      'Keep the previous film last frame as continuity truth for room geometry, viewpoint, lighting, and ongoing story; never replace the actual transition start frame.',
      'The fresh live camera reference contains a newly detected real visitor and the current real room. Bring that visitor into the destination as a natural optional presence without replacing existing people or forcing them into the planned action.',
      'Render one coherent real surveillance-camera frame with documentary deep focus, plausible scale, contact, occlusion, material texture, and room-lit shadows.',
      scenePlanEntry?.castUse ? `Optional cast memory: ${scenePlanEntry.castUse}` : '',
      scenePlanEntry?.stillPrompt || scenePlanEntry?.imageDescription || scenePlanEntry?.storyBeat || scenePlanEntry?.beat || '',
    ].filter(Boolean).join(' ');

    try {
      logger.info(`new visitor transition target: scene ${sceneContext?.index || 0}, camera=${cameraReferencePath}`);
      const targetFrame = await imageModel.prompt(prompt, {
        ...imageOptions,
        ...(castContextConfig?.model || {}),
        imagePath: startImagePath,
        images: referencePaths.map((referenceImage) => ({ path: referenceImage })),
        priorityReferencePath: cameraReferencePath,
        contextReference: { enabled: true },
        prompt,
        sceneContext,
        frameRole: 'new-visitor-first-last-destination',
        runwareTimeoutMs: resolvePositiveInt(castContextConfig?.timeoutMs, 120000),
      });
      const outputPath = String(targetFrame?.image?.path || targetFrame?.imagePath || '').trim();
      if (!outputPath) {
        return null;
      }
      this.pendingPersonaReferencePath = '';
      ...
      return {
        ...
        json: {
          ...(targetFrame?.json || {}),
          metadata: {
            ...(targetFrame?.json?.metadata || {}),
            prompt,
            sourceLastFramePath: startImagePath,
            newVisitorReferencePath: cameraReferencePath,
            newVisitorTransitionTarget: true,
          },
        },
      };
    } catch (error) {
      logger.warn(`new visitor transition target skipped: scene ${sceneContext?.index || 0}, ${error?.message || error}`);
      return null;
    }
  }
      ...
      try {
        ...
        if (savedReference?.path) {
          const previousDescription = String(
            this.getOpeningImageConfig()?.personaReferenceDescription || ''
          ).trim();
          const nextDescription = String(
            savedReference.metadata?.personDescription
            || capturedReference.metadata?.personDescription
            || ''
          ).trim();
          const nextStrength = Number(
            savedReference.metadata?.personStrength
            || capturedReference.metadata?.personStrength
            || 0
          );
          this.pendingPersonaReferenceIsNew = isLikelyNewVisitorReference({
            previousDescription,
            nextDescription,
            nextStrength,
          });
          ...
          markPersonaReferenceCaptureTimestampOnContext(this);
          logger.info(
            `Updated async persona reference: ${savedReference.path}`
            + ` | newVisitor=${this.pendingPersonaReferenceIsNew}`
          );
        }
  ...
  buildPersonaReferenceImageOptions(options = {}) {
    ...
    return {
      ...
      negative_prompt: [
        ...
        'recast camera person, substitute actor, duplicate face, duplicated subject',
      ].filter(Boolean).join(', '),
  ...
  buildSceneContextFluxPrompt({ scenePlanEntry = {}, sceneContext = {}, contextImage = null } = {}) {
    ...
    const groundedPrompt = buildCameraGroundedPrompt({
      ...
      actorAction: scenePlanEntry?.actorAction,
      actorsInteraction: scenePlanEntry?.actorsInteraction,
      locationAction: scenePlanEntry?.locationAction,
      storyEvent: scenePlanEntry?.storyEvent,
      castReferences: scenePlanEntry?.castReferences,
  ...
  buildDriftCorrectionPrompt({ lastFrame, referenceImage, nextScenePlanEntry, nextSceneContext }) {
    const driftCorrection = this.getDriftCorrectionConfig();
    const currentPersonOnly = driftCorrection?.currentPersonOnly === true;
    const previousPrompt = currentPersonOnly
      ? ''
      : String(lastFrame?.json?.metadata?.prompt || '').trim();
    const openingImageConfig = this.getOpeningImageConfig();
    const openingContinuityAnchor = currentPersonOnly
      ? ''
      : String(openingImageConfig?.continuityAnchor || '').trim();
    const openingContinuityVision = currentPersonOnly
      ? ''
      : String(openingImageConfig?.continuityVisionText || '').trim();
    const personaReferenceIdentityClause = currentPersonOnly
      ? ''
    ...
    const hasFreshCameraReference = referenceImage?.sourceType === 'cameraShot';

    return truncatePromptText([
      'Use this image as the exact visual anchor for a drift-correction still.',
      'Treat the real person, room geometry, camera viewpoint, and practical lighting in the input as primary truth.',
      'Do not preserve an obviously illustrated, decal-like, opaque splash graphic, floating prop, synthetic overlay, or physically impossible contact merely because it appears in the input.',
      'Repair such local artifacts as physically credible real material under the same room light while preserving scale, contact, occlusion, shadows, and underlying surface texture.',
      ...
      currentPersonOnly
        ? 'Use only the current live camera person as a person reference. Do not recover, name, or combine people from earlier shots, opening context, or prior text.'
        : '',
      ...
      driftCorrectionLevel === 'aggressive'
        ...
        : '',
      personaReferenceIdentityClause,
      openingContinuityAnchor
        ? `Canonical opening continuity: ${truncatePromptText(openingContinuityAnchor, 360)}`
        : '',
      !openingContinuityAnchor && openingContinuityVision
        ? `Canonical opening continuity: ${truncatePromptText(openingContinuityVision, 360)}`
        : '',
      'Correct generative drift and restore realism, readability, and continuity.',
      hasFreshCameraReference
        ? 'A fresh live camera reference may contain a newly arrived real visitor. Use it as optional room and cast evidence without replacing existing people or forcing the visitor into the planned action.'
        : '',
      nextTitle ? `Prepare the next scene title: ${nextTitle}.` : '',
      nextBeat
        ? `Prepare the next beat without adding new props that are not already visible: ${truncatePromptText(nextBeat, 420)}`
        : '',
      previousPrompt
        ? `Preserve from the previous shot: ${truncatePromptText(previousPrompt, 520)}`
        : '',
    ].filter(Boolean).join(' '), 2900);
  }

  buildDriftReferenceRolesPrompt({
    primaryInputPath,
    referenceImagePath,
    openingImagePath,
    ...
    const roles = [
      'Reference image order is deliberate.',
      'Reference image 1 is the generated last frame: preserve its immediate action and camera continuity.',
    ];
    let nextIndex = 2;

    if (referenceImagePath && referenceImagePath !== primaryInputPath) {
      roles.push(`Reference image ${nextIndex} is the current real camera capture: it controls the visible person identity, room geometry, left-right layout, and practical lighting.`);
      nextIndex += 1;
    }
    if (openingImagePath && openingImagePath !== primaryInputPath && openingImagePath !== referenceImagePath) {
      roles.push(`Reference image ${nextIndex} is the canonical opening room image: preserve its geometry and left-right layout.`);
      nextIndex += 1;
    }
    if (contextScreenshotPaths.length > 0) {
      roles.push(`Reference images ${nextIndex} onward are older continuity only; use them to resolve conflicts, never as a collage or a new scene.`);
    }
  ...
  async correctLastFrameForNextScene({ lastFrame, nextScenePlanEntry, nextSceneContext }) {
    ...
    const currentPersonOnly = driftCorrection?.currentPersonOnly === true;
    const openingImageConfig = this.getOpeningImageConfig();
    const openingImagePath = !currentPersonOnly && typeof openingImageConfig?.imagePath === 'string'
      ? path.resolve(openingImageConfig.imagePath)
      : '';
    const contextBufferConfig = this.getContextBufferConfig();
    const useContextBuffer = !currentPersonOnly && contextBufferConfig.enabled;
    ...
    // The current-person-only mode never sends historic people. Otherwise
    // Runware FLUX.2 flex accepts at most ten images, including the primary
    // last frame, with fresh camera and opening anchors taking priority.
    const priorityReferencePaths = [referenceImagePath, openingImagePath]
      .filter(Boolean)
      .filter((entryPath, index, paths) => (
        entryPath !== primaryInputPath && paths.indexOf(entryPath) === index
      ));
    const availableContextSlots = Math.max(
      0,
      MAX_DRIFT_CORRECTION_REFERENCE_IMAGES - 1 - priorityReferencePaths.length
    );
    const selectedContextScreenshotPaths = contextScreenshotPaths
      .filter((entryPath) => (
        entryPath !== primaryInputPath && !priorityReferencePaths.includes(entryPath)
      ))
      .slice(-availableContextSlots);
    const secondaryReferencePaths = [
      ...priorityReferencePaths,
      ...selectedContextScreenshotPaths,
    ];
    ...
    const correctionPrompt = [
      this.buildDriftCorrectionPrompt({
        lastFrame,
        referenceImage,
        nextScenePlanEntry,
        nextSceneContext,
      }),
      (typeof this.buildDriftReferenceRolesPrompt === 'function'
        ? this.buildDriftReferenceRolesPrompt.bind(this)
        : Generator.prototype.buildDriftReferenceRolesPrompt.bind(this))({
        primaryInputPath,
        referenceImagePath,
        openingImagePath,
        contextScreenshotPaths: selectedContextScreenshotPaths,
      }),
    ].filter(Boolean).join(' ');
    const correctionOptions = {
      ...
      negative_prompt: [
        ...
        'illustration, graphic decal, painted splash, floating prop, synthetic overlay',
      ].filter(Boolean).join(', '),
  ...
  async generateSceneClip({
    ...
    const personaReferenceIdentityClause = typeof this.buildPersonaReferenceIdentityClause === 'function'
      ? this.buildPersonaReferenceIdentityClause()
      : '';
    const clipOptions = {
      ...options,
  ...
  async continueSceneLoop({
    ...
    for (let index = startIndex; index < clipCount; index += 1) {
      ...
      const newVisitorTransitionFrame = typeof this.generateNewVisitorTransitionTarget === 'function'
        ? await this.generateNewVisitorTransitionTarget({
        ...
        : null;
      let castContextEndFrame = newVisitorTransitionFrame?.image?.path
        ? {
            path: newVisitorTransitionFrame.image.path,
            prompt: newVisitorTransitionFrame.json?.metadata?.prompt || '',
          }
        : null;
      if (castContextEndFrame) {
      ...
      const shouldGenerateCastContextFrame = !castContextEndFrame
      ...
      const castContextFrame = shouldGenerateCastContextFrame
        ...
        : null;
      if (!castContextEndFrame && castContextFrame?.image?.path) {
      ...
      let endFrameOverride = castContextEndFrame;
      ...
      const castContextDecision = plannedSingleImageContinuation
        ...
        : newVisitorTransitionFrame?.image?.path
          ? 'generated as First/Last destination from fresh real visitor reference'
          : castContextEndFrame
