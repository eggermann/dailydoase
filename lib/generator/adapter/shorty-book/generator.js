...
export const compactFluxPromptForProvider = ({
  ...
  if (normalizedPrompt.length <= maxCharacters) {
    return translateSemanticTermsForProductionPrompt(normalizedPrompt, scenePlanEntry);
  }

  const compacted = [
    'Edit the supplied photograph into one realistic semantic scene.',
    (scenePlanEntry.semanticAnchorEnglish || scenePlanEntry.semanticAnchor)
      ? `Inherited semantic anchor: ${compactPromptFragment(
        scenePlanEntry.semanticAnchorEnglish || scenePlanEntry.semanticAnchor,
        100
      )}`
      : '',
    (scenePlanEntry.semanticCollisionEnglish || scenePlanEntry.semanticCollision)
      ? `Fresh semantic collision: ${compactPromptFragment(
        scenePlanEntry.semanticCollisionEnglish || scenePlanEntry.semanticCollision,
        100
      )}`
      : '',
    scenePlanEntry.inheritedConsequence
      ? `Inherited visible state: ${compactPromptFragment(scenePlanEntry.inheritedConsequence)}`
      : '',
    scenePlanEntry.semanticCollisionPhysicalization
      ? `Physical rule: ${compactPromptFragment(scenePlanEntry.semanticCollisionPhysicalization, 280)}`
      : '',
    scenePlanEntry.monsterTactic
      ? `Monster tactic: ${compactPromptFragment(scenePlanEntry.monsterTactic)}`
      : '',
    scenePlanEntry.semanticAction
      ? `Visible action: ${compactPromptFragment(scenePlanEntry.semanticAction)}`
      : '',
    scenePlanEntry.offscreenMonsterAction
      ? `Off-screen action: ${compactPromptFragment(scenePlanEntry.offscreenMonsterAction)}`
      : '',
    scenePlanEntry.visibleEvidenceOfAgency
      ? `Visible agency evidence: ${compactPromptFragment(scenePlanEntry.visibleEvidenceOfAgency)}`
      : '',
    scenePlanEntry.monsterPresence
      ? `Monster presence: ${compactPromptFragment(scenePlanEntry.monsterPresence)}`
      : '',
    scenePlanEntry.localConsequence
      ? `Visible consequence: ${compactPromptFragment(scenePlanEntry.localConsequence)}`
      : '',
    scenePlanEntry.stillPrompt
      ? `Decisive still: ${compactPromptFragment(scenePlanEntry.stillPrompt, 360)}`
      : '',
    'Preserve the exact Kaufhaus camera viewpoint, crop, exposure, floor plan, ceiling, ducts, windows, columns, walls, elevators, floor, lens perspective, and at least eighty percent of the photographed room.',
    'Keep exactly one recognizable Green Monster and confine mutation to its body, attached anatomy, nearby lamps, objects, reflections, or shadows.',
    'No humans, portraits, silhouettes, duplicate creatures, poster layout, text, logos, panels, borders, or magenta background.',
  ].filter(Boolean).join(' ');
...
const readSemanticCollision = (scenePlanEntry = {}) => {
  const structuredAnchor = String(
    scenePlanEntry?.semanticAnchorEnglish || scenePlanEntry?.semanticAnchor || ''
  ).trim();
  const structuredCollision = String(
    scenePlanEntry?.semanticCollisionEnglish || scenePlanEntry?.semanticCollision || ''
  ).trim();
  if (structuredAnchor || structuredCollision) {
    return {
      anchor: structuredAnchor,
      collision: structuredCollision,
    };
  }

  const semanticCue = String(scenePlanEntry?.semanticCue || '').trim();
  const anchorMatch = semanticCue.match(/Anchor \([^)]*\): ([\s\S]*?)\. Collision A \(/);
  const collisionMatch = semanticCue.match(
    /Collision A \([^)]*\): ([\s\S]*?)\.(?: Context:| Dramatic function:)/
  );

  return {
    anchor: String(anchorMatch?.[1] || '').trim(),
    collision: String(collisionMatch?.[1] || '').trim(),
  };
};
...
class Generator extends PostTo {
  ...
  buildSceneContextFluxPrompt({
    ...
    const semanticCollision = readSemanticCollision(scenePlanEntry);
    ...
    if (protagonistAlreadyComposited) {
      return [
        'Transform the supplied Kaufhaus-and-monster photograph into the next unmistakably different surreal scene.',
        storyBeat ? `Required physical event: ${storyBeat}` : '',
        basePrompt ? `Required visible anatomy and action: ${basePrompt}` : '',
        semanticCollision.anchor ? `Semantic Anchor: ${semanticCollision.anchor}.` : '',
        semanticCollision.collision ? `Semantic Collision: ${semanticCollision.collision}.` : '',
        'Make this collision readable at thumbnail size. Rebuild at least one third of the monster silhouette, branching anatomy, and attached mechanisms around the required event while preserving its face and identity.',
        'A color shift, new glow, minor texture change, or unchanged reference pose is not a semantic transformation and is forbidden.',
        'Keep the real Kaufhaus unmistakably recognizable: preserve the exact camera viewpoint, crop, floor plan, major structural geometry, concrete floor, fluorescent ceiling grid, exposed ducts, windows, walls, doors, elevators, and mirrored columns.',
        'Preserve at least eighty percent of the visible room unchanged, including its original exposure, brightness, color balance, surface texture, lens perspective, and architectural objects.',
        'Do not replace the location with a generic warehouse or fantasy interior. Do not apply a global cinematic color grade and do not darken the whole room.',
        'Keep the existing single Green Monster as the only protagonist and only living figure.',
        'Never create a second body or person-shaped form: no humanoid sculpture, mannequin, statue, human-like plant, duplicate monster, portrait, or person-shaped shadow.',
        'Keep these two terms in productive conflict. Do not explain or harmonize them.',
        'Use the preceding scene direction only for monster anatomy, physical action, and nearby semantic consequences. Ignore any conflicting request inside it to change the location, global palette, exposure, camera, or architectural style.',
        'Make the semantic collision visibly transform the monster, its attached organic-mechanical details, the nearest overhead lamps, and nearby floor reflections or shadows.',
        'Confine surreal mutation to the monster and its immediate surroundings. The untouched Kaufhaus photograph remains the dominant image.',
        'No humans, portraits, extra creatures, poster panels, borders, labels, callout lines, numbers, logos, or text.',
        'Output one realistic edited Kaufhaus photograph, not a poster or illustration.',
      ].filter(Boolean).join(' ');
    }
    if (sceneContextImageConfig?.allowPeople === false) {
      const promptOnlyIdentity = sceneContextImageConfig?.protagonistReferenceMode === 'promptOnly';
      return [
        'Use the first supplied image as the immutable Kaufhaus location plate.',
        'Preserve at least eighty percent of that photograph unchanged: exact camera viewpoint, crop, exposure, brightness, color balance, floor plan, concrete floor, fluorescent ceiling grid, ducts, windows, walls, columns, elevators, furniture, and architectural objects.',
        'Never replace, darken, redesign, extend, or globally restyle the Kaufhaus. Never create a generic dark warehouse or fantasy interior.',
        promptOnlyIdentity
          ? 'Build identity from the realistic Green Monster vocabulary: one solemn elongated green bark face, amber glowing eyes, two large leaf-shaped ears, one trunk-like mouth tendril, a rooted botanical body, branching plant anatomy, attached flower organs, hanging amber lamps, and aged brass mechanisms. Preserve these identity traits without reproducing one fixed silhouette.'
          : 'Use the separate creature image only as an identity and material vocabulary: realistic botanical fibers, face, glowing eyes, leaf anatomy, roots, attached lamps, and aged brass mechanisms. Ignore its flat magenta background completely.',
        'Construct exactly one fresh scene-specific incarnation of the Green Monster. Preserve identity, but visibly rebuild its pose, silhouette, branching anatomy, and attached mechanisms; do not paste or reproduce the reference pose.',
        semanticCollision.anchor ? `Semantic Anchor: ${semanticCollision.anchor}.` : '',
        semanticCollision.collision ? `Semantic Collision: ${semanticCollision.collision}.` : '',
        'Keep the two semantic terms in productive conflict; do not explain or harmonize them.',
        storyBeat ? `Surreal scene event: ${storyBeat}` : '',
        'The semantic conflict must become an unmistakable physical mutation of the monster, not merely a decorative glow or unchanged reference cutout. Make the planned surreal event clearly visible around and through its body.',
        'Place the newly constructed monster naturally on the open Kaufhaus floor with physically credible scale, perspective, contact shadow, and reflections.',
        'Confine semantic mutations to the monster, its attached anatomy, the nearest overhead lamps, and nearby floor reflections. The untouched raw Kaufhaus photograph must remain the dominant image.',
        'No humans, portraits, silhouettes, extra creatures, separate plant figures, poster layout, panels, borders, labels, callout lines, numbers, logos, text, or magenta backdrop.',
        'Output one realistic edited Kaufhaus photograph, not an illustration or concept-art environment.',
      ].filter(Boolean).join(' ');
    }
    const protagonistRule = sceneContextImageConfig?.allowPeople === false
      ? 'The isolated Green Monster is the only protagonist and the only living figure. Do not create humans, portraits, faces, silhouettes, crowds, information panels, poster layouts, borders, callout lines, numbers, labels, or text.'
    ...
    const referenceRule = 'Use the separate creature image only for Green Monster identity, realistic botanical materials, face, glowing eyes, and attached lamp anatomy. Ignore its flat magenta isolation background completely. Never copy a reference backdrop, former poster composition, or graphic page layout.';
    ...
    const semanticStillDirection = translateSemanticTermsForProductionPrompt([
      String(scenePlanEntry?.inheritedConsequence || '').trim(),
      String(scenePlanEntry?.semanticCollisionPhysicalization || '').trim(),
      String(scenePlanEntry?.stillPrompt || '').trim(),
      String(scenePlanEntry?.monsterTactic || '').trim(),
      String(scenePlanEntry?.semanticAction || '').trim(),
      String(scenePlanEntry?.monsterPresence || '').trim(),
      String(scenePlanEntry?.localConsequence || scenePlanEntry?.roomConsequence || '').trim(),
      String(scenePlanEntry?.viewpoint || '').trim(),
    ].filter(Boolean).join(' '), scenePlanEntry);
    const sceneDirection = semanticStillDirection || groundedPrompt || basePrompt;

    return translateSemanticTermsForProductionPrompt([
      'Use the first provided scene context image as the immutable Kaufhaus location plate for this scene.',
      'Preserve at least eighty percent of the photographed room unchanged: exact camera viewpoint, crop, exposure, brightness, color balance, lens perspective, floor plan, floor, ceiling, ducts, windows, columns, walls, elevators, furniture, and structural geometry.',
      'The result must remain immediately recognizable as this exact Kaufhaus photograph. Do not replace, dissolve, darken, redesign, extend, or globally restyle it as a generic warehouse, exhibition hall, or fantasy interior.',
      referenceRule,
      protagonistRule,
      personaReferenceIdentityClause,
      ...
      semanticCollision.anchor ? `Semantic Anchor: ${semanticCollision.anchor}.` : '',
      semanticCollision.collision ? `Semantic Collision: ${semanticCollision.collision}.` : '',
      'Construct one fresh scene-specific incarnation of the Green Monster from this semantic conflict. The identity reference is a vocabulary, not a fixed pose or fixed composite.',
      'Derive all variable scene content from the semantic conflict and the protagonist action. Confine surreal mutation to the monster, semantically justified people, movable objects, light, reflections, shadows, and one nearby architectural detail; leave the rest of the location plate untouched.',
      sceneDirection ? `Scene-specific direction: ${sceneDirection}` : '',
  ...
  buildSceneContextSemanticReconstructionPrompt({ scenePlanEntry = {}, sceneContext = {} } = {}) {
    const semanticCollision = readSemanticCollision(scenePlanEntry);
    const storyBeat = selectSceneStoryBeat({ scenePlanEntry, sceneContext });
    const visibleAction = selectFluxStillDirection({ scenePlanEntry, sceneContext });

    return [
      'Edit this newly generated Kaufhaus-and-monster seed into the final semantic scene.',
      'Preserve the exact photographed Kaufhaus viewpoint, exposure, floor plan, ceiling, ducts, windows, columns, walls, elevators, furniture, and at least eighty percent of the visible room.',
      'Preserve the Green Monster identity already visible in the seed: elongated bark face, amber eyes, leaf ears, trunk-like mouth tendril, roots, botanical fibers, attached lamps, and aged brass mechanisms.',
      semanticCollision.anchor ? `Semantic Anchor: ${semanticCollision.anchor}.` : '',
      semanticCollision.collision ? `Semantic Collision: ${semanticCollision.collision}.` : '',
      'Reconstruct the monster through the conflict: visibly change its pose, silhouette, branching anatomy, and attached mechanisms while keeping it recognizably the same organism.',
      storyBeat ? `Required physical event: ${storyBeat}` : '',
      visibleAction ? `Required visible subject action: ${visibleAction}` : '',
      'Use that scene direction only for monster anatomy, physical action, and nearby consequences; ignore its location, global palette, exposure, camera, and architectural style language.',
      'The result must show at least one unmistakable physical mutation in the monster body and one visible consequence in a nearby lamp, object, reflection, or shadow. A simple glow, unchanged cutout, or copied reference pose is insufficient.',
      'Keep exactly one living organism. All flowers, faces, lamps, roots, and branches must remain visibly attached parts of that one monster.',
      'No humans, extra creatures, separate plant figures, generic fantasy room, global darkening, poster layout, text, labels, logos, or magenta background.',
  ...
  async generateSceneContextFluxFrame({
    ...
    const protagonistAlreadyComposited = typeof protagonistAlreadyCompositedOverride === 'boolean'
      ...
      : sceneContextImageConfig?.protagonistAlreadyComposited === true;
    const promptOnlyIdentity = sceneContextImageConfig?.protagonistReferenceMode === 'promptOnly';
    const referenceImages = !protagonistAlreadyComposited
      && !promptOnlyIdentity
      && protagonistReferencePath
      && await fs.pathExists(protagonistReferencePath)
  ...
  shouldApplyDriftCorrection({ nextScenePlanEntry, nextSceneContext } = {}) {
    const driftCorrection = this.getDriftCorrectionConfig();
  ...
  buildDriftCorrectionPrompt({ lastFrame, nextScenePlanEntry, nextSceneContext }) {
    ...
    const endFrameContinuity = String(
    ...
    return [
      ...
      endFrameContinuity ? `Actual visible end-frame continuity: ${endFrameContinuity}` : '',
  ...
  async analyzeLastFrameForNextScene({ lastFrame, nextScenePlanEntry, nextSceneContext } = {}) {
    ...
    try {
      ...
      const continuity = summarizeVisionStoryContext(outputText) || outputText;
      if (!continuity) {
        return lastFrame;
      }
  ...
  async promptSceneImagesOnly(options, fileName) {
    ...
    for (let index = 0; index < scenePlan.length; index += 1) {
      ...
      const generatedFrame = await this.generateSceneContextFluxFrame({
        ...
        contextImageOverride: continueFromPreviousScene
          ? { path: previousSceneOutputPath }
          : null,
        protagonistAlreadyCompositedOverride: continueFromPreviousScene
          ? true
          : undefined,
      });
      ...
      generatedScenes.push({
        ...
        title: scenePlanEntry?.title || '',
        storyBeat: sceneContext.storyBeat,
        startFrameStrategy,
        startFrameReason: scenePlanEntry?.startFrameReason || '',
        semanticCue: scenePlanEntry?.semanticCue || '',
        semanticAnchor: scenePlanEntry?.semanticAnchor || '',
        semanticAnchorEnglish: scenePlanEntry?.semanticAnchorEnglish
          || scenePlanEntry?.semanticAnchor
          || '',
        semanticCollision: scenePlanEntry?.semanticCollision || '',
        semanticCollisionEnglish: scenePlanEntry?.semanticCollisionEnglish
          || scenePlanEntry?.semanticCollision
          || '',
        semanticCollisionPhysicalization: scenePlanEntry?.semanticCollisionPhysicalization || '',
        semanticDerivation: scenePlanEntry?.semanticDerivation || null,
        semanticConflict: scenePlanEntry?.semanticConflict || '',
        monsterIntent: scenePlanEntry?.monsterIntent || '',
        monsterPresenceMode: scenePlanEntry?.monsterPresenceMode || '',
        offscreenMonsterAction: scenePlanEntry?.offscreenMonsterAction || '',
        visibleEvidenceOfAgency: scenePlanEntry?.visibleEvidenceOfAgency || '',
        localConsequence: scenePlanEntry?.localConsequence || '',
        consequenceId: scenePlanEntry?.consequenceId || '',
        inheritsConsequenceId: scenePlanEntry?.inheritsConsequenceId || '',
        clue: scenePlanEntry?.clue || '',
        clueSource: scenePlanEntry?.clueSource || 'none',
        tensionLevel: Number.isFinite(Number(scenePlanEntry?.tensionLevel))
          ? Number(scenePlanEntry.tensionLevel)
          : null,
        tensionCause: scenePlanEntry?.tensionCause || '',
        nextSceneHook: scenePlanEntry?.nextSceneHook || '',
        semanticSourceMode: continueFromPreviousScene
          ? 'previousSceneImage'
          : 'locationAndIdentityReferences',
        prompt: generatedFrame?.json?.metadata?.prompt
          || scenePlanEntry?.stillPrompt
          || scenePlanEntry?.imageDescription
          || '',
        scenePlanPrompt: scenePlanEntry?.stillPrompt || scenePlanEntry?.imageDescription || '',
