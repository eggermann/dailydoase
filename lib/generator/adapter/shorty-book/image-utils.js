...
export const resolveImageModel = (imgType, models) => {
  const forcedType = String(process.env.FRESHWEB_IMAGE_TYPE || '').trim();
  if (forcedType === 'aiqtech-NSFW-Real' && models.aiqtech) return models.aiqtech;
  if (forcedType === 'imageActorInScene' && models.imageActorInScene) return models.imageActorInScene;
  if (forcedType === 'flux' && models.flux) return models.flux;
