...
test('normalizes explicit and legacy reflection appearances', () => {
  ...
  expect(normalizeSceneAppearance({}, {
    ...
    mode: 'reflection',
    subject: 'archival close-up',
    surface: 'the existing eyeglass lenses',
    inferred: true,
  }));
});
...
test('legacy reflection remains optically contained instead of becoming a sticker', () => {
  const result = compileFluxEditPrompt({
    scene: {
  ...
  expect(result.appearance.subject).toBe('single archival close-up');
  expect(result.appearance.surface).toBe('the existing eyeglass lenses');
  expect(result.prompt).toContain('remain optically contained inside the reflective surface');
});
...
test('compiles WAN as motion without scene title or story explanation', () => {
  ...
  expect(result.prompt).toContain('stays attached to both eyeglass lenses');
