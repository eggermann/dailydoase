...
test('compiles First/Last as one motion and locked CCTV rule', () => {
  const result = compileWanFirstLastMotionPrompt({
    scene: {
      storyBeat: 'The room becomes a nightmare.',
      videoPrompt: 'The painter rises in the room while the fixed camera holds steady.',
      motionCue: 'He rises from kneeling, takes two steps, and draws without stopping.',
      cameraCue: 'Static aisle-end CCTV hold.',
    },
    model: 'alibaba:wan@2.7',
  });

  expect(result.prompt).toBe('Locked-off CCTV view. He rises from kneeling, takes two steps, and draws without stopping.');
  expect(result.prompt).not.toMatch(/room|nightmare|static|hold steady|camera movement/i);
