...
test('direct Mirelo success muxes audio and returns the merged video', async () => {
  ...
  expect(mireloAI.prompt).toHaveBeenCalledWith(
    '/tmp/silent-trailer.mp4',
...
test('Runware Mirelo WAV fallback is muxed into the final trailer', async () => {
  generateRunwareMireloFallback.mockResolvedValue({
    file: '/tmp/runware-fallback.wav',
    cost: 0.18,
  });
  muxVideoAndAudio.mockResolvedValue('/tmp/out/merged/trailer-with-sound.mp4');
  ...
  expect(result).toBe('/tmp/out/merged/trailer-with-sound.mp4');
  expect(muxVideoAndAudio).toHaveBeenCalledWith(
    '/tmp/silent-trailer.mp4',
    '/tmp/runware-fallback.wav',
    '/tmp/out/merged',
    { outputName: 'trailer-with-sound.mp4' }
  );
});
