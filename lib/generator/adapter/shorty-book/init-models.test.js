...
import {
  ...
  shouldInitCastContextImageModel,
} from './init-models.js';
...
test('shouldInitCastContextImageModel follows the optional cast-context switch', () => {
  expect(shouldInitCastContextImageModel({
    sceneLoop: { castContext: { enabled: true } },
  })).toBe(true);
  expect(shouldInitCastContextImageModel({
    sceneLoop: { castContext: { enabled: false } },
  })).toBe(false);
});
