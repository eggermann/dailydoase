...
export const VIEWPOINT_MOTION_RULE = [
  'Use one physically coherent, continuous viewpoint movement chosen to reveal the event.',
].join(' ');
...
export const buildWanPrompt = ({ scene = {}, allowPeople = true } = {}) => {
  ...
  const durationRule = Number(durationSeconds) <= 2
    ? 'Keep the viewpoint nearly fixed; use one tiny adjustment at most.'
    : 'Use one restrained viewpoint movement at most.';
