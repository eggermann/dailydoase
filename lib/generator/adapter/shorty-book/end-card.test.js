...
test('uses the dedicated readable layout for vertical trailer end cards', async () => {
  const outputDir = await fs.mkdtemp(path.join(os.tmpdir(), 'dailydoase-end-card-portrait-'));
  const outputPath = path.join(outputDir, 'end-card-portrait.png');
  const result = await renderExhibitionEndCard({
    dossierPath,
    outputPath,
    width: 576,
