...
export const isMonsterExplicitlyAbsent = (scene = {}) => {
  ...
  const remainingVisibleEvidence = presence
    .replace(/\b(?:not visible|unseen|off-screen|offscreen|no monster|without (?:the )?monster|absent|none)\b/gi, ' ');
  ...
  return /\b(absent|none|not visible|unseen|off-screen|offscreen|no monster|without (?:the )?monster)\b/i
    .test(presence);
};
