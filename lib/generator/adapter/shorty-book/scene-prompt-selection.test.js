...
test('builds an environment FLUX prompt without monster construction', () => {
  ...
  expect(prompt).toContain('grounded practical-effects aesthetic');
  expect(prompt).toContain('physically accurate imperfect materials');
  expect(prompt).toContain('low-key motivated practical lighting');
  expect(prompt).toContain('ARRI Alexa 35 live-action tonal response');
  expect(prompt).toContain('fine analog film grain');
  expect(prompt).toContain('Never render illustration, comic-book art');
  expect(prompt).toContain('must never change the mandatory live-action practical-effects rendering');
