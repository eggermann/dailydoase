...
export const addMireloAudioAndUpload = async ({
  ...
  try {
    ...
    logger.warn('Mirelo prompt failed, returning original video:', error?.message || error);
    return videoSource;
  }
  if (!dataFromMirelo) {
    logger.warn('Mirelo returned no audio file, returning original video.');
    return videoSource;
  }
