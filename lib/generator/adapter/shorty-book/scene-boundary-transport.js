...
const readPeopleCount = (value) => {
  const count = Number(value);
  return Number.isFinite(count) && count >= 0 ? Math.floor(count) : null;
};

export const resolveSceneBoundaryTransportPolicy = ({
  nextScene = {},
  currentPeopleCount,
  cameraPeopleCount,
  cameraImageAvailable = false,
} = {}) => {
  const plannedVideoMode = normalizeText(nextScene.videoMode) || 'singleImage';
  const plannedFrameSource = normalizeText(nextScene.frameSource) || 'lastFrame';
  const plannedUsesCamera = nextScene.useCameraShot === true
    || plannedFrameSource === 'newImage';
  const plannedCameraTransport = cameraImageAvailable
    && plannedUsesCamera
    && (plannedVideoMode === 'firstLast' || plannedFrameSource === 'newImage');
  const previousCount = readPeopleCount(currentPeopleCount);
  const freshCount = readPeopleCount(cameraPeopleCount);
  const peopleChanged = previousCount !== null
    && freshCount !== null
    && previousCount !== freshCount;
  const shouldAskModel = Boolean(cameraImageAvailable && (plannedCameraTransport || peopleChanged));
  ...
  return {
    ...
    plannedFrameSource,
    plannedUsesCamera,
    plannedCameraTransport,
    peopleChanged,
    currentPeopleCount: previousCount,
    cameraPeopleCount: freshCount,
    reason: shouldAskModel
      ? (peopleChanged
        ? 'fresh camera person count changed'
        : 'next scene already requests camera transport')
      : (cameraImageAvailable
        ? 'next scene plans generated-frame continuation and person count is unchanged'
        : 'fresh camera image unavailable'),
  };
};
...
export const createSceneBoundaryTransportDecider = ({
  ...
  return async ({ completedScene = {}, nextScene = {}, generatedLastFramePath = '', cameraShot = {}, roomMemory = '', storyTransport = '', transportPolicy = null } = {}) => {
    const messages = [{
      role: 'system',
      content: [
        ...
        'The complete scene planner is authoritative. Do not replace a planned lastFrame + singleImage continuation merely because a newer camera pose or closer framing is available.',
        ...
        'The physical cameras stay fixed. Never invent a dolly, zoom, pan, tilt, cross-dissolve, cinematic reframe, or a new high-corner angle.',
        'For locationReturn, transitionPrompt describes subject or room motion connecting the two supplied endpoints. First/Last interpolation supplies the transport itself.',
        'For continue, keep transitionPrompt empty unless one short causal residue is needed.',
      ...
      content: [
        ...
        transportPolicy ? `Transport policy: ${JSON.stringify(transportPolicy)}` : '',
