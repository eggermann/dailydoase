...
export const createStoryTransportController = ({
  ...
  return {
    ...
    rememberCameraBoundary({
      ...
      const observedActors = people.actors.map((actor) => ({
        ...actor,
        status: 'observed-at-scene-boundary',
        lastScene: Number(sceneIndex) || undefined,
        lastStoryState: 'Observed by the live camera between completed shots.',
      }));
      actorReferences.touchMany(observedActors);
      return {
        people: { ...people, actors: observedActors },
        actorReferences: actorReferences.values(),
      };
    },
