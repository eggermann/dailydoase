...
export const createStoryTransportDraft = ({
  ...
  return {
    ...
    // All streams still contribute source cues in every iteration. Rotating the
    // lead word prevents the same first stream from becoming the permanent
    // headline of an otherwise changing live sequence.
...
export const completeStoryTransport = ({
  ...
  const beats = Array.isArray(scenePlan)
    ? scenePlan.map(readSceneBeat).filter(Boolean)
    : [];
...
const describeCastMemory = (cast = {}, maxPlannerReferences = 3) => {
  ...
  return `CAST MEMORY — optional recent library (${references.length}/${cast.actorReferenceLimit || references.length} newest): ${cards.join('; ')}.`;
};
