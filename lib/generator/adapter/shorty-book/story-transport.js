...
export const formatStoryTransportForPrompt = (transport = {}) => {
  const semanticWords = compactList(transport.topics);
  const lines = [
    `Current topic word: ${normalizeText(transport.topic) || 'n/a'}.`,
    ...(semanticWords.length > 0
      ? [
          `Semantic word stream (every term is mandatory for this iteration): ${semanticWords.join(' | ')}.`,
          'Give every term a visible, causally connected role across the scene plan; never silently drop a term.',
        ]
      : []),
