...
const saveSemanticScenePlanArtifacts = async ({
  ...
  await Promise.all([
    ...
    fs.writeJson(path.join(promptDir, 'validated-scene-plan.json'), validatedScenePlan, { spaces: 2 }),
    fs.writeJson(
      path.join(promptDir, 'semantic-validation-report.json'),
      semanticValidation?.reports || [],
      { spaces: 2 }
    ),
  ]);
};
