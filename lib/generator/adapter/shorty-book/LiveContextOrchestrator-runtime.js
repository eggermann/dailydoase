...
const SERIES_CONTEXT = pickEnvValue('FRESHWEB_SERIES_CONTEXT');
const SOURCE_CUE_MIX_TYPE = String(
  pickEnvValue('FRESHWEB_SOURCE_CUE_MIX_TYPE') || 'random'
).trim().toLowerCase() || 'random';
...
const CONFIG = {
  story: {
    ...
    sourceCueMixType: SOURCE_CUE_MIX_TYPE,
...
const promptFunktion = async (streams, config) => {
  ...
  const sourceCues = await buildSourceCues({
    ...
    configMode: CONFIG.story.mode,
    requestedMixType: CONFIG.story.sourceCueMixType,
  ...
  const storyTransportDraft = storyTransportController.beginIteration({
    ...
    seriesContext: CONFIG.story.seriesContext,
    sourceCues,
    visionStoryContext: structuredVisionStoryContext,
    referenceImagePath: openingCameraShot,
  });
  const storyTransportPrompt = formatStoryTransportForPrompt(storyTransportDraft);
