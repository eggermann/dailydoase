...
const ADVANCE_ON_FAILURE = parseBoolean(
  pickEnvValue('FRESHWEB_ADVANCE_ON_FAILURE'),
  false
);
const FAILURE_RECOVERY_ITERATIONS = parseOptionalPositiveNumber(
  pickEnvValue('FRESHWEB_FAILURE_RECOVERY_ITERATIONS')
);
const FAILURE_RECOVERY_DELAY_MS = parsePositiveNumber(
  pickEnvValue('FRESHWEB_FAILURE_RECOVERY_DELAY_MS'),
  1000
);
const SEMANTIC_STEP_TIMEOUT_MS = parsePositiveNumber(
  pickEnvValue('FRESHWEB_SEMANTIC_STEP_TIMEOUT_MS'),
  90000
);
...
const SCENE_PLAN_TIMEOUT_MS = parsePositiveNumber(
  pickEnvValue('FRESHWEB_SCENE_PLAN_TIMEOUT_MS'),
  45000
);
const SCENE_PLAN_ALLOW_NEUTRAL_FALLBACK = parseBoolean(
  pickEnvValue('FRESHWEB_SCENE_PLAN_ALLOW_NEUTRAL_FALLBACK'),
  true
);
...
const FLUX_VARIANT = pickEnvValue('FRESHWEB_FLUX_VARIANT')
  || 'schnell';
const IMAGE_TYPE = pickEnvValue('FRESHWEB_IMAGE_TYPE');
const IMAGE_MODEL = pickEnvValue('FRESHWEB_IMAGE_MODEL');
const IMAGE_PROVIDER = pickEnvValue('FRESHWEB_IMAGE_PROVIDER');
    ...
    image: {
      ...
      ...(IMAGE_MODEL || IMAGE_PROVIDER ? {
        model: {
          ...
          ...(IMAGE_PROVIDER ? { provider: IMAGE_PROVIDER, hfProvider: IMAGE_PROVIDER } : {}),
        },
      } : {}),
    },
    openingStart: {
...
const sceneGenerator = createWebcamSceneGenerator({
  ...
  timeoutMs: SCENE_PLAN_TIMEOUT_MS,
});
...
const buildSceneLoopConfig = () => ({
  enabled: true,
  semanticWords: CONFIG.story.words
...
const promptFunktion = async (streams, config) => {
  ...
  const { sourceCues, sourceCueRecords } = await buildSourceCueBundle({
    ...
    semanticStepTimeoutMs: SEMANTIC_STEP_TIMEOUT_MS,
  });
  ...
  try {
    scenePlanResult = await generateScenePlanWithFallback({
      ...
      allowNeutralFallback: SCENE_PLAN_ALLOW_NEUTRAL_FALLBACK,
      startFrameStrategy: CONFIG.story.startFrameStrategy,
...
const adapterConfig = {
  ...
  model: {
    ...
    advanceOnFailure: ADVANCE_ON_FAILURE,
    ...(FAILURE_RECOVERY_ITERATIONS !== null
      ? { failureRecoveryIterations: FAILURE_RECOVERY_ITERATIONS }
      : {}),
    failureRecoveryDelayMs: FAILURE_RECOVERY_DELAY_MS,
