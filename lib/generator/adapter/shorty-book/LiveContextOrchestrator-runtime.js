...
import {
  ...
  createConfiguredWebcamImagePromptHandler,
...
const MIN_SCENE_COUNT = parsePositiveNumber(
  pickEnvValue('FRESHWEB_MIN_SCENE_COUNT'),
  1
);
...
const SCENE_LENGTH_TAKTMUSTER_COUNT = parsePositiveNumber(
  pickEnvValue('FRESHWEB_SCENE_LENGTH_TAKT_COUNT', 'FRESHWEB_SCENE_LENGTH_TAKT', 'FRESHWEB_TAKT'),
  4
);
const SCENE_LENGTH_TAKTMUSTER_ZAEHLER = parsePositiveNumber(
  pickEnvValue('FRESHWEB_SCENE_LENGTH_TAKT_ZAEHLER'),
  4
);
const SCENE_LENGTH_TAKTMUSTER_NENNER = parsePositiveNumber(
  pickEnvValue('FRESHWEB_SCENE_LENGTH_TAKT_NENNER'),
  4
);
const SCENE_LENGTH_TAKTMUSTER_INITIAL_SHIFT = Math.max(0, Math.floor(parseFiniteNumber(
  pickEnvValue('FRESHWEB_SCENE_LENGTH_TAKT_SHIFT'),
  0
)));
const SCENE_LENGTH_PRIMARY_TAKTMUSTER_COUNT = parsePositiveNumber(
  pickEnvValue('FRESHWEB_SCENE_LENGTH_PRIMARY_TAKT_COUNT'),
  2
);
const SCENE_LENGTH_PRIMARY_TAKTMUSTER_ZAEHLER = parsePositiveNumber(
  pickEnvValue('FRESHWEB_SCENE_LENGTH_PRIMARY_TAKT_ZAEHLER'),
  4
);
const SCENE_LENGTH_PRIMARY_TAKTMUSTER_NENNER = parsePositiveNumber(
  pickEnvValue('FRESHWEB_SCENE_LENGTH_PRIMARY_TAKT_NENNER'),
  4
);
...
const SCENE_PLAN_REPLAY_PATH = pickEnvValue('FRESHWEB_SCENE_PLAN_REPLAY_PATH');
...
const SCENE_LENGTH_SUM_MAX_SECONDS = parsePositiveNumber(
  pickEnvValue('FRESHWEB_SCENE_LENGTH_SUM_MAX_SECONDS'),
  4
);
...
const ITERATION_START_MODE = String(
  pickEnvValue('FRESHWEB_ITERATION_START_MODE')
  || (CHAIN_FROM_PREVIOUS_LOOP_LAST_FRAME ? 'chain' : 'cameraReset')
).trim();
...
const CAST_CONTEXT_NEGATIVE_PROMPT = pickEnvValue('FRESHWEB_CAST_CONTEXT_NEGATIVE_PROMPT')
  || 'collage, split screen, picture-in-picture, thumbnails, duplicate face, broken anatomy';
...
const ASYNC_WEBCAM_PERSONA_REFERENCE_UPDATES = USE_WEBCAM_PERSONA_REFERENCE
  ...
  : false;
const SCENE_BOUNDARY_TRANSPORT_ENABLED = parseBoolean(
  pickEnvValue('FRESHWEB_SCENE_BOUNDARY_TRANSPORT'),
  true
);
const ROOM_MEMORY_SIZE = parsePositiveNumber(
  pickEnvValue('FRESHWEB_ROOM_MEMORY_SIZE'),
  10
);
...
const CAMERA_IMAGE_PATHS = parsePipeList(
  pickEnvValue('FRESHWEB_CAMERA_IMAGE_PATHS', 'FRESHWEB_OPENING_IMAGE_PATHS'),
  []
);
...
const CAMERA_SOURCE_LABEL = pickEnvValue('FRESHWEB_CAMERA_SOURCE_LABEL')
  || (CAMERA_IMAGE_URLS.length > 0 || CAMERA_IMAGE_PATHS.length > 0 ? 'source frame' : 'webcam shot');
const HAS_CONFIGURED_CAMERA_IMAGE = Boolean(
  CAMERA_IMAGE_PATH
  || CAMERA_IMAGE_PATHS.length > 0
  || CAMERA_IMAGE_URLS.length > 0
);
...
const CONFIG = {
  story: {
    ...
    scenePlanReplayPath: SCENE_PLAN_REPLAY_PATH,
    ...
    sceneLengthTaktmusterCount: SCENE_LENGTH_TAKTMUSTER_COUNT,
    sceneLengthTaktmusterZaehler: SCENE_LENGTH_TAKTMUSTER_ZAEHLER,
    sceneLengthTaktmusterNenner: SCENE_LENGTH_TAKTMUSTER_NENNER,
    sceneLengthTaktmusterInitialShift: SCENE_LENGTH_TAKTMUSTER_INITIAL_SHIFT,
    sceneLengthPrimaryTaktmusterCount: SCENE_LENGTH_PRIMARY_TAKTMUSTER_COUNT,
    sceneLengthPrimaryTaktmusterZaehler: SCENE_LENGTH_PRIMARY_TAKTMUSTER_ZAEHLER,
    sceneLengthPrimaryTaktmusterNenner: SCENE_LENGTH_PRIMARY_TAKTMUSTER_NENNER,
    ...
    sceneLengthSumMaxSeconds: SCENE_LENGTH_SUM_MAX_SECONDS,
    ...
    iterationStartMode: ITERATION_START_MODE,
...
const createTaktmusterRuntime = ({
  ...
  const resolvedInitialShift = Math.max(0, Math.floor(Number(initialShift) || 0));
  for (let step = 0; step < resolvedInitialShift; step += 1) {
    taktmuster.getNext();
  }
  ...
  return {
    ...
    zaehler,
    nenner,
    initialShift: resolvedInitialShift,
...
const sceneLengthTaktmusterRuntime = createTaktmusterRuntime({
  taktCnt: CONFIG.story.sceneLengthTaktmusterCount,
  zaehler: CONFIG.story.sceneLengthTaktmusterZaehler,
  nenner: CONFIG.story.sceneLengthTaktmusterNenner,
  type: CONFIG.story.sceneLengthTaktmusterType,
  initialShift: CONFIG.story.sceneLengthTaktmusterInitialShift,
});

const sceneLengthPrimaryTaktmusterRuntime = createTaktmusterRuntime({
  taktCnt: CONFIG.story.sceneLengthPrimaryTaktmusterCount,
  zaehler: CONFIG.story.sceneLengthPrimaryTaktmusterZaehler,
  nenner: CONFIG.story.sceneLengthPrimaryTaktmusterNenner,
  type: CONFIG.story.sceneLengthTaktmusterType,
});

const nextSummedSceneLength = () => Math.min(
  CONFIG.story.sceneLengthSumMaxSeconds,
  sceneLengthPrimaryTaktmusterRuntime.nextSceneLength()
...
const applySceneCountBias = (value) => {
  ...
  if (!Number.isFinite(parsed) || parsed <= 0) {
    return CONFIG.story.minSceneCount;
  }
  const bias = Number(CONFIG.story.sceneCountBias);
  if (!Number.isFinite(bias) || Math.abs(bias) < 0.0001) {
    return Math.max(CONFIG.story.minSceneCount, Math.floor(parsed));
  }
  return Math.max(CONFIG.story.minSceneCount, Math.floor(parsed + bias));
};

const resolveSceneCount = () => {
  const resolvedCount = resolveSceneCountFromConfig({
  ...
  return Math.max(CONFIG.story.minSceneCount, resolvedCount);
};
...
const CAMERA_MAX_LAST_VALID_REUSES = parsePositiveNumber(
  pickEnvValue('FRESHWEB_CAMERA_MAX_LAST_VALID_REUSES'),
  2
);
const MAX_GENERATIONS_WITHOUT_PERSON = parsePositiveNumber(
  pickEnvValue('FRESHWEB_MAX_GENERATIONS_WITHOUT_PERSON'),
  2
);
const CAMERA_PERSON_SCAN_INTERVAL_MS = parsePositiveNumber(
  pickEnvValue('FRESHWEB_CAMERA_PERSON_SCAN_INTERVAL_MS'),
  10000
);
...
let consecutiveLastValidCameraReuses = 0;
let consecutiveGenerationsWithoutPerson = 0;
...
const captureBestPersonaReferenceShot = async ({ burstCount = ASYNC_WEBCAM_PERSONA_REFERENCE_BURST_COUNT } = {}) => {
  ...
  for (let index = 0; index < totalCaptures; index += 1) {
    const captured = await captureValidatedCameraShot({
      ...
      reuseLocalConfiguredImage: true,
    });
...
const sceneBoundaryTransportDecider = createSceneBoundaryTransportDecider({
  openai,
  fallbackOpenai: localMistralChatFallback,
  model: CONFIG.models.chatModel,
  fallbackModel: LOCAL_MISTRAL_MODEL,
});
const roomMemory = createRoomMemory({ maxSize: ROOM_MEMORY_SIZE });
...
const applySceneLoopDefaultsToPlan = (scenePlan = [], {
  storyDrivenIterationStart = false,
} = {}) => applyWebcamScenePlanVideoModeDefaults(scenePlan, {
  ...
  preserveFirstSceneVideoMode: storyDrivenIterationStart,
});
...
const image = {
  ...
  seed: CONFIG.models.imageSeed,
  prompts: {
    create: createConfiguredWebcamImagePromptHandler({
      model: WEBCAM_PERSONA_REFERENCE_MODEL,
...
const video = {
  prompts: {
    create: createWebcamFirstLastPrompt({
      ...
      cameraStyle: CONFIG.story.cameraStyle,
      videoModel: CONFIG.models.firstLastVideoModel || 'alibaba:wan@2.6-flash',
    }),
  },
  model: {
...
const video2 = {
  prompts: {
    create: createWebcamSingleImagePrompt({
      ...
      cameraSourceLabel: CONFIG.camera.sourceLabel,
      cameraStyle: CONFIG.story.cameraStyle,
      videoModel: CONFIG.models.singleVideoModel || 'alibaba:wan@2.6-flash',
    }),
  },
  model: {
...
let localCameraImageCursor = 0;
let activeLocalCameraImageIndex = -1;
...
const resolveConfiguredCameraImage = async ({ reuseLocalConfiguredImage = false } = {}) => {
  ...
  if (CONFIG.camera.imagePaths.length > 0) {
    if (!reuseLocalConfiguredImage || activeLocalCameraImageIndex < 0) {
      activeLocalCameraImageIndex = localCameraImageCursor;
      localCameraImageCursor += 1;
    }
    const imageIndex = activeLocalCameraImageIndex;
    const configuredPath = CONFIG.camera.imagePaths[imageIndex % CONFIG.camera.imagePaths.length];
    return {
      imagePath: path.resolve(configuredPath),
      imageSource: `configured-image-${(imageIndex % CONFIG.camera.imagePaths.length) + 1}`,
    };
  }
...
const MAX_QUEUED_PERSON_AGE_MS = 10 * 60 * 1000;
const protagonistFramings = new Set(['full-body', 'upper-body']);
...
const claimNextQueuedPersonShot = async () => {
  ...
  const entry = (Array.isArray(queue?.entries) ? queue.entries : [])
    .filter((candidate) => {
      const acceptedAtMs = Date.parse(String(candidate?.acceptedAt || ''));
      const isFresh = Number.isNaN(acceptedAtMs) || Date.now() - acceptedAtMs <= MAX_QUEUED_PERSON_AGE_MS;
      return candidate?.id
        && candidate?.imagePath
        && !consumedIds.has(candidate.id)
        && isFresh
        && protagonistFramings.has(String(candidate?.framing || ''));
    })
    .sort((left, right) => {
      const leftAcceptedAt = Date.parse(String(left?.acceptedAt || '')) || 0;
      const rightAcceptedAt = Date.parse(String(right?.acceptedAt || '')) || 0;
...
const rememberValidCameraShot = (shot = {}) => {
  if (shot?.imagePath) {
    ...
    consecutiveLastValidCameraReuses = 0;
    // A technically usable frame is not necessarily a newly confirmed person.
    // Keep the generation gate counting until presence vision actually confirms one.
    if (shouldResetPersonlessGenerationCount({ requirePerson: REQUIRE_PERSON_IN_CAMERA, shot })) {
      consecutiveGenerationsWithoutPerson = 0;
    }
  }
  return shot;
};
...
const reuseLastValidCameraShot = (reason = 'no-person') => {
  ...
  if (consecutiveLastValidCameraReuses >= CAMERA_MAX_LAST_VALID_REUSES) {
    return null;
  }
  consecutiveLastValidCameraReuses += 1;
  console.warn(
    `${ANSI_YELLOW}[freshweb-middle-cost-4-3] keeps last valid camera reference `
    + `(${reason}, ${consecutiveLastValidCameraReuses}/${CAMERA_MAX_LAST_VALID_REUSES}); no wait${ANSI_RESET}`
  );
  return {
    ...lastValidCameraShot,
...
const hasExhaustedLastValidCameraReuses = () => (
  Boolean(lastValidCameraShot?.imagePath)
  && consecutiveLastValidCameraReuses >= CAMERA_MAX_LAST_VALID_REUSES
);

const logWaitingForFreshCameraShot = (reason) => {
  console.warn(
    `${ANSI_YELLOW}[freshweb-middle-cost-4-3] last valid camera reference was reused `
    + `${CAMERA_MAX_LAST_VALID_REUSES} times (${reason}); holding generation for a fresh person shot${ANSI_RESET}`
  );
};
...
const captureValidatedCameraShot = async ({
  ...
  waitForPerson = true,
  fallbackToLastValid = false,
  reuseLocalConfiguredImage = false,
} = {}) => {
  let fallbackPath = '';
  let emptyFrameCount = 0;
  let usableShotAttempts = 0;
  let hasResolvedConfiguredImageForAttempt = false;
  let waitForFreshPerson = waitForPerson;
  let hasConfirmedPerson = !REQUIRE_PERSON_IN_CAMERA;

  const reuseLastValidOrWaitForFresh = (reason) => {
    const reusedShot = fallbackToLastValid ? reuseLastValidCameraShot(reason) : null;
    if (reusedShot || !fallbackToLastValid || !hasExhaustedLastValidCameraReuses()) {
      return { reusedShot, shouldWait: false };
    }
    waitForFreshPerson = true;
    logWaitingForFreshCameraShot(reason);
    return { reusedShot: null, shouldWait: true };
  };

  while (true) {
    const captured = await resolveConfiguredCameraImage({
      reuseLocalConfiguredImage: reuseLocalConfiguredImage || hasResolvedConfiguredImageForAttempt,
    });
    hasResolvedConfiguredImageForAttempt = true;
    ...
    if (cameraPresenceDetector && cameraPresenceGuardAvailable) {
      try {
        ...
        if (!gateResult.shouldCheckVision) {
          emptyFrameCount += 1;
          if (!waitForFreshPerson) {
            const fallback = reuseLastValidOrWaitForFresh('unchanged-camera-frame');
            if (!fallback.shouldWait) return fallback.reusedShot;
          }
          console.warn(
        ...
        const describedPerson = presence.hasPerson
          ? null
          : await describeCameraPerson({ imagePath });
        const recoveredPerson = describedPerson?.hasPerson === true;
        if (recoveredPerson) {
          console.log(
            `[freshweb-middle-cost-4-3] camera presence recovered by detailed local vision: `
            + `${describedPerson.personDescription || 'real person visible'}`
          );
        }
        const hasPerson = presence.hasPerson || recoveredPerson;
        hasConfirmedPerson = hasConfirmedPerson || hasPerson;
        ...
        if (!hasPerson) {
          emptyFrameCount += 1;
          if (!waitForFreshPerson) {
            const fallback = reuseLastValidOrWaitForFresh('no-person');
            if (!fallback.shouldWait) return fallback.reusedShot;
          }
          const detail = [presence.outputText, describedPerson?.visionText]
            .filter(Boolean)
            .join(' | ');
          console.warn(
    ...
    if (usableShotAttempts >= attempts) {
      if (waitForFreshPerson && fallbackToLastValid) {
        usableShotAttempts = 0;
        if (CAMERA_EMPTY_FRAME_WAIT_MS > 0) {
          await wait(CAMERA_EMPTY_FRAME_WAIT_MS);
        }
        continue;
    ...
    if (!waitForFreshPerson) {
      const fallback = reuseLastValidOrWaitForFresh('unusable-shot');
      if (!fallback.shouldWait) return fallback.reusedShot;
    }
    if (CAMERA_EMPTY_FRAME_WAIT_MS > 0) {
      await wait(CAMERA_EMPTY_FRAME_WAIT_MS);
...
const hasConfirmedCameraPerson = (shot = {}) => (
  !REQUIRE_PERSON_IN_CAMERA || shot?.hasConfirmedPerson === true
);
...
const resolveOpeningCameraShot = async () => {
  ...
  if (queuedShot) {
    consecutiveGenerationsWithoutPerson = 0;
    return { ...queuedShot, hasConfirmedPerson: true };
  }

  let shot = await captureValidatedCameraShot();
  if (hasConfirmedCameraPerson(shot)) {
    consecutiveGenerationsWithoutPerson = 0;
    return shot;
  }

  consecutiveGenerationsWithoutPerson += 1;
  if (consecutiveGenerationsWithoutPerson <= MAX_GENERATIONS_WITHOUT_PERSON) {
    console.warn(
      `${ANSI_YELLOW}[freshweb-middle-cost-4-3] no confirmed person for generation `
      + `${consecutiveGenerationsWithoutPerson}/${MAX_GENERATIONS_WITHOUT_PERSON}; using this camera fallback${ANSI_RESET}`
    );
    return shot;
  }

  console.warn(
    `${ANSI_YELLOW}[freshweb-middle-cost-4-3] ${MAX_GENERATIONS_WITHOUT_PERSON} generations without a person; `
    + `holding the next generation and scanning the webcam every ${CAMERA_PERSON_SCAN_INTERVAL_MS}ms${ANSI_RESET}`
  );
  while (!hasConfirmedCameraPerson(shot)) {
    await wait(CAMERA_PERSON_SCAN_INTERVAL_MS);
    shot = await captureValidatedCameraShot();
  }
  consecutiveGenerationsWithoutPerson = 0;
  return shot;
};

const resolveRunningIterationCameraShot = async () => captureValidatedCameraShot({
  waitForPerson: false,
  fallbackToLastValid: true,
  reuseLocalConfiguredImage: true,
});

const resolveSceneBoundaryCameraShot = async () => {
  const captured = await resolveConfiguredCameraImage({ reuseLocalConfiguredImage: false });
  const visionText = await getFrameVision(
    { image: { path: captured.imagePath } },
    { prompt: VISION_PROMPT }
  );
  return {
...
const persistOpeningPersonaReference = async ({
  ...
  const iterationLabel = String(Math.max(1, Number(storyRunIndex) || 1)).padStart(4, '0');
  const targetPath = path.join(
    targetDir,
    `opening-persona-reference-iteration-${iterationLabel}${ext}`
  );
...
const loadScenePlanReplay = async (replayPath = '') => {
  const configuredPath = String(replayPath || '').trim();
  if (!configuredPath) {
    return null;
  }
  ...
  if (!(await fs.pathExists(resolvedPath))) {
    throw new Error(`Scene-plan replay file not found: ${resolvedPath}`);
  }

  const payload = await fs.readJson(resolvedPath);
  const scenePlan = Array.isArray(payload)
    ? payload
    : payload?.rawScenePlan || payload?.scenePlan || payload?.appliedScenePlan;

  if (!Array.isArray(scenePlan) || scenePlan.length === 0) {
    throw new Error(`Scene-plan replay file has no reusable scene plan: ${resolvedPath}`);
  }

  return {
    path: resolvedPath,
    payload,
    scenePlan: scenePlan.map((scene) => ({ ...scene })),
    sourceCues: Array.isArray(payload?.sourceCues) ? payload.sourceCues : [],
  };
};
...
const saveCameraSnapshotArtifact = async ({
  ...
  const payload = {
      ...
      storyRunIndex,
      iterationStartMode: CONFIG.story.iterationStartMode,
      plannedIterationStart: scenePlan[0]
        ? {
            frameSource: scenePlan[0].frameSource,
            videoMode: scenePlan[0].videoMode,
            freshImage: scenePlan[0].freshImage,
            useCameraShot: scenePlan[0].useCameraShot,
          }
        : null,
      storyTransport,
      storyTransportArtifactPath,
  ...
  await saveWebcamScenePlanArtifact({
    outputDir,
    fileName: `scene-generator.camera-snapshot.iteration-${iterationLabel}.json`,
    payload,
  });
...
const buildSceneLoopConfig = () => ({
  ...
  ...(isReferenceImageActorMode(CONFIG.story.mode)
    ...
    : {}),
  openingImage: {
    promptSource: HAS_CONFIGURED_CAMERA_IMAGE ? '' : CONFIG.story.openingPromptSource,
  ...
  castContext: {
    ...
    model: {
      ...(CONFIG.render.castContext.model || {}),
    },
  },
});
...
const logSceneLoopSummary = ({
  ...
  console.log('[freshweb-middle-cost-4-3] minSceneCount:', CONFIG.story.minSceneCount);
  console.log(
  ...
  console.log(
    ...
    `${CONFIG.story.sceneLengthTaktmusterType} | ${CONFIG.story.sceneLengthTaktmusterCount} x ${CONFIG.story.sceneLengthTaktmusterZaehler}/${CONFIG.story.sceneLengthTaktmusterNenner} | shift ${CONFIG.story.sceneLengthTaktmusterInitialShift}`
  );
  console.log(
    '[freshweb-middle-cost-4-3] sceneLengthSum:',
    `${CONFIG.story.sceneLengthPrimaryTaktmusterCount} x ${CONFIG.story.sceneLengthPrimaryTaktmusterZaehler}/${CONFIG.story.sceneLengthPrimaryTaktmusterNenner} + shifted length rhythm | max ${CONFIG.story.sceneLengthSumMaxSeconds}s`
  );
  ...
  console.log('[freshweb-middle-cost-4-3] iterationStartMode:', CONFIG.story.iterationStartMode);
  ...
  for (const scene of scenePlan) {
    ...
    console.log(
      `  ${scene.index}. ${scene.title} | ${scene.videoMode} | ${scene.frameSource}`
      + ` | ${scene.visualPreparation || 'continueFrame'} | ${durationLabel}`
    );
  }
};
...
const promptFunktion = async (streams, config) => {
  ...
  const storyDrivenIterationStart = CONFIG.story.iterationStartMode.toLowerCase() === 'storydriven'
    && currentStoryRunIndex > 1;
  const {
    imagePath: openingCameraShot,
    imageSource,
    visionText: capturedVisionText = '',
  } = await resolveOpeningCameraShot();
  const scenePlanReplay = await loadScenePlanReplay(CONFIG.story.scenePlanReplayPath);
  const requestedSceneCount = scenePlanReplay?.scenePlan.length || resolveSceneCount();
  const replaySceneLengths = scenePlanReplay?.scenePlan.map((scene) => Number(scene?.durationSeconds));
  const requestedSceneLengths = replaySceneLengths?.every((duration) => Number.isFinite(duration) && duration > 0)
    ? replaySceneLengths
    : await refreshResolvedSceneLengths(requestedSceneCount);
  const sceneContextImages = await resolveSceneContextImages({ sceneCount: requestedSceneCount });
  const sourceCues = scenePlanReplay
    ? scenePlanReplay.sourceCues
  ...
  if (scenePlanReplay) {
    console.log(`[freshweb-middle-cost-4-3] scene-plan replay: ${scenePlanReplay.path}`);
  }
  ...
  const openingRoomMemoryEntry = roomMemory.remember({
    iteration: currentStoryRunIndex,
    sceneIndex: 0,
    imagePath: openingCameraShot,
    imageSource,
    location: structuredVisionStoryContext.location,
    setup: structuredVisionStoryContext.setupSummary,
    assets: structuredVisionStoryContext.assets,
    actors: structuredVisionStoryContext.actors,
  });
  ...
  const storyTransportPrompt = [
    ...
    roomMemory.formatForPrompt(),
  ].join(' ');
  const roomMemoryDir = path.join(path.resolve(config.outputDir), 'room-memory');
  await fs.ensureDir(roomMemoryDir);
  await fs.writeJson(
    path.join(roomMemoryDir, `iteration-${String(currentStoryRunIndex).padStart(4, '0')}-opening.json`),
    {
      latest: openingRoomMemoryEntry,
      entries: roomMemory.values(),
      plannerPrompt: roomMemory.formatForPrompt(),
    },
    { spaces: 2 }
  );
  ...
  const {
    ...
    ? {
        scenePlan: scenePlanReplay.scenePlan,
        effectiveSceneLengths: requestedSceneLengths,
      }
    : await generateScenePlanWithFallback({
        ...
        iterationStartMode: CONFIG.story.iterationStartMode,
        hasPreviousIteration: currentStoryRunIndex > 1,
  ...
  const sanitizedScenePlan = isReferenceImageActorMode(CONFIG.story.mode)
    ...
    : runtimeScenePlan;
  const scenePlan = sanitizedScenePlan;
  ...
  const openingFluxContextPrompt = openingFluxContextActive
    ? buildOpeningFluxContextPrompt({
        ...
        cameraSourceLabel: CONFIG.camera.sourceLabel,
        cameraStyle: CONFIG.story.cameraStyle,
        model: CONFIG.render.openingStart.model?.model || CONFIG.models.webcamPersonaReferenceModel || 'bfl:6@1',
      })
    : '';
  ...
  config.sceneLoop.iterationStartMode = CONFIG.story.iterationStartMode;
  config.sceneLoop.storyRunIndex = currentStoryRunIndex;
  config.sceneLoop.currentCameraImagePath = resolvedOpeningPersonaReferencePath;
  let activeBoundaryPeopleCount = Number(storyTransport?.people?.count)
    || storyTransport?.people?.actors?.length
    || 0;
  config.sceneLoop.onSceneBoundary = SCENE_BOUNDARY_TRANSPORT_ENABLED
    ? async ({
        sceneContext,
        scenePlanEntry,
        nextScenePlanEntry,
        ...
        const captured = await resolveSceneBoundaryCameraShot();
        const boundaryNumber = Math.max(1, Number(sceneContext?.index) || 1);
        const boundaryDir = path.join(path.resolve(config.outputDir), 'parts', 'scene-boundaries');
        const extension = path.extname(captured.imagePath) || '.jpg';
        const archivedCameraPath = path.join(
          boundaryDir,
          `iteration-${String(currentStoryRunIndex).padStart(4, '0')}-after-scene-${String(boundaryNumber).padStart(2, '0')}${extension}`
        );
        await fs.ensureDir(boundaryDir);
        if (path.resolve(captured.imagePath) !== path.resolve(archivedCameraPath)) {
          await fs.copy(captured.imagePath, archivedCameraPath, { overwrite: true });
        }

        const visionContext = extractVisionStoryContext(captured.visionText);
        const rememberedRoom = roomMemory.remember({
          iteration: currentStoryRunIndex,
          sceneIndex: boundaryNumber,
          imagePath: archivedCameraPath,
          imageSource: captured.imageSource,
          location: visionContext.location,
          setup: visionContext.setupSummary,
          assets: visionContext.assets,
          actors: visionContext.actors,
        });
        const roomMemoryArtifactPath = path.join(
          path.resolve(config.outputDir),
          'room-memory',
          `iteration-${String(currentStoryRunIndex).padStart(4, '0')}-after-scene-${String(boundaryNumber).padStart(2, '0')}.json`
        );
        await fs.ensureDir(path.dirname(roomMemoryArtifactPath));
        await fs.writeJson(roomMemoryArtifactPath, {
          latest: rememberedRoom,
          entries: roomMemory.values(),
          plannerPrompt: roomMemory.formatForPrompt(),
        }, { spaces: 2 });
        const cameraShot = {
          imagePath: archivedCameraPath,
          imageSource: captured.imageSource,
          visionText: captured.visionText,
          visionSummary: summarizeVisionStoryContext(captured.visionText),
        };
        const cameraPeopleCount = visionContext.actors.length;
        const transportPolicy = resolveSceneBoundaryTransportPolicy({
          nextScene: nextScenePlanEntry,
          currentPeopleCount: activeBoundaryPeopleCount,
          cameraPeopleCount,
          cameraImageAvailable: Boolean(archivedCameraPath),
        });
        let decision;
        if (!transportPolicy.shouldAskModel) {
          decision = {
            command: 'continue',
            reason: transportPolicy.reason,
            storyBridge: '',
            transitionPrompt: '',
            skippedModel: true,
          };
          ...
          try {
            decision = await sceneBoundaryTransportDecider({
              completedScene: scenePlanEntry,
              nextScene: nextScenePlanEntry,
              generatedLastFramePath,
              cameraShot,
              roomMemory: roomMemory.formatForPrompt(),
              storyTransport: formatStoryTransportForPrompt(storyTransport),
              transportPolicy,
            });
          } catch (error) {
            decision = {
              command: 'continue',
              reason: `GPT boundary decision failed: ${error?.message || error}`,
              storyBridge: '',
              transitionPrompt: '',
        ...
        const cameraTransportApplied = ['locationReturn', 'cameraReset'].includes(decision.command);
        let rememberedCast = null;
        if (cameraTransportApplied) {
          activeBoundaryPeopleCount = cameraPeopleCount;
          if (transportPolicy.peopleChanged) {
            rememberedCast = storyTransportController.rememberCameraBoundary({
              visionStoryContext: visionContext,
              referenceImagePath: archivedCameraPath,
              iteration: currentStoryRunIndex,
              sceneIndex: boundaryNumber,
            });
          }
        ...
        const result = {
          ...decision,
          transportPolicy,
          cameraImagePath: archivedCameraPath,
          cameraVisionText: captured.visionText,
          cameraVisionSummary: cameraShot.visionSummary,
          cameraHasPeople: visionContext.actors.length > 0,
          cameraTransportApplied,
          rememberedCast,
          generatedLastFramePath,
          completedVideoFile: videoFile,
          rememberedRoom,
          roomMemoryArtifactPath,
          roomMemory: roomMemory.values(),
        };
        const artifactPath = path.join(
          boundaryDir,
          `iteration-${String(currentStoryRunIndex).padStart(4, '0')}-after-scene-${String(boundaryNumber).padStart(2, '0')}.json`
        );
        await fs.writeJson(artifactPath, result, { spaces: 2 });
        console.log(
          `[freshweb-middle-cost-4-3] boundary ${boundaryNumber}->${boundaryNumber + 1}: ${result.command} | ${result.reason}`
        );
        console.log(`[freshweb-middle-cost-4-3] boundary camera: ${archivedCameraPath}`);
        console.log(`[freshweb-middle-cost-4-3] boundary artifact: ${artifactPath}`);
        return result;
      }
    : undefined;
  ...
  config.sceneLoop.openingImage = {
    ...
    personaReferenceModel: USE_WEBCAM_PERSONA_REFERENCE
      ...
      : undefined,
    promptSource: HAS_CONFIGURED_CAMERA_IMAGE
      ? ''
...
const adapterConfig = {
  ...
  story: {
    mode: CONFIG.story.mode,
    cameraStyle: CONFIG.story.cameraStyle,
    cameraFirstLastMaxDurationSeconds: CONFIG.story.cameraFirstLastMaxDurationSeconds,
  },
  model: {
  ...
  words: CONFIG.story.words,
  semanticStream: {
    enabled: !CONFIG.story.scenePlanReplayPath,
    streams: [],
  },
  video,
  video2,
