...
const GPT_OWNS_RHYTHM = parseBoolean(
  pickEnvValue('FRESHWEB_GPT_OWNS_RHYTHM'),
  false
);
const GPT_RHYTHM_MIN_SCENE_COUNT = parsePositiveNumber(
  pickEnvValue('FRESHWEB_GPT_RHYTHM_MIN_SCENE_COUNT'),
  2
);
const GPT_RHYTHM_MAX_SCENE_COUNT = parsePositiveNumber(
  pickEnvValue('FRESHWEB_GPT_RHYTHM_MAX_SCENE_COUNT'),
  7
);
const GPT_RHYTHM_MIN_DURATION_SECONDS = parsePositiveNumber(
  pickEnvValue('FRESHWEB_GPT_RHYTHM_MIN_DURATION_SECONDS'),
  2
);
const GPT_RHYTHM_MAX_DURATION_SECONDS = parsePositiveNumber(
  pickEnvValue('FRESHWEB_GPT_RHYTHM_MAX_DURATION_SECONDS'),
  6
);
...
const ROOM_REANCHOR_EVERY = parsePositiveNumber(
  pickEnvValue('FRESHWEB_ROOM_REANCHOR_EVERY'),
  4
);
const FALSE_VISITOR_REFERENCE_EVERY = parsePositiveNumber(
  pickEnvValue('FRESHWEB_FALSE_VISITOR_REFERENCE_EVERY'),
  4
);
const FALSE_VISITOR_REFERENCE_AFTER_CONTINUATIONS = parsePositiveNumber(
  pickEnvValue('FRESHWEB_FALSE_VISITOR_REFERENCE_AFTER_CONTINUATIONS'),
  3
);
const FALSE_VISITOR_REFERENCE_CONTINUATION_INTERVAL = parsePositiveNumber(
  pickEnvValue('FRESHWEB_FALSE_VISITOR_REFERENCE_CONTINUATION_INTERVAL'),
  3
);
const FALSE_VISITOR_REFERENCE_IMAGE_PATH = pickEnvValue('FRESHWEB_FALSE_VISITOR_REFERENCE_IMAGE_PATH')
  || path.resolve('lib/generator/adapter/assets/gelbnase-reference.png');
...
const CONFIG = {
  story: {
    ...
    gptOwnsRhythm: GPT_OWNS_RHYTHM,
    gptRhythmMinSceneCount: GPT_RHYTHM_MIN_SCENE_COUNT,
    gptRhythmMaxSceneCount: GPT_RHYTHM_MAX_SCENE_COUNT,
    gptRhythmMinDurationSeconds: GPT_RHYTHM_MIN_DURATION_SECONDS,
    gptRhythmMaxDurationSeconds: GPT_RHYTHM_MAX_DURATION_SECONDS,
  ...
  render: {
    ...
    driftCorrection: {
      ...
      roomEvolution: {
        reanchorEvery: ROOM_REANCHOR_EVERY,
      },
      falseVisitorReference: {
        every: FALSE_VISITOR_REFERENCE_EVERY,
        afterContinuations: FALSE_VISITOR_REFERENCE_AFTER_CONTINUATIONS,
        continuationInterval: FALSE_VISITOR_REFERENCE_CONTINUATION_INTERVAL,
        imagePath: FALSE_VISITOR_REFERENCE_IMAGE_PATH,
      },
      contextBuffer: {
...
const resolveConfiguredCameraImage = async () => {
  if (CONFIG.camera.imagePaths.length > 0) {
    const imageIndex = localCameraImageCursor;
    localCameraImageCursor += 1;
    const imagePath = CONFIG.camera.imagePaths[imageIndex % CONFIG.camera.imagePaths.length];
    return {
      imagePath,
      imageSource: `local-sequence-${(imageIndex % CONFIG.camera.imagePaths.length) + 1}`,
    };
  }
...
const logSceneLoopSummary = ({
  ...
  console.log(
    '[freshweb-middle-cost-4-3] rhythm owner:',
    CONFIG.story.gptOwnsRhythm
      ? `GPT (${CONFIG.story.gptRhythmMinSceneCount}-${CONFIG.story.gptRhythmMaxSceneCount} scenes, ${CONFIG.story.gptRhythmMinDurationSeconds}-${CONFIG.story.gptRhythmMaxDurationSeconds}s each)`
      : 'runtime'
  );
...
const promptFunktion = async (streams, config) => {
  ...
  const {
    ...
    rhythm: CONFIG.story.gptOwnsRhythm
      ? {
          enabled: true,
          minSceneCount: CONFIG.story.gptRhythmMinSceneCount,
          maxSceneCount: CONFIG.story.gptRhythmMaxSceneCount,
          minDurationSeconds: CONFIG.story.gptRhythmMinDurationSeconds,
          maxDurationSeconds: CONFIG.story.gptRhythmMaxDurationSeconds,
        }
      : null,
