...
const CREATIVE_CUTS_ENABLED = parseBoolean(
  pickEnvValue('FRESHWEB_CREATIVE_CUTS_ENABLED'),
  false
);
...
const CONFIG = {
  story: {
    ...
    creativeCutsEnabled: CREATIVE_CUTS_ENABLED,
  ...
  models: {
    ...
    falFirstLastFallbacks: [
      { type: 'falFirstLast', model: 'fal-ai/wan/v2.7/image-to-video' },
    ],
    falSingleFallbacks: [
...
const promptFunktion = async (streams, config) => {
  ...
  const {
    ...
    allowCreativeCuts: CONFIG.story.creativeCutsEnabled,
  ...
  const sanitizedScenePlan = isReferenceImageActorMode(CONFIG.story.mode)
    ? sanitizeWebcamCameraScenePlan(runtimeScenePlan, {
        ...
        creativeCutsEnabled: CONFIG.story.creativeCutsEnabled,
      })
    : runtimeScenePlan;
...
const adapterConfig = {
  ...
  semanticStreamStaticTest: CONFIG.story.staticTestMode,
