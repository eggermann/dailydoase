...
const CAST_CONTEXT_MAX_REFERENCES = parsePositiveNumber(
  pickEnvValue('FRESHWEB_CAST_CONTEXT_MAX_REFERENCES'),
  3
);
...
const DRIFT_CORRECTION_CURRENT_PERSON_ONLY = parseBoolean(
  pickEnvValue('FRESHWEB_DRIFT_CORRECTION_CURRENT_PERSON_ONLY'),
  false
);
...
const CONFIG = {
  ...
  render: {
    ...
    castContext: {
      ...
      maxReferences: CAST_CONTEXT_MAX_REFERENCES,
      model: {
    ...
    driftCorrection: {
      ...
      currentPersonOnly: DRIFT_CORRECTION_CURRENT_PERSON_ONLY,
      contextBuffer: {
...
const claimNextQueuedPersonShot = async () => {
  ...
  const candidates = (Array.isArray(queue?.entries) ? queue.entries : [])
    ...
    .sort((left, right) => (
      Date.parse(right.acceptedAt || right.createdAt || 0)
      - Date.parse(left.acceptedAt || left.createdAt || 0)
    ));
  let entry = null;
...
const buildSceneLoopConfig = () => ({
  ...
  castContext: {
    ...
    maxReferences: CONFIG.render.castContext.maxReferences,
    model: {
...
const promptFunktion = async (streams, config) => {
  ...
  const {
    ...
    emergencyScenePlan: createEmergencyScenePlan,
  });
