...
import { createSceneBoundaryTransportDecider } from './scene-boundary-transport.js';
...
const promptFunktion = async (streams, config) => {
  ...
  config.sceneLoop.onSceneBoundary = SCENE_BOUNDARY_TRANSPORT_ENABLED
    ? async ({
        ...
        const rememberedCast = storyTransportController.rememberCameraBoundary({
        ...
        const liveTransport = {
          ...storyTransport,
          people: rememberedCast.people,
          cast: {
            ...(storyTransport.cast || {}),
            actorReferences: rememberedCast.actorReferences,
          },
        };
        const cameraShot = {
          imagePath: archivedCameraPath,
        ...
        try {
          decision = await sceneBoundaryTransportDecider({
            ...
            storyTransport: formatStoryTransportForPrompt(liveTransport),
          });
        } catch (error) {
          decision = {
            command: 'continue',
