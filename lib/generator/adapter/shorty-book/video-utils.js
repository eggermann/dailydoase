  ...
  if (useSingleImageMode) {
    ...
    if (hasEndImage) {
      ...
      lastEndFrame = await generateImage(streams, {
        ...imageOptions,
        // A first-last transition must grow out of the frame that is already
        // on screen. Persona references may stay in `images`, but must never
        // replace this source image with an older cast snapshot.
        imagePath: startFrame.image.path,
        sceneContext,
        frameRole: 'end',
      });
...
export const buildVideoConfig = ({
  ...
  const cameraPersonOverride = String(options?.personaReferenceIdentityClause || '').trim();
  const totalPrompt = addStaticPrompt(
    [cameraPersonOverride, generatedPrompt].filter(Boolean).join(' '),
    videoModel.staticPrompt
  );

  mergedConfig.prompt = totalPrompt ?? (options.useImagePrompt
    ? startFrame.json.metadata.prompt
    : '');
  if (typeof options.endImageStream === 'string' && options.endImageStream.trim()) {
    mergedConfig.endImageStream = options.endImageStream;
  }
