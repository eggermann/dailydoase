...
export const DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT = [
  DEFAULT_SCENE_SYSTEM_PROMPT,
  'REFERENCE IMAGE: Use detected people and room as visual source material. Identity, actor count, position, orientation, geometry, typography, objects, and transformations may drift when the story or surreal mode benefits.',
  'EXISTING-ROOM-FIRST: Treat visible people, objects, artwork, furniture, openings, surfaces, light, and room geometry as the primary stage and prop inventory. Every scene should make at least one detected feature carry the action.',
  'Let semantic cues alter the use, meaning, pressure, or relationship of those existing features. Vary whether each cue acts on a person, the room, or both together.',
  'New surreal elements may appear, but they should visibly grow from an existing form, object, light condition, or action so the real camera origin remains readable.',
  'A mirror or reflective transformation may show one continuous reflection of the visible actor, but must not create an independent duplicate person.',
  'Use ordered source cues as causes. Their consequence may happen through people, room, objects, atmosphere, abstraction, typography, or transformation. Carry a readable residue into the next scene.',
...
const TRIPPY_CAMERA_SCENE_PROMPT_APPENDIX = [
  ...
  'Preserve continuity only where it strengthens the sequence. Stage one readable event or transformation per scene.',
].join(' ');
...
const appendPlannedSpatialState = (prompt, scenePlanEntry = {}) => {
  const basePrompt = normalizeString(prompt);
  const normalizedBasePrompt = basePrompt.toLowerCase();
  const spatialState = [
    normalizeString(scenePlanEntry?.actorTrajectory),
    normalizeString(scenePlanEntry?.finalPose),
  ].filter((value) => value && !normalizedBasePrompt.includes(value.toLowerCase()));

  return [basePrompt, ...spatialState].filter(Boolean).join(' ');
};
...
export const createWebcamFirstLastPrompt = ({
  ...
  const plannedPrompt = appendPlannedSpatialState(
    scenePlanEntry?.videoPrompt || buildFallbackVideoPrompt(
      scenePlanEntry,
