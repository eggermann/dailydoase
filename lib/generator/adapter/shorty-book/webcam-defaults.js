...
export const sanitizeWebcamCameraScenePlan = (scenePlan = []) => {
  ...
  const laterScenes = sanitizedPlan.slice(1);
  const needsLaterFreshReanchor = laterScenes.length >= 3;
  const hasLaterFreshReanchor = laterScenes.some((scene) => (
    normalizeString(scene?.videoMode) === 'singleImage'
    && normalizeString(scene?.frameSource) === 'newImage'
    && scene?.useCameraShot === true
  ));

  if (!needsLaterFreshReanchor || hasLaterFreshReanchor) {
    return sanitizedPlan;
  }

  let reanchorIndex = -1;
  for (let index = sanitizedPlan.length - 1; index >= 1; index -= 1) {
    if (normalizeString(sanitizedPlan[index]?.videoMode) === 'singleImage') {
      reanchorIndex = index;
      break;
    }
  }
  ...
  if (reanchorIndex < 1) {
    reanchorIndex = sanitizedPlan.length - 1;
  }

  sanitizedPlan[reanchorIndex] = {
    ...sanitizedPlan[reanchorIndex],
    videoMode: 'singleImage',
    frameSource: 'newImage',
    freshImage: true,
