...
import {
  ...
  compileWanFirstLastMotionPrompt,
  compileWanMotionPrompt,
} from './visual-prompt-compiler.js';

export const DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT = [
  DEFAULT_SCENE_SYSTEM_PROMPT,
  'REFERENCE IMAGE TRUTH: Keep detected people recognizable: same face, age, body, hair, and clothing. Never replace, merge, age, or duplicate them silently. Only an explicit story event changes identity.',
  'OBSERVED PEOPLE: A person already in the frame stays the same observed cast member. A new person must visibly enter through a frame edge, doorway, or opening, or arrive after an explicit hard CCTV cut. Never let an unannounced person simply exist in the next frame.',
  'PROP PHYSICS: Surreal props are welcome, but every new object states scale (handheld, table-sized, or room-sized) and physical support (hand, table, floor, wall, or existing object). Keep its scale readable against the observed person and room.',
  'CAMERA GRAMMAR: For every scene choose one fixed high corner, ceiling, doorway, checkout-monitor, or aisle-end security-camera angle that reveals its cause and consequence. A new scene may cut to another motivated fixed camera; never use operator movement. Use real 1989 Einkaufszentrum CCTV: wide practical coverage, VHS noise, interlace, deep focus, clipped highlights, crushed blacks, chroma bleed, lens grime, uneven auto-exposure, low bitrate; never cinema.',
  'EXISTING-ROOM: Use visible people, objects, light, geometry.',
  'Semantic cues alter existing people, room, or their relationship.',
  'Surreal elements grow visibly from existing form, object, light, or action.',
  'Reflections remain attached to their surface, never independent duplicates.',
  'Ordered cues cause visible consequences. Carry residue forward.',
  'MODE STATE MACHINE: Scene 1 of the first iteration is newImage + singleImage + freshImage=true + useCameraShot=true. A later story-driven iteration may instead continue from lastFrame, transition firstLast toward current camera context, or reset to newImage when the story benefits. Later continuation is lastFrame + singleImage + freshImage=false + useCameraShot=false. Later destination transition is lastFrame + firstLast + freshImage=false + useCameraShot=true. No other combination is valid.',
  'Default to continuation. Use destination transition only for a clear reachable end pose in the same room.',
  'FINAL CHECK: scene states are causal and distinct; mode tuple matches the state machine.',
].join(' ');

const TRIPPY_CAMERA_SCENE_PROMPT_APPENDIX = [
  'LTX-TRIPPY: Cues may alter actors, light, texture, geometry, typography, objects, identity, or the room itself.',
  'Identity changes must be readable story events, not accidental frame-to-frame drift. Keep supplied camera perspective fixed while the surreal event changes what it records.',
  'Stage one readable event or transformation per scene.',
].join(' ');
...
export const createConfiguredWebcamImagePromptHandler = ({
  model = 'bfl:6@1',
  cameraStyle = '',
} = {}) => async (prompt, sceneContext, scenePlanEntry) => {
  const compilation = compileFluxEditPrompt({
    scene: scenePlanEntry || {
      singleImagePrompt: buildFallbackStillPrompt(prompt),
    },
    currentState: 'The supplied source image is composition truth.',
    model,
    cameraStyle,
    referenceRoles: [
      { role: 'current frame / composition truth' },
      { role: 'identity truth when supplied' },
    ],
  });
  if (sceneContext && typeof sceneContext === 'object') {
    sceneContext.fluxPromptCompiler = compilation;
  }
  return compilation.prompt;
};

const defaultWebcamImagePromptHandler = createConfiguredWebcamImagePromptHandler();

export const createWebcamImagePromptHandler = (...args) => defaultWebcamImagePromptHandler(...args);
...
export const createWebcamFirstLastPrompt = ({
  ...
  cameraStyle = '',
  videoModel = 'alibaba:wan@2.6-flash',
  ...
  if (isReferenceImageActorMode(configMode)) {
    const compilation = compileWanFirstLastMotionPrompt({
      scene: scenePlanEntry || {},
      model: videoModel,
      frameRoles: [
        { role: 'first frame / current visual truth', path: frameContext.startFrame?.image?.path },
        { role: 'last frame / destination visual truth', path: frameContext.endFrame?.image?.path },
      ],
    });
    if (sceneContext && typeof sceneContext === 'object') {
      sceneContext.visualPromptCompiler = compilation;
    }
    return compilation.prompt;
  }
...
export const createWebcamSingleImagePrompt = ({
  ...
  if (isReferenceImageActorMode(configMode)) {
    const compilation = compileWanMotionPrompt({
      scene: scenePlanEntry || {},
      model: videoModel,
      cameraStyle,
      frameRoles: [
...
export const describeWebcamCameraScenePlanIssues = (scenePlan = [], {
  ...
  scenePlan.forEach((scene, index) => {
    ...
    if (index === 0) {
      if (storyDrivenIterationStart) {
        const isCameraReset = videoMode === 'singleImage'
          && frameSource === 'newImage'
          && useCameraShot
          && freshImage;
        const isContinuation = videoMode === 'singleImage'
          && frameSource === 'lastFrame'
          && !useCameraShot
          && !freshImage;
        const isCameraTransition = videoMode === 'firstLast'
          && frameSource === 'lastFrame'
          && useCameraShot
          && !freshImage;
        if (!isCameraReset && !isContinuation && !isCameraTransition) {
          issues.push(`${sceneLabel}: unsupported story-driven iteration start tuple`);
        }
        return;
      }
...
export const sanitizeWebcamCameraScenePlan = (
  ...
  const sanitizedPlan = scenePlan.map((scene, index) => {
    ...
    normalized = isFirst && storyDrivenIterationStart
      ? normalized.videoMode === 'firstLast'
        ...
        : normalized.frameSource === 'lastFrame'
          ? {
              ...normalized,
      ...
      ? {
          ...normalized,
          videoMode: normalizeString(firstClipVideoMode) === 'firstLast'
            ? 'firstLast'
            : 'singleImage',
          frameSource: 'newImage',
          freshImage: true,
      ...
      : isTrippyFlavor && trippyReanchorEnabled
        ? (() => {
...
export const applyWebcamScenePlanVideoModeDefaults = (
  scenePlan = [],
  {
    ...
    firstClipVideoMode = 'singleImage',
    preserveFirstSceneVideoMode = false,
  } = {}
  ...
  return {
    ...
    videoMode: isFirst && !preserveFirstSceneVideoMode
      ? firstClipVideoMode
