...
import { buildWanPrompt, selectWanMotionDirection } from './scene-prompt-selection.js';
import {
...
export const DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT = [
  DEFAULT_SCENE_SYSTEM_PROMPT,
  'When configMode is "reference-image-actor", keep every scene grounded in the currently visible real shot.',
  'In reference-image-actor mode, source cues must shape the emotional arc and implied off-screen story inside the real shot.',
  ...
  'In reference-image-actor mode, let the source cues drive the storybook progression mainly through title, beat, storyBeat, motionCue, cameraCue, and emotional change, not by replacing the visible shot with literal off-screen objects.',
  'In reference-image-actor mode, favor embodied, visually readable subscene beats over explanatory, analytical, or topic-summary phrasing.',
  'In reference-image-actor mode, map wordstream changes through visible location emphasis, actor movement, and sensory atmosphere shifts (light, texture, tension) inside the same room.',
  'For each reference-image-actor scene, decide whether the cue acts mainly on the subject, on the room, or on both together, and make that interaction explicit in beat, storyBeat, motionCue, and cameraCue.',
  ...
  'In reference-image-actor mode, do not literalize unseen source-cue nouns into visible props; convert them into body action, room emphasis, lighting pressure, and camera behavior inside the existing shot.',
  'In reference-image-actor mode, do not turn the source cues into a generic host presentation, review, explanation, or discussion unless the source cues explicitly call for that format.',
  'When reference-image-actor vision context is provided, use its location description, actor descriptions, and visible-shot description as the basis for every scene beat, stillPrompt, imageDescription, and video prompt.',
  'In reference-image-actor mode, scene changes should come from recomposing, reframing, or evolving the visible location and actor state described by vision, not from hardcoded banned-word lists or hand-written object filters.',
  'In reference-image-actor mode, scene 1 must always start from a fresh live reference image.',
  'In reference-image-actor mode, scene 1 must always use videoMode "singleImage".',
  'For every reference-image-actor scene, first decide the new story point or subscene moment, then choose frameSource and videoMode to fit that moment.',
  'In reference-image-actor mode, a different setting means a different visible setup inside the same real shot: a new pose, gaze direction, body position, framing emphasis, relation to the window or artwork, foreground-background balance, or lighting emphasis that is already plausible from the source image.',
  ...
  'Preserve the real camera orientation from the source webcam frame: do not mirror or horizontally flip the shot, and keep the left-right room layout consistent with the source image.',
  ...
  'For reference-image-actor mode, videoPrompt and singleImagePrompt must describe only what can happen inside the visible shot: facial expression, body movement, gaze shift, hand movement, posture change, lighting change, focus change, or camera motion.',
  'In reference-image-actor mode, keep the same subject identity, room, and overall setting unless a fresh reference image is explicitly requested.',
  'In reference-image-actor mode, keep the location static across the sequence and follow the beat through expression, gesture, props, framing, and lighting inside that same space.',
  'Default to videoMode "singleImage" in reference-image-actor mode when in doubt, especially for small emotional or gestural changes that should preserve actor and background continuity.',
  ...
  'After scene 1, keep later reference-image-actor scenes chained from the previous generated last frame instead of restarting from a fresh reference image.',
  'When a later scene uses videoMode "firstLast", treat the previous last frame as the start image and make the destination stillPrompt describe the fresh reference-image-actor end state you want to arrive at.',
  'For every later firstLast reference-image-actor scene, set useCameraShot=true and freshImage=false.',
  'For every later singleImage reference-image-actor scene, set frameSource="lastFrame", useCameraShot=false, and freshImage=false.',
  'For later scenes in reference-image-actor mode, only these structures are valid: singleImage from lastFrame or firstLast from lastFrame toward a fresh reference-image-actor end shot.',
  'For later scenes in reference-image-actor mode, useCameraShot=true is only for the fresh reference-image-actor destination of a firstLast scene.',
  'In reference-image-actor mode, if videoMode is "firstLast", frameSource must be "lastFrame" and the destination should be a fresh reference-image-actor end shot.',
  'Across the whole reference-image-actor plan, vary the subscene logic through expression, gesture, gaze, framing emphasis, and emotional progression even if several consecutive scenes all use singleImage.',
  'Do not force firstLast just to create variety; continuity is more important than mode variety in reference-image-actor mode.',
  ...
  'Before you output JSON, check that every later singleImage scene starts from lastFrame and that every firstLast reference-image-actor scene has useCameraShot=true.',
...
const ACTOR_LANGUAGE_PATTERN = /\b(?:he|his|him|she|her|hers|man|woman|person|people|figure|subject|visitor|worker|performer|witness|customer|shopper|face|eyes?|mouth|jaw|chin|hands?|shoulders?|torso|posture|breath)\b/i;
...
const refineSceneFromWordstream = ({
  ...
  const repeatedDominantSignature = previousScene
    ...
    : false;
  const actorMismatch = !allowPeople && !actorVisible && sceneContainsActorLanguage(scene);
...
export const resolveWebcamScenePlanSystemPrompt = ({
  ...
  if (isReferenceImageActorMode(configMode)) {
...
export const createWebcamFirstLastPrompt = ({
  ...
  const usesMonsterPipeline = /monster/i.test(String(cameraSourceLabel || ''));
  if (isReferenceImageActorMode(configMode) && usesMonsterPipeline) {
    return buildWanPrompt({
      scene: { ...scenePlanEntry, videoPrompt: plannedPrompt },
      allowPeople,
    });
  }
...
export const createWebcamSingleImagePrompt = ({
  ...
  const fallbackPrompt = buildFallbackVideoPrompt(
    scenePlanEntry,
    startFramePrompt || 'Continue the current frame with subtle motion.'
  );
  const plannedPrompt = selectWanMotionDirection({
    scenePlanEntry,
    wasTransitionBeat,
    fallbackPrompt,
  });
  ...
  if (isReferenceImageActorMode(configMode)) {
    return buildCameraGroundedPrompt({
      ...
      promptFlavor,
      preserveBasePrompt: String(promptFlavor || '').toLowerCase() === 'wancinematicsurreal',
      lockActorCount,
      previousSceneMemory,
      cameraSourceLabel,
    });
  ...
  scenePlan.forEach((scene, index) => {
    ...
    if (index === 0) {
      if (videoMode !== 'singleImage') {
        issues.push(`${sceneLabel}: opening reference-image-actor scene must use videoMode "singleImage"`);
      }
      if (frameSource !== 'newImage') {
        issues.push(`${sceneLabel}: opening reference-image-actor scene must start from frameSource "newImage"`);
      }
      if (!useCameraShot) {
        issues.push(`${sceneLabel}: opening reference-image-actor scene must set useCameraShot=true`);
      }
      if (!freshImage) {
        issues.push(`${sceneLabel}: opening reference-image-actor scene must set freshImage=true`);
      }
      return;
    }
    ...
    if (videoMode === 'firstLast') {
      if (frameSource !== 'lastFrame') {
        issues.push(`${sceneLabel}: firstLast reference-image-actor scene must start from frameSource "lastFrame"`);
      }
      if (!useCameraShot) {
        issues.push(`${sceneLabel}: firstLast reference-image-actor scene must set useCameraShot=true for the fresh reference-image-actor destination`);
      }
      return;
    }

    if (videoMode !== 'singleImage') {
      issues.push(`${sceneLabel}: reference-image-actor scene must use videoMode "singleImage" or "firstLast"`);
      return;
    }

    if (frameSource === 'newImage') {
      issues.push(`${sceneLabel}: later singleImage reference-image-actor scene must start from frameSource "lastFrame"`);
      if (useCameraShot) {
        issues.push(`${sceneLabel}: later singleImage reference-image-actor scene must set useCameraShot=false`);
      }
      if (freshImage) {
        issues.push(`${sceneLabel}: later singleImage reference-image-actor scene must set freshImage=false`);
      }
      return;
    }
    ...
    if (frameSource === 'lastFrame') {
      if (useCameraShot) {
        issues.push(`${sceneLabel}: singleImage reference-image-actor scene with frameSource "lastFrame" must set useCameraShot=false`);
      }
      if (freshImage) {
        issues.push(`${sceneLabel}: singleImage reference-image-actor scene with frameSource "lastFrame" must set freshImage=false`);
      }
      return;
    }

    issues.push(`${sceneLabel}: reference-image-actor scene has unsupported frameSource "${frameSource || 'unknown'}"`);
  });
...
export const sanitizeWebcamCameraScenePlan = (
  scenePlan = [],
  {
    ...
    sceneFlavor = 'default',
    allowPeople = false,
    trippyReanchorInterval = 2,
    cameraReanchorInterval = 0,
    startFrameStrategy = {},
  } = {}
) => {
  const isTrippyFlavor = normalizeString(sceneFlavor).toLowerCase() === 'ltxtrippy';
  const resolvedTrippyReanchorInterval = Number(trippyReanchorInterval);
  const trippyReanchorEnabled = Number.isFinite(resolvedTrippyReanchorInterval)
    && resolvedTrippyReanchorInterval > 0;
  const resolvedCameraReanchorInterval = Number(cameraReanchorInterval);
  const cameraReanchorEnabled = Number.isFinite(resolvedCameraReanchorInterval)
    && resolvedCameraReanchorInterval > 0;
  ...
  const sanitizedPlan = scenePlan.map((scene, index) => {
    ...
    normalized = isFirst
      ...
      : isTrippyFlavor
        ? (() => {
            const shouldReanchor = trippyReanchorEnabled
              ? (((index + 1) % resolvedTrippyReanchorInterval) === 1)
              : false;
        ...
        : cameraReanchorEnabled && (((index + 1) % resolvedCameraReanchorInterval) === 1)
        ? {
            ...normalized,
            videoMode: 'singleImage',
            frameSource: 'newImage',
    ...
    if (startFrameStrategy.enabled === true) {
      const configuredStrategy = resolveConfiguredStartFrameStrategy({
        scene: normalized,
        sceneIndex: index,
        sceneCount: scenePlan.length,
        config: startFrameStrategy,
      });
      normalized = applyPlannedStartFrameStrategy({
        ...normalized,
        startFrameStrategy: configuredStrategy,
      }, index);
    }
