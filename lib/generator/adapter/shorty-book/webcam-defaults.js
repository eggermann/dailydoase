...
export const DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT = [
  ...
  'MODE STATE MACHINE: Scene 1 is newImage + singleImage + freshImage=true + useCameraShot=true. Later continuation is lastFrame + singleImage + freshImage=false + useCameraShot=false. Later destination transition is lastFrame + firstLast + freshImage=false + useCameraShot=true. No other combination is valid.',
  'Default to continuation. Use destination transition only for a clear reachable end pose in the same room.',
  'For later new objects or material changes set requiresDocumentaryEndFrame=true; FLUX then creates the real destination before WAN motion.',
  'FINAL CHECK: scene states are causal and distinct; mode tuple matches the state machine.',
].join(' ');

export const DOCUMENTARY_CCTV_IMAGE_CONTRACT = [
  'Render as an unembellished real CCTV webcam frame with documentary deep focus.',
  'Every object must have plausible physical scale, contact, occlusion, material texture, and room-lit shadows; preserve visible texture through transparent material.',
  'Avoid graphic decals, painted effect shapes, glossy 3D-render styling, and illustration.',
].join(' ');
...
export const createWebcamImagePromptHandler = async (prompt, sceneContext, scenePlanEntry) => {
  ...
  if (scenePlanEntry?.stillPrompt) {
    imagePrompt = scenePlanEntry.stillPrompt;
  } else if (scenePlanEntry?.imageDescription) {
    imagePrompt = scenePlanEntry.imageDescription;
  } else {
    imagePrompt = buildFallbackStillPrompt(prompt);
  }
  return `${imagePrompt} ${DOCUMENTARY_CCTV_IMAGE_CONTRACT}`.trim();
};
...
const buildIncomingMotion = (scenePlan = [], sceneContext = {}) => {
  const currentIndex = Math.max(1, Number(sceneContext?.index || 1));
  if (currentIndex <= 1) {
    return '';
  }
  ...
  return normalizeString(previousScenePlanEntry.exitMotion)
    || normalizeString(previousScenePlanEntry.motionCue);
};
...
export const createWebcamFirstLastPrompt = ({
  ...
  const incomingMotion = buildIncomingMotion(frameContext.scenePlan, sceneContext);
...
export const sanitizeWebcamCameraScenePlan = (
  ...
  const sanitizedPlan = scenePlan.map((scene, index) => {
    ...
    normalized = isFirst
      ...
      : normalized.requiresDocumentaryEndFrame === true
        ? {
            ...normalized,
            videoMode: 'firstLast',
