...
const ACTOR_LANGUAGE_PATTERN = /\b(?:he|his|him|she|her|hers|face|eyes?|mouth|jaw|chin|hands?|shoulders?|torso|posture|breath)\b/i;
...
const pickCueVariant = (variants = [], cueText = '', sceneIndex = 0, salt = 0) => {
  if (!Array.isArray(variants) || variants.length === 0) {
    return null;
  }
  return variants[(hashCueText(cueText) + sceneIndex + salt) % variants.length];
};
...
const sceneLooksCueDriven = (scene = {}, {
  cueText = '',
  previousScene = null,
  previousCueText = '',
  storyContext = {},
  ...
  const beatHasCue = fieldStronglyContainsCueLanguage(scene.title, cueText)
    || fieldStronglyContainsCueLanguage(scene.beat, cueText)
    || fieldStronglyContainsCueLanguage(scene.storyBeat, cueText);
  const hasConcreteSupport = [
    scene.motionCue,
    scene.cameraCue,
    scene.stillPrompt,
    scene.imageDescription,
  ].some((value) => !fieldLooksWeak(value, {
    cueText,
    storyContext,
    actorVisible,
    minKeywordCount: 3,
  ...
  const repeated = previousScene
    ? buildSceneDominantSignature(scene, cueText) === buildSceneDominantSignature(previousScene, previousCueText || cueText)
    : false;

  if (!actorVisible && sceneContainsActorLanguage(scene)) {
    return false;
  }

  return beatHasCue && hasConcreteSupport && !repeated;
};
...
const CUE_ACTOR_BEAT_VARIANTS = [
  ({ cueLabel, roomAnchor }) => `The cue "${cueLabel}" lands as a visible interruption, pulling posture and gaze toward ${roomAnchor}.`,
  ({ cueLabel, roomAnchor }) => `The cue "${cueLabel}" changes the room pressure first, and the actor answers it through a small shift toward ${roomAnchor}.`,
  ({ cueLabel, roomAnchor }) => `The cue "${cueLabel}" arrives as a new read on the same space, changing how the actor relates to ${roomAnchor}.`,
  ({ cueLabel, roomAnchor }) => `The cue "${cueLabel}" sharpens the moment, making the actor re-balance against ${roomAnchor} inside the same shot.`,
  ({ cueLabel, roomAnchor }) => `The cue "${cueLabel}" tilts the scene, and the actor's body starts to answer that change around ${roomAnchor}.`,
  ({ cueLabel, roomAnchor }) => `The cue "${cueLabel}" sticks in the room and becomes readable through a fresh posture change near ${roomAnchor}.`,
];

const CUE_ROOM_BEAT_VARIANTS = [
  ({ cueLabel, roomAnchor }) => `The cue "${cueLabel}" changes the room through crop, light, and pressure around ${roomAnchor}.`,
  ({ cueLabel, roomAnchor }) => `The cue "${cueLabel}" reorganizes the visible space so ${roomAnchor} becomes newly charged in the frame.`,
  ({ cueLabel, roomAnchor }) => `The cue "${cueLabel}" turns the same room into a different visual event centered on ${roomAnchor}.`,
  ({ cueLabel, roomAnchor }) => `The cue "${cueLabel}" lands as a visible shift in framing and atmosphere around ${roomAnchor}.`,
  ({ cueLabel, roomAnchor }) => `The cue "${cueLabel}" bends attention across the room until ${roomAnchor} becomes the shot's active pressure point.`,
  ({ cueLabel, roomAnchor }) => `The cue "${cueLabel}" leaves a visible trace in the room, tightening crop and atmosphere around ${roomAnchor}.`,
];

const CUE_ACTOR_MOTION_VARIANTS = [
  ({ roomAnchor }) => `He checks his posture, lets the gaze slide toward ${roomAnchor}, then holds on the new tension.`,
  ({ roomAnchor }) => `His weight shifts first, then the face follows, as if ${roomAnchor} has started to pull the scene off-axis.`,
  ({ roomAnchor }) => `He pauses, tightens through the jaw and shoulders, and turns a fraction toward ${roomAnchor}.`,
  ({ roomAnchor }) => `A small recoil moves through his upper body before he steadies himself against the pull of ${roomAnchor}.`,
  ({ roomAnchor }) => `He lets the cue travel through the face first, then answers it with a short body check toward ${roomAnchor}.`,
  ({ roomAnchor }) => `His stance resets in place, breath catches, and the gaze fixes harder on ${roomAnchor}.`,
];

const CUE_ROOM_MOTION_VARIANTS = [
  ({ roomAnchor }) => `The room holds still while light, focus, and edge tension gather around ${roomAnchor}.`,
  ({ roomAnchor }) => `The frame drifts into a new balance, making ${roomAnchor} feel heavier and more watchful.`,
  ({ roomAnchor }) => `A quiet shift in crop and light turns ${roomAnchor} into the place where the scene now lives.`,
  ({ roomAnchor }) => `The visible room absorbs the cue through a slow change in emphasis, with ${roomAnchor} taking over the shot.`,
  ({ roomAnchor }) => `Depth, focus, and the hold of the frame tighten until ${roomAnchor} becomes the dominant event.`,
  ({ roomAnchor }) => `The shot settles into a different pressure pattern, letting ${roomAnchor} carry the visible change.`,
];

const CUE_ACTOR_CAMERA_VARIANTS = [
  ({ roomAnchor }) => `Keep the shot grounded and ease the frame toward ${roomAnchor} without losing the actor's face.`,
  ({ roomAnchor }) => `Let the camera lean slightly into ${roomAnchor} while holding the actor's new posture readable.`,
  ({ roomAnchor }) => `Use one restrained drift so the actor and ${roomAnchor} tighten into the same visual line.`,
  ({ roomAnchor }) => `Reframe just enough to keep ${roomAnchor} active behind the actor's shift in expression.`,
  ({ roomAnchor }) => `Hold the same setup, then bias the frame toward ${roomAnchor} as the body change lands.`,
  ({ roomAnchor }) => `Keep continuity steady and let a small camera correction make ${roomAnchor} feel closer to the actor.`,
];

const CUE_ROOM_CAMERA_VARIANTS = [
  ({ roomAnchor }) => `Let the camera search the room slightly until ${roomAnchor} becomes the clear visual anchor.`,
  ({ roomAnchor }) => `Hold the shot family, but rebalance the frame so ${roomAnchor} dominates the room read.`,
  ({ roomAnchor }) => `Use one subtle camera drift that makes ${roomAnchor} carry the whole cue change.`,
  ({ roomAnchor }) => `Tighten the framing pressure around ${roomAnchor} without inventing a new setup.`,
  ({ roomAnchor }) => `Keep the room fixed and let the camera settle into a new emphasis on ${roomAnchor}.`,
  ({ roomAnchor }) => `Move the frame just enough for ${roomAnchor} to become the visible destination inside the same space.`,
];
...
const buildWordstreamSceneProfile = ({
  ...
  const roomAnchor = normalizeRoomAnchorForScene(pickRoomAnchor(storyContext, sceneIndex), actorVisible);
  const cueLabel = compactCueLabel(cueText) || normalizeString(cueText) || 'current cue';
  const titleSuffix = cueLabel ? `: ${cueLabel}` : '';
  const beatTemplate = pickCueVariant(
    actorVisible ? CUE_ACTOR_BEAT_VARIANTS : CUE_ROOM_BEAT_VARIANTS,
    cueText,
    sceneIndex,
    1
  );
  const motionTemplate = pickCueVariant(
    actorVisible ? CUE_ACTOR_MOTION_VARIANTS : CUE_ROOM_MOTION_VARIANTS,
    cueText,
    sceneIndex,
    2
  );
  const cameraTemplate = pickCueVariant(
    actorVisible ? CUE_ACTOR_CAMERA_VARIANTS : CUE_ROOM_CAMERA_VARIANTS,
    cueText,
    sceneIndex,
    3
  );
  const beatLead = beatTemplate
    ? beatTemplate({ cueLabel, roomAnchor, actorText, locationText })
    : `The cue "${cueLabel}" changes the same shot around ${roomAnchor}.`;
  const motion = motionTemplate
    ? motionTemplate({ cueLabel, roomAnchor, actorText, locationText })
    : `The visible shot shifts around ${roomAnchor} without breaking continuity.`;
  const camera = cameraTemplate
    ? cameraTemplate({ cueLabel, roomAnchor, actorText, locationText })
    : `Keep the same setup while letting ${roomAnchor} become more dominant.`;

  const stillPrompt = actorVisible
    ? `${actorText} in ${locationText}, caught in a fresh visible shift of posture and gaze while ${roomAnchor} stays readable.`
    : `${locationText}, with ${roomAnchor} carrying a newly visible change in crop, light, and atmosphere.`;
  const imageDescription = actorVisible
    ? `${actorText} remains in ${locationText}; posture, gaze, and room tension redirect toward ${roomAnchor} while the same setup stays intact.`
    : `The same ${locationText}; framing, light, and spatial emphasis shift until ${roomAnchor} becomes the active part of the shot.`;
  const storyBeat = actorVisible
    ? `The wordstream cue "${cueLabel}" changes the visible shot through posture, gaze, and room emphasis around ${roomAnchor}, without adding new props.`
    : `The wordstream cue "${cueLabel}" changes the visible room through framing, light, and spatial emphasis around ${roomAnchor}, without inventing a visible actor or new props.`;
...
const refineSceneFromWordstream = ({
  ...
  const actorVisible = hasVisibleActorInStoryContext(storyContext);
  const unsupportedVisibleEntities = findUnsupportedVisibleEntities(scene, storyContext);
  ...
  const cueDriven = sceneLooksCueDriven(scene, {
    cueText,
    previousScene,
    previousCueText,
    storyContext,
  ...
  const rewriteVisibleFields = actorMismatch || (unsupportedVisibleEntities.length > 0 && !cueDriven);
  const rewriteDynamicFields = repeatedDominantSignature || !cueDriven || actorMismatch;
  const rewriteBeat = fieldLooksWeak(scene.beat, {
    cueText,
    previousValue: previousScene?.beat,
    storyContext,
    actorVisible,
    minKeywordCount: 5,
  }) || rewriteDynamicFields;
  const rewriteStoryBeat = fieldLooksWeak(scene.storyBeat, {
    cueText,
    previousValue: previousScene?.storyBeat,
    storyContext,
    actorVisible,
    minKeywordCount: 5,
  }) || rewriteDynamicFields || !fieldContainsCueLanguage(scene.storyBeat, cueText);
  const rewriteMotionCue = fieldLooksWeak(scene.motionCue, {
    cueText,
  ...
  const rewriteCameraCue = fieldLooksWeak(scene.cameraCue, {
    cueText,
  ...
  const rewriteStillPrompt = fieldLooksWeak(scene.stillPrompt, {
    cueText,
    previousValue: previousScene?.stillPrompt,
    storyContext,
    actorVisible,
    minKeywordCount: 5,
    requireSpatialSignal: true,
  }) || rewriteVisibleFields;
  const rewriteImageDescription = fieldLooksWeak(scene.imageDescription, {
    cueText,
    previousValue: previousScene?.imageDescription,
    storyContext,
    actorVisible,
    minKeywordCount: 5,
    requireSpatialSignal: true,
  ...
  const shouldRewriteVideoPrompt = rewriteVisibleFields
    ...
    || fieldLooksWeak(scene.videoPrompt, {
      cueText,
      previousValue: previousScene?.videoPrompt,
      storyContext,
      actorVisible,
      minKeywordCount: 6,
    });
  const shouldRewriteSingleImagePrompt = rewriteVisibleFields
    ...
    || rewriteStillPrompt
    || rewriteMotionCue
    || fieldLooksWeak(scene.singleImagePrompt, {
      cueText,
      previousValue: previousScene?.singleImagePrompt,
      storyContext,
      actorVisible,
      minKeywordCount: 6,
    });
...
export const sanitizeWebcamCameraScenePlan = (
  ...
  const sanitizedPlan = scenePlan.map((scene, index) => {
    ...
    if (hasWordstream && hasVisionContext) {
