...
export const DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT = [
  DEFAULT_SCENE_SYSTEM_PROMPT,
  'REFERENCE IMAGE: Use detected people and room as visual source material. Identity, actor count, position, orientation, geometry, and objects may drift only as a photographic event inside that same real camera world; do not introduce symbolic or illustrative objects.',
  'Source cues are private causes: silently translate each into an ordinary room relation or action; never make its noun, name, species, age, product, place, or phrase visible. Show their consequence through people, room operations, existing objects, or material conditions. An added prop needs an ordinary physical function, never a topic-representing role. Artwork, posters, labels, and screens stay flat objects; never transfer their graphic style to the footage. Carry a readable physical residue into the next scene.',
...
const buildGeneratedWordstreamMotion = ({
  ...
  const fallbackFragment = actorVisible
    ? compactWordstreamFragment(roomFocus, 18)
    : compactWordstreamFragment(joinPromptFragments(storyContext.description, roomFocus), 18);
...
const buildGeneratedWordstreamCamera = ({
  ...
  const fallbackFragment = compactWordstreamFragment(
    joinPromptFragments(roomFocus, storyContext.setupSummary),
    18
  );
...
const buildGeneratedWordstreamVisual = ({
  ...
  return {
    text: compactWordstreamFragment(
      fragment?.text
      || joinPromptFragments(storyContext.description, storyContext.setupSummary),
      18
    ),
...
const buildWordstreamSceneProfile = ({
  ...
  const cueTextValue = normalizeString(cueText);
  ...
  return {
    title: normalizeString(scene.title) || `Room consequence ${sceneIndex + 1}`,
    beat,
    stillPrompt,
    imageDescription,
    storyBeat,
...
const refineSceneFromWordstream = ({
  ...
  const preservePlannerScene = normalizeString(scene.beat)
  ...
  if (preservePlannerScene) {
    return {
      ...scene,
      title: normalizeString(scene.title) || profile.title,
      storyBeat: normalizeString(scene.storyBeat) || profile.storyBeat,
  ...
  const preserveCueDrivenPlannerBeat = normalizeString(scene.beat)
    && unsupportedCoreVisibleEntities.length === 0
    && !repeatedDominantSignature
    && !actorMismatch
    && normalizeString(scene.stillPrompt)
  ...
  const rewriteStoryBeat = !normalizeString(scene.storyBeat)
    || rewriteDynamicFields
    || !normalizeString(scene.storyBeat)
  ...
  const sanitizedPlan = scenePlan.map((scene, index) => {
    ...
    normalized = isFirst
      ...
      : isTrippyFlavor && trippyReanchorEnabled
        ...
        : creativeCutsEnabled && normalized.creativeCut === true
        ? {
            ...normalized,
            videoMode: 'singleImage',
            frameSource: 'newImage',
