...
const WORDSTREAM_QUALIFIER_WORDS = new Set([
  'bare', 'brief', 'close', 'dim', 'direct', 'faint', 'full', 'gentle', 'hard',
  'held', 'high', 'low', 'quick', 'quiet', 'readable', 'real', 'same', 'sharp',
  'slight', 'small', 'soft', 'steady', 'still', 'subtle', 'sudden', 'tight',
  'tilted', 'upper', 'visible', 'wide'
]);
...
const isConcreteEntityKeyword = (keyword) => keyword.length >= 4
  && !WORDSTREAM_BODY_WORDS.has(keyword)
  && !WORDSTREAM_ABSTRACT_WORDS.has(keyword)
  && !WORDSTREAM_QUALIFIER_WORDS.has(keyword)
  && !MOTION_LANGUAGE_PATTERN.test(keyword);

const findUnsupportedEntitySpansInText = ({
  value = '',
  storyContext = {},
  cueText = '',
} = {}) => {
  ...
  extractCandidateEntitySpans(sanitizedValue).forEach((span) => {
    const concreteKeywords = span.filter((keyword) => isConcreteEntityKeyword(keyword));
...
const scoreWordstreamFragment = (fragment, {
  ...
  const unsupportedEntities = findUnsupportedEntitySpansInText({
    value: fragment.text,
    storyContext,
    cueText,
  });
...
const buildWordstreamLeadSentence = ({
  cueLabel = '',
  roomFocus = '',
} = {}) => ensureWordstreamSentence(
  `The cue "${cueLabel}" disturbs the same shot around ${roomFocus || 'the visible room'}.`
);
...
const buildWordstreamFollowSentence = ({
  fragmentText = '',
  fallbackText = '',
} = {}) => {
  const fragment = compactWordstreamFragment(fragmentText, 18);
  if (!fragment) {
    return ensureWordstreamSentence(fallbackText);
  }
  if (/^(?:he|she|his|her|they|their|the room|the frame|the camera|light|focus|crop|wall|window|ceiling|door|map|globe|background|foreground)\b/i.test(fragment)) {
    return ensureWordstreamSentence(fragment);
  }
  return ensureWordstreamSentence(fragment);
};
...
const findUnsupportedEntitiesInFields = (
  ...
  fields.forEach((field) => {
    findUnsupportedEntitySpansInText({
      value: scene?.[field],
      storyContext,
      cueText,
...
const findUnsupportedVisibleEntities = (scene = {}, storyContext = {}, cueText = '') => findUnsupportedEntitiesInFields(
  scene,
  storyContext,
  CAMERA_VISIBLE_TEXT_FIELDS,
  cueText
);
...
const buildGeneratedWordstreamMotion = ({
  ...
  return {
    text: buildWordstreamFollowSentence({
      fragmentText: fragment?.text,
      fallbackText: actorVisible
        ? `The "${compactCueLabel(cueText) || cueText || 'current cue'}" cue registers through a tightened stance, redirected gaze, and checked breath toward ${roomFocus}.`
        : `The "${compactCueLabel(cueText) || cueText || 'current cue'}" cue registers through crop, light, and pressure gathering at ${roomFocus}.`,
    }),
...
const buildGeneratedWordstreamCamera = ({
  ...
  return {
    text: buildWordstreamFollowSentence({
      fragmentText: fragment?.text,
      fallbackText: `Keep ${roomFocus} readable while the "${compactCueLabel(cueText) || cueText || 'current cue'}" shift changes the frame.`,
    }),
...
const buildGeneratedWordstreamVisual = ({
  ...
  return {
    text: compactWordstreamFragment(fragment?.text, 18),
...
const buildWordstreamSceneProfile = ({
  ...
  const visibleSentence = visualFocus.text
    ? ensureWordstreamSentence(visualFocus.text)
    : ensureWordstreamSentence(
      actorVisible
        ? `${actorText} stays inside ${locationText} while ${roomFocus} becomes the active part of the frame`
        : `${locationText} stays in view while ${roomFocus} becomes the active part of the frame`
    );
  const beat = joinUniqueWordstreamSentences(
    buildWordstreamLeadSentence({
      cueLabel: cueTextValue,
      roomFocus,
    }),
    motion.text
  );
  const storyBeat = actorVisible
    ? `The wordstream cue "${cueTextValue}" meshes with ${locationText} and the visible actor, turning ${roomFocus} into the next readable scene change without adding new props.`
    : `The wordstream cue "${cueTextValue}" meshes with ${locationText} and turns ${roomFocus} into the next readable scene change without inventing a visible actor or new props.`;
  const stillPrompt = actorVisible
    ? `${actorText} in ${locationText}, ${compactWordstreamFragment(visibleSentence, 22)}, with frame tension gathering at ${roomFocus}`
    : `${locationText}, ${compactWordstreamFragment(visibleSentence, 22)}, with frame tension gathering at ${roomFocus}`;
  const imageDescription = actorVisible
    ? `${actorText} remains in ${locationText}. ${visibleSentence} ${ensureWordstreamSentence(`Frame tension gathers at ${roomFocus}`)}`
    : `${ensureWordstreamSentence(`${locationText} remains in view`)} ${visibleSentence} ${ensureWordstreamSentence(`Frame tension gathers at ${roomFocus}`)}`;
  const videoPrompt = joinUniqueWordstreamSentences(beat, camera.text);
  const singleImagePrompt = joinUniqueWordstreamSentences(stillPrompt, motion.text);
  ...
  return {
    ...
    imageDescription,
    storyBeat,
    motionCue: motion.text,
    cameraCue: camera.text,
    videoPrompt,
    singleImagePrompt,
  };
};
...
const refineSceneFromWordstream = ({
  ...
  const actorVisible = hasVisibleActorInStoryContext(storyContext);
  const unsupportedVisibleEntities = findUnsupportedVisibleEntities(scene, storyContext, cueText);
  ...
  const rewriteVisibleFields = actorMismatch || unsupportedVisibleEntities.length > 0;
  const rewriteDynamicFields = repeatedDominantSignature
    || !cueDriven
    || actorMismatch;
  const rewriteBeat = fieldLooksWeak(scene.beat, {
    ...
    actorVisible,
    minKeywordCount: 5,
  }) || rewriteDynamicFields || unsupportedBeatEntities.length > 0;
  const rewriteStoryBeat = fieldLooksWeak(scene.storyBeat, {
    ...
    actorVisible,
    minKeywordCount: 5,
  }) || rewriteDynamicFields || !fieldContainsCueLanguage(scene.storyBeat, cueText) || unsupportedStoryBeatEntities.length > 0;
