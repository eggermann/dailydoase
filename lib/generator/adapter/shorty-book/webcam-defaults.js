...
const buildFirstLastMotionPrompt = ({ motionCue = '' } = {}) => {
  const action = compactScenePrompt(
    normalizeString(motionCue, 'One clear movement connects the start and end frames.'),
    1,
    24
  ).replace(/[.]+$/g, '').trim();

  return compactScenePrompt([
    'Locked-off CCTV view.',
    action ? `${action}.` : '',
    'Natural continuous body motion; room and camera remain fixed.',
  ].filter(Boolean).join(' '), 3, 48);
};
...
export const createWebcamFirstLastPrompt = ({
  ...
  if (isReferenceImageActorMode(configMode)) {
    return buildFirstLastMotionPrompt({
