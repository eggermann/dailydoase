...
export const DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT = [
  ...
  'Across the whole camera plan, vary the subscene logic instead of repeating one pattern: mix continuation beats and destination beats when the story naturally supports them.',
  ...
  'Choose those two scene types from the story logic: continuation for subtle emotional development, and firstLast for a visible shift toward a new setup in the same room.',
  ...
  'Before you output JSON, check the later scenes and confirm the plan includes the required mix, that every later singleImage scene starts from lastFrame, and that every firstLast camera scene has useCameraShot=true.',
