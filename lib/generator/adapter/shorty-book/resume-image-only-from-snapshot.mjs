...
const explicitScenePlan = explicitScenePlanPath
  ...
  : null;
const untrustedSavedScenePlan = Array.isArray(explicitScenePlan)
  ? explicitScenePlan
  : (Array.isArray(snapshot.runtimeScenePlan) ? snapshot.runtimeScenePlan : []);
const savedSourceCueRecords = Array.isArray(snapshot.sourceCueRecords)
  ? snapshot.sourceCueRecords
  : [];
const savedPlanLanguageValidation = validateEnglishScenePlanContent(untrustedSavedScenePlan);
const savedPlanHasEnglishSemanticTerms = untrustedSavedScenePlan.every((scene) => (
  String(scene?.semanticAnchorEnglish || '').trim()
  && String(scene?.semanticCollisionEnglish || '').trim()
));
const completeSavedScenePlan = savedPlanLanguageValidation.valid && savedPlanHasEnglishSemanticTerms
  ? compactScenePlanFields(untrustedSavedScenePlan)
  : buildNeutralFallbackScenePlan({
      sceneCount: untrustedSavedScenePlan.length,
      sceneLengths: untrustedSavedScenePlan.map((scene) => scene?.durationSeconds),
      sourceCueRecords: savedSourceCueRecords,
  });
if (!savedPlanLanguageValidation.valid || !savedPlanHasEnglishSemanticTerms) {
  console.warn('[image-only-resume] saved runtime plan rejected; using neutral validated fallback');
}
