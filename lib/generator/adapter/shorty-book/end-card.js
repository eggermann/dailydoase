...
export const renderExhibitionEndCard = async ({
  ...
  const usePortraitTrailerLayout = useCompactTrailerLayout && height > width;
  const posterSvg = `
  ...
  `;
  const portraitTrailerSvg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
      <defs>
        <linearGradient id="portraitVeil" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="#120d0a" stop-opacity="0.72"/>
          <stop offset="48%" stop-color="#120d0a" stop-opacity="0.46"/>
          <stop offset="100%" stop-color="#120d0a" stop-opacity="0.84"/>
        </linearGradient>
      </defs>
      <rect width="100%" height="100%" fill="url(#portraitVeil)"/>
      <rect x="0" y="0" width="${width}" height="10" fill="#54b62d"/>
      <style>
        .title { font-family: "DejaVu Serif", serif; font-size: ${Math.round(width * 0.095)}px; font-weight: 700; fill: #f4ef21; letter-spacing: -1.4px; }
        .detail { font-family: "DejaVu Sans", sans-serif; font-size: ${Math.round(width * 0.031)}px; font-weight: 700; fill: #fff8e8; letter-spacing: 0.35px; }
        .accent { font-family: "DejaVu Sans", sans-serif; font-size: ${Math.round(width * 0.034)}px; font-weight: 700; fill: #54b62d; letter-spacing: 0.8px; }
        .subtitle { font-family: "DejaVu Sans", sans-serif; font-size: ${Math.round(width * 0.034)}px; font-weight: 700; fill: #ff43a7; letter-spacing: 1px; }
      </style>
      <text x="${width / 2}" y="${Math.round(height * 0.13)}" text-anchor="middle" class="title">FORMEN DER</text>
      <text x="${width / 2}" y="${Math.round(height * 0.205)}" text-anchor="middle" class="title">ABWEICHUNG</text>
      <line x1="${Math.round(width * 0.16)}" y1="${Math.round(height * 0.27)}" x2="${Math.round(width * 0.84)}" y2="${Math.round(height * 0.27)}" stroke="#54b62d" stroke-width="4"/>
      <text x="${width / 2}" y="${Math.round(height * 0.33)}" text-anchor="middle" class="detail">${venue}</text>
      <text x="${width / 2}" y="${Math.round(height * 0.395)}" text-anchor="middle" class="accent">VERNISSAGE · ${vernissage.replace('VERNISSAGE ', '')}</text>
      <text x="${width / 2}" y="${Math.round(height * 0.46)}" text-anchor="middle" class="detail">${exhibition}</text>
      <text x="${width / 2}" y="${Math.round(height * 0.525)}" text-anchor="middle" class="subtitle">AUSSTELLUNG · SOUNDS · PERFORMANCE</text>
    </svg>
  `;
  const svg = usePortraitTrailerLayout
    ? portraitTrailerSvg
    : useCompactTrailerLayout
      ? compactTrailerSvg
    : (backgroundImageDataUri ? overlaySvg : posterSvg);

  const resolvedOutputPath = path.resolve(outputPath);
  await fs.ensureDir(path.dirname(resolvedOutputPath));
  if (usePortraitTrailerLayout) {
    const portraitBase = resolvedBackgroundImagePath
      ? sharp(resolvedBackgroundImagePath).resize(width, height, { fit: 'cover' })
      : sharp({
        create: {
          width,
          height,
    ...
    await portraitBase
      .composite([{ input: Buffer.from(portraitTrailerSvg) }])
      .png()
