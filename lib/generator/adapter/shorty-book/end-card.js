...
export const renderExhibitionEndCard = async ({
  ...
  const compactTrailerSvg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
      ...
      <text x="${width / 2}" y="158" text-anchor="middle" class="subtitle">GREEN MONSTER WARE HAUS</text>
      <line x1="${Math.round(width * 0.16)}" y1="181" x2="${Math.round(width * 0.84)}" y2="181" stroke="#54b62d" stroke-width="3"/>
      <text x="${width / 2}" y="213" text-anchor="middle" class="detail">${venue}</text>
      <text x="${width / 2}" y="244" text-anchor="middle" class="accent">VERNISSAGE · ${vernissage.replace('VERNISSAGE ', '')}</text>
      <text x="${width / 2}" y="275" text-anchor="middle" class="detail">${exhibition}</text>
      <text x="${width / 2}" y="301" text-anchor="middle" class="subtitle">AUSSTELLUNG · SOUNDS · PERFORMANCE</text>
    </svg>
  `;
