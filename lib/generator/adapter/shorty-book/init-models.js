...
export const initModels = async ({ config, imageDir }) => {
  const initVideoModelWithFallbacks = async (primaryType, baseConfig = {}, defaultType = '') => {
    ...
    if (initErrors.length > 0) {
      logger.warn(
        `Video fallback init had recoverable errors: ${initErrors.map(({ type, message }) => `${type}: ${message}`).join(' | ')}`
      );
    }
  ...
  const {
    ...
    ? await initVideoModelWithFallbacks('wanFirstLast', firstLastModelConfig, 'wanFirstLast')
