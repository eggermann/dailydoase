...
export const shouldInitCastContextImageModel = (config = {}) => resolveBoolean(
  config?.sceneLoop?.castContext?.enabled,
  false
);
...
export const initModels = async ({ config, imageDir }) => {
  ...
  const mireloMode = String(config?.sceneLoop?.mireloMode || '').trim().toLowerCase();
  const mireloDisabled = ['off', 'none', 'disabled'].includes(mireloMode);
  const mireloAI = mireloDisabled
    ? null
  ...
  const personaReferenceImageModel = shouldInitPersonaReferenceImageModel(config)
    ...
    : null;
  const castContextImageModel = shouldInitCastContextImageModel(config)
    ? await ImageActorInScene.init({
      ...(config?.sceneLoop?.castContext?.model || {}),
      skipCollectionCounter: true,
    })
    : null;
  ...
  if (castContextImageModel) {
    castContextImageModel.imageDir = partsDir;
  }
