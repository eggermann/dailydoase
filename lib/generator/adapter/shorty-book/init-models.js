...
export const initModels = async ({ config, imageDir }) => {
  const mireloAI = await new PostToMirelo_VideoSound().init(config.mireloAI ?? {});
  const videoModelFirstLast = await new PostToWan22_FirstLastFrame().init((config.video?.model) ?? {});
  const videoModelSingle = await new PostToWan22_5B_ImageVideo().init((config.video?.model2) ?? {});
  ...
  const imageInitConfig = resolveImageInitConfig(config.image);
