...
const USE_WEBCAM_PERSONA_REFERENCE = STORY_MODE === 'camera'
  ? parseBoolean(
    pickEnvValue(
...
const getRemoteCameraImageCacheDir = () => path.join(CONFIG.camera.outputDir, '.remote-camera-cache');

const getRemoteImagePath = (imageUrl, imageIndex, { prefix = 'remote', cacheDir = getRemoteCameraImageCacheDir() } = {}) => {
...
const getRemoteCameraImagePath = (imageUrl, imageIndex) => getRemoteImagePath(imageUrl, imageIndex, {
  prefix: 'camera',
});

const getRemoteProtagonistImagePath = (imageUrl) => getRemoteImagePath(imageUrl, 0, {
  prefix: 'protagonist',
});

const getRemoteSceneContextImagePath = (imageUrl, imageIndex) => getRemoteImagePath(imageUrl, imageIndex, {
  prefix: 'scene-context',
});

const downloadRemoteImageToCache = async (imageUrl, imagePath) => {
...
const buildFolderImagesApiUrl = (folderUrl) => {
  ...
  return `https://dailydoase.de/api/folder-images?url=${encodeURIComponent(resolvedFolderUrl)}`;
};

const loadSceneContextImageUrls = async () => {
  const explicitUrls = CONFIG.sceneContextImage.urls.filter(Boolean);
  const apiUrl = CONFIG.sceneContextImage.apiUrl || buildFolderImagesApiUrl(CONFIG.sceneContextImage.folderUrl);
  if (!apiUrl) {
    return explicitUrls;
  }
  ...
  try {
    ...
    const apiUrls = Array.isArray(payload?.items)
      ? payload.items
        ...
        .map((entry) => {
          if (/^https?:\/\//i.test(entry)) {
            return entry.replace(/^http:\/\//i, 'https://');
          }
          if (entry.startsWith('/')) {
            return `https://dailydoase.de/v${entry}`;
          }
          try {
            return new URL(entry, 'https://dailydoase.de/v/').toString();
          } catch {
            return '';
          }
        })
        .filter(Boolean)
      : [];
    const combinedUrls = [...explicitUrls, ...apiUrls];
    if (!CONFIG.sceneContextImage.startAfterProtagonist || !CONFIG.camera.protagonistImageUrl) {
      return combinedUrls;
    }
    const protagonistFile = path.basename(new URL(CONFIG.camera.protagonistImageUrl).pathname);
    const protagonistIndex = combinedUrls.findIndex((entry) => {
      try {
        return path.basename(new URL(entry).pathname) === protagonistFile;
      } catch {
        return false;
      }
    });
    if (protagonistIndex < 0 || protagonistIndex >= combinedUrls.length - 1) {
      return combinedUrls;
    }
    return combinedUrls.slice(protagonistIndex + 1);
  } catch (error) {
    console.warn(`[freshweb-middle-cost-4-3] scene context image API failed: ${error?.message || error}`);
    return explicitUrls;
  }
};
