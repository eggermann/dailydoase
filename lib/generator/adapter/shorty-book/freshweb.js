...
const claimNextQueuedPersonShot = async () => {
  ...
  await fs.outputJson(CAMERA_PERSON_QUEUE_CONSUMED_PATH, {
    ids: [...consumedIds, entry.id].slice(-128),
  ...
  while (true) {
    ...
    if (!VALIDATE_CAMERA_SHOT) {
      return {
        imagePath,
        visionText: '',
        imageSource: emptyFrameCount > 0 ? `captured-waited-${emptyFrameCount}` : 'captured',
      };
    }
    ...
    if (!looksLikeBadCameraShot(visionText)) {
      return {
        ...
        imageSource: usableShotAttempts === 0
          ? (emptyFrameCount > 0 ? `captured-waited-${emptyFrameCount}` : 'captured')
...
const promptFunktion = async (streams, config) => {
  ...
  config.sceneLoop.openingImage = {
    ...
    sourceType: openingFluxContextActive ? 'fluxContext' : 'cameraShot',
