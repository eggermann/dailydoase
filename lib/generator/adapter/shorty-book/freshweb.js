...
const getRemoteCameraImagePath = (imageUrl, imageIndex) => {
  const resolvedUrl = String(imageUrl || '').trim();
  const hash = crypto.createHash('sha1').update(`${imageIndex}:${resolvedUrl}`).digest('hex').slice(0, 12);
  let extension = '.jpg';
  try {
  ...
  return path.join(
    getRemoteCameraImageCacheDir(),
    `${String(imageIndex + 1).padStart(3, '0')}-${hash}${safeExtension}`
  );
};
