...
test('runtime defaults and camera sanitization preserve semantic enforcement fields', () => {
  const semanticFields = {
    sceneFocus: 'trace',
    semanticAnchorEnglish: 'exhibition',
    semanticCollisionEnglish: 'hunger',
    semanticDerivation: {
      anchorContribution: 'The exhibition protects each object.',
      collisionContribution: 'Hunger consumes available material.',
      contradiction: 'Protection survives only if visibility is consumed.',
      physicalization: 'The monster removes glowing filaments from lamps.',
      causalResult: 'A dark lamp route remains overhead.',
    },
    tensionCause: 'The dark lamp route advances toward the elevator.',
    consequenceId: 'scene-01-dark-lamp-route',
    inheritsConsequenceId: '',
    clueSource: 'consequence',
    offscreenMonsterAction: 'The monster removes filaments outside direct view.',
    visibleEvidenceOfAgency: 'Lamps extinguish after the camera looks away.',
  };
  const planned = [{
    index: 1,
    videoMode: 'firstLast',
  ...
  const defaulted = applyWebcamScenePlanVideoModeDefaults(planned, {
    firstClipVideoMode: 'singleImage',
  });
  const [sanitized] = sanitizeWebcamCameraScenePlan(defaulted);

  for (const field of Object.keys(semanticFields)) {
    expect(sanitized[field]).toEqual(semanticFields[field]);
  }
});
...
test('resolveWebcamScenePlanSystemPrompt appends trippy guidance for ltxTrippy flavor', () => {
  ...
  expect(prompt).toContain('sceneFlavor is "ltxTrippy"');
  expect(prompt).toContain('allow surreal transformations of light, texture, background geometry, color cast, atmosphere, and implied props');
  expect(prompt).toContain('prompt-dominant spectacle is more important than strict shot continuity');
});
...
test('camera default prompt tells the planner to choose frameSource and videoMode from the story point', () => {
  ...
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain(
    'For each reference-image-actor scene, decide whether the cue acts mainly on the subject, on the room, or on both together'
  );
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain(
    'Preserve the real camera orientation from the source webcam frame'
  );
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain(
  ...
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain(
    'Default to videoMode "singleImage" in reference-image-actor mode when in doubt'
  );
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain(
...
test('sanitizeWebcamCameraScenePlan ties cue pressure to subject and room when rebuilding weak scenes', () => {
  const scenePlan = sanitizeWebcamCameraScenePlan([
    {
      title: 'Beat 1',
      beat: 'The room feels tense.',
  ...
  expect(combined).toMatch(/grand guignol bread/i);
  expect(combined).toMatch(/bald man|subject|actor|man/i);
  expect(combined).toMatch(/door frame|wall|room/i);
});
...
test('sanitizeWebcamCameraScenePlan preserves planned people when trailer policy allows them', () => {
  const scenePlan = sanitizeWebcamCameraScenePlan([
    {
      title: 'Human collision',
      beat: 'The cue "1983 Kaufhaus coral" makes one visitor brace under the fluorescent fixtures.',
      stillPrompt: 'One visitor stands beside the green monster in the same Kaufhaus hall under fluorescent fixtures.',
      imageDescription: 'The visitor and green monster remain inside the concrete Kaufhaus hall.',
      storyBeat: 'The cue "1983 Kaufhaus coral" changes human behavior and room pressure.',
      motionCue: 'The visitor steps backward as the green monster opens its glowing eyes.',
      cameraCue: 'The camera pushes toward the visitor and mirrored column.',
      videoPrompt: 'The visitor steps backward while the camera pushes toward the mirrored column.',
      singleImagePrompt: 'One visitor beside the green monster steps backward in the Kaufhaus hall.',
      frameSource: 'newImage',
      videoMode: 'singleImage',
      durationSeconds: 3,
      freshImage: true,
    ...
    visionStoryContext: 'Location: Concrete Kaufhaus hall with mirrored columns and fluorescent fixtures. Actors: (None visibly present in the frame)*. Description: Empty hall under fluorescent light.',
    sourceCues: ['1983 Kaufhaus coral'],
    allowPeople: true,
  });
  ...
  expect(combinedPrompt).toMatch(/visitor/i);
});
