...
test('camera default prompt keeps creative choices open and enforces only transport state', () => {
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain('may drift only as a photographic event');
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain('do not introduce symbolic or illustrative objects');
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain('Show their consequence through people, room operations, existing objects, or material conditions');
  ...
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT.length).toBeLessThan(4500);
});
...
test('sanitizeWebcamCameraScenePlan keeps an enabled creative cut as a fresh generated Runware start', () => {
  const scenePlan = sanitizeWebcamCameraScenePlan([
    {
      title: 'Opening',
      videoMode: 'singleImage',
...
test('createWebcamFirstLastPrompt keeps planned story text in camera mode', async () => {
  ...
  expect(prompt).toContain('Fixed real 1989 Einkaufszentrum CCTV viewpoint');
...
test('createWebcamFirstLastPrompt keeps CCTV as a camera rule and prioritizes continuous action', async () => {
  const createPrompt = createWebcamFirstLastPrompt({
    configMode: 'reference-image-actor',
    cameraStyle: 'Real 1989 German Einkaufszentrum CCTV footage: fixed high security-camera angle, slight VHS noise.',
    getFrameVision: async () => 'Actors: a painter kneels at an easel. Location: an exhibition studio.',
    getContinuityFrameVision: async () => 'Actors: a painter kneels at an easel. Location: an exhibition studio.',
  });
  ...
  const prompt = await createPrompt(
    ...
    { index: 1, durationSeconds: 5 },
    {
      scenePlan: [{
        index: 1,
        durationSeconds: 5,
        storyBeat: 'The painter turns the measured figure into a moving crowd.',
        videoPrompt: 'The painter rises, steps to the canvas, and draws three quick figures.',
        motionCue: 'He rises from kneeling, takes two steps, and draws without stopping.',
        cameraCue: 'Static aisle-end CCTV hold with no operator movement.',
      }],
      startFrame: { image: { path: 'start.png' } },
      endFrame: { image: { path: 'end.png' } },
    }
  ...
  expect(prompt).toContain('Fixed real 1989 Einkaufszentrum CCTV viewpoint');
  expect(prompt).toContain('camera stays locked while people and objects move continuously');
  expect(prompt).toContain('He rises from kneeling, takes two steps');
  expect(prompt).toContain('do not freeze, hold a pose, or delay the change');
