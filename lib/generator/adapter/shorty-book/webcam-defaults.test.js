...
test('still-image prompts compile explicit FLUX current-state and visible-change sections', async () => {
  const prompt = await createWebcamImagePromptHandler('', {}, {
    stillPrompt: 'A newly invented woman steps physically into the room.',
  });

  expect(prompt).toContain('CURRENT VISUAL STATE:');
  expect(prompt).toContain('NEXT VISIBLE CHANGE:');
  expect(prompt).toContain('CAMERA REALITY:');
  expect(prompt).toContain('A newly invented woman steps physically into the room');
});
...
test('camera default prompt preserves identity and b1 CCTV camera grammar', () => {
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain('Keep detected people recognizable');
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain('fixed high corner, ceiling, doorway');
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain('1989 Einkaufszentrum CCTV: wide practical coverage');
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain('A new person must visibly enter');
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain('states scale (handheld, table-sized, or room-sized)');
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain('Carry residue forward.');
  ...
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain('Scene 1 of the first iteration is newImage + singleImage + freshImage=true + useCameraShot=true');
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT.length).toBeLessThan(4800);
});
...
test('story-driven defaults preserve GPT first-scene transport mode', () => {
  const scenePlan = applyWebcamScenePlanVideoModeDefaults(
    [{ title: 'Visitor may enter', videoMode: 'firstLast' }],
    {
      resolveConfiguredVideoMode: () => 'singleImage',
      scenePlanControlsVideoMode: true,
      preserveFirstSceneVideoMode: true,
    }
  );

  expect(scenePlan[0].videoMode).toBe('firstLast');
});
...
test('story-driven sanitizer preserves all three supported iteration starts', () => {
  const options = { storyDrivenIterationStart: true };
  const continuation = sanitizeWebcamCameraScenePlan([{
    title: 'Continue',
    videoMode: 'singleImage',
    frameSource: 'lastFrame',
    freshImage: false,
  ...
  const transition = sanitizeWebcamCameraScenePlan([{
    title: 'Transition',
    videoMode: 'firstLast',
    frameSource: 'lastFrame',
    freshImage: false,
  ...
  const reset = sanitizeWebcamCameraScenePlan([{
    title: 'Reset',
    videoMode: 'singleImage',
    frameSource: 'newImage',
    freshImage: true,
  ...
  expect(continuation[0]).toMatchObject({ videoMode: 'singleImage', frameSource: 'lastFrame', freshImage: false, useCameraShot: false });
  expect(transition[0]).toMatchObject({ videoMode: 'firstLast', frameSource: 'lastFrame', freshImage: false, useCameraShot: true });
  expect(reset[0]).toMatchObject({ videoMode: 'singleImage', frameSource: 'newImage', freshImage: true, useCameraShot: true });
  expect(describeWebcamCameraScenePlanIssues(transition, options)).toEqual([]);
});

test('sanitizeWebcamCameraScenePlan permits a configured first-last opening from fresh webcam shot', () => {
  const scenePlan = sanitizeWebcamCameraScenePlan([
    {
...
test('createWebcamFirstLastPrompt sends only motion and locked CCTV rule', async () => {
  ...
  expect(prompt).toBe('Locked-off CCTV view. He flinches back from the desk.');
  expect(prompt).not.toContain('The lecture turns into a private nightmare');
  expect(prompt).not.toContain('The professor in the study');
  expect(prompt).not.toContain('CONTINUITY:');
});

test('createWebcamSingleImagePrompt sends WAN motion and camera, not story prose', async () => {
  ...
  expect(prompt).not.toContain('The scholarly mask slips');
  expect(prompt).not.toContain('The professor in the study');
...
test('createWebcamSingleImagePrompt does not leak previous story memory into WAN', async () => {
  const createPrompt = createWebcamSingleImagePrompt({
    configMode: 'camera',
    getFrameVision: async () => 'Subject: the professor. Setting: the study.',
  });
  ...
  expect(prompt).not.toContain('He tries to continue speaking while the room keeps pressing in');
  expect(prompt).not.toContain('The scholarly mask slips');
  expect(prompt).not.toContain('The professor in the study');
  expect(prompt).toContain('He swallows, glances aside, and tries to hold the pose');
});

test('createWebcamSingleImagePrompt leaves surreal still construction to FLUX', async () => {
  const createPrompt = createWebcamSingleImagePrompt({
    configMode: 'camera',
  ...
  expect(prompt).not.toContain('He craves fries but keeps the feeling internal');
  expect(prompt).not.toContain('plate of fries');
  expect(prompt).toContain('He leans in slightly and narrows his eyes');
  expect(prompt).toContain('CONTINUITY:');
});

test('createWebcamSingleImagePrompt keeps transition motion when firstLast becomes singleImage', async () => {
  const createPrompt = createWebcamSingleImagePrompt({
    configMode: 'camera',
    getFrameVision: async () => 'Subject: the professor. Setting: the study.',
  });
  ...
  expect(prompt).not.toContain('The professor in the study');
  expect(prompt).not.toContain('The room closes in around him');
  expect(prompt).toContain('He rises fast and turns toward the window');
  expect(prompt).toContain('The camera pushes in with him');
  expect(prompt).toContain('CONTINUITY:');
});

test('createWebcamSingleImagePrompt uses visual continuity contract without re-describing identity', async () => {
  const createPrompt = createWebcamSingleImagePrompt({
    configMode: 'camera',
    getFrameVision: async () => [
  ...
  expect(prompt).not.toContain('The professor in the study');
  expect(prompt).toContain('Keep identity, anatomy, room layout, lighting, framing, lens behavior, and image texture stable');
