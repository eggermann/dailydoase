...
test('createWebcamFirstLastPrompt uses a lean motion-only prompt in camera mode', async () => {
  ...
  expect(prompt).toContain('Locked-off CCTV view');
  expect(prompt).toContain('He flinches back from the desk');
  expect(prompt).toContain('Natural continuous body motion; room and camera remain fixed');
  expect(prompt).not.toContain('The lecture turns into a private nightmare');
  expect(prompt).not.toContain('The camera edges closer');
  expect(prompt).not.toContain('The professor in the study');
  expect(prompt).not.toMatch(/\b(?:static|hold steady)\b/i);
});
