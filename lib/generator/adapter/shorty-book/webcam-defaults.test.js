...
test('sanitizeWebcamCameraScenePlan rewrites camera scenes from source cues and vision without literal prop carryover', () => {
  ...
  expect(scenePlan[0].title).toContain('Wordstream shift 1: american mary homity pie');
  expect(scenePlan[0].stillPrompt).toContain('room');
  expect(scenePlan[0].storyBeat).toContain('wordstream cue "american mary homity pie"');
  expect(scenePlan[0].videoPrompt).toContain('"american mary homity pie"');
  ...
  expect(scenePlan[1].title).toContain('Wordstream shift 2: lexikon der filmbegriffe erotic');
  expect(scenePlan[1].motionCue).toMatch(/weight|face|gaze|breath|shoulders|jaw|stance/i);
});
...
test('sanitizeWebcamCameraScenePlan rewrites repeated adjacent cue scenes into different dominant motion and camera states', () => {
  ...
  expect(scenePlan[0].storyBeat).toContain('"chicharr chicken"');
  expect(scenePlan[1].storyBeat).toContain('"grand guignol bread"');
...
test('sanitizeWebcamCameraScenePlan falls back to room-grounded wording when no actor is visible', () => {
  ...
  expect(scenePlan[0].storyBeat).toContain('"hammer filme tarantula film"');
  expect(scenePlan[0].storyBeat).toContain('without inventing a visible actor');
...
test('createWebcamFirstLastPrompt keeps planned story text in camera mode', async () => {
  ...
  expect(prompt).toContain('Timing: one 3.2-second transition with one clear start-to-end arc.');
  expect(prompt).not.toContain('4.8-second transition');
