...
test('sanitizeWebcamCameraScenePlan rewrites camera scenes from source cues and vision without literal prop carryover', () => {
  ...
  expect(scenePlan[0].storyBeat).toContain('current wordstream cue');
  expect(scenePlan[0].videoPrompt).not.toMatch(/bowl|mixture|ingredient|pie|oven/i);
  ...
  expect(scenePlan[1].motionCue).toMatch(/eyes|breath|shoulders|mouth|hands/i);
});
