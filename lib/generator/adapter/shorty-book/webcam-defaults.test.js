...
test('camera default prompt keeps creative choices open and enforces only transport state', () => {
  ...
  expect(DEFAULT_WEBCAM_CAMERA_SCENE_SYSTEM_PROMPT).toContain('requiresDocumentaryEndFrame=true');
...
test('createWebcamImagePromptHandler adds documentary material realism to FLUX destination prompts', async () => {
  const prompt = await createWebcamImagePromptHandler('fallback', {}, {
    stillPrompt: 'A transparent puddle spreads across the existing concrete floor.',
  });

  expect(prompt).toContain('A transparent puddle spreads across the existing concrete floor.');
  expect(prompt).toContain('real CCTV webcam frame with documentary deep focus');
  expect(prompt).toContain('plausible physical scale, contact, occlusion, material texture');
  expect(prompt).toContain('preserve visible texture through transparent material');
});
...
test('sanitizeWebcamCameraScenePlan routes a planned documentary destination through firstLast', () => {
  const scenePlan = sanitizeWebcamCameraScenePlan([
    {
      title: 'Opening',
      videoMode: 'singleImage',
    ...
    {
      title: 'Physical water consequence',
      videoMode: 'singleImage',
      frameSource: 'lastFrame',
      freshImage: false,
      useCameraShot: false,
      requiresDocumentaryEndFrame: true,
    },
  ]);
...
test('createWebcamSingleImagePrompt keeps chosen surveillance-camera style in the WAN prompt', async () => {
  ...
  expect(prompt).toContain('Real 1989 German Einkaufszentrum CCTV footage: locked high security-camera angle, slight VHS noise.');
...
test('createWebcamSingleImagePrompt carries previous scene memory into the next scene prompt', async () => {
      ...
      scenePlan: [
        {
          ...
          storyBeat: 'The scholarly mask slips.',
          motionCue: 'His face tightens and his gaze locks upward.',
          exitMotion: 'His gaze is already lifting upward and his shoulders continue turning toward the window.',
        },
        {
          index: 2,
  ...
  expect(prompt).toContain('Start with the previous motion already underway: His gaze is already lifting upward');
  expect(prompt).toContain('The professor in the study');
});
