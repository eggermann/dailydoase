...
test('camera prompt chain writes inspectable mock video prompts', async () => {
  ...
  expect(promptChain[0].prompt).toContain(promptChain[0].scene.videoPrompt);
  expect(promptChain[0].prompt.toLowerCase()).toContain(promptChain[0].scene.cameraCue.toLowerCase());
  expect(promptChain[0].prompt).not.toContain('Timing: one');
  if (secondScene) {
    expect(secondScene.prompt).toContain(secondScene.scene.videoPrompt);
  }
  expect(await fs.pathExists(promptChainPath)).toBe(true);
