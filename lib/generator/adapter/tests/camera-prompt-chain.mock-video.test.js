...
test('camera prompt chain writes inspectable mock video prompts', async () => {
  ...
  expect(promptChain[0].prompt).toContain(`Timing: one ${expectedFirstHeldDuration}-second held shot`);
  expect(promptChain[0].prompt).toContain('Beat:');
  expect(promptChain[0].prompt).toContain('Motion:');
  expect(promptChain[0].prompt).toContain('Camera:');
  if (secondScene) {
    ...
    const expectedDurationLabel = secondScene.videoMode === 'firstLast'
      ? `Timing: one ${expectedSecondDuration}-second transition`
      : `Timing: one ${expectedSecondDuration}-second held shot`;
