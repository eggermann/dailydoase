...
const formatPromptLogField = (prompt, maxLength = 1000) => {
  ...
  return `  ${colorText(String('prompt').padEnd(13), 'dim')}${colorText(text, 'magenta')}`;
};
...
test('camera prompt chain writes inspectable mock video prompts', async () => {
  ...
  logSection('vision story context', normalizeInlineText(visionStoryContext), 'green');
  ...
  if (secondScene) {
    const expectedSecondDuration = PROMPT_CHAIN_SCENE_REQUEST.requestedSceneLengths[1];
    const expectedDurationLabel = secondScene.videoMode === 'firstLast'
      ? `Fit the action to a ${expectedSecondDuration}-second transition`
      : `Fit the action to a ${expectedSecondDuration}-second held shot`;
    expect(secondScene.prompt).toContain(expectedDurationLabel);
  }
  expect(await fs.pathExists(promptChainPath)).toBe(true);
