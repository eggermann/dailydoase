...
const PROMPT_STATIC_PREFIXES = [
  'Keep continuity with the same actor',
  'Keep continuity with the same actors',
  'Keep continuity with the same setting',
  'Keep continuity with the same visible subject and setting',
  'Keep the same base location:',
  'Keep the same established actor identity:',
  'Keep the same established actor identities:',
  'Keep the same base shot setup:',
  'Preserve this continuity:',
  'Keep the same location:',
  'Keep the same actor already established:',
  'Keep the same actors already established:',
  'Stay grounded in this visible shot description:',
  'Use this visible shot description for continuity:',
  'Keep the same visible subject and setting as',
  'Keep the same visible subject and setting.',
  'Keep the visible start frame grounded in',
  'Arrive in the same location family, still reading as',
  'End in the same location family, still reading as',
  'End with the same actor still reading as',
  'End with the same actors still reading as',
  'End on the same subject and setting, still reading as',
  'Arrive in a scene that clearly reads as',
  'By the end, the same actor',
  'By the end, the same actors',
  'By the end, the same setting',
  'By the end, the same subject and setting',
];
const PROMPT_BEAT_PREFIX = 'Follow this planned beat inside the shot:';
const PROMPT_SCENE_SETUP_PREFIXES = [
  'Keep these visible anchors, props, and secondary figures readable when they are already in frame:',
  'If already visible, keep these anchors readable:',
  'Let the shot travel toward this destination setup:',
  'Drive the shot toward:',
  'Let the held shot emphasize this readable setup:',
  'Let this held shot settle into:',
];
const PROMPT_RULE_PREFIX = 'Only describe believable motion, expression changes, and camera movement inside this shot.';

const startsWithAny = (value, prefixes = []) => prefixes.some((prefix) => value.startsWith(prefix));

const colorizePromptText = (value, maxLength = 1000) => {
  const text = compactLogText(value, maxLength);
  if (!text) {
    return '';
  }
  ...
  return sentences.map((sentence) => {
    if (startsWithAny(sentence, PROMPT_STATIC_PREFIXES)) {
      mode = 'static';
      return colorText(sentence, 'cyan');
    }

    if (sentence.startsWith(PROMPT_BEAT_PREFIX)) {
      mode = 'beat';
      return colorText(sentence, 'magenta');
    }

    if (startsWithAny(sentence, PROMPT_SCENE_SETUP_PREFIXES)) {
      mode = 'beat';
      return colorText(sentence, 'magenta');
    }

    if (sentence.startsWith(PROMPT_RULE_PREFIX)) {
      mode = 'dynamic';
      return colorText(sentence, 'yellow');
    }
    ...
    if (mode === 'dynamic') {
      return colorText(sentence, 'green');
    }
...
const formatPromptLogField = (prompt, maxLength = 1000) => {
  ...
  return `  ${colorText(String('prompt').padEnd(13), 'dim')}${colorizePromptText(prompt, maxLength)}`;
};
...
test('camera prompt chain writes inspectable mock video prompts', async () => {
  ...
  logSection('vision story context', compactLogText(visionStoryContext, 360), 'green');
  ...
  const expectedFirstHeldDuration = PROMPT_CHAIN_SCENE_REQUEST.requestedSceneLengths[0];
  const expectedSecondTransitionDuration = PROMPT_CHAIN_SCENE_REQUEST.requestedSceneLengths[1];
  expect(promptChain[0].prompt).toContain('Keep continuity with the same actor');
  expect(promptChain[0].prompt).toContain(`Fit the action to a ${expectedFirstHeldDuration}-second held shot`);
  expect(promptChain[0].prompt).toContain('Only describe believable motion');
  expect(promptChain[1].prompt).toContain(`Fit the action to a ${expectedSecondTransitionDuration}-second transition`);
  expect(await fs.pathExists(promptChainPath)).toBe(true);
