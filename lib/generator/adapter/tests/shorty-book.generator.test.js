...
describe('Generator.continueSceneLoop', () => {
  ...
  test('uses a cast-context image as a First/Last destination when GPT plans First/Last', async () => {
    tempDir = await createTempDir();
    const promptDir = path.join(tempDir, 'parts', 'scene-prompts');
    await fs.ensureDir(promptDir);
    const previousFrame = {
  ...
  test('does not let a cast-context image override a GPT-planned single-image scene', async () => {
    tempDir = await createTempDir();
    const promptDir = path.join(tempDir, 'parts', 'scene-prompts');
    await fs.ensureDir(promptDir);
    const previousFrame = {
    ...
    const context = {
      ...
      generateSceneClip: jest.fn().mockResolvedValue({
        videoData: { file: path.join(tempDir, 'scene-02.mp4') },
        generatedPrompt: 'hold the room while the visitor approaches',
        mergedConfig: {},
      }),
      finalizeSceneLoopResult: jest.fn().mockResolvedValue({ file: path.join(tempDir, 'scene-loop.mp4') }),
      resolveChainedStartFrameFromLastEnd: jest.fn().mockImplementation(function keepLastFrame() {
    ...
    await Generator.prototype.continueSceneLoop.call(context, {
      streams: [],
      options: {},
      fileName: 'cast-single-image',
      sceneLoop: {
        captureLastFrame: false,
        scenePlan: [
          { title: 'Previous shot', videoMode: 'singleImage', frameSource: 'lastFrame' },
          { title: 'Hold with visitor memory', videoMode: 'singleImage', frameSource: 'lastFrame' },
        ],
      },
      clipCount: 2,
      imageOptions: {},
...
describe('Generator.generateSceneClip', () => {
  test('adds the current camera-person anchor to a single-image video prompt', () => {
    const context = {
      config: {},
  ...
  test('keeps the first-last video prompt free of the redundant camera-person anchor', () => {
    const context = {
      config: {},
      addStaticPrompt: (prompt) => prompt,
      getOpeningImageConfig: jest.fn(() => ({
    ...
    const mergedConfig = Generator.prototype.buildVideoConfig.call(
      context,
      'Fixed CCTV view. The visitor crosses the aisle without stopping.',
      { model: {} },
      { json: { metadata: { prompt: '' } } },
      {},
      { useSingleImage: false }
    ...
    expect(mergedConfig.prompt).toContain('Photoreal continuous CCTV observation of the source frame');
    expect(mergedConfig.prompt).toContain('Fixed CCTV view. The visitor crosses the aisle without stopping.');
    expect(mergedConfig.prompt).not.toContain('saved webcam anchor');
  });
