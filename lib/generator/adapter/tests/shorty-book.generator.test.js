...
test('compactFluxPromptForProvider preserves the semantic chain below provider limit', () => {
  const scenePlanEntry = {
    ...
    consequence: 'A longer path of extinguished lamps remains.',
  ...
  expect(compacted).toContain(scenePlanEntry.stillPrompt);
  expect(compacted).toContain(scenePlanEntry.consequence);
  expect(compacted).not.toContain('semanticCollisionPhysicalization');
});

test('compactFluxPromptForProvider preserves Semantic Stream terms during compaction', () => {
  const compacted = compactFluxPromptForProvider({
    prompt: 'Duplicated production prose. '.repeat(300),
    scenePlanEntry: {
      ...
      sceneFocus: 'objects',
      stillPrompt: 'The Rolltreppe steps fold into a moving wall.',
    },
  });

  expect(compacted).toContain('Rolltreppe steps fold into a moving wall');
});

test('compactFluxPromptForProvider keeps source Semantic Stream terms in semantic prose', () => {
  const compacted = compactFluxPromptForProvider({
    prompt: 'Duplicated production prose. '.repeat(300),
    scenePlanEntry: {
      semanticAnchor: 'Kaufhaus',
      ...
      sceneFocus: 'objects',
      consequence: 'Betriebsform bends the Kaufhaus ducts.',
      stillPrompt: 'Kaufhaus lamps fail under the Betriebsform.',
    },
  });

  expect(compacted).toContain('Betriebsform bends the Kaufhaus ducts');
});

test('compactFluxPromptForProvider preserves a provider-ready Semantic Stream prompt', () => {
  const compacted = compactFluxPromptForProvider({
  ...
  expect(compacted).toBe('The Kaufhaus lamps collapse under the Betriebsform.');
});
...
describe('Generator.resolveStartFrame', () => {
  ...
  test('generates a FLUX-Kontext opening frame from the camera shot when the mode is active', async () => {
    ...
    const context = {
      ...
      resolveMonsterContinuityReference: jest.fn().mockResolvedValue({
        path: openingImagePath,
        source: 'testCompleteMonsterScene',
      }),
    ...
    const startFrame = await Generator.prototype.resolveStartFrame.call(
      ...
      {},
      {
        ...
        scenePlanEntry: { sceneFocus: 'monster' },
        interval: 3,
        storyRunIndex: 3,
        model: {
  ...
  test('generates a FLUX-Kontext opening frame from the first scene context image with the protagonist as reference', async () => {
    ...
    const completeMonsterScenePath = path.join(tempDir, 'complete-monster-scene.jpg');
    await fs.writeFile(protagonistPath, 'protagonist-reference');
    await fs.writeFile(sceneContextPath, 'scene-context');
    await fs.writeFile(completeMonsterScenePath, 'complete-monster-scene');
    ...
    expect(openingFluxContextModel.prompt).toHaveBeenCalledWith(prompt, expect.objectContaining({
      ...
      images: [{ path: completeMonsterScenePath }],
...
describe('Generator.continueSceneLoop', () => {
  ...
  test('reseeds the original monster when a monster scene follows a free scene', async () => {
    tempDir = await createTempDir();
    const promptDir = path.join(tempDir, 'parts', 'scene-prompts');
    await fs.ensureDir(promptDir);
    const monsterSeedPath = path.join(tempDir, 'original-monster-seed.png');
    await fs.writeFile(monsterSeedPath, 'original-monster-seed');

    const context = {
      imageDir: tempDir,
      getAsyncPersonaReferenceConfig: jest.fn().mockReturnValue({}),
      scheduleAsyncPersonaReferenceCapture: jest.fn().mockResolvedValue(''),
      generateSceneContextFluxFrame: jest.fn().mockResolvedValue({
        image: { path: monsterSeedPath },
        json: { metadata: { monsterReferenceIncluded: true } },
      }),
      generateSceneClip: jest.fn().mockResolvedValue({
        videoData: { file: path.join(tempDir, 'scene-02.mp4') },
        generatedPrompt: 'The original monster enters the Kaufhaus.',
        mergedConfig: {},
      }),
      finalizeSceneLoopResult: jest.fn().mockResolvedValue({
        file: path.join(tempDir, 'scene-loop.mp4'),
    ...
    const scenePlan = [
      { title: 'Empty aisle', sceneFocus: 'location', monsterPresence: 'absent' },
      {
        title: 'Monster return',
        sceneFocus: 'monster',
        monsterPresence: 'visible at the elevator',
    ...
    await Generator.prototype.continueSceneLoop.call(context, {
      streams: [],
      options: {},
      fileName: 'monster-return',
      sceneLoop: {
        scenePlan,
        startFrameStrategy: { enabled: true },
        subsequentClipsUseSingleImage: true,
        captureLastFrame: false,
      },
      clipCount: 2,
      ...
      clipResults: [],
      startFrame: {
        image: { path: path.join(tempDir, 'free-scene-last-frame.png') },
        json: { metadata: { prompt: 'empty Kaufhaus' } },
      },
      startIndex: 1,
      loopStartsFromLastFrame: false,
    });

    expect(context.generateSceneContextFluxFrame).toHaveBeenCalledWith(expect.objectContaining({
      scenePlanEntry: scenePlan[1],
      sceneContext: expect.objectContaining({ index: 2 }),
    }));
    expect(context.generateSceneClip).toHaveBeenCalledWith(expect.objectContaining({
      startFrame: expect.objectContaining({ image: { path: monsterSeedPath } }),
    }));
  });
});

describe('Generator.generateSceneClip', () => {
  test('quantizes active Runware Wan 2.6 Flash only at request submission', async () => {
    const sceneContext = {
      index: 1,
      total: 2,
      durationSeconds: 2.6,
      curvedDurationSeconds: 2.1,
    };
    const runwareVideoType = {
      config: {
        type: 'runwareImageToVideo',
        model: 'alibaba:wan@2.6-flash',
      },
      prompt: jest.fn().mockResolvedValue({ file: '/tmp/runware-wan.mp4' }),
    };
    const context = {
      prepareVideoGeneration: jest.fn().mockResolvedValue({
        videoType: runwareVideoType,
        videoTypeCandidates: [],
        videoModel: {},
        generatedPrompt: 'Move existing objects only.',
      }),
      ...
      generateVideoData: Generator.prototype.generateVideoData,
    ...
    await Generator.prototype.generateSceneClip.call(context, {
      ...
      streams: [],
      options: { sceneContext, mireloAI: {} },
      fileName: 'scene-01',
      useSingleImage: true,
      imageOptions: {},
    });

    expect(runwareVideoType.prompt).toHaveBeenCalledWith(
      '/tmp/start-frame.png',
      expect.objectContaining({ duration_seconds: 3 })
    );
    expect(sceneContext).toMatchObject({
      preQuantizedDurationSeconds: 2.6,
      providerDurationSeconds: 3,
      durationSeconds: 3,
      providerDurationRule: 'runware-wan-2.6-flash-integer-2-15',
    });
  ...
  test('saves every scene from one run in the shared image-only folder', async () => {
    ...
    const scenePlan = [
      ...
      },
      {
        ...
        sceneFocus: 'monster',
        monsterPresence: 'visible near the escalator',
    ...
    expect(summary.scenes[0]).toMatchObject({
      sceneFocus: 'location',
      event: scenePlan[0].storyBeat,
      stillPrompt: scenePlan[0].stillPrompt,
      generatedImagePath: path.join(outputDir, 'run-02-scene-01.png'),
    });
    ...
    expect(summary.scenes[1].generatedImagePath).toBe(path.join(outputDir, 'run-02-scene-02.png'));
...
describe('Generator.buildSceneContextFluxPrompt', () => {
  test('uses environment mode when scene focus is objects', () => {
    const context = {
    ...
    expect(prompt).toContain('The escalator steps fold into a moving wall.');
    expect(prompt).not.toContain('Do not show the Green Monster');
    expect(prompt).not.toContain('fresh scene-specific incarnation');
  });

  test('removes a prior monster from a chained trace scene', () => {
    const context = {
      getOpeningImageConfig: () => ({
    ...
    const prompt = Generator.prototype.buildSceneContextFluxPrompt.call(context, {
      scenePlanEntry: {
        ...
        sceneFocus: 'trace',
        stillPrompt: 'Fluorescent light bends into rigid shards after an unseen event.',
      },
      sceneContext: { index: 2, total: 6 },
      contextImage: { path: '/tmp/location-elevators.jpeg' },
    });

    expect(prompt).toContain('Compose the new scene from its planned subjects and event.');
    expect(prompt).not.toContain('monster-shaped shadow');
    expect(prompt).toContain('Preserve environmental continuity');
  });

  test('uses creature identity only for monster-focused scenes', () => {
    const context = {
      getOpeningImageConfig: () => ({}),
      getSceneContextImageConfig: () => ({
        allowPeople: false,
    ...
    expect(prompt).toContain('One Green Monster fractures nearby lamps.');
    expect(prompt).not.toContain('at least eighty percent');
    expect(prompt).not.toContain('fresh scene-specific incarnation');
  });
});

describe('Generator.generateSceneContextFluxFrame focus routing', () => {
  test('omits the monster reference for a location scene and uses a complete monster scene for a monster scene', async () => {
    const localTempDir = await createTempDir();
    const contextPath = path.join(localTempDir, 'kaufhaus.jpg');
    const protagonistPath = path.join(localTempDir, 'isolated-monster.jpg');
    const completeMonsterScenePath = path.join(localTempDir, 'complete-monster-scene.jpg');
    await fs.writeFile(contextPath, 'kaufhaus');
    await fs.writeFile(protagonistPath, 'monster');
    await fs.writeFile(completeMonsterScenePath, 'monster in Kaufhaus');
    const prompt = jest.fn().mockResolvedValue({ image: { path: contextPath }, json: {} });
    const context = {
      openingFluxContextModel: { prompt },
      getSceneContextImageConfig: () => ({
        enabled: true,
        images: [{ path: contextPath }],
        model: {},
        protagonistAlreadyComposited: false,
      ...
      getOpeningImageConfig: () => ({
        referenceImagePath: protagonistPath,
        monsterContinuityAnchorPath: completeMonsterScenePath,
      }),
      resolveMonsterContinuityReference: Generator.prototype.resolveMonsterContinuityReference,
      buildPersonaReferenceIdentityClause: () => 'Keep the same monster identity.',
      buildSceneContextFluxPrompt: Generator.prototype.buildSceneContextFluxPrompt,
      buildSceneContextSemanticReconstructionPrompt: Generator.prototype.buildSceneContextSemanticReconstructionPrompt,
    };

    try {
      await Generator.prototype.generateSceneContextFluxFrame.call(context, {
        scenePlanEntry: {
          sceneFocus: 'location',
          stillPrompt: 'The elevator doors open by themselves.',
        },
        sceneContext: { index: 1 },
      });
      await Generator.prototype.generateSceneContextFluxFrame.call(context, {
        scenePlanEntry: {
          sceneFocus: 'monster',
          stillPrompt: 'The Green Monster waits beside the elevator.',
          monsterPresence: 'visible beside the elevator',
        },
        sceneContext: { index: 1 },
      });
      context.lastMonsterFrame = { image: { path: contextPath } };
      await Generator.prototype.generateSceneContextFluxFrame.call(context, {
        scenePlanEntry: {
          sceneFocus: 'mixed',
          stillPrompt: 'The same monster crosses the Kaufhaus aisle.',
          monsterPresence: 'visible in the aisle',
        },
        sceneContext: { index: 1 },
      });

      expect(prompt).toHaveBeenNthCalledWith(1, expect.any(String), expect.objectContaining({
        images: [],
        contextReference: undefined,
      }));
      expect(prompt).toHaveBeenNthCalledWith(2, expect.any(String), expect.objectContaining({
        images: [{ path: protagonistPath }],
        contextReference: { enabled: true },
      }));
      expect(prompt).toHaveBeenNthCalledWith(3, expect.any(String), expect.objectContaining({
        images: [{ path: protagonistPath }],
        contextReference: { enabled: true },
      }));
    } finally {
...
describe('Generator.buildSceneContextSemanticReconstructionPrompt', () => {
  test('keeps reconstruction monster-free for a trace scene', () => {
    const prompt = Generator.prototype.buildSceneContextSemanticReconstructionPrompt.call({
      buildPersonaReferenceIdentityClause: () => '',
    }, {
      scenePlanEntry: {
        sceneFocus: 'trace',
        stillPrompt: 'Three attached lamps shatter into rigid shards after an unseen event.',
      },
      sceneContext: { index: 1, total: 6 },
    });

    expect(prompt).not.toContain('Do not show or introduce the monster');
    expect(prompt).toContain('Three attached lamps shatter into rigid shards');
  });
});
...
describe('Generator.shouldApplyDriftCorrection', () => {
  ...
  test('runs image-to-image repair only for the planned drift strategy', async () => {
    await expect(decide('driftCorrectedLastFrame')).resolves.toBe(true);
    await expect(decide('rawLastFrame')).resolves.toBe(false);
    await expect(decide('locationReanchor')).resolves.toBe(false);
  });
});
...
describe('Generator.resolveChainedStartFrameFromLastEnd', () => {
  ...
  test('submits an 8-frame FIFO screenshot buffer as context reference for kontext drift correction', async () => {
    ...
    const expectedSecondaryRefs = [...new Set([
      screenshotPaths[7],
      screenshotPaths[8],
      ...expectedBuffer,
    ])];
