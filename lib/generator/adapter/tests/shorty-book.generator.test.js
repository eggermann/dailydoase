...
import { createVisitorMemory, rememberVisitor } from '../shorty-book/visitor-memory.js';

const createTempDir = async () => fs.mkdtemp(path.join(os.tmpdir(), 'dailydoase-shorty-book-'));

describe('isLikelyNewVisitorReference', () => {
  test('ignores wording and position changes around the same visible identity', () => {
    expect(isLikelyNewVisitorReference({
      previousDescription: 'Bald person with glasses, short beard, black sleeveless shirt, foreground left.',
      nextDescription: 'A bald visitor wearing glasses and a black sleeveless shirt with short beard at frame right.',
      nextStrength: 93,
    })).toBe(false);
  });

  test('detects a materially different real visitor description', () => {
    expect(isLikelyNewVisitorReference({
      previousDescription: 'Bald person with glasses, short beard, black sleeveless shirt.',
      nextDescription: 'Young visitor with long red hair, green coat and yellow shoulder bag.',
      nextStrength: 91,
    })).toBe(true);
  });

  test('does not invent a visitor when local vision found no person', () => {
    expect(isLikelyNewVisitorReference({
      previousDescription: 'Bald person with glasses and beard.',
      nextDescription: 'Empty exhibition room.',
      nextStrength: 0,
    })).toBe(false);
  });
});

describe('Lost Audience visitor memory', () => {
  test('recognizes a returning visitor after another visitor was seen', () => {
    let memory = createVisitorMemory({ capacity: 3 });
    const first = rememberVisitor(memory, {
      description: 'Bald visitor with glasses, short beard, black sleeveless shirt.',
      strength: 93,
      referencePath: '/tmp/visitor-one.jpg',
      sceneIndex: 1,
    });
    memory = first.memory;
    const second = rememberVisitor(memory, {
      description: 'Young visitor with long red hair, green coat and yellow shoulder bag.',
      strength: 91,
      referencePath: '/tmp/visitor-two.jpg',
      sceneIndex: 2,
    });
    const returnEvent = rememberVisitor(second.memory, {
      description: 'Bald person with glasses and a short beard in a black sleeveless shirt.',
      strength: 95,
      referencePath: '/tmp/visitor-one-return.jpg',
      sceneIndex: 5,
    });

    expect(returnEvent.event).toMatchObject({
      type: 'returning',
      interaction: 'shared-attention',
      visitor: {
        id: first.event.visitor.id,
        sightings: 2,
        lastSeenScene: 5,
        referencePaths: ['/tmp/visitor-one.jpg', '/tmp/visitor-one-return.jpg'],
      },
    });
    expect(returnEvent.memory.visitors).toHaveLength(2);
  });
});
...
describe('Generator.generateImage', () => {
  ...
  test('creates a First/Last destination from a real new visitor while preserving the previous last frame as start', async () => {
    tempDir = await createTempDir();
    const previousLastFramePath = path.join(tempDir, 'previous-last-frame.png');
    const cameraVisitorPath = path.join(tempDir, 'fresh-real-camera-visitor.jpg');
    const castMemoryPath = path.join(tempDir, 'old-cast-memory.jpg');
    const destinationPath = path.join(tempDir, 'new-visitor-destination.png');
    await Promise.all([
      fs.writeFile(previousLastFramePath, 'previous-last-frame'),
      fs.writeFile(cameraVisitorPath, 'real-camera-visitor'),
      fs.writeFile(castMemoryPath, 'old-cast-memory'),
    ]);
    const castContextImageModel = {
      prompt: jest.fn().mockResolvedValue({
    ...
    const context = {
      ...
      castContextImageModel,
      driftCorrectionModel: null,
      personaReferenceImageModel: null,
      pendingPersonaReferencePath: cameraVisitorPath,
      pendingPersonaReferenceIsNew: true,
      getCastContextConfig: Generator.prototype.getCastContextConfig,
    };

    const target = await Generator.prototype.generateNewVisitorTransitionTarget.call(context, {
      startFrame: { image: { path: previousLastFramePath } },
      scenePlanEntry: {
        videoMode: 'firstLast',
        stillPrompt: 'The real visitor enters at the edge of the continuing room event.',
        castReferences: [{ referenceImage: castMemoryPath }],
      },
      sceneContext: { index: 3 },
      imageOptions: { width: 1184, height: 880 },
    });
    ...
    expect(target).toMatchObject({
      ...
      json: {
        metadata: {
          sourceLastFramePath: previousLastFramePath,
          newVisitorReferencePath: cameraVisitorPath,
          newVisitorTransitionTarget: true,
        },
      },
    });
    expect(castContextImageModel.prompt).toHaveBeenCalledWith(
      expect.stringContaining('fresh live camera reference contains a newly detected real visitor'),
      expect.objectContaining({
        imagePath: previousLastFramePath,
        images: [{ path: cameraVisitorPath }, { path: castMemoryPath }],
        priorityReferencePath: cameraVisitorPath,
        frameRole: 'new-visitor-first-last-destination',
      })
    );
    expect(context.pendingPersonaReferencePath).toBe('');
    expect(context.pendingPersonaReferenceIsNew).toBe(false);
  });
...
describe('Generator.continueSceneLoop', () => {
  ...
  test('keeps the previous last frame as First/Last start when a real new visitor supplies the destination', async () => {
    tempDir = await createTempDir();
    const promptDir = path.join(tempDir, 'parts', 'scene-prompts');
    await fs.ensureDir(promptDir);
    const previousFrame = {
      image: { path: path.join(tempDir, 'previous-last-frame.png') },
      json: { metadata: { prompt: 'previous moving room frame' } },
    };
    const visitorDestination = {
      image: { path: path.join(tempDir, 'real-visitor-destination.png') },
      json: { metadata: { prompt: 'new real visitor enters the continuing room' } },
    };
    const context = {
      imageDir: tempDir,
      continueSceneLoop: Generator.prototype.continueSceneLoop,
      generateNewVisitorTransitionTarget: jest.fn().mockResolvedValue(visitorDestination),
      generateCastContextFrame: jest.fn(),
      generateSceneClip: jest.fn().mockResolvedValue({
        videoData: { file: path.join(tempDir, 'scene-02.mp4') },
        generatedPrompt: 'continue forward motion into the visitor destination',
        mergedConfig: {},
      }),
      finalizeSceneLoopResult: jest.fn().mockResolvedValue({ file: path.join(tempDir, 'scene-loop.mp4') }),
      resolveChainedStartFrameFromLastEnd: jest.fn().mockImplementation(function keepLastFrame() {
    ...
    await Generator.prototype.continueSceneLoop.call(context, {
      streams: [],
      options: {},
      fileName: 'new-real-visitor-transition',
      sceneLoop: {
        captureLastFrame: false,
        scenePlan: [
          { title: 'Previous shot', videoMode: 'singleImage', frameSource: 'lastFrame' },
          { title: 'Visitor enters', videoMode: 'firstLast', frameSource: 'lastFrame' },
        ],
      },
      clipCount: 2,
      imageOptions: {},
    ...
    expect(context.generateSceneClip).toHaveBeenCalledWith(expect.objectContaining({
      ...
      useSingleImage: false,
      endFrameOverride: {
        path: visitorDestination.image.path,
        prompt: 'new real visitor enters the continuing room',
      },
    }));
    ...
    expect(decision.applied).toMatchObject({
      ...
      endFramePath: visitorDestination.image.path,
      castContext: 'generated as First/Last destination from fresh real visitor reference',
    });
  });
  ...
  test('waits for the camera-person refresh before rendering the next scene', async () => {
    ...
    for (let attempt = 0; attempt < 40 && generatedSceneIndexes.length === 0; attempt += 1) {
...
describe('Generator.resolveChainedStartFrameFromLastEnd', () => {
  ...
  test('builds a gentler prompt for moderate drift handling', () => {
    ...
    expect(prompt).toContain('Do not preserve an obviously illustrated, decal-like, opaque splash graphic');
    expect(prompt).toContain('Repair such local artifacts as physically credible real material');
    expect(prompt).toContain('underlying surface texture');
    expect(prompt.length).toBeLessThanOrEqual(2900);
  });

  test('keeps Runware drift prompts below the provider limit with long scene history', () => {
    const longText = 'A long generated surveillance description with room geometry and visitor movement. '.repeat(100);
    const context = {
      config: {
        sceneLoop: {
          openingImage: {
            continuityAnchor: longText,
          },
        },
        driftCorrection: { level: 'moderate' },
      },
      getOpeningImageConfig: Generator.prototype.getOpeningImageConfig,
      getDriftCorrectionConfig: Generator.prototype.getDriftCorrectionConfig,
    };
    ...
    const prompt = Generator.prototype.buildDriftCorrectionPrompt.call(context, {
      lastFrame: { json: { metadata: { prompt: longText } } },
      referenceImage: { sourceType: 'cameraShot' },
      nextScenePlanEntry: { title: 'Next scene', storyBeat: longText },
      nextSceneContext: { index: 2 },
    });

    expect(prompt.length).toBeLessThanOrEqual(2900);
    expect(prompt).toContain('fresh live camera reference');
    expect(prompt).toContain('Prepare the next scene title: Next scene.');
  });
