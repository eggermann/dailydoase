...
describe('resolveRoomEvolutionPhase', () => {
  test('develops the room for three steps, then re-anchors with a residue', () => {
    expect(resolveRoomEvolutionPhase({ previousStep: 0, reanchorEvery: 4 }))
      .toEqual({ step: 1, every: 4, reanchor: false });
    expect(resolveRoomEvolutionPhase({ previousStep: 3, reanchorEvery: 4 }))
      .toEqual({ step: 4, every: 4, reanchor: true });
    expect(resolveRoomEvolutionPhase({ previousStep: 4, reanchorEvery: 4 }))
      .toEqual({ step: 1, every: 4, reanchor: false });
  });
});

describe('Generator Gelbnasen continuity', () => {
  let tempDir = '';
  ...
  test('continues the yellow trace after three story continuations without a new visitor', async () => {
    tempDir = await createTempDir();
    const falseVisitorReferencePath = path.join(tempDir, 'gelbnase-reference.png');
    await fs.writeFile(falseVisitorReferencePath, 'yellow trace');
    const context = {
      config: {
        driftCorrection: {
          falseVisitorReference: {
            afterContinuations: 3,
            continuationInterval: 3,
            imagePath: falseVisitorReferencePath,
          },
        },
      },
      gelbnasenContinuationCount: 2,
      gelbnasenTraceStage: 0,
      pendingPersonaReferencePath: '',
      getDriftCorrectionConfig: Generator.prototype.getDriftCorrectionConfig,
    };

    await expect(
      Generator.prototype.resolveDriftCorrectionReferenceImage.call(context, {
        lastFrame: { image: { path: path.join(tempDir, 'last-frame.png') } },
      })
    ).resolves.toEqual({
      path: falseVisitorReferencePath,
      prompt: 'a small yellow-hat, onion-nose visitor trace enters the surveillance memory',
      sourceType: 'falseVisitorReference',
    });
    expect(context.gelbnasenContinuationCount).toBe(3);
    expect(context.gelbnasenTraceStage).toBe(1);
  });
});
...
describe('Generator.continueSceneLoop', () => {
  ...
  test('uses a cast-context image as the one anchor for a planned single-image scene', async () => {
    tempDir = await createTempDir();
    const promptDir = path.join(tempDir, 'parts', 'scene-prompts');
    await fs.ensureDir(promptDir);
    const previousFrame = {
    ...
    await Generator.prototype.continueSceneLoop.call(context, {
      ...
      sceneLoop: {
        ...
        scenePlan: [
          ...
          { title: 'Return of the visitor', videoMode: 'singleImage', frameSource: 'lastFrame' },
        ],
      },
      clipCount: 2,
      imageOptions: {},
  ...
  test('records a failed scene and finalizes the remaining loop', async () => {
    tempDir = await createTempDir();
    const promptDir = path.join(tempDir, 'parts', 'scene-prompts');
    await fs.ensureDir(promptDir);
    const startFrame = {
    ...
    const context = {
      imageDir: tempDir,
      continueSceneLoop: Generator.prototype.continueSceneLoop,
      generateSceneClip: jest.fn().mockRejectedValue(new Error('video provider unavailable')),
      finalizeSceneLoopResult: jest.fn().mockResolvedValue({ file: path.join(tempDir, 'scene-loop.mp4') }),
    };
    ...
    await Generator.prototype.continueSceneLoop.call(context, {
      streams: [],
      options: {},
      fileName: 'failed-scene-loop',
      sceneLoop: {
        captureLastFrame: false,
        firstClipUseSingleImage: true,
        subsequentClipsUseSingleImage: true,
        scenePlan: [
          { title: 'Unavailable opening', videoMode: 'singleImage', frameSource: 'lastFrame' },
        ],
      },
      clipCount: 1,
      imageOptions: {},
    ...
    await expect(fs.readJSON(path.join(promptDir, 'failed-scene-loop-scene-01-scene-failure.json')))
      .resolves.toMatchObject({
        sceneIndex: 1,
        sceneTitle: 'Unavailable opening',
        failure: 'video provider unavailable',
        action: 'retry-once-then-skip',
      });
    expect(context.finalizeSceneLoopResult).toHaveBeenCalledWith(expect.objectContaining({ clipResults: [] }));
  });
...
describe('Generator.resolveChainedStartFrameFromLastEnd', () => {
  ...
  test('uses the corrected image as the next start frame when drift correction is enabled', async () => {
    ...
    const context = {
      ...
      buildDriftCorrectionPrompt: Generator.prototype.buildDriftCorrectionPrompt,
      buildDriftReferenceRolesPrompt: Generator.prototype.buildDriftReferenceRolesPrompt,
  ...
  test('can use a fresh camera shot as the FLUX Kontext reference image', async () => {
    ...
    expect(context.driftCorrectionModel.prompt).toHaveBeenCalledWith(
      expect.stringContaining('A fresh live camera reference may contain newly arrived visitors.'),
      expect.objectContaining({
  ...
  test('keeps the exact video last frame when cast context will become the first-last target', async () => {
    const lastFrame = {
      image: { path: '/tmp/raw-last-frame.png' },
      json: { metadata: { prompt: 'previous shot prompt' } },
    };
  ...
  test('keeps drift correction to ten total images and retains the newest distinct context frames', async () => {
    const tempDir = await createTempDir();
    ...
    const context = {
      ...
      config: {
        driftCorrection: {
          ...
          referenceImage: {
            captureFn: jest
              .fn()
              .mockResolvedValueOnce(screenshotPaths[10])
              .mockResolvedValueOnce(screenshotPaths[11]),
            promptSource: '',
          },
          model: {
            model: 'black-forest-labs/FLUX.1-Kontext-dev',
      ...
      contextScreenshotBuffer: screenshotPaths.slice(0, 10),
      driftCorrectionModel: {
        prompt: jest.fn().mockResolvedValue({
    ...
    const expectedBuffer = [...screenshotPaths.slice(1, 10), screenshotPaths[11]];
    const expectedSecondaryRefs = [
      screenshotPaths[10],
      screenshotPaths[7],
      ...expectedBuffer
        .filter((entryPath) => entryPath !== screenshotPaths[7])
        .slice(-7),
    ];
    expect(context.contextScreenshotBuffer).toEqual(expectedBuffer);
    const sentPrompt = context.driftCorrectionModel.prompt.mock.calls[0][0];
    expect(sentPrompt).toContain('Canonical opening continuity: Location: white wall room. Actor: bald man in dark jacket.');
    expect(sentPrompt).toContain('Reference image 1 is the current generated last frame');
    expect(sentPrompt).toContain('Reference image 2 is the freshest real camera capture');
    expect(sentPrompt).toContain('Reference image 3 is the canonical opening room reference');
    expect(context.driftCorrectionModel.prompt).toHaveBeenCalledWith(
      expect.stringContaining('Reference image 1 is the current generated last frame'),
      expect.objectContaining({
