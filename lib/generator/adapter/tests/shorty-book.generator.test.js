...
test('compactFluxPromptForProvider uses English terms instead of source-language words', () => {
  ...
  expect(compacted).toContain('escalator steps fold into a moving wall');
  expect(compacted).not.toContain('Kaufhaus');
  expect(compacted).not.toContain('Rolltreppe');
});
...
describe('Generator.buildSceneContextFluxPrompt', () => {
  test('uses environment mode when scene focus is objects', () => {
    ...
    expect(prompt).toContain('Do not show the Green Monster');
  ...
  test('removes a prior monster from a chained trace scene', () => {
    ...
    expect(prompt).toContain('Remove or move the monster completely outside the visible frame.');
    expect(prompt).toContain('Do not leave a monster-shaped shadow');
  ...
  test('uses creature identity only for monster-focused scenes', () => {
    ...
    expect(prompt).toContain('The separate protagonist image is mandatory and is the canonical identity.');
    expect(prompt).toContain('Generate the exact same individual Green Monster');
...
describe('Generator.buildSceneContextSemanticReconstructionPrompt', () => {
  test('keeps reconstruction monster-free for a trace scene', () => {
    ...
    expect(prompt).toContain('Do not show or introduce the monster');
