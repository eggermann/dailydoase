...
import { buildVideoConfig } from '../shorty-book/video-utils.js';
...
describe('buildVideoConfig', () => {
  test('puts a real camera person ahead of fictional story visitor details in video prompts', () => {
    const cameraPersonOverride = 'The real person in the saved webcam anchor has priority over story character details.';
    const config = buildVideoConfig({
      config: {},
      addStaticPrompt: (prompt) => prompt,
      generatedPrompt: 'A blonde visitor enters the gallery.',
      videoModel: { model: {}, staticPrompt: '' },
      startFrame: { json: { metadata: { prompt: 'camera frame' } } },
      options: { personaReferenceIdentityClause: cameraPersonOverride },
    });

    expect(config.prompt).toBe(`${cameraPersonOverride} A blonde visitor enters the gallery.`);
  });
});
...
describe('Generator.resolveChainedStartFrameFromLastEnd', () => {
  ...
  test('can use a fresh camera shot as the FLUX Kontext reference image', async () => {
    ...
    expect(context.driftCorrectionModel.prompt.mock.calls[0][1].contextReference).toBeUndefined();
    expect(startFrame).toMatchObject({
      image: { path: correctedImagePath },
      json: {
        metadata: {
  ...
  test('submits an 8-frame FIFO screenshot buffer as context reference for kontext drift correction', async () => {
    ...
    const expectedSecondaryRefs = [
      screenshotPaths[8],
      screenshotPaths[7],
      ...expectedBuffer.filter((entryPath) => ![
        screenshotPaths[8],
        screenshotPaths[7],
      ].includes(entryPath)),
    ];
    expect(expectedSecondaryRefs).toHaveLength(9);
