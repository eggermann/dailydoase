...
const sanitize = (value, key = '') => {
  if (typeof value === 'string' && /^data:(image|video|audio)\//i.test(value)) {
    const mediaType = value.slice(5, value.indexOf(';') > 5 ? value.indexOf(';') : 40);
    return `[${mediaType} data URI, ${value.length} chars]`;
  }
