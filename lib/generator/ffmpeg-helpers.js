...
export async function concatMp4Lossless(mp4Paths, outMp4, workDir, {
  ...
  const resolvedTrimLeadingSeconds = Math.max(0, Number(trimLeadingSeconds) || 0);
  const allStreamsMatch = streamKeys.every((value) => value === streamKeys[0]);
  if (allStreamsMatch && resolvedTrimLeadingSeconds === 0) {
    await sh('ffmpeg', ['-y', '-f', 'concat', '-safe', '0', '-i', listFile, '-c', 'copy', outMp4]);
    return outMp4;
  }

  logger.warn(
    resolvedTrimLeadingSeconds > 0
      ? `concatMp4Lossless trims ${resolvedTrimLeadingSeconds}s from later clip starts and re-times each clip.`
      : 'concatMp4Lossless detected mismatched stream parameters; falling back to re-encode concat.'
  );
  const firstStreamParts = String(streamKeys[0] || '').split('|');
  const targetWidth = Number(firstStreamParts[1]) || Math.max(...streamKeys.map((value) => Number(String(value).split('|')[1]) || 0));
  const targetHeight = Number(firstStreamParts[2]) || Math.max(...streamKeys.map((value) => Number(String(value).split('|')[2]) || 0));
  const targetFps = parseFrameRate(firstStreamParts[3]) || 24;
  const clipStreams = await Promise.all(mp4Paths.map((inputPath) => probeVideoStream(inputPath)));
  const ffmpegArgs = ['-y'];
  ...
  const filterParts = mp4Paths.map((_, index) => {
    const clipDuration = Number(clipStreams[index]?.duration) || 0;
    const leadingTrim = index > 0 && clipDuration > 0
      ? Math.min(resolvedTrimLeadingSeconds, clipDuration * 0.25)
      : 0;
    const filters = [
      ...(leadingTrim > 0
        ? [
            `trim=start=${leadingTrim.toFixed(3)}`,
            `setpts=(PTS-STARTPTS)*${(clipDuration / (clipDuration - leadingTrim)).toFixed(8)}`,
          ]
        : ['setpts=PTS-STARTPTS']),
  ...
  const totalDuration = clipStreams.reduce((sum, stream) => sum + (Number(stream?.duration) || 0), 0);
  if (resolvedTrimLeadingSeconds > 0 && totalDuration > 0) {
    const padAllowance = (resolvedTrimLeadingSeconds * Math.max(1, mp4Paths.length - 1)) + (1 / targetFps);
    const targetFrameCount = Math.max(1, Math.round(totalDuration * targetFps));
    filterParts.push(
      `${concatInputs}concat=n=${mp4Paths.length}:v=1:a=0[joined]`,
      `[joined]tpad=stop_mode=clone:stop_duration=${padAllowance.toFixed(3)},fps=${targetFps},trim=end_frame=${targetFrameCount},setpts=PTS-STARTPTS[outv]`
    );
  } else {
