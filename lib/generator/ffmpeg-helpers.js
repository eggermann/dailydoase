...
export async function concatTrailerWithCollisionCuts(mp4Paths, outMp4, {
  ...
  const collisionCutPath = globalForwardDolly
    ? path.join(path.dirname(outMp4), `${path.basename(outMp4, path.extname(outMp4))}-pre-dolly.mp4`)
    : outMp4;
  const args = ['-y'];
  for (const inputPath of mp4Paths) {
  ...
  if (globalForwardDolly) {
    // One continuous forward push gives clips made by separate WAN calls a
    // shared direction. It does not hide the collision; it makes every return
    // from that collision feel like entering deeper into the same Kaufhaus.
    await sh('ffmpeg', [
      '-y',
      '-i', collisionCutPath,
      '-vf', `zoompan=z='min(zoom+0.00045,1.08)':d=1:s=${targetWidth}x${targetHeight}:fps=${targetFps},format=yuv420p`,
      '-c:v', 'libx264',
      '-preset', 'veryfast',
      '-pix_fmt', 'yuv420p',
      '-an',
    ...
    await fs.unlink(collisionCutPath);
  }
