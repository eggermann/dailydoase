...
export async function normalizeVideoDurationSeconds(inputVideoPath, targetDurationSeconds, {
  ...
  const actual = await probeVideoDurationSeconds(inputVideoPath);
  if (Math.abs(actual - target) < 0.03) {
    return inputVideoPath;
  }

  const resolvedTargetFps = Number(targetFps) > 0 ? Number(targetFps) : 24;
  const stopDuration = Math.max(0, target - actual);
  ...
  const videoFilter = stopDuration > 0
    ? `fps=${resolvedTargetFps},tpad=stop_mode=clone:stop_duration=${stopDuration.toFixed(3)}`
