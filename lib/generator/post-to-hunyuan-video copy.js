...
class PostToHunYuanVideo extends PostTo {
  constructor(modelConfig = {}) {
    ...
    this.config.space = modelConfig.space ?? 'LPX55/HunYuan-Keyframe2VID-Control-Lora';
    ...
    this.config.fallbacks = modelConfig.fallbacks ?? [
      // Primary target
      'LPX55/HunYuan-Keyframe2VID-Control-Lora',
      // API‑enabled SVD key‑frame interpolation Spaces
      'kylin/video-keyframe-interp',
      'John6666/svd_keyframe_api'
    ];
  ...
  async init() {
    ...
    for (const space of this.config.fallbacks) {
      try {
        this._cli = await Client.connect(space, {
          hf_token: process.env.HF_TOKEN ?? process.env.HF_API_TOKEN ?? undefined
        });
        this.config.spaceActive = space;
        console.log(`🛰️  Connected to "${space}"`);
        break;
      } catch (err) {
        console.warn(`⚠️  Space "${space}" unreachable: ${err.message}`);
        console.warn('⏭️  Trying next fallback…');
      }
    }
    if (!this._cli) throw new Error('No fallback Space reachable');
    return this;
  }
