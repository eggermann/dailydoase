...
const DEFAULT_RUNWARE_MAX_RETRIES = 2;
const DEFAULT_RUNWARE_RETRY_DELAY_MS = 750;
...
async function runRunwareImageToImage({
  ...
  parameters = {},
  timeoutMs = 0,
  maxRetries = DEFAULT_RUNWARE_MAX_RETRIES,
  retryDelayMs = DEFAULT_RUNWARE_RETRY_DELAY_MS,
}) {
...
export class PostToHF_ImageActorInScene extends PostTo {
  ...
  async prompt(promptText, options = {}) {
    ...
    const runwareMaxRetries = Math.max(
      0,
      Number(options.runwareMaxRetries ?? this.config.runwareMaxRetries ?? DEFAULT_RUNWARE_MAX_RETRIES) || 0
    );
    const runwareRetryDelayMs = Math.max(
      0,
      Number(options.runwareRetryDelayMs ?? this.config.runwareRetryDelayMs ?? DEFAULT_RUNWARE_RETRY_DELAY_MS) || 0
    );
    ...
    if (useRunware) {
      imageResult = await runRunwareImageToImage({
        ...
        maxRetries: runwareMaxRetries,
        retryDelayMs: runwareRetryDelayMs,
      });
    ...
    if (imageResult.buffer) {
      ...
      await downloadToFile(imageResult.url, savePath, {
        timeoutMs: 15 * 60 * 1000,
        maxRetries: useRunware ? runwareMaxRetries : 0,
        retryDelayMs: runwareRetryDelayMs,
      });
    } else {
