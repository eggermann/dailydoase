...
import {
  imagePathToDataUrl as runwareImagePathToDataUrl,
  resolveRunwareKey,
...
export const shouldUseDirectRunwareImageEdit = ({
  model,
  provider,
  runwareKey,
} = {}) => {
  if (!runwareKey || normalizeProvider(provider)?.toLowerCase() !== 'runware') {
    return false;
  }
  return /^(?:bfl|runware):/i.test(String(model || '').trim());
};

const isRemoteImageReference = (value = '') => /^(?:https?:|data:image\/)/i.test(String(value).trim());

export const buildRunwareReferenceImages = async ({ inputPath, referencePaths = [] } = {}) => {
  const paths = [inputPath, ...referencePaths]
    .map((entry) => String(entry || '').trim())
    .filter(Boolean);
  const uniquePaths = [...new Set(paths)].slice(0, 10);

  return Promise.all(uniquePaths.map(async (entry) => (
    isRemoteImageReference(entry) ? entry : runwareImagePathToDataUrl(entry)
  )));
};

const runRunwareImageToImage = async ({
  model,
  apiKey,
  prompt,
  inputPath,
  ...
  const referenceImages = await buildRunwareReferenceImages({ inputPath, referencePaths });
  const { task, result } = await submitRunwareImageJob({
    apiKey,
    model,
    prompt,
  ...
  if (!url) {
    throw new Error('Runware image response returned no imageURL');
  }
  ...
  return {
    ...
    payload: {
      ...task,
      inputs: { referenceImages: referencePaths.slice(0, 10) },
    },
  };
};
...
export class PostToHF_ImageActorInScene extends PostTo {
  ...
  async prompt(promptText, options = {}) {
    ...
    const useDirectRunware = shouldUseDirectRunwareImageEdit({
      model: usedModel,
      provider: hfProvider,
      runwareKey,
    });
    const providerLabel = useDirectRunware ? 'Runware' : (useDirectFal ? 'FAL' : 'HF');
    ...
    const imageResult = useDirectRunware
      ? await runRunwareImageToImage({
        model: usedModel,
        apiKey: runwareKey,
        prompt: mergedPrompt,
        inputPath: imagePath,
    ...
    const json = {
      ...
      contextReferenceMode: useDirectRunware && referencePaths.length > 0
        ? 'runware-referenceImages'
        : (contextReference?.enabled ? 'promptOnly' : 'disabled'),
      parameters: {
        guidance_scale: parameters.guidance_scale ?? 2.5,
        num_inference_steps: parameters.num_inference_steps ?? 28,
        ...(parameters.seed !== undefined ? { seed: parameters.seed } : {}),
