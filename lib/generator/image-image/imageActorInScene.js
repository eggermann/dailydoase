...
import { buildContextReferenceBoard } from './context-reference-board.js';
...
export class PostToHF_ImageActorInScene extends PostTo {
  ...
  async prompt(promptText, options = {}) {
    ...
    let contextReferenceBoard = null;

    if (contextReference?.enabled) {
      contextReferenceBoard = await buildContextReferenceBoard({
        imageDir: this.imageDir,
        primaryImagePath: imagePath,
        referenceImages,
        layout: contextReference.layout,
        name: contextReference.name || 'flux-context-board',
      });
      if (contextReferenceBoard?.imagePath) {
        imagePath = contextReferenceBoard.imagePath;
      }
    }
    ...
    if (contextReferenceBoard?.imagePath) {
      console.log('[image-actor-in-scene:HF] Context board:', contextReferenceBoard.imagePath);
    }
    ...
    const json = {
      ...
      referenceImages: referenceImages.map((entry) => entry?.path || entry?.url).filter(Boolean),
      contextReferenceBoardPath: contextReferenceBoard?.imagePath || '',
      contextReferencePrimaryPath: contextReferenceBoard?.primaryImagePath || '',
      contextReferenceBufferSize: contextReferenceBoard?.referenceImagePaths?.length || 0,
      parameters: {
        guidance_scale: parameters.guidance_scale ?? 2.5,
        num_inference_steps: parameters.num_inference_steps ?? 28,
