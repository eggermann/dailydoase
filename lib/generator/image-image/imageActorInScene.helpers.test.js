...
describe('imageActorInScene fal helpers', () => {
  ...
  test('uses direct Runware for a supported image model and Runware key', () => {
    expect(shouldUseDirectRunwareImageEdit({
      model: 'bfl:6@1',
      provider: 'runware',
  ...
  test('builds a capped, deduplicated Runware multi-image reference set', async () => {
    const tempPath = path.join(os.tmpdir(), `dailydoase-runware-reference-${Date.now()}.png`);
    ...
    try {
      const references = await buildRunwareReferenceImages({
        inputPath: tempPath,
        referencePaths: [tempPath, 'https://example.com/reference.jpg'],
      });
      expect(references).toHaveLength(2);
      expect(references[0]).toMatch(/^data:image\/png;base64,/);
      expect(references[1]).toBe('https://example.com/reference.jpg');
    } finally {
