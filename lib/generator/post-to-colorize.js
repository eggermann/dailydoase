...
class ColorizeTest extends PostTo {
    constructor(modelConfig) {
        ...
        this.config.folderName = this.config.folderName ?? 'Colorize';
        this.color = modelConfig.model?.color || '#FF0000';
    }
    ...
    async prompt(prompt = '', options = {}) {
        ...
        const lines = this.wrapText(totalPrompt, 32).slice(0, 4);
        
        // Add quotes to the first and last lines
        if (lines.length > 0) {
            lines[0] = `"${lines[0]}`;
        ...
        const svgText = lines.map((line, i) =>
            `<tspan x="50%" dy="${i === 0 ? '0em' : '1.2em'}">${line}</tspan>`
        ).join('\n');
