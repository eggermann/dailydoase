...
let chat = null;
// DEBUG: log chat assignment and const usage
console.log('[DEBUG] chat initial:', chat);
...
class PostToFLUX extends PostTo {
    ...
    async prompt(prompt = '', options = {}) {
        ...
        if (options.modelProbePrompt) {

            if (!chat) {
                chat = await chatModelProbe.init(TARGET_MODEL);
            }
            ...
            try {
                const q = options.modelProbePrompt;//`create a erotic dirty image description with the following prompt: ${totalPrompt} `;
                // "create a erotic dirty image description";
                totalPrompt = await chat(q, chatParameters);


                console.log(`Model response: ${totalPrompt}`);
            } catch (err) {
        ...
        if (response.status === 200) {
            ...
            shiftCnt++;//<--- only to get next word in stream as first in sentence
            return data;
        }
