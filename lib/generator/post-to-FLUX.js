...
class PostToFLUX extends PostTo {
     ...
     */
    _setupImageDir() {
        // Optionally set baseDir here if needed
     ...
     * @returns {Promise<object>} Save result paths
     */
    async prompt(prompt = '', options = {}) {
        ...
        const mergedOptions = { ...defaultOptions, ...options, ...options.image.model };
        ...
        if (mergedOptions.num_inference_steps < minSteps) {
            console.warn(`⚠️  num_inference_steps too low (${mergedOptions.num_inference_steps}); clamping to ${minSteps}`);
        ...
        if (response.status === 200) {
            ...
            const data = {
                image: await saveJPG(this.imageDir, name, { buffer }),
                json: await saveJSON(this.imageDir, name, { prompt: totalPromptWithStatic, modelEndpoint: this.endpoint })
            }
