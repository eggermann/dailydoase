...
const logger = createLogger('flux', { envKeys: ['FLUX_DEBUG', 'GENERATOR_DEBUG'] });
const ANSI = {
    reset: '\x1b[0m',
    yellow: '\x1b[33m',
    magenta: '\x1b[35m',
...
const wrapLogText = (value, width = 92, indent = '  ') => {
...
const formatFrameRole = (value) => {
    if (value === 'start') return 'start still';
    if (value === 'end') return 'end still';
    if (value === 'opening') return 'opening still';
    if (value === 'final-end') return 'final end still';
    return 'still prompt';
};
...
const buildCompactFluxError = (err) => {
    const code = err?.code || err?.cause?.code || '';
    const hostname = err?.hostname || err?.cause?.hostname || '';

    if (code === 'ENOTFOUND' && hostname) {
        return `FLUX network error: cannot resolve ${hostname}`;
    }
    if (code === 'ECONNREFUSED' && hostname) {
        return `FLUX network error: connection refused by ${hostname}`;
    }
    if (code === 'ETIMEDOUT' || code === 'ECONNABORTED') {
        return 'FLUX network error: request timed out';
    }
    return `FLUX request failed: ${err?.message || 'unknown error'}`;
};
...
class PostToFLUX extends PostTo {
    ...
    constructor(modelConfig = {}) {
        ...
        logger.payload('config', this.config);
        logger.debug('imageDir set to:', this.imageDir);
    }
    ...
    async prompt(prompt = '', options = {}) {
        ...
        console.log(`${ANSI.bold}${ANSI.magenta}[flux] ${formatSceneLabel(options.sceneContext)} ${formatFrameRole(options.frameRole)}${ANSI.reset}`);
        console.log(wrapLogText(totalPrompt));
        console.log('');
        ...
        if (mergedOptions.num_inference_steps < minSteps) {
            console.warn(
                `${ANSI.yellow}[flux] steps${ANSI.reset} num_inference_steps too low (${mergedOptions.num_inference_steps}); clamping to ${minSteps}`
            );
        ...
        try {
            ...
            if (err.response) {
                logger.error(`AxiosError: ${err.message} (status: ${err.response.status})`);
                if (err.response.data) {
                    try {
                        const errJson = JSON.parse(Buffer.from(err.response.data).toString('utf-8'));
                        logger.error(`API error: ${errJson.error || err.response.data}`);
                    } catch {
                        logger.error(
                            `API error: ${Buffer.from(err.response.data).toString('utf-8')}`
                        );
                    }
                }
            } else {
                logger.error(buildCompactFluxError(err));
            }

            throw new Error(buildCompactFluxError(err));
        }
        ...
        if (!cType.startsWith('image/')) {
            ...
            logger.error(`Unexpected content-type: ${cType}`);
            logger.error(`Response (first 200 chars): ${msg.slice(0, 200)}...`);
        ...
        logger.error(`API error: received status ${response.status}`);
        return false;
    }
}
