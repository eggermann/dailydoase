...
export const LTX_MIN_DIMENSION = 256;
...
export class PostToLtxDistilled_ImageVideo extends PostTo {
  ...
  normalizeDimensions(height, width) {
    let normalizedHeight = this.nextMul32(height);
    let normalizedWidth = this.nextMul32(width);

    if (normalizedHeight >= LTX_MIN_DIMENSION && normalizedWidth >= LTX_MIN_DIMENSION) {
      return { height: normalizedHeight, width: normalizedWidth };
    }

    const scale = Math.max(
      LTX_MIN_DIMENSION / normalizedHeight,
      LTX_MIN_DIMENSION / normalizedWidth,
      1
    );

    normalizedHeight = this.nextMul32(normalizedHeight * scale);
    normalizedWidth = this.nextMul32(normalizedWidth * scale);

    return {
      height: Math.max(LTX_MIN_DIMENSION, normalizedHeight),
      width: Math.max(LTX_MIN_DIMENSION, normalizedWidth),
    };
  }
  ...
  buildPayload(basePrompt, tmpImagePath, options = {}) {
    ...
    ({ height: h, width: w } = this.normalizeDimensions(h, w));
