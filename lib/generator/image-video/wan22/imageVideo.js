...
const MIXED_SELF_HOSTED_DEFAULT_MODEL_ID = 'Wan-AI/Wan2.2-I2V-A14B-Diffusers';
...
export class PostToWan22_5B_ImageVideo extends PostTo {
  ...
  async buildSelfHostedPayload(tmpInputImage, options = {}, durationSeconds) {
    ...
    const usesMixedSpace = normalizeSpaceId(this.runtime.space) === normalizeSpaceId(MIXED_SELF_HOSTED_SPACE);
    ...
    if (usesMixedSpace) {
      return {
        payload: {
          ...
          model_id: options.custom_model_id
            ?? this.config.custom_model_id
            ?? process.env.WAN22_SINGLE_MODEL_ID
            ?? MIXED_SELF_HOSTED_DEFAULT_MODEL_ID,
          prompt,
          ...
          resolution_profile: resolutionProfile === SELF_HOSTED_HIGH_PROFILE ? 'balanced 720p-ish' : 'small 480p-ish',
          custom_max_area: customMaxArea,
        },
        metadata: {
          resolutionProfile,
          customMaxArea,
