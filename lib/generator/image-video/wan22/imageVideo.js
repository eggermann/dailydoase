...
const normalizeSpaceId = (value) => String(value || '').trim().toLowerCase();
...
export const resolveWanSingleImageRuntime = (config = {}) => {
  ...
  const usesWanSSpace = normalizeSpaceId(resolvedSpace) === normalizeSpaceId(selfHostedSpace);
  return {
    selfHostedHugginfaceModel: usesWanSSpace,
    space: resolvedSpace,
    endpoint: usesWanSSpace ? SELF_HOSTED_ENDPOINT : DEFAULT_ENDPOINT,
  };
};
...
export class PostToWan22_5B_ImageVideo extends PostTo {
  ...
  async buildSelfHostedPayload(tmpInputImage, options = {}, durationSeconds) {
    ...
    return {
      payload: {
        ...
        param_6: options.prompt ?? '',
        param_7: options.negative_prompt ?? this.config.negative_prompt ?? '',
        param_8: numFrames,
        param_9: clampNumber(options.sampling_steps ?? this.config.sampling_steps, 8, 60, 8),
        param_10: clampNumber(options.guide_scale ?? this.config.guide_scale, 1, 8, 1),
        param_11: fps,
        param_12: seed,
  ...
  extractVideoUrl(result) {
    ...
    return toPublicFileUrl(this._cli, chosen);
  }
