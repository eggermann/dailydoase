import {
  ...
  resolveWanFirstLastRuntime,
  resolveWanFirstLastSelfHostedTiming,
} from './firstLastFrame.js';
...
describe('resolveWanFirstLastRuntime', () => {
  ...
  test('promotes fps for short self-hosted clips so min frame count does not stretch to four seconds', () => {
    const timing = resolveWanFirstLastSelfHostedTiming({
      durationSeconds: 1,
      requestedFps: 8,
    });

    expect(timing.fps).toBe(24);
    expect(timing.numFrames).toBe(33);
  });

  test('keeps short self-hosted two-second clips close to target duration', () => {
    const payload = buildWanFirstLastSelfHostedPayload({
      tmpStart: '/tmp/start.png',
      tmpEnd: '/tmp/end.png',
      options: {
    ...
    expect(payload.fps).toBeGreaterThanOrEqual(16);
