...
describe('resolveWanSingleImageRuntime', () => {
  ...
  test('uses the multimodalart input_image field for repo and direct Space URLs', () => {
    expect(resolveWanInputImageFieldName('multimodalart/wan2-1-fast')).toBe('input_image');
    expect(resolveWanInputImageFieldName('https://huggingface.co/spaces/multimodalart/wan2-1-fast')).toBe('input_image');
    expect(resolveWanInputImageFieldName('https://multimodalart-wan2-1-fast.hf.space')).toBe('input_image');
    expect(resolveWanInputImageFieldName('Wan-AI/Wan-2.2-5B')).toBe('image');
  });
