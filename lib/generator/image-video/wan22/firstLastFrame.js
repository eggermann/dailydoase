...
export class PostToWan22_FirstLastFrame extends PostTo {
  ...
  async init() {
    ...
    try {
      ...
      console.warn('Error checking HF token or service i sdown:', this.config.space, e);
    }
    return this;
  }
  ...
  async prompt(startImageStream, options = {}) {
    ...
    if (!url) {
      const sample = JSON.stringify(result?.data, null, 2);
      console.error('Wan-2.2 FirstLast: Could not extract video URL. Raw data sample:', sample?.slice(0, 2000));
