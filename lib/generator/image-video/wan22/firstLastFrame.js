...
export const buildWanFirstLastSelfHostedPayload = ({
  ...
  const fps = clampNumber(
    options.fps ?? options.frames_per_second ?? config.fps ?? config.frames_per_second ?? SELF_HOSTED_DEFAULT_FPS,
    8,
    24,
    SELF_HOSTED_DEFAULT_FPS
  );
  const numFrames = roundToSteppedRange(
    (Number(durationSeconds) || config.duration_seconds || defaultVideoVars.duration_seconds) * fps,
