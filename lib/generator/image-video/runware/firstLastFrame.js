...
const logger = createLogger('runware:first-last', { envKeys: ['RUNWARE_DEBUG', 'GENERATOR_DEBUG'] });
...
export class PostToRunware_FirstLastFrame extends PostTo {
  ...
  async prompt(startImageStream, options = {}) {
    ...
    const inputId = `${Date.now()}-${randomUUID()}`;
    const tmpStart = `${inputDir}/${inputId}-start.png`;
    const tmpEnd = `${inputDir}/${inputId}-end.png`;
    ...
    await endSharp
      ...
      .png()
      .toFile(tmpEnd);
    logger.payload('first-last-inputs', {
      sourceStartImage: startImageStream,
      sourceEndImage: options.endImageStream || startImageStream,
      renderedStartImage: tmpStart,
      renderedEndImage: tmpEnd,
    });
    ...
    const loggedPayload = {
      ...payload,
      inputFramePaths,
      // The two rendered PNGs above are the exact image inputs. Do not copy
      // their large data URIs into the human-readable request record.
      frameImages: payload.frameImages.map(({ frame }) => ({ frame })),
    };
    const requestArtifact = await saveJSON(
      `${this.imageDir}/${inputId}-runware-first-last-request.json`,
      {
        provider: 'runware',
    ...
    let result;
    try {
      result = await submitRunwareVideoJob({
      ...
      await saveJSON(`${this.imageDir}/${inputId}-runware-first-last-error.json`, {
        provider: 'runware',
        model: options.runwareModel || this.config.model,
