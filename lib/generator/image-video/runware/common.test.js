import { describe, expect, jest, test } from '@jest/globals';

import {
  buildRunwareVideoTask,
  normalizeRunwarePositivePrompt,
  normalizeRunwareFrameImages,
  normalizeRunwareDurationSeconds,
  requestRunware,
} from './common.js';

describe('normalizeRunwareDurationSeconds', () => {
  test('rounds decimal scene timing and clamps it to Runware limits', () => {
    expect(normalizeRunwareDurationSeconds(1.6)).toBe(2);
    expect(normalizeRunwareDurationSeconds(3.2)).toBe(3);
    expect(normalizeRunwareDurationSeconds(3.6)).toBe(4);
    expect(normalizeRunwareDurationSeconds(99)).toBe(15);
  });
});

describe('requestRunware', () => {
  test('aborts a stalled request at the supplied timeout', async () => {
    const originalFetch = globalThis.fetch;
    globalThis.fetch = jest.fn((url, options = {}) => new Promise((resolve, reject) => {
      options.signal?.addEventListener('abort', () => reject(options.signal.reason), { once: true });
    }));

    try {
      await expect(requestRunware({
        apiKey: 'runware-test-key',
        body: [{ taskType: 'imageInference' }],
        timeoutMs: 5,
      })).rejects.toThrow('Runware request timed out after 5ms');
    } finally {
      globalThis.fetch = originalFetch;
    }
  });

  test('retries a transient fetch failure with the same task payload', async () => {
    const originalFetch = globalThis.fetch;
    const body = [{ taskType: 'imageInference', taskUUID: 'stable-task-id' }];
    globalThis.fetch = jest.fn()
      .mockRejectedValueOnce(new TypeError('fetch failed', {
        cause: new Error('socket disconnected'),
      }))
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        statusText: 'OK',
        json: async () => ({ data: [{ taskUUID: 'stable-task-id' }] }),
      });

    try {
      await expect(requestRunware({
        apiKey: 'runware-test-key',
        body,
        maxRetries: 2,
        retryDelayMs: 0,
      })).resolves.toEqual({ data: [{ taskUUID: 'stable-task-id' }] });
      expect(globalThis.fetch).toHaveBeenCalledTimes(2);
      expect(globalThis.fetch.mock.calls[0][1].body).toBe(JSON.stringify(body));
      expect(globalThis.fetch.mock.calls[1][1].body).toBe(JSON.stringify(body));
    } finally {
      globalThis.fetch = originalFetch;
    }
  });

  test('reports the transport cause after retries are exhausted', async () => {
    const originalFetch = globalThis.fetch;
    const cause = Object.assign(new Error('connection reset by peer'), { code: 'ECONNRESET' });
    globalThis.fetch = jest.fn().mockRejectedValue(new TypeError('fetch failed', { cause }));

    try {
      await expect(requestRunware({
        apiKey: 'runware-test-key',
        body: [{ taskType: 'imageInference' }],
        maxRetries: 1,
        retryDelayMs: 0,
      })).rejects.toThrow('Runware transport failed: fetch failed (ECONNRESET: connection reset by peer)');
      expect(globalThis.fetch).toHaveBeenCalledTimes(2);
    } finally {
      globalThis.fetch = originalFetch;
    }
  });
});

describe('normalizeRunwareFrameImages', () => {
  test('nests the current Runware image field while accepting the legacy adapter field', () => {
    expect(normalizeRunwareFrameImages([
      { frame: 'first', inputImage: 'data:image/png;base64,abc' },
    ])).toEqual([
      { frame: 'first', image: 'data:image/png;base64,abc' },
    ]);
  });
});

describe('buildRunwareVideoTask', () => {
  test('builds Wan 2.7 first/last interpolation with current image fields', () => {
    const task = buildRunwareVideoTask({
      taskUUID: 'task-1',
      model: 'alibaba:wan@2.7',
      prompt: 'Move between both frames.',
      durationSeconds: 3,
      width: 512,
      height: 384,
      resolution: '720p',
      frameImages: [
        { frame: 'first', inputImage: 'start-image' },
        { frame: 'last', inputImage: 'end-image' },
      ],
    });

    expect(task.inputs.frameImages).toEqual([
      { frame: 'first', image: 'start-image' },
      { frame: 'last', image: 'end-image' },
    ]);
    expect(task.settings).toEqual({ audio: false, promptExtend: false });
    expect(task.providerSettings).toBeUndefined();
    expect(task.resolution).toBe('720p');
    expect(task.width).toBeUndefined();
    expect(task.height).toBeUndefined();
  });

  test('keeps Wan 2.6 provider settings and dimensions for single-frame video', () => {
    const task = buildRunwareVideoTask({
      taskUUID: 'task-2',
      model: 'alibaba:wan@2.6-flash',
      durationSeconds: 3,
      width: 512,
      height: 384,
      frameImages: [{ frame: 'first', image: 'start-image' }],
    });

    expect(task.providerSettings.alibaba.shotType).toBe('single');
    expect(task.settings).toBeUndefined();
    expect(task.width).toBe(512);
    expect(task.height).toBe(384);
  });

  test('never sends a decimal duration to Runware', () => {
    const task = buildRunwareVideoTask({
      taskUUID: 'task-duration-contract',
      model: 'alibaba:wan@2.6-flash',
      durationSeconds: 3.2,
      frameImages: [{ frame: 'first', image: 'start-image' }],
    });

    expect(task.duration).toBe(3);
  });

  test('always sends Runware a valid prompt string when a scene prompt is structured', () => {
    expect(normalizeRunwarePositivePrompt({ prompt: 'A visitor slowly turns toward the camera.' }))
      .toBe('A visitor slowly turns toward the camera.');
    expect(normalizeRunwarePositivePrompt(null)).toBe('Subtle documentary camera motion.');

    const task = buildRunwareVideoTask({
      taskUUID: 'task-prompt-contract',
      model: 'alibaba:wan@2.6-flash',
      prompt: { text: 'A still figure takes one slow step.' },
      durationSeconds: 3,
      frameImages: [{ frame: 'first', image: 'start-image' }],
    });

    expect(task.positivePrompt).toBe('A still figure takes one slow step.');
  });
});
