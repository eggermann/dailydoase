...
describe('normalizeRunwarePositivePrompt', () => {
  test('converts structured scene prompts into valid Runware prompt text', () => {
    expect(normalizeRunwarePositivePrompt({ prompt: 'A visitor turns toward the exit.' }))
      .toBe('A visitor turns toward the exit.');
    expect(normalizeRunwarePositivePrompt(undefined)).toBe('Subtle documentary camera motion.');
  });
});
