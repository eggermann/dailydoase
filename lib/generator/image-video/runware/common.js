...
const RUNWARE_MIN_DURATION_SECONDS = 2;
const RUNWARE_MAX_DURATION_SECONDS = 15;
const MIME_BY_EXT = {
  '.jpg': 'image/jpeg',
  '.jpeg': 'image/jpeg',
  '.png': 'image/png',
...
const describeTransportError = (error) => {
  const message = String(error?.message || error || 'unknown transport error').trim();
...
const isRetryableTransportError = (error) => {
  ...
  return /abort|fetch failed|network|econnreset|econnrefused|enotfound|socket|timed out|timeout/.test(details);
};
...
// Runware accepts only whole clip seconds. Keep the intended timing as close
// as possible while making every request valid at the provider boundary.
export const normalizeRunwareDurationSeconds = (value) => {
  const requestedSeconds = Number(value);
  const roundedSeconds = Number.isFinite(requestedSeconds)
    ? Math.round(requestedSeconds)
    : RUNWARE_MIN_DURATION_SECONDS;
  return Math.min(RUNWARE_MAX_DURATION_SECONDS, Math.max(RUNWARE_MIN_DURATION_SECONDS, roundedSeconds));
};
...
// Scene planners occasionally return a structured prompt. Runware only accepts
// a non-empty string here, so normalize once at the provider boundary rather
// than allowing one malformed scene to stop the exhibition loop.
export const normalizeRunwarePositivePrompt = (value) => {
  if (typeof value === 'string') return value.trim().slice(0, 1_500) || 'Subtle documentary camera motion.';
  if (Array.isArray(value)) {
    return value.map((entry) => normalizeRunwarePositivePrompt(entry))
      .filter(Boolean)
      .join(' ')
      .trim()
      .slice(0, 1_500) || 'Subtle documentary camera motion.';
  }
  if (value && typeof value === 'object') {
    return normalizeRunwarePositivePrompt(
      value.positivePrompt ?? value.prompt ?? value.text ?? value.content ?? value.message
    );
  }
  return 'Subtle documentary camera motion.';
};
...
export const buildRunwareVideoTask = ({
  ...
  const task = {
    ...
    taskUUID,
    model,
    positivePrompt: normalizeRunwarePositivePrompt(prompt),
    duration: normalizeRunwareDurationSeconds(durationSeconds),
    numberResults: 1,
...
export const requestRunware = async ({
  body,
  apiKey,
  timeoutMs = 0,
  maxRetries = 0,
  ...
  let response;
  let json;
  const retryCount = Math.max(0, Math.floor(Number(maxRetries) || 0));
  const retryPauseMs = Math.max(0, Number(retryDelayMs) || 0);
  ...
  for (let attempt = 0; attempt <= retryCount; attempt += 1) {
    ...
    try {
      ...
      const transportError = controller?.signal.aborted
        ? new Error(`Runware request timed out after ${requestTimeoutMs}ms`, { cause: error })
        : new Error(`Runware transport failed: ${describeTransportError(error)}`, { cause: error });
      const shouldRetry = attempt < retryCount && isRetryableTransportError(transportError);
      if (!shouldRetry) {
        throw transportError;
      }
      logger.warn(
        `Transient Runware transport failure; retry ${attempt + 1}/${retryCount}: ${transportError.message}`
      );
      await sleep(retryPauseMs * (attempt + 1));
    } finally {
