...
const RUNWARE_MODELS_WITH_FIXED_INFERENCE_CONTROLS = new Set([
  'bfl:3@1',
]);
const FLUX_KONTEXT_DIMENSIONS = [
...
export const resolveRunwareImageDimensions = ({ model, width, height } = {}) => {
  const requestedWidth = Number(width);
  const requestedHeight = Number(height);
  const modelHasFixedDimensions = RUNWARE_MODELS_WITH_FIXED_INFERENCE_CONTROLS.has(
    String(model || '')
  );

  if (!modelHasFixedDimensions) {
    return { width: requestedWidth, height: requestedHeight };
  }

  const exactDimensions = FLUX_KONTEXT_DIMENSIONS.find((candidate) => (
  ...
  const requestedAspectRatio = requestedWidth > 0 && requestedHeight > 0
    ? requestedWidth / requestedHeight
    : 1184 / 880;
  return FLUX_KONTEXT_DIMENSIONS.reduce((closest, candidate) => {
    const closestDistance = Math.abs((closest.width / closest.height) - requestedAspectRatio);
    const candidateDistance = Math.abs((candidate.width / candidate.height) - requestedAspectRatio);
    return candidateDistance < closestDistance ? candidate : closest;
  }, FLUX_KONTEXT_DIMENSIONS[2]);
};
...
export const buildRunwareImageTask = ({
  ...
  const dimensions = resolveRunwareImageDimensions({ model, width, height });
  const task = {
  ...
  const modelHasFixedInferenceControls = RUNWARE_MODELS_WITH_FIXED_INFERENCE_CONTROLS.has(
    String(model || '')
  );
  if (!modelHasFixedInferenceControls && negativePrompt) {
    task.negativePrompt = negativePrompt;
  }
  if (Number.isFinite(dimensions.width) && dimensions.width > 0) task.width = dimensions.width;
  if (Number.isFinite(dimensions.height) && dimensions.height > 0) task.height = dimensions.height;
  if (!modelHasFixedInferenceControls && Number.isFinite(Number(steps)) && Number(steps) > 0) {
    task.steps = Number(steps);
  }
  if (!modelHasFixedInferenceControls && Number.isFinite(Number(guidanceScale))) {
    task.CFGScale = Number(guidanceScale);
  }
