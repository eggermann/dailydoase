...
const findTaskResult = (response = {}, taskUUID = '') => {
  const data = Array.isArray(response?.data) ? response.data : [];
  return data.find((item) => item?.taskUUID === taskUUID) || null;
};

export const buildRunwareImageInferenceTask = ({
  taskUUID = randomUUID(),
  model,
  prompt,
  width,
  ...
  const task = {
    ...
    taskUUID,
    model,
    positivePrompt: String(prompt || ''),
    numberResults: 1,
    inputs: {
      referenceImages: [...new Set(referenceImages.filter(Boolean))].slice(0, 10),
    },
  };
...
export const submitRunwareImageJob = async ({
  apiKey,
  model,
  prompt,
  width,
  ...
  const initialResponse = await requestRunware({ apiKey, body: [task] });
  const initialResult = findTaskResult(initialResponse, task.taskUUID);
  if (typeof initialResult?.imageURL === 'string' && initialResult.imageURL) {
    return { task, result: initialResult };
  }
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    const response = await requestRunware({
      apiKey,
      body: [{ taskType: 'getResponse', taskUUID: task.taskUUID }],
    });
    const result = findTaskResult(response, task.taskUUID);
    if (!result || result.status === 'processing') {
      continue;
    }
    if (result.status === 'success' && typeof result.imageURL === 'string' && result.imageURL) {
      return { task, result };
    }
    const error = Array.isArray(response?.errors)
      ? response.errors.find((item) => item?.taskUUID === task.taskUUID) || response.errors[0]
      : null;
    if (error) {
      throw new Error(`Runware image request failed: ${extractText(error)}`.trim());
    }
  }

  throw new Error(`Runware image request timed out after ${timeoutMs}ms`);
};
...
export default {
  ...
  imagePathToDataUrl,
  requestRunware,
  buildRunwareImageInferenceTask,
  submitRunwareImageJob,
