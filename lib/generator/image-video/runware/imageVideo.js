...
// WAN 2.6 accepts whole seconds only. The trailer rhythm may use fractional
// beats (for example 3.2 seconds); ffmpeg still preserves that final timeline,
// but the provider must receive the nearest valid whole-second render length.
export const normalizeRunwareVideoDuration = (value, fallback = 3) => {
  ...
  const safeDuration = Number.isFinite(requestedDuration) && requestedDuration > 0
    ? requestedDuration
    : fallback;
  return Math.min(15, Math.max(2, Math.round(safeDuration)));
};
