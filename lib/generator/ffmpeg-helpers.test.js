...
describe('concatMp4Lossless', () => {
  test('normalizes differing pixel aspect ratios before re-encoded concat', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ffmpeg-sar-concat-'));
    const firstPath = path.join(tmpDir, 'first.mp4');
    const secondPath = path.join(tmpDir, 'second.mp4');
    const outputPath = path.join(tmpDir, 'output.mp4');

    for (const [videoPath, color, sar] of [[firstPath, 'red', '1'], [secondPath, 'blue', '29933/29920']]) {
      execFileSync('ffmpeg', ['-hide_banner', '-loglevel', 'error', '-y', '-f', 'lavfi', '-i', 'color=c=' + color + ':s=320x240:r=12', '-vf', 'setsar=' + sar, '-t', '1', '-c:v', 'libx264', '-pix_fmt', 'yuv420p', videoPath]);
    }

    await concatMp4Lossless([firstPath, secondPath], outputPath, tmpDir, { trimLeadingSeconds: 0.125 });
    expect((await probeVideoStream(outputPath)).duration).toBeCloseTo(2, 1);
  });

  test('trims later scene starts while preserving total duration and first canvas', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ffmpeg-concat-'));
    const firstPath = path.join(tmpDir, 'first.mp4');
    const secondPath = path.join(tmpDir, 'second.mp4');
    const outputPath = path.join(tmpDir, 'output.mp4');

    for (const [videoPath, color, size] of [
      [firstPath, 'red', '320x240'],
      [secondPath, 'blue', '640x360'],
    ]) {
      execFileSync('ffmpeg', [
        '-hide_banner',
        '-loglevel', 'error',
    ...
    await concatMp4Lossless([firstPath, secondPath], outputPath, tmpDir, {
      trimLeadingSeconds: 0.125,
    });

    const stream = await probeVideoStream(outputPath);
    expect(stream.width).toBe(320);
    expect(stream.height).toBe(240);
    expect(stream.fps).toBeCloseTo(12, 1);
    expect(stream.duration).toBeCloseTo(2, 1);
  });
});
