...
import { concatMp4Lossless, extractLastFrame, forceVideoEndImage, normalizeVideoOutput, probeVideoStream } from './ffmpeg-helpers.js';
...
describe('concatMp4Lossless', () => {
  test('normalizes differing pixel aspect ratios before re-encoded concat', async () => {
    ...
    for (const [videoPath, color, size, sar] of [[firstPath, 'red', '320x240', '1'], [secondPath, 'blue', '640x360', '29933/29920']]) {
      execFileSync('ffmpeg', ['-hide_banner', '-loglevel', 'error', '-y', '-f', 'lavfi', '-i', 'color=c=' + color + ':s=' + size + ':r=12', '-vf', 'setsar=' + sar, '-t', '1', '-c:v', 'libx264', '-pix_fmt', 'yuv420p', videoPath]);
    }
