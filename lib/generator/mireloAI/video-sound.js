...
export class PostToMirelo_VideoSound extends PostTo {
  ...
  async runVideoToVideo(videoInput, options = {}) {
    ...
    try {
      ...
      if (assetUrl && isHttpUrl(assetUrl)) {
        ...
        await saveJsonSidecar(savePath, { endpoint: '/video-to-video', ...payload, response_meta: { ok: true } });
        return savePath;
      }

      // Persist JSON response if no direct asset
      const jsonPath = await saveJsonSidecar(path.join(this.imageDir, `${Date.now()}-mirelo-v2v.json`), { endpoint: '/video-to-video', ...payload, response: out ?? json });
      ...
      if ((options.return_input_on_error ?? this.config.return_input_on_error) && isHttpUrl(video_url)) {
        try {
          ...
          await saveJsonSidecar(savePathIn, {
            endpoint: '/video-to-video',
            from_input: true,
      ...
      const errPath = await saveJsonSidecar(path.join(this.imageDir, `${Date.now()}-mirelo-v2v.error.json`), {
        endpoint: '/video-to-video',
        ...payload,
  ...
  async runVideoToVideoFormData(localVideoPath, options = {}) {
    ...
    if (assetUrl && isHttpUrl(assetUrl)) {
      ...
      await downloadToFile(assetUrl, savePath);
      await saveJsonSidecar(savePath, { endpoint: '/video-to-video', formData: true });
      return savePath;
    }
    const jsonPath = await saveJsonSidecar(path.join(this.imageDir, `${Date.now()}-mirelo-v2v-form.json`), { response: json });
    return jsonPath;
  }
  ...
  async runVideoToSfx(videoInput, options = {}) {
    ...
    try {
      ...
      if (process.env.MIRELO_DEBUG === 'true') {
        try {
          console.log('[Mirelo] /video-to-sfx response keys:', Object.keys(json || {}));
          console.log('[Mirelo] /video-to-sfx chosen asset URL:', assetUrl);
        } catch {}
      }
      ...
      if (assetUrl && isHttpUrl(assetUrl)) {
        ...
        await downloadToFile(assetUrl, savePath);
        await saveJsonSidecar(savePath, { endpoint: '/video-to-sfx', ...payload, response_meta: { ok: true } });
        return savePath;
      }
 
      // Persist JSON response if no direct asset
      const jsonPath = await saveJsonSidecar(
        path.join(this.imageDir, `${Date.now()}-mirelo-sfx.json`),
        { endpoint: '/video-to-sfx', ...payload, response: out ?? json }
      );
 
      // Optional fallback: return original input if requested and we had no asset
      if ((options.return_input_on_error ?? this.config.return_input_on_error) && isHttpUrl(video_url)) {
        try {
          const fnameIn = `${Date.now()}-mirelo-sfx.input.mp4`;
          const savePathIn = path.join(this.imageDir, fnameIn);
          await downloadToFile(video_url, savePathIn);
          await saveJsonSidecar(savePathIn, {
            endpoint: '/video-to-sfx',
      ...
      const errPath = await saveJsonSidecar(path.join(this.imageDir, `${Date.now()}-mirelo-sfx.error.json`), {
        endpoint: '/video-to-sfx',
        ...payload,
        error: String(e?.message || e),
        auth_mode_used: this.lastAuthInfo?.mode,
      ...
      // Optional: return the original input movie as a fallback asset on error
      if ((options.return_input_on_error ?? this.config.return_input_on_error) && isHttpUrl(video_url)) {
        try {
          const fnameIn = `${Date.now()}-mirelo-sfx.input.mp4`;
          const savePathIn = path.join(this.imageDir, fnameIn);
  ...
  /**
   * Video -> SFX (multipart/form-data, audio-only)
   */
  async runVideoToSfxFormData(localVideoPath, options = {}) {
    const p = path.resolve(localVideoPath);
    if (!(await fs.pathExists(p))) throw new Error(`runVideoToSfxFormData: file not found: ${p}`);
    ...
    form.append('duration', String(options.duration ?? this.config.duration));
    form.append('num_samples', String(options.num_samples ?? this.config.num_samples));
    form.append('steps', String(options.steps ?? this.config.steps));
    if (typeof options.seed === 'number' || typeof this.config.seed === 'number') {
      form.append('seed', String(typeof options.seed === 'number' ? options.seed : this.config.seed));
    }
    form.append('creativity_coef', String(options.creativity_coef ?? this.config.creativity_coef));
    form.append('return_audio_only', 'true');
    if (typeof options.text_prompt === 'string' && options.text_prompt.trim()) form.append('text_prompt', options.text_prompt);

    let json;
    try {
      json = await this._postFormData('/video-to-sfx', form);
    } catch (e) {
      const msg = String(e?.message || e);
      // Fallback for providers that don't support multipart (404/401/400/415)
      if (/(?:401|404|415|400)|Unsupported Media Type|Not Found|Bad Request|Unauthorized/i.test(msg)) {
        if (process.env.MIRELO_DEBUG === 'true') {
          console.warn('[Mirelo] /video-to-sfx form-data unsupported; falling back to upload+JSON. Reason:', msg);
        }
        const video_url = await this._ensureVideoUrl(localVideoPath);
        return await this.runVideoToSfx(video_url, options);
      }
      throw e;
    }

    const out = json?.outputs?.[0] || json?.data?.[0] || json;
    const assetUrl = out?.audio_url || out?.url || out?.audio || null;
    if (assetUrl && isHttpUrl(assetUrl)) {
      const fname = `${Date.now()}-mirelo-sfx.wav`;
      const savePath = path.join(this.imageDir, fname);
      await downloadToFile(assetUrl, savePath);
      await saveJsonSidecar(savePath, { endpoint: '/video-to-sfx', formData: true });
      return savePath;
    }
    const jsonPath = await saveJsonSidecar(path.join(this.imageDir, `${Date.now()}-mirelo-sfx-form.json`), { response: json });
    return jsonPath;
  }
  ...
  async prompt(videoInput, options = {}) {
    ...
    if (options?.audioOnly) {
      return wantsForm ? this.runVideoToSfxFormData(videoInput, options) : this.runVideoToSfx(videoInput, options);
    }
