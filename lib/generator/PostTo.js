...
export default class PostTo {
    ...
    async checkSignature() {
        if (!this.firstTime) {
            ...
            await this.handleNewSeries(this.imageDir);
    ...
    registerCleanupOnExit() {
        const cleanup = () => {
            if (this.imageDir && fs.existsSync(this.imageDir)) {
                ...
                // A generation root commonly starts with one `parts` directory.
                // That directory can already contain costly media, so it is not
                // empty merely because it is the root's only direct entry.
                if (files.length === 0) {
    ...
    async handleNewSeries(seriesPath) {
        const options = this.config;
        try {
            await fs.writeFile(
                path.join(seriesPath, 'info.json'),
        ...
        try {
            const commitHash = getGitCommitHash(PROJECT_ROOT);
            if (commitHash) {
            ...
            console.warn('[PostTo] Failed to update generation-by-commit.txt:', error?.message || error);
        }
    }
}
