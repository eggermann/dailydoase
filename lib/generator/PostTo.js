...
export default class PostTo {
    ...
    addStaticPrompt(prompt, staticPrompt) {
        ...
        // Build parts, trim and remove empties
        const parts = [pre || '', prompt || '', post || '']
            .map(p => String(p).trim())
            .filter(p => p.length);
        ...
        // Remove spaces before punctuation, collapse multiple commas, ensure single space after comma,
        // collapse multiple spaces and trim
        result = result
            .replace(/\s+([,\.!?:;])/g, '$1') // remove space before punctuation
            .replace(/,+/g, ',')              // collapse multiple commas
            .replace(/,\s*/g, ', ')           // ensure one space after comma
