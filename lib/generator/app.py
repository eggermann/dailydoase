...
import os
# --------------------------------------------------------------------------- #
# Configure CUDA caching allocator to reduce fragmentation on long‑running   #
# sessions. We pass all three options in **one** assignment because the      #
# parser expects a single comma‑separated string.                            #
# --------------------------------------------------------------------------- #
...
# Optional memory‑efficient attention backend
try:
...
import torch
print(f"[debug] PYTORCH_CUDA_ALLOC_CONF = {os.getenv('PYTORCH_CUDA_ALLOC_CONF')}")
import numpy as np
import torchaudio
...
device = 'cuda'
dtype = torch.float16  # use fp16 to lower VRAM
...
try:
    ...
    transformer = NagWanTransformer3DModel.from_single_file(wan_path, torch_dtype=dtype)
    ...
    # save VRAM on 1× L40S (48 GB)
    pipe.enable_attention_slicing()
    pipe.enable_model_cpu_offload()

    # Use xFormers attention if it’s available – saves >20 % VRAM
    if _HAS_XFORMERS:
        try:
            pipe.enable_xformers_memory_efficient_attention()
        except Exception as _e:
            print(f"xFormers could not be enabled: {_e}")
    ...
    torch.cuda.empty_cache()  # ensure we release any GPU memory before fallback
    pipe = None
...
def generate_video_with_audio(
    ...
    try:
        ...
        # Free unused GPU memory to mitigate fragmentation between runs
