...
const DEFAULT_OPTIONS = {
  ...
  audio_negative_prompt:
    'Static, motionless, still, ugly, bad quality, worst quality, poorly drawn, low resolution, blurry, lack of details, unclear words',
...
class GenVeo3Video extends PostTo {
  constructor(modelConfig = {}) {
    ...
    //this.config.space = modelConfig.space ?? 'eggman-poff/VEO3-Free';
    this.config.space = modelConfig.space ?? 'ginigen/VEO3-Free';
    //    this.config.space = modelConfig.space ?? 'eggman-poff/VEO3-Free';

    this.config.folderName = modelConfig.folderName ?? 'eggman-poff/VEO3-Free-dd';
  ...
  async prompt(streams, options = {}) {
    ...
    if (this.keepPrompt) {
      ...
      basePrompt = this.keepPrompt;
      this.keepPrompt = null;
    } else {
      ...
      basePrompt = await promptCreator.default(streams, options);
      ...
      // Always treat prompTags as a string, never JSON
      // If you need an array: const tagsArray = prompTags.split(/[ ,]+/).filter(Boolean);
    ...
    try {
      ...
      this.keepPrompt = basePrompt; // keep the prompt for next iteration
      return false;
    }
