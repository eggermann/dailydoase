...
//        console.log(`Scanning directory: ${dir}`);
        ...
        try {
            ...
            for (const item of items) {
                ...
                try {
                    ...
                    if (itemError && itemError.code === 'ENOENT') {
                        continue;
                    }
    ...
    /**
     * Rebuild cache after a filesystem event. FileWatcher debounces calls, so
     * one published trailer costs one scan instead of permanent polling.
     */
    refreshFromFilesystem() {
        if (!this._imgPath) return;

        try {
            this.initialize(this._imgPath);
        } catch (error) {
            console.error('Filesystem cache refresh failed:', error);
        }
    }
