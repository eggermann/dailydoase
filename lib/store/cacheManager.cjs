...
class CacheManager {
    ...
    /**
     * Add or replace one changed media file without rescanning GENERATIONS.
     */
    upsertFile(fullPath) {
        ...
        try {
            const { stats, resolvedPath } = getFileStats(normalizedPath);
            if (!stats.isFile()) return;

            const metadata = createFileMetadata(file, stats, resolvedPath);
            const folderName = path.basename(path.dirname(resolvedPath));
            const previousFiles = this._cache[folderName] || [];
            const keptFiles = previousFiles.filter((item) => item.fullPath !== resolvedPath);
            this._totalFiles -= previousFiles.length - keptFiles.length;
            this._cache[folderName] = keptFiles;

            const insertIndex = keptFiles.findIndex((item) => item.mtime > metadata.mtime);
            if (insertIndex === -1) {
                keptFiles.push(metadata);
            } else {
                keptFiles.splice(insertIndex, 0, metadata);
            }
            this._totalFiles++;
        } catch (error) {
            if (error.code !== 'ENOENT') {
                console.warn(`Skipping changed file ${file}:`, error);
            }
        }
    }

    /**
     * Remove a deleted file or directory and its descendants from cache.
     */
    removePath(fullPath) {
        const normalizedPath = path.normalize(fullPath);
        const nestedPath = `${normalizedPath}${path.sep}`;

        Object.entries(this._cache).forEach(([folderName, files]) => {
            const keptFiles = files.filter((item) => (
                item.fullPath !== normalizedPath && !item.fullPath.startsWith(nestedPath)
            ));
            this._totalFiles -= files.length - keptFiles.length;
...
//        console.log(`Scanning directory: ${dir}`);
    ...
    initialize(imageDir) {
        ...
        try {
            ...
            // A complete scan may contain thousands of files. Sort each folder once
            // after it is full instead of sorting after every single file.
            Object.values(this._cache).forEach((files) => {
    ...
    /**
     * Update only paths touched by the publisher. No polling and no full scan.
     */
    refreshChangedPaths(changedPaths) {
        changedPaths.forEach((changedPath) => {
            const normalizedPath = path.normalize(changedPath);
            if (!this._imgPath || !normalizedPath.startsWith(`${this._imgPath}${path.sep}`)) {
                return;
            }
            ...
            try {
                ...
                if (stats.isDirectory()) {
                    if (!fileWatcher.isWatching(normalizedPath)) {
                        this.scanDir(normalizedPath);
                    }
                } else if (stats.isFile()) {
                ...
                if (error.code === 'ENOENT') {
                    this.removePath(normalizedPath);
                    return;
                }
                console.warn(`Failed to refresh changed path ${normalizedPath}:`, error);
            }
        });
    }
