...
class FileWatcher {
     ...
     */
    watch(dir, ctx) {
        if (process.env.DISABLE_FILE_WATCH === '1') return;
        if (this._watchers.has(dir)) return;
        for (const watchedDir of this._watchers.keys()) {
            if (dir.startsWith(`${watchedDir}${path.sep}`)) return;
        }

        try {
        //    console.log(`[fileWatcher] start watching: ${dir} @ ${new Date().toISOString()}`);
            const watcher = fs.watch(dir, { recursive: true }, (eventType, filename) => {
                if (!filename) return;
                if (filename === '.DS_Store') return;

       //         console.log(`[fileWatcher] ${new Date().toISOString()} dir:${dir} event:${eventType}, file:${filename}`);
