 ...
 */
function createFileMetadata(fileName, stats, resolvedPath) {
    ...
    const jsonPath = resolvedPath + JSON_EXTENSION;// resolvedPath.replace(path.extname(fileName), JSON_EXTENSION);
...
async function getGeneration(fileMetadata, includeImageData = false) {
    try {
        // Use createFileMetadata to generate metadata
        const metadata = createFileMetadata(
            fileMetadata.file,
            { mtime: fileMetadata.mtime }, // Pass as object because createFileMetadata expects a 'stats' object with an 'mtime' property
            fileMetadata.fullPath
        );

        let result = {
            src: `/${metadata.url.replace(/^\/+/, '')}`,
            json: { name: metadata.file },
            metadata
        };
