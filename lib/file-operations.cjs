...
const fileOperations = {
    ...
    /**
     * Reads and parses a JSON file using store's generation data
    ...
    async getFile(dir) {
        try {
            ...
            const fullPath = path.join(imgPath, dir);

            if (!fs.existsSync(fullPath)) {
                throw new Error(`File not found: ${fullPath}`);
            }

            // Read the file directly instead of using store's getGeneration
            // since we only need the raw file content
