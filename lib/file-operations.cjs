...
const fileOperations = {
    ...
    async getFile(key, filename) {
        try {
            const cacheFolder = path.join(await store.getCache(), key);

            get file by key and filename
            
            const filePath = path.join(cacheFolder, filename);

            if (!fs.existsSync(filePath)) {
                throw new Error(`File ${filename} not found in cache folder ${cacheFolder}`);
            }

            return await fs.promises.readFile(filePath, { encoding: "base64" });
        } catch (e) {
            console.error(`getFileFromCacheFolder error for ${key}/${filename}:`, e);
            throw e;
        }
