...
const COMMAND_CANDIDATES = {
  ...
  imagesnap: ['/usr/local/bin/imagesnap', '/opt/homebrew/bin/imagesnap', 'imagesnap'],
};
...
const getCommandCandidates = (command) => (
  Array.isArray(COMMAND_CANDIDATES[command]) ? COMMAND_CANDIDATES[command] : [command]
);

const resolveCommand = async (command, args = ['-h']) => {
  const candidates = getCommandCandidates(command);
  for (const candidate of candidates) {
    try {
      await runCommand(candidate, args, { timeoutMs: 2_000 });
      return candidate;
    } catch (error) {
...
const hasCommand = async (command, args = ['-h']) => {
  try {
    return Boolean(await resolveCommand(command, args));
  } catch (_) {
    return false;
  }
};
...
const isCameraBusyError = (error) => {
  ...
  return CAMERA_BUSY_PATTERNS.some((pattern) => message.includes(pattern));
};
...
const captureWithImageSnap = async (savePath, options) => {
  const imageSnapPath = await resolveCommand('imagesnap', ['-h']);
  if (!imageSnapPath) {
    throw new Error('imagesnap is not installed');
  }
  const args = [];
  ...
  await runCommand(imageSnapPath, args, options);
  return savePath;
};

const captureWithFfmpeg = async (savePath, options) => {
  const ffmpegPath = await resolveCommand('ffmpeg', ['-version']);
  if (!ffmpegPath) {
  ...
  const maxAttempts = Math.max(1, Number(options.captureRetries) || 2);
  const retryDelayMs = Math.max(0, Number(options.captureRetryDelayMs) || 750);
  let lastError = null;

  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      await runCommand(ffmpegPath, args, options);
      return savePath;
    } catch (error) {
      lastError = error;
      if (!isCameraBusyError(error) || attempt >= maxAttempts) {
        break;
      }
