...
import { createLogger } from '../../generator/logger.js';

const logger = createLogger('yt-upload:gate', { envKeys: ['YT_UPLOAD_DEBUG', 'GENERATOR_DEBUG'] });
...
export async function gateConcatAndUpload({ imageDir, options = {}, allDats } = {}) {
  ...
  if (!exceeds) {
    logger.debug('No action needed: total duration within maxDuration threshold.');
  ...
  logger.debug('concat output path:', outPath);
...
export async function uploadToYouTubeWithGeneratedMetadata({
  ...
  try {
    ...
    logger.warn('Metadata generation failed, falling back to defaults:', err?.message || err);
  }
