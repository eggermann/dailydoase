...
// Optional OpenAI import; function gracefully falls back if no API key.
let OpenAI = null;
try { OpenAI = (await import('openai')).default; } catch {}
...
export async function generateYouTubeMetadataFromDir({ dirPath, projectName, language = 'en' } = {}) {
  ...
  const sampleText = prompts.slice(0, prompts.length-1).join('\n---\n');

  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey || !OpenAI) {
    const fb = fallbackMeta(prompts, { projectName, language });
    try { await saveJSON(path.join(dirPath, 'youtube-metadata'), { mode: 'fallback', dirPath, projectName, language, prompts: prompts.slice(0, 6), result: fb }); } catch {}
    return fb;
  }
