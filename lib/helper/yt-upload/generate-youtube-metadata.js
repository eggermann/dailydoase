...
export async function generateYouTubeMetadataFromDir({ dirPath, projectName, language = 'en' } = {}) {
  ...
  const sampleText = prompts.slice(0, 6).join('\n---\n');

  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey || !OpenAI) {
    return fallbackMeta(prompts, { projectName, language });
  }
...
function fallbackMeta(prompts, { projectName, language }) {
  ...
  const descLines = [
    `${projectName || 'Art Project'} — A short study in cinematic staging and identity.`,
    '',
