...
const resolveGenerationsPath = (overridePath) =>
    path.resolve(overridePath || process.env.GENERATIONS_PATH || './tests/GENERATIONS');

let GENERATIONS_PATH = resolveGenerationsPath();//process.cwd()
 ...
 */
function initCache(overwriteGENERATIONS_PATH = GENERATIONS_PATH) {
    GENERATIONS_PATH = resolveGenerationsPath(overwriteGENERATIONS_PATH);

    //  console.log(`Initializing cache at: ${GENERATIONS_PATH}`);
