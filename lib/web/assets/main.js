...
const _ = {
    ...
    pollingMainPageImage: () => {
        ...
        console.log('Polling for new image...');
    ...
    setMenuToggle: () => {
        const btn = document.querySelector('.c-aside__menu__button');
        btn?.addEventListener('click', () => {
            _.mainAside.classList.toggle('opened-menu');
        });

        // Mark menu as scrolled to reduce button prominence while sticky
        const menuEl = _.mainAside;
        if (menuEl) {
            const updateScrolled = () => {
                if (menuEl.scrollTop > 0) menuEl.classList.add('scrolled');
                else menuEl.classList.remove('scrolled');
            };
            menuEl.addEventListener('scroll', updateScrolled, { passive: true });
            // initialize
            updateScrolled();
        }
    }
