...
const routes = [
    {
        "/youtube": async function (req, res) {
            ...
            const folderPath = path.join(
                __dirname,
                this.imageDir || '../../GENERATIONS/youtube', // Fallback if this.imageDir is undefined
                this.options.folderName || 'test' // Fallback if this.folderName is undefined
            );

            await ensureDirectoryExists(folderPath);

            __.getNext(); // External function call (assuming it updates some state)
            ...
            if (!validFiles.length) {
                return res.status(404).send("No valid files found in the folder");
            }

            // Sort files by last modified date (most recent first)
            validFiles.sort((a, b) => b.mtime - a.mtime);

            // Select file in a cyclic order
            const pos = (__.youtubeCnt++ % validFiles.length);
            const selectedFile = validFiles[pos].file;
    ...
    {
        "/rnd": async (req, res) => {
            const absolutPath = _.getRandomImage();
