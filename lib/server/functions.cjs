  ...
  // @TODO
        ...
        let files = fs.readdirSync(p)
            .filter(file => {
                ...
                try {
                    ...
                    console.log('getAllFilesFromFolderForImages XXXX ', err);
    ...
    getRandomFolder: () => {
        ...
        let files = fs.readdirSync(p).filter(file => {
            return fs.lstatSync(path.join(p, file)).isDirectory()
        });
    ...
    createMenu(dir, actFolderName = "/") {
        let files = fs.readdirSync(dir).filter(file => {
            ...
            .map((file) => {
                ...
                const fileCnt = Math.floor(fs.readdirSync(path.join(dir, folder)).length / 2);
