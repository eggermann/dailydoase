...
const getLiveTrailerRank = (folderName) => {
    if (folderName === 'CANK-TRAILER-GOOD-1') return 0;
    if (folderName === 'CANK-TRAILER-GOOD-2') return 1;
    if (folderName === 'CANK-TRAILER-GOOD-3') return 2;
    if (folderName === 'CANK-TRAILER') return 3;
    if (folderName === 'CANK') return 4;
    return -1;
};
...
const registerPartials= function (){
...
const _ = {
    ...
    createMenu(dir, actFolderName = "/") {
        ...
        // Live trailer folders stay directly below Home.
        // Every other folder follows by its newest file, newest first.
        const sortedFolders = allFolders.sort((a, b) => {
            const liveTrailerRankA = getLiveTrailerRank(a);
            const liveTrailerRankB = getLiveTrailerRank(b);
            if (liveTrailerRankA !== liveTrailerRankB) {
                if (liveTrailerRankA !== -1) return -1;
                if (liveTrailerRankB !== -1) return 1;
            }

            const filesA = store.getFolder(a);
            const filesB = store.getFolder(b);
            const newestA = filesA.length ? Math.max(...filesA.map(f => f.mtime || 0)) : 0;
            const newestB = filesB.length ? Math.max(...filesB.map(f => f.mtime || 0)) : 0;
            return newestB - newestA;
        });

        // map them to your menu structure
        let menu = sortedFolders.map(folder => {
