...
const routes = {
    ...
    "/:model/:folderName/": async (req, res) => {
        ...
        let images = await Promise.all(storeFunction.getAllFilesFromFolderForImages(actFolderName));
     
      //  console.log("req.params.actFolderName",images);
    ...
    "/:model/:folderName/:file": (req, res) => {
        const url = `${req.params.folderName}/${req.params.file}`;
        ...
        if (suffix === ".json") {
            ...
            const images = storeFunction.getFile(req.params.folderName,req.params.file);
