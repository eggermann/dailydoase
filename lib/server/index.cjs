...
const {
    readRepeatAllAfter,
    writeRepeatAllAfter,
} = require('./continuous-player-doase.cjs');
const { chooseContinuousPlayerFolder } = require('./continuous-player-folder.cjs');
...
const continuousPlayerFallbackFolder = "lib/generator/adapter/tests/GENERATIONS/87-freshweb-prompt-fast-wan-strict-4-3-test/parts";
...
const getNewestContinuousPartsFolder = () => {
    ...
    const stack = [continuousGenerationsRoot];
    let newestFolder = "";
    let newestMtimeMs = -1;
    ...
    while (stack.length) {
        ...
        for (const entry of entries) {
            ...
            const relativeEntryPath = path.relative(repoRoot, entryPath).split(path.sep).join("/");

            if (relativeEntryPath.includes("/archive/")) {
                continue;
            }
            ...
            if (entry.name === "parts") {
                ...
                if (stat.mtimeMs > newestMtimeMs) {
                    ...
                    newestFolder = relativeEntryPath;
                }
                continue;
            }
    ...
    if (!newestFolder) {
        throw new Error("No parts folder found under lib/generator/adapter/tests/GENERATIONS");
    }

    return newestFolder;
};

const folderHasContinuousVideos = (folder) => {
    try {
        const { absolutePath } = getContinuousFolderPath(folder);
        return fs.readdirSync(absolutePath, { withFileTypes: true })
            .some((entry) => entry.isFile() && /\.(mp4|webm|mov)$/i.test(entry.name));
    } catch (error) {
        return false;
    }
};

const getDefaultContinuousPartsFolder = () => chooseContinuousPlayerFolder({
    expectedFolder: getNewestContinuousPartsFolder(),
    fallbackFolder: continuousPlayerFallbackFolder,
    hasVideos: folderHasContinuousVideos,
});
...
const getContinuousVideoFiles = (folder, { repeatAllAfter } = {}) => {
    const { absolutePath, relativePath } = getContinuousFolderPath(folder);
    const playerDoase = repeatAllAfter === undefined
        ? readRepeatAllAfter(absolutePath)
        : writeRepeatAllAfter(absolutePath, repeatAllAfter);
    ...
    return {
        ...
        files,
        playerDoase: {
            repeatAllAfter: playerDoase.repeatAllAfter,
            allPartsSinceMs: playerDoase.allPartsSinceMs,
        },
    };
};
...
<body>
    <div class="page">
        <section class="meta">
            ...
            <div class="playback-controls">
                ...
                <label title="After Tail size × 3 × 10 clips, play every part since the marker once, then return to the tail.">
                    <input id="repeat-all-after-three" type="checkbox">
                    3× see all
                </label>
            </div>
        </section>
    ...
    <script>
        (() => {
            ...
            const getPlaylistUrl = (repeatAllAfter) => {
                const params = new URLSearchParams({ folder });
                if (repeatAllAfter !== undefined) {
                    params.set('repeatAllAfter', String(repeatAllAfter));
                }
                return '/continuous-video-api?' + params.toString();
            };
            const videos = [
            ...
            const repeatAllAfterThreeEl = document.getElementById('repeat-all-after-three');
            ...
            let repeatAllAfter = -1;
            let allRoundStartIndex = -1;
            const failedClipIds = new Set();
            const crossfadeMs = 260;
            const handoffWindowSeconds = 0.32;
            const fullscreenOverlayIdleMs = 1600;
            const minimumClipAgeMs = 4000;
            const repeatAllAfterThreeModules = 3;
            const tailRoundsPerRepeatModule = 10;
            ...
            const syncRepeatAllAfterThreeControl = (value) => {
                repeatAllAfter = Number(value) === repeatAllAfterThreeModules
                    ? repeatAllAfterThreeModules
                    : -1;
                repeatAllAfterThreeEl.checked = repeatAllAfter === repeatAllAfterThreeModules;
                repeatAllAfterThreeEl.disabled = loopMode !== 'tail' || pingPongEnabled;
            };
            ...
            const getLoopRange = () => {
                ...
                if (playAllRoundActive && allRoundStartIndex >= 0) {
                    return { start: allRoundStartIndex, end: playlist.length - 1 };
                }
            ...
            const getNextStep = () => {
                ...
                if (currentIndex < start || currentIndex > end) {
                    return { index: start, direction: 1 };
                }
                        ...
                        index: (() => {
                            ...
                            if (loopMode === 'tail') {
                                ...
                                if (
                                    ...
                                    && repeatAllAfter > 0
                                    && allRoundStartIndex >= 0
                                    && tailRoundsCompleted % (
                                        repeatAllAfter * tailRoundsPerRepeatModule
                                    ) === 0
                                ) {
                                    playAllRoundActive = true;
                                    deactivateAllRoundAfterStep = false;
                                    return allRoundStartIndex;
                                }
                            }
                            return start;
                        })(),
            ...
            const updateStatus = () => {
                ...
                videoCountEl.textContent = playlist.length > 0 && currentIndex >= 0
                    ...
                    : '0/0';
                syncTailControls();
                syncRepeatAllAfterThreeControl(repeatAllAfter);
            };
            ...
            const fetchPlaylist = async ({ nextRepeatAllAfter } = {}) => {
                ...
                const response = await fetch(getPlaylistUrl(nextRepeatAllAfter), { cache: 'no-store' });
                if (!response.ok) {
                    throw new Error('Failed to load playlist');
                }
                const data = await response.json();
                latestFolderFileCount = data.count || 0;
                syncRepeatAllAfterThreeControl(data.playerDoase?.repeatAllAfter);
                ...
                preloadedIndex = preloadedClipId
                    ...
                    : preloadedIndex;
                allRoundStartIndex = playlist.findIndex((item) => (
                    item.mtimeMs >= Number(data.playerDoase?.allPartsSinceMs || Infinity)
                ));
            ...
            const completeTransitionToSlot = async (nextSlot) => {
                ...
                preloadedIndex = -1;
                if (
                    deactivateAllRoundAfterStep
                    && playAllRoundActive
                    && currentIndex === allRoundStartIndex
                ) {
            ...
            repeatAllAfterThreeEl.addEventListener('change', async () => {
                const nextRepeatAllAfter = repeatAllAfterThreeEl.checked
                    ? repeatAllAfterThreeModules
                    : -1;
                try {
                    await fetchPlaylist({ nextRepeatAllAfter });
                    tailRoundsCompleted = 0;
                    playAllRoundActive = false;
                    deactivateAllRoundAfterStep = false;
                    await preloadIndex(getNextStep());
                    ...
                    syncRepeatAllAfterThreeControl(repeatAllAfter);
                    setError('Could not save the 3× see all setting.');
                    console.warn('Could not save repeat-all setting', error);
                }
            });
...
const routes = {
    "/continuous-video": async (req, res) => {
        ...
        try {
            const folder = requestedFolder.trim()
                ? requestedFolder
                : getDefaultContinuousPartsFolder();
            if (folder.trim()) {
    ...
    "/continuous-video-api": async (req, res) => {
        try {
            const repeatAllAfter = Object.prototype.hasOwnProperty.call(req.query, 'repeatAllAfter')
                ? req.query.repeatAllAfter
                : undefined;
            res.json(getContinuousVideoFiles(req.query.folder, { repeatAllAfter }));
        } catch (error) {
