...
const renderContinuousVideoPage = (folder) => `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <script>
        (() => {
            ...
            const getNextIndex = () => {
                ...
                if (currentIndex < start || currentIndex > end) return start;
                return currentIndex >= end ? start : currentIndex + 1;
            };

            const updateStatus = () => {
                nowPlayingEl.textContent = playlist[currentIndex]?.name || 'waiting for files';
            ...
            const completeTransitionToSlot = async (nextSlot) => {
                ...
                updateStatus();
                void preloadIndex(getNextIndex());
                await playVideo(nextVideo);
            };

            const preloadIndex = async (index) => {
            ...
            videos.forEach((video, index) => {
                video.addEventListener('timeupdate', async () => {
                    ...
                    try {
                        await fetchPlaylist();
                        await preloadIndex(getNextIndex());
