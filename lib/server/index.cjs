...
const __ = {dailymotionCnt: 0, youtubeCnt: 0, maxJsonFiles: 1000};
...
//            console.log('----->',folder)
    ...
    {
        '/v/:version/:img/img'://bei klick auf bild.
            async (req, res) => {
                ...
                const template = _.getImageTemplate();
                const data = {menu, isHome: true}
                const result = '<!DOCTYPE html> ' + template(data);
                res.end(img);
            }
...
module.exports.init = (getNext) => {
    ...
    routes.forEach(r => {//add routes to express
