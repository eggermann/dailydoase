...
const isLiveTrailerFolder = (folderName) => folderName === 'CANK'
    || folderName === 'CANK-TRAILER'
    || folderName.startsWith('CANK-TRAILER-GOOD-');
...
const getNewestMediaGeneration = async (predicate) => {
    const cache = store.getCache() || {};
    let newest = null;

    Object.values(cache).forEach((folderFiles) => {
        folderFiles.forEach((file) => {
            if (!file || !file.file) {
                return;
            }

            if (file.file.startsWith('info') || file.file.toLowerCase().endsWith('.json')) {
                return;
            }

            if (typeof predicate === 'function' && !predicate(file)) {
                return;
            }

            if (!newest || new Date(file.mtime) > new Date(newest.mtime)) {
                newest = {
                    ...file,
    ...
    return store.getGeneration(newest, false);
};

const sortCankImagesNewestFirst = (images) => {
    const getPrefix = (image) => {
        const fileName = image?.metadata?.file || "";
        const match = fileName.match(/^(\d+)-/);
        return match ? Number(match[1]) : null;
    };

    return [...images].sort((left, right) => {
        const leftPrefix = getPrefix(left);
        const rightPrefix = getPrefix(right);
        ...
        return new Date(right.metadata?.mtime || 0) - new Date(left.metadata?.mtime || 0);
    });
};
...
const buildFolderImagesResponse = async (model, folderName, req) => {
    const images = await storeFunction.getAllFilesFromFolderForImages(folderName);
    const sortedImages = req.query.sort === "desc" ? [...images].reverse() : images;
    const origin = `${req.protocol}://${req.get("host")}`;
    const items = sortedImages.map((image, index) => ({
        index: index + 1,
        src: image.src,
        file: image.metadata?.file || "",
        folderName: image.metadata?.folderName || folderName,
        prompt: image.json?.prompt || "",
        absoluteUrl: `${origin}/v${image.src}`,
    }));
...
const parseFolderFromAnyUrl = (rawUrl) => {
    ...
    const pathMatch = parsed.pathname.match(/\/v\/([^/]+)\/?/);
    if (!pathMatch) {
        throw new Error("URL must include /v/<folder>");
    }

    return pathMatch[1];
};

const renderContinuousVideoPage = (folder = "", initialError = "", requestedFolder = "") => `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    <script>
        (() => {
            ...
            const requestedFolder = ${JSON.stringify(requestedFolder)};
            const initialError = ${JSON.stringify(initialError)};
            const followNewestFolder = !requestedFolder.trim();
            const playlistUrl = followNewestFolder
                ? '/continuous-video-api'
                : ('/continuous-video-api?folder=' + encodeURIComponent(folder));
            const videos = [
            ...
            const updateStatus = () => {
                ...
                const currentFolderIndex = currentClip
                    ? latestFolderFiles.findIndex((item) => (
                        item.id === currentClip.id || item.name === currentClip.name
                    ))
                    : -1;
                const displayTotal = latestFolderFileCount || latestFolderFiles.length || playlist.length;
                const displayIndex = currentFolderIndex >= 0
                    ? currentFolderIndex + 1
                    : (playlist.length > 0 && currentIndex >= 0 ? currentIndex + 1 : 0);
                ...
                videoPromptEl.textContent = folder
                    ...
                    : 'paste a folder path';
                videoCountEl.textContent = displayTotal > 0 && displayIndex > 0
                    ? String(displayIndex) + '/' + String(displayTotal)
                    : '0/0';
                syncTailControls();
            };
            ...
            const fetchPlaylist = async () => {
                ...
                playlistRefreshPromise = (async () => {
                    ...
                    if (
                        ...
                        window.location.replace('/continuous-video');
                        return playlist;
                    }
                    latestFolderFiles = Array.isArray(data.files) ? data.files : [];
                    latestFolderFileCount = data.count || latestFolderFiles.length || 0;
                    const nextPlaylist = getEligibleFiles(latestFolderFiles);
...
const routes = {
    "/continuous-video": async (req, res) => {
        ...
        try {
            ...
            res.send(renderContinuousVideoPage(folder, "", requestedFolder));
        } catch (error) {
            res.set("content-type", "text/html");
            res.send(renderContinuousVideoPage(requestedFolder, error.message, requestedFolder));
        }
    },

    "/continuous-video-api": async (req, res) => {
        try {
            const requestedFolder = typeof req.query.folder === "string"
                ? req.query.folder.trim()
                : "";
            const folder = requestedFolder || getNewestContinuousPartsFolder();
            res.json(getContinuousVideoFiles(folder));
        } catch (error) {
    ...
    "/": async (req, res) => {
        ...
        try {
            ...
            const latestVideo = await getNewestMediaGeneration((file) => (
                /\.(mp4|webm|mov)$/i.test(file.file)
            ));
            const latestImage = latestVideo || await storeFunction.newestFile();
    ...
    "/api/folder-images": async (req, res) => {
        try {
            const folder = parseFolderFromAnyUrl(req.query.url);
            const payload = await buildFolderImagesResponse("v", folder, req);
            res.json(payload);
        } catch (error) {
            res.status(400).json({ error: error.message });
        }
    ...
    "/:model/:folderName/": async (req, res) => {
        ...
        try {
            ...
            let images = await storeFunction.getAllFilesFromFolderForImages(actFolderName);
//console.log(`Found ${images.length} images in folder ${images}`,actFolderName);
            if (isLiveTrailerFolder(actFolderName)) {
                images = sortCankImagesNewestFirst(images);
    ...
    "/:model/:folderName/api/images": async (req, res) => {
        try {
            const payload = await buildFolderImagesResponse(
                req.params.model,
            ...
            console.error("Error handling folder API request:", error);
...
module.exports.init = (getNext = () => { }, config = { hello: "default" }) => {
    ...
    app.use("", router);
    return app.listen(port, "0.0.0.0", () => console.log(`Server running at http://0.0.0.0:${port}/`));
};

module.exports.__test = {
    buildFolderImagesResponse,
    parseFolderFromAnyUrl,
};
