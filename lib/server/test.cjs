...
if (isOnServer()) {
    const candidates = [
        path.resolve(__dirname, '../../lib/GENERATIONS'),
        path.resolve(__dirname, '../../GENERATIONS'),
        path.resolve(__dirname, '../../tests/GENERATIONS'),
    ];
    const existing = candidates.find((candidate) => require('fs').existsSync(candidate));
    saveItemPath = existing || candidates[0];
}
